...
class MediaMetaRepositoryTest extends RepositoryTestBase {

    private static MediaMetaRepository metaRepo;
    ...
    static void setupClass() {
        metaRepo = new MediaMetaRepository();
        station = stationRepo.create("MediaMetaStation");
        account = accountRepo.create("media-meta@test.com", "Page", "Meta");
    ...
    private StationFile createFile(String hash) {
        return mediaFileRepo.create(pageId, station.id(), hash, hash + ".png", "image/png", 8);
    }
    ...
    void moveFileToFolder() {
        ...
        try {
            ...
            assertNull(mediaFileRepo.findById(file.id()).orElseThrow().folderId());
    ...
    void assignTagAndFindAssignments() {
        ...
        try {
            ...
            mediaFileRepo.delete(fileA.id());
            mediaFileRepo.delete(fileB.id());
            metaRepo.deleteTag(tag.id());
        }
    }
