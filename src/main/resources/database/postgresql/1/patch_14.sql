...
-- For account-bound users, GDPR consent lives in gdpr_consent. Anonymous submitters
-- have no account, so the proof of acceptance is captured inline as a single JSONB
-- blob. The proof is write-once and never queried by individual field, so packing it
-- into one nullable column keeps the schema light and lets the ConsentProof record
-- grow new fields without future ALTER TABLEs.
ALTER TABLE ember_schema.form_response               ADD COLUMN consent_proof JSONB;
ALTER TABLE ember_schema.waiting_list_entry          ADD COLUMN consent_proof JSONB;
ALTER TABLE ember_schema.waitlist_verification_token ADD COLUMN consent_proof JSONB;

COMMENT ON COLUMN ember_schema.form_response.consent_proof
    IS 'JSONB-encoded ConsentProof (consentVersion, privacyVersion, tosVersion, ipAddress, country, userAgent, consentedAt) captured at the time of an anonymous public submission. NULL for member-attached responses (whose consent lives in gdpr_consent).';
COMMENT ON COLUMN ember_schema.waiting_list_entry.consent_proof
    IS 'JSONB-encoded ConsentProof captured at the time of public submission. NULL for entries created by managers via the admin UI.';
COMMENT ON COLUMN ember_schema.waitlist_verification_token.consent_proof
    IS 'JSONB-encoded ConsentProof captured at the original public submission. Preserved across the e-mail verification step and copied onto the resulting waiting_list_entry.';
