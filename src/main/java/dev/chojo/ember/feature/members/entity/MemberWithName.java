...
public record MemberWithName(
    ...
    public static MemberWithName from(
        ...
        if (m.accountId() == null) {
            return new MemberWithName(m.id(), m.stationId(), 0, m.displayName(), "", profileComplete, identity);
        }
        ...
        return new MemberWithName(m.id(), m.stationId(), m.accountId(), name, email, profileComplete, identity);
    }
}
