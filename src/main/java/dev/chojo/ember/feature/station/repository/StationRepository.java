...
public class StationRepository {
    ...
    public boolean updatePdfHidesInstanceUrl(int id, boolean hides) {
        return query("UPDATE station SET pdf_hides_instance_url = :hides WHERE id = :id;")
                .single(call().bind("hides", hides).bind("id", id))
                .update()
                .changed();
    }
     ...
     *
     * <p>Switched off, every nickname stays where it is and none is read, so switching it back on
     * gives everybody their name back rather than asking them to type it again.
     */
    public boolean updateNicknamesEnabled(int id, boolean enabled) {
        return query("UPDATE station SET nicknames_enabled = :enabled WHERE id = :id;")
