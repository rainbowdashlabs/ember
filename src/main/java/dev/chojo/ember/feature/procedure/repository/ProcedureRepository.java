...
public class ProcedureRepository {
    ...
    public Optional<ProcedureTemplate> findTemplateById(int id) {
        return Query.query("SELECT * FROM procedure_template WHERE id = :id;")
    ...
    public ProcedureTemplate createTemplate(int stationId, String name, String description, int createdBy) {
        return Query.query("""
                        INSERT INTO procedure_template(station_id, name, description, created_by)
                        VALUES(:station_id, :name, :description, :created_by)
                        RETURNING *;""")
                .single(Call.of()
    ...
    public boolean updateTemplate(int id, String name, String description) {
        return Query.query("UPDATE procedure_template SET name = :name, description = :description WHERE id = :id;")
                .single(Call.of()
                        .bind("name", name)
                        .bind("description", description)
                        .bind("id", id))
    ...
    public boolean archiveTemplate(int id) {
        return Query.query("UPDATE procedure_template SET archived = TRUE WHERE id = :id;")
                .single(Call.of().bind("id", id))
                .update()
                .changed();
    }
    ...
    public List<ProcedureTemplateItem> findTemplateItems(int templateId) {
        return Query.query("SELECT * FROM procedure_template_item WHERE template_id = :template_id ORDER BY position;")
    ...
    public ProcedureTemplateItem createTemplateItem(
        ...
        return Query.query("""
                        INSERT INTO procedure_template_item(template_id, title, description, public, user_assigned, position)
                        VALUES(:template_id, :title, :description, :public, :user_assigned, :position)
                        RETURNING *;""")
                .single(Call.of()
    ...
    public boolean updateTemplateItem(
        ...
        return Query.query("""
                        ...
                        SET title = :title, description = :description, public = :public,
                            user_assigned = :user_assigned, position = :position
                        WHERE id = :id;""")
                .single(Call.of()
                        .bind("title", title)
                        .bind("description", description)
    ...
    public boolean deleteTemplateItem(int id) {
        return Query.query("DELETE FROM procedure_template_item WHERE id = :id;")
                .single(Call.of().bind("id", id))
                .delete()
                .changed();
    }
    ...
    public List<int[]> findTemplateItemDependencies(int templateId) {
        return Query.query("""
                        ...
                        FROM procedure_template_item_dependency d
                        JOIN procedure_template_item i ON i.id = d.item_id
    ...
    public void setTemplateItemDependencies(int templateId, List<int[]> dependencies) {
        Query.query("""
                        DELETE FROM procedure_template_item_dependency
                        WHERE item_id IN (SELECT id FROM procedure_template_item WHERE template_id = :template_id);""").single(Call.of().bind("template_id", templateId)).delete();
        for (int[] dep : dependencies) {
            Query.query("""
                            INSERT INTO procedure_template_item_dependency(item_id, depends_on_item_id)
                            VALUES(:item_id, :depends_on_item_id)
    ...
    public List<Procedure> findProceduresByStation(int stationId, ProcedureStatus status) {
        ...
        return Query.query("SELECT * FROM procedure WHERE station_id = :station_id ORDER BY created_at DESC;")
                .single(Call.of().bind("station_id", stationId))
                .map(Procedure.map())
                .all();
    }
    ...
    public List<Procedure> findProceduresByAssignee(
        ...
        if (status != null) {
            return Query.query("""
                            ...
                            JOIN procedure_assignee pa ON pa.procedure_id = p.id
                            WHERE p.station_id = :station_id AND pa.member_id = :member_id AND p.status = :status""" + publicFilter + " ORDER BY p.created_at DESC;")
                    .single(Call.of()
                            .bind("station_id", stationId)
        ...
        return Query.query("""
                        SELECT p.* FROM procedure p
                        JOIN procedure_assignee pa ON pa.procedure_id = p.id
                        WHERE p.station_id = :station_id AND pa.member_id = :member_id""" + publicFilter + " ORDER BY p.created_at DESC;")
    ...
    public Optional<Procedure> findProcedureById(int id) {
        return Query.query("SELECT * FROM procedure WHERE id = :id;")
                .single(Call.of().bind("id", id))
                .map(Procedure.map())
                .first();
    }
    ...
    public boolean updateProcedure(int id, String name, String description, boolean isPublic, Instant dueAt) {
        return Query.query("""
                        UPDATE procedure SET name = :name, description = :description,
                            public = :public, due_at = :due_at
                        WHERE id = :id;""")
                .single(Call.of()
                        .bind("name", name)
                        .bind("description", description)
    ...
    public boolean resolveProcedure(int id) {
        return Query.query(
                        "UPDATE procedure SET status = 'RESOLVED', resolved_at = NOW() WHERE id = :id AND status = 'OPEN';")
                .single(Call.of().bind("id", id))
                .update()
                .changed();
    }
    ...
    public boolean deleteProcedure(int id) {
        return Query.query("DELETE FROM procedure WHERE id = :id;")
                .single(Call.of().bind("id", id))
                .delete()
                .changed();
    }
    ...
    public List<Integer> findAssigneeIds(int procedureId) {
        return Query.query("SELECT member_id FROM procedure_assignee WHERE procedure_id = :procedure_id;")
    ...
    public void addAssignee(int procedureId, int memberId) {
        Query.query("""
                        INSERT INTO procedure_assignee(procedure_id, member_id)
                        VALUES(:procedure_id, :member_id)
                        ON CONFLICT DO NOTHING;""")
    ...
    public List<ProcedureItem> findItems(int procedureId) {
        return Query.query("SELECT * FROM procedure_item WHERE procedure_id = :procedure_id ORDER BY position;")
                .single(Call.of().bind("procedure_id", procedureId))
    ...
    public Optional<ProcedureItem> findItemById(int id) {
        return Query.query("SELECT * FROM procedure_item WHERE id = :id;")
                .single(Call.of().bind("id", id))
                .map(ProcedureItem.map())
                .first();
    }
    ...
    public ProcedureItem createItem(
        ...
        return Query.query("""
                        INSERT INTO procedure_item(procedure_id, title, description, public, user_assigned, position)
                        VALUES(:procedure_id, :title, :description, :public, :user_assigned, :position)
                        RETURNING *;""")
                .single(Call.of()
    ...
    public boolean deleteItem(int id) {
        return Query.query("DELETE FROM procedure_item WHERE id = :id;")
                .single(Call.of().bind("id", id))
                .delete()
                .changed();
    }
    ...
    public boolean updateItemNote(int id, String note) {
        return Query.query("UPDATE procedure_item SET note = :note WHERE id = :id;")
    ...
    public List<int[]> findItemDependencies(int procedureId) {
        return Query.query("""
                        SELECT d.item_id, d.depends_on_item_id
                        FROM procedure_item_dependency d
                        JOIN procedure_item i ON i.id = d.item_id
    ...
    public void setItemDependencies(int procedureId, List<int[]> dependencies) {
        Query.query("""
                        DELETE FROM procedure_item_dependency
                        WHERE item_id IN (SELECT id FROM procedure_item WHERE procedure_id = :procedure_id);""").single(Call.of().bind("procedure_id", procedureId)).delete();
        for (int[] dep : dependencies) {
    ...
    public void addItemDependency(int itemId, int dependsOnItemId) {
        Query.query("""
                        INSERT INTO procedure_item_dependency(item_id, depends_on_item_id)
                        VALUES(:item_id, :depends_on_item_id)
                        ON CONFLICT DO NOTHING;""")
    ...
    public int countOpenByAssigneeWithAvailableItems(int stationId, int memberId) {
        return Query.query("""
                        ...
                        FROM procedure p
                        JOIN procedure_assignee pa ON pa.procedure_id = p.id
                        JOIN procedure_item pi ON pi.procedure_id = p.id
    ...
    public int countOpenByStation(int stationId) {
        return Query.query("SELECT count(*) FROM procedure WHERE station_id = :station_id AND status = 'OPEN';")
                .single(Call.of().bind("station_id", stationId))
                .map(row -> row.getInt("count"))
                .first()
                .orElse(0);
