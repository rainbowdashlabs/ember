 ...
 */
public record EventFederationShare(int id, int eventId, ShareScope scope) {

    /**
     * Creates a row mapping for database result set conversion.
     */
    public static RowMapping<EventFederationShare> map() {
        return row -> new EventFederationShare(
                row.getInt("id"), row.getInt("event_id"), row.getEnum("scope", ShareScope.class));
    }
}
