...
- **Wenn etwas schiefgeht, steht jetzt da, was.** Statt "Das hat nicht funktioniert" benennt eine Fehlermeldung, was passiert ist, ob du es selbst beheben kannst oder ob es an Ember liegt, und was als Nächstes zu tun ist. Sieht es nach einem Fehler in Ember aus, bietet dieselbe Meldung an, ihn zu melden, und die Meldung erreicht die Betreiber deiner Installation mit allem, was sie zum Auffinden brauchen.
- **Ist der Fehler eine Regel, wird nicht mehr um eine Fehlermeldung gebeten.** Etwas, das du nicht darfst, ein bereits vergebener Name, eine zu große Datei: Das steht jetzt klar da, ohne Melde-Knopf. So sind die Meldungen, die ankommen, auch die, die sich zu lesen lohnen.
- **Was geklappt hat, wird nicht mehr als Fehlschlag gemeldet.** Speichern, Löschen, Einladen und Übergeben meldeten einen Fehlschlag, wenn nur die Liste dahinter nicht neu geladen werden konnte, und viele haben es daraufhin ein zweites Mal gemacht. Beides wird jetzt unterschieden, und eine Ansicht, die nur veraltet ist, sagt das auch.
- **Jeder Fehler trägt einen kurzen Code.** Eine Meldung, die sagt, dass etwas schiefgegangen ist, zeigt jetzt auch einen Code wie F-021, kurz genug, um ihn am Telefon vorzulesen, und er geht in die Fehlermeldung mit ein. Wer sich das ansieht, wird damit auf genau die eine Stelle geführt statt auf eine Beschreibung dessen, was du getan hast.
- **Fehlermeldungen sind auf Deutsch.** Der Satz, der benennt, was schiefgegangen ist, kam bisher auf Englisch an, wo er vom Server selbst stammte. Er ist jetzt durchgehend auf Deutsch, und was noch nicht übersetzt ist, wird weiterhin angezeigt statt verschluckt.
...
- **Zeilen und Karten, die eine Seite öffnen, lassen sich in einem neuen Tab öffnen.** Einträge in den Listen der Wache verhalten sich jetzt wie die Links, die sie immer schon waren: mit der mittleren Maustaste in einem eigenen Tab öffnen, mit der rechten die Adresse kopieren, mit der Tastatur erreichen und als Link vorgelesen bekommen. Am Klick selbst ändert sich nichts.
...
- **Das öffentliche Wiki führte auf unleserliche Adressen zurück.** Ein Ordner oder ein Artikel im öffentlichen Wiki führte auf eine Adresse mit der internen Kennung der Wache statt mit ihrem Namen, sodass man von der lesbaren Adresse weggeführt wurde und beim Teilen den falschen Link kopierte. Alle Links auf den öffentlichen Seiten verwenden jetzt den lesbaren Namen, sofern die Wache einen hat.
...
- **Bei Formularen ohne Anmeldung ließ sich einstellen, wer antworten darf.** Ein Kontaktformular und eine Umfrage auf einer öffentlichen Seite boten dieselbe Auswahl aus Mitgliedsarten, Gruppen, Tags und einzelnen Personen wie eine interne Umfrage, und nichts wertete sie aus: Wer den Link hatte, konnte in jedem Fall antworten. Die Auswahl erscheint jetzt nur noch bei Umfragen, die Mitglieder der Wache beantworten.
