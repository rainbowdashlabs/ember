  ...
  if (!isInsideDir(file, 'typography')) {
    for (let i = 0; i < templateLines.length; i++) {
      ...
      if (/<h[123][\s>]/i.test(line)) {
        warn(file, templateStartLine + i, `Raw <${line.match(/<(h[123])/i)?.[1]}> usage. Consider using PageHeader, SectionHeader, or SubHeader.`)
      }
    }
  }
  ...
  if (!isInsideComponents(file)) {
    for (let i = 0; i < templateLines.length; i++) {
      ...
      for (const match of classMatches) {
        ...
        if (classes.length > 6) {
          warn(file, templateStartLine + i, `Element has ${classes.length} CSS classes (max 6). Consider extracting to a component.`)
        }
      }
    }
  }
