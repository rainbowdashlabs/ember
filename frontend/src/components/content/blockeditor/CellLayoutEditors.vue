 ...
 * {@code (config, [content], [stationUid])} prop shape and the two emits
...
<template>
    <div class="space-y-3">
        ...
        <ImageGalleryEditor v-else-if="kind === 'IMAGE_GALLERY'" :config="config" :station-uid="stationUid"
                            @update:config="config = $event"/>
        ...
        <HeroBannerEditor v-else-if="kind === 'HERO_BANNER'" :config="config" :station-uid="stationUid"
                          @update:config="config = $event"/>
