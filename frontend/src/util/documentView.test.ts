...
const saveBlob = vi.fn((_blob: Blob, _filename: string) => Promise.resolve('DONE'))

vi.mock('@/util/saveBlob', () => ({
    SaveResult: {DONE: 'DONE', DISMISSED: 'DISMISSED', UNAVAILABLE: 'UNAVAILABLE'},
...
describe('presentDocument', () => {
    beforeEach(() => {
        saveBlob.mockReset()
        saveBlob.mockResolvedValue('DONE')
        closeDocument()
        pointer('fine')
    })

    it('saves the document at a desk', async () => {
        const blob = new Blob(['%PDF'], {type: 'application/pdf'})

        await presentDocument(blob, 'report.pdf')

        expect(saveBlob).toHaveBeenCalledWith(blob, 'report.pdf')
        expect(getViewedDocument().value).toBeNull()
    })

    it('opens the document in the hand instead of saving it', async () => {
        pointer('coarse')
        const blob = new Blob(['%PDF'], {type: 'application/pdf'})
    ...
    it('saves what nothing can draw, even in the hand', async () => {
        pointer('coarse')
        const blob = new Blob(['zip'], {type: 'application/zip'})

        await presentDocument(blob, 'export.zip')
     ...
     */
    it('saves a list of values rather than showing it', async () => {
        pointer('coarse')
        const blob = new Blob(['a,b'], {type: 'text/csv'})

        await presentDocument(blob, 'list.csv')
    ...
    it('takes the kind from the name where the type says nothing', async () => {
        pointer('coarse')

        await presentDocument(new Blob(['%PDF'], {type: 'application/octet-stream'}), 'report.pdf')

        expect(saveBlob).not.toHaveBeenCalled()
        expect(getViewedDocument().value).toMatchObject({filename: 'report.pdf'})
    })

    it('lets go of the document when the reader closes it', async () => {
        pointer('coarse')
        await presentDocument(new Blob(['%PDF'], {type: 'application/pdf'}), 'report.pdf')
