...
const {config: denied, loading, error, runWith} = useConfigPanel<string[]>({
  initial: [],
  fetch: async () => (await clusterGovernance.getDeniedModules()).deniedModules,
})
...
async function save() {
  saved.value = false
  await runWith(async () => {
    await clusterGovernance.setDeniedModules(denied.value)
    saved.value = true
    return (await clusterGovernance.getDeniedModules()).deniedModules
  }, {busy})
}
</script>
