...
<template>
  ...
  <div class="flex items-center gap-3">
    <TextInput :model-value="filterText" :placeholder="t('membersList.filter')" class="flex-1" @update:model-value="(v: string | undefined) => emit('update:filterText', v ?? '')" />
    <div class="relative">
      <SecondaryButton class="text-sm" @click="showColumnPicker = !showColumnPicker">
    ...
    <SecondaryButton class="text-sm" @click="emit('clearFilters')">
      <font-awesome-icon :icon="['fas', 'xmark']" class="mr-1" />
      {{ t('membersList.clearFilters') }}
    </SecondaryButton>
    <SecondaryButton class="text-sm" @click="showSaveFilter = !showSaveFilter">
      <font-awesome-icon :icon="['fas', 'download']" class="mr-1" />
      {{ t('membersList.saveFilter') }}
    </SecondaryButton>
    <SecondaryButton class="text-sm" @click="emit('toggleExport')">
      <font-awesome-icon :icon="['fas', 'download']" class="mr-1" />
      {{ exportMode ? t('membersList.export.cancel') : t('membersList.export.button') }}
    </SecondaryButton>
    <template v-if="exportMode">
      <span class="text-xs text-(--text-muted)">{{ t('membersList.export.selected', { count: selectedCount }) }}</span>
      <PrimaryButton class="text-sm" :disabled="selectedCount === 0" @click="emit('exportContinue')">
