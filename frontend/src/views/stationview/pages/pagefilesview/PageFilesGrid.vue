...
const emit = defineEmits<{
    ...
    'toggle-select': [fileId: number, value: boolean]
...
<template>
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        ...
        <div v-for="e in files" :key="'e-' + e.file.id"
            ...
            <label class="absolute top-1 left-1 z-10 flex items-center bg-(--bg)/90 backdrop-blur-sm rounded p-1 cursor-pointer"
                   @click.stop>
                <CheckboxInput :model-value="isSelected(e.file.id)"
                               @update:model-value="(v: boolean) => emit('toggle-select', e.file.id, v)"/>
            </label>
            ...
            <div class="absolute top-1 right-1 flex items-center gap-1" @click.stop>
                ...
                <IconButton :icon="['fas', 'pen']" :label="t('stationPages.editor.editFileMeta')"
                            class="bg-(--bg)/90 backdrop-blur-sm rounded-full !p-1 text-xs"
                            @click="emit('edit-file', e.file)"/>
            </div>
        </div>
    </div>
</template>
