...
import PageFilesGridItem from './pagefilesgrid/PageFilesGridItem.vue'
...
function onEdit(file: PageFile) {
    closeMenu()
    emit('edit-file', file)
}

function onDelete(file: PageFile) {
    closeMenu()
...
<template>
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        ...
        <PageFilesGridItem v-for="(e, idx) in files" :key="'e-' + e.file.id"
