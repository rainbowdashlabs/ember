...
<template>
    <div class="space-y-3">
        <CalloutEditor v-if="kind === 'CALLOUT'" :config="config" :content="content"
                       @update:config="emit('update:config', $event)" @update:content="emit('update:content', $event)"/>
        <QuoteEditor v-else-if="kind === 'QUOTE'" :config="config" :content="content" :station-uid="stationUid"
                     @update:config="emit('update:config', $event)" @update:content="emit('update:content', $event)"/>
        <DividerEditor v-else-if="kind === 'DIVIDER'" :config="config" @update:config="emit('update:config', $event)"/>
        <SpacerEditor v-else-if="kind === 'SPACER'" :config="config" @update:config="emit('update:config', $event)"/>
        ...
        <FileDownloadEditor v-else-if="kind === 'FILE_DOWNLOAD'" :config="config" :station-uid="stationUid"
                            @update:config="emit('update:config', $event)"/>
        <CountdownEditor v-else-if="kind === 'COUNTDOWN'" :config="config" @update:config="emit('update:config', $event)"/>
        ...
        <StatsCounterEditor v-else-if="kind === 'STATS_COUNTER'" :config="config" @update:config="emit('update:config', $event)"/>
        <TabsEditor v-else-if="kind === 'TABS'" :config="config" @update:config="emit('update:config', $event)"/>
        <AchievementsEditor v-else-if="kind === 'ACHIEVEMENTS'" :config="config" @update:config="emit('update:config', $event)"/>
        ...
        <PageLinkEditor v-else-if="kind === 'PAGE_LINK'" :config="config" :station-uid="stationUid"
                        @update:config="emit('update:config', $event)"/>
        <MapEditor v-else-if="kind === 'MAP'" :config="config" @update:config="emit('update:config', $event)"/>
        <AddressCardEditor v-else-if="kind === 'ADDRESS_CARD'" :config="config" @update:config="emit('update:config', $event)"/>
        <MemberSpotlightEditor v-else-if="kind === 'MEMBER_SPOTLIGHT'" :config="config" @update:config="emit('update:config', $event)"/>
        ...
        <ExternalLinkCardEditor v-else-if="kind === 'EXTERNAL_LINK_CARD'" :config="config" :station-uid="stationUid"
                                @update:config="emit('update:config', $event)"/>
        <BlogSignupEditor v-else-if="kind === 'BLOG_SIGNUP'" :config="config" @update:config="emit('update:config', $event)"/>
        <AudioEmbedEditor v-else-if="kind === 'AUDIO_EMBED'" :config="config" :station-uid="stationUid"
                          @update:config="emit('update:config', $event)"/>
        <PollEmbedEditor v-else-if="kind === 'POLL_EMBED'" :config="config" @update:config="emit('update:config', $event)"/>
        <CellQuizEditor v-else-if="kind === 'QUIZ_TEASER'" :config="config" :station-uid="stationUid"
                        @update:config="emit('update:config', $event)"/>
        <FormsCtaEditor v-else-if="kind === 'FORMS_CTA'" :config="config" @update:config="emit('update:config', $event)"/>
