    ...
    <template v-for="m in eligibleMembers" :key="`reg-${m.id}`">
      <template v-if="getRegistration(m.id)">
        <div class="flex items-center gap-1">
          ...
          <SecondaryButton :title="t('eventsUpcoming.withdraw')" class="!p-0 !bg-transparent !border-0" @click="emit('withdraw', getRegistration(m.id)!.id)">
            ...
            <ErrorBadge v-else-if="getRegistration(m.id)!.status === RegistrationStatus.DECLINED">{{ t('eventsUpcoming.statusDeclined') }} <font-awesome-icon :icon="['fas', 'xmark']" class="ml-1 h-3 w-3"/></ErrorBadge>
          </SecondaryButton>
