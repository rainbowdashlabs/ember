...
async function reorderChecklist(fromIndex: number, toIndex: number) { const items = [...checklist.value]; const [moved] = items.splice(fromIndex, 1); if (!moved) return; items.splice(toIndex, 0, moved); checklist.value = items; try { await api.reorderChecklist({ orderedIds: items.map(i => i.id) }) } catch { await loadDetails() } }
