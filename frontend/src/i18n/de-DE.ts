/*
 *     SPDX-License-Identifier: AGPL-3.0-only
 *
 *     Copyright (C) RainbowDashLabs and Contributor
 */
export default {
    mailProviders: {
        BREVO: {
            intro: 'Brevo vergibt eigene SMTP-Zugangsdaten. Du findest sie in Brevo rechts oben unter deinem Kontonamen → „SMTP & API“.',
            userLabel: 'Anmelde-E-Mail',
            userHint: 'Die E-Mail-Adresse, die Brevo auf der Seite „SMTP & API“ als Login anzeigt - in der Regel die E-Mail-Adresse deines Brevo-Kontos.',
            secretLabel: 'SMTP-Schlüssel',
            secretHint: 'Ein SMTP-Schlüssel beginnt mit „xsmtpsib-“. Der reguläre API-Schlüssel („xkeysib-“) funktioniert für den Versand nicht.',
            webhookStep1: 'Öffne in Brevo „Einstellungen → Transaktionsbezogene E-Mails → Webhook“ '
                + 'oder direkt app.brevo.com/webhooks/outbound/create.',
            webhookStep2: 'Klicke auf „Add a new webhook“ und füge die Adresse von oben ein.',
            webhookStep3: 'Wähle mindestens „delivered“, „softBounce“, „hardBounce“, „blocked“, '
                + '„spam“ und „deferred“ und speichere.',
            webhookLink: 'https://help.brevo.com/hc/de/articles/360007666479',
        },
        RAPIDMAIL: {
            intro: 'RapidMail erstellt SMTP-Zugangsdaten pro Projekt. Du findest sie unter Transaktionsmails → Projekte verwalten → Bearbeiten.',
            userLabel: 'SMTP-Benutzername',
            userHint: 'Der generierte Benutzername aus deinem Transaktionsmail-Projekt - nicht dein RapidMail-Login.',
            secretLabel: 'SMTP-Passwort',
            secretHint: 'Das generierte Passwort aus demselben Transaktionsmail-Projekt.',
        },
        SWEEGO: {
            intro: 'Sweego erstellt SMTP-Zugangsdaten im Dashboard, sobald deine Domain verifiziert ist.',
            userLabel: 'SMTP-Login',
            userHint: 'Der generierte SMTP-Login aus dem Sweego-Dashboard.',
            secretLabel: 'SMTP-Passwort',
            secretHint: 'Das generierte SMTP-Passwort aus dem Sweego-Dashboard.',
            webhookStep1: 'Öffne im Sweego-Dashboard „Webhooks“ und lege einen neuen an.',
            webhookStep2: 'Füge die Adresse von oben ein und wähle den Kanal „email“.',
            webhookStep3: 'Wähle mindestens „delivered“, „soft-bounce“, „hard_bounce“ und '
                + '„complaint“ und speichere.',
            webhookStep4: 'Sweego signiert seine Meldungen. Hinterlege das dort angezeigte '
                + 'Webhook-Secret in der Konfiguration, dann prüft Ember jede Meldung darauf.',
            hostHint: 'Sweego gibt jedem Konto einen eigenen Relay-Server, zum Beispiel '
                + '„prod-mta-05.swg-srv.net“. Host und Port stehen unter „Credentials → SMTP“; der '
                + 'Port ist dort oft 2525.',
            credentialsLink: 'https://app.sweego.io/credentials/smtp',
            webhookAppLink: 'https://app.sweego.io/webhooks',
            webhookLink: 'https://learn.sweego.io/docs/webhooks/setup',
        },
        TWILIO: {
            intro: 'Twilio SendGrid nutzt einen API-Schlüssel für den SMTP-Versand. Als Benutzername wird automatisch „apikey“ verwendet.',
            secretLabel: 'API-Schlüssel',
            secretHint: 'Erstelle den Schlüssel in SendGrid unter Settings → API Keys; er beginnt mit „SG.“.',
            webhookStep1: 'Öffne in SendGrid „Settings → Mail Settings → Event Webhook“.',
            webhookStep2: 'Schalte den Event Webhook ein und füge die Adresse von oben als '
                + '„HTTP Post URL“ ein.',
            webhookStep3: 'Wähle „Delivered“, „Bounced“, „Blocked“, „Deferred“ und „Spam Reports“ '
                + 'und speichere.',
            webhookLink: 'https://www.twilio.com/docs/sendgrid/for-developers/tracking-events/event',
        },
    },
    mailFreeTier: {
        title: 'Kostenlos versendbar',
        hint: 'Was die Anbieter ohne Bezahlung zulassen. Die Zahlen stammen von den Anbietern '
            + 'selbst und ändern sich gelegentlich; ein Klick führt zur jeweiligen Preisseite.',
        perDay: '{count} Mails/Tag',
        ownServer: 'Was dein Server hergibt',
        trialOnly: 'Nur 60 Tage testweise, danach kostenpflichtig',
        paidOnly: 'Kein kostenloses Kontingent',
        checked: 'Stand {date}',
        noTerms: 'Keine fremden Bedingungen',
    },
    mailWebhook: {
        title: 'Rückmeldungen zur Zustellung',
        intro: 'Dein Anbieter bestätigt beim Versand nur, dass er die Mail übernommen hat. Ob sie '
            + 'ankommt, entscheidet sich danach zwischen ihm und dem Empfängerserver. Trage diese '
            + 'Adresse bei deinem Anbieter ein, damit Ember es erfährt.',
        purpose: 'Danach siehst du zu jeder Mail, ob sie zugestellt, abgewiesen oder blockiert '
            + 'wurde. Und wird nicht deine Nachricht, sondern der Absendeserver abgelehnt, wechselt '
            + 'Ember selbsttätig auf den nächsten Anbieter deiner Kette.',
        stepsTitle: 'Bei {provider} einrichten',
        unsupported: 'Für {provider} kann Ember noch keine Rückmeldungen entgegennehmen. Der '
            + 'Versand und der Wechsel auf den nächsten Anbieter bei einem fehlgeschlagenen '
            + 'Versand funktionieren; eine Mail, die angenommen und erst danach abgewiesen wird, '
            + 'bleibt aber unsichtbar.',
        noProvider: 'Richte zuerst einen Anbieter ein.',
        docs: 'Dokumentation des Anbieters',
        openSettings: 'Direkt zu den Webhook-Einstellungen',
        copy: 'Adresse kopieren',
        copied: 'Adresse kopiert.',
        copyFailed: 'Kopieren nicht möglich. Markiere die Adresse und kopiere sie von Hand.',
        replaceFailed: 'Die Adresse konnte nicht neu erzeugt werden. Die bisherige gilt weiterhin.',
        replace: 'Neuen Schlüssel erzeugen',
        replaceHint: 'Ein neuer Schlüssel macht die alte Adresse sofort ungültig. Trage die neue '
            + 'danach bei deinem Anbieter ein, sonst kommen keine Rückmeldungen mehr an.',
        unavailable: 'Noch keine Adresse verfügbar.',
        signingSecret: 'Signatur-Secret des Anbieters',
        signingSecretSaved: 'Signatur-Secret gespeichert.',
        signingSecretStored: 'Hinterlegt, zum Ändern neu eingeben',
        signingSecretHint: 'Trägst du das Secret von {provider} hier ein, wird jede Meldung darauf '
            + 'geprüft und nicht nur anhand der Adresse angenommen. Leer lassen schaltet die Prüfung '
            + 'wieder ab.',
    },
    transferImport: {
        completed: 'Import abgeschlossen.',
        failed: 'Import fehlgeschlagen: {error}',
        morePending: 'noch {count} weitere Schritte ausstehend',
    },
    adminStationImport: {
        backToStations: 'Zurück zu den Wachen',
    },
    publicConsent: {
        label: 'Ich habe die {privacy} und die {tos} gelesen und stimme ihnen zu.',
        privacy: 'Datenschutzerklärung',
        tos: 'Nutzungsbedingungen',
        loadError: 'Die Rechtsdokumente konnten nicht geladen werden. Bitte später erneut versuchen.',
        required: 'Bitte stimme der Datenschutzerklärung und den Nutzungsbedingungen zu.',
    },
    landing: {
        meta: {
            title: 'Ember - Verwaltung für Jugendfeuerwehren',
        },
        tryNow: 'Jetzt ausprobieren',
        login: 'Anmelden',
        helpCenter: 'Hilfe',
        hero: {
            titleLine1: 'Schluss mit Excel-Listen',
            titleLine2: 'und WhatsApp-Gruppen.',
            lede: 'Ember ist eine offene Plattform für Jugendfeuerwehren. Termine, Anwesenheit, Material - an einem Ort, ohne Abo.',
            ctaCreate: 'Wache anlegen',
            ctaHost: 'Selbst hosten',
            ctaDemo: 'Demo ansehen',
            linkDemo: 'oder Demo ansehen →',
            linkLoginDemo: 'oder direkt anmelden →',
            linkLogin: 'oder anmelden →',
        },
        week: {
            eyebrow: 'Diese Woche',
            footer: 'Beispiel - so sieht eine Woche in Ember aus.',
            entries: [
                {day: 'Mi', time: '18:00', text: 'Übung - Grundausbildung'},
                {day: 'Do', time: '19:30', text: 'Gruppenleitersitzung'},
                {day: 'Sa', time: '09:00', text: 'Jugendflamme-Vorbereitung'},
                {day: 'So', time: '-',     text: 'Materialcheck'},
            ],
        },
        evening: {
            eyebrow: 'Ein Übungsabend',
            titleLine1: 'Mittwoch.',
            titleLine2: '18:00.',
            titleLine3: 'Übung.',
            kicker: 'Zwei Stunden ohne Klemmbrett. Die Anwesenheit landet beim Tippen direkt in der Statistik.',
            beats: [
                {time: '17:50', text: 'Termin steht seit Wochen im Kalender. Erinnerung war schon raus.'},
                {time: '17:55', text: 'Ausbildungsinhalt liegt im Wiki - zum Vorbereiten und Nachschlagen.'},
                {time: '18:00', text: 'Vorab abgemeldete sind schon eingetragen. Die übrigen: ein Tap.'},
                {time: '19:30', text: 'Übung beendet. Anwesenheits-Statistik schreibt sich selbst.'},
                {time: '19:45', text: 'Nächster Termin steht. Erinnerung läuft automatisch.'},
                {time: '20:00', text: 'Heimweg. Nichts, was später nachgetragen werden muss.'},
            ],
        },
        material: {
            eyebrow: 'Material',
            headline: 'Wer trägt welche Größe.',
            lede: 'Jede Jugendliche, jeder Jugendliche bekommt eigene Ausrüstung - Schutzjacke, Hose, Stiefel. Ember weiß welche Größe wo ist. Wenn Lukas wächst, läuft der Größentausch geordnet ab. Wenn etwas verschwindet, ist klar, wer es zuletzt hatte.',
            bullets: [
                'Ausrüstung pro Mitglied, mit Größenverlauf.',
                'Größentäusche mit Versandstatus zwischen Wachen oder für euch.',
                'Verlust und Beschaffung in einer Übersicht.',
            ],
            card: {
                title: 'Übersicht - Material',
                count: '{n} offene Vorgänge',
                aria: 'Beispiel: Material-Übersicht',
            },
            sections: {
                exchanges: 'Offene Größentäusche',
                procurements: 'Offene Beschaffungen',
                losses: 'Verluste',
            },
            columns: {
                entry: 'Eintrag',
                type: 'Typ',
                owner: 'Besitzer',
                status: 'Status',
                note: 'Notiz',
                requested: 'Angefragt',
                lostSince: 'Verloren seit',
            },
            type: {
                INTERNAL: 'Intern',
                EXTERNAL: 'Extern',
                MIXED: 'Gemischt',
            },
            exchangeStatus: {
                ANNOUNCED: 'Angekündigt',
                SHIPPED: 'Im Versand',
            },
            mock: {
                exchanges: [
                    {item: 'Schutzjacke', size: '152 → 158', type: 'INTERNAL', owner: 'Lukas Hartmann', status: 'SHIPPED'},
                    {item: 'Schutzhose',  size: '152 → 158', type: 'INTERNAL', owner: 'Mia Klein',      status: 'ANNOUNCED'},
                ],
                procurements: [
                    {item: 'Sicherheitsstiefel', size: 'Gr. 41', type: 'INTERNAL', notes: '-',                  requested: '08.06.'},
                    {item: 'JF-Polo',            size: 'Gr. S',  type: 'EXTERNAL', notes: '4× neue Mitglieder', requested: '09.06.'},
                ],
                losses: [
                    {item: 'JF-Handschuhe', size: 'Unisize', type: 'INTERNAL', owner: 'Tim Becker', lostAt: '12.06.'},
                ],
            },
        },
        features: {
            eyebrow: 'Funktionen',
            headline: 'Alles, was eine Jugendfeuerwehr verwalten muss.',
            items: [
                {title: 'Anwesenheit',  body: 'Wer war da, wer nicht.'},
                {title: 'Termine',      body: 'Übungen und Wettkämpfe planen.'},
                {title: 'Material',     body: 'Schläuche, Helme, Funkgeräte im Blick.'},
                {title: 'Mitglieder',   body: 'Eine zentrale Liste, kein Karteikasten.'},
                {title: 'Anmeldungen',  body: 'Formulare ohne Papier.'},
                {title: 'Nachrichten',  body: 'Was in der Wehr läuft.'},
                {title: 'Wiki',         body: 'Wissen, das bleibt.'},
                {title: 'Lernen',       body: 'Lernkarten, Übungsfragen und eigene Quizze.'},
                {title: 'Dienstdoku',   body: 'Übungen lückenlos festhalten.'},
                {title: 'Webseite',     body: 'Eure Wache im Netz einfach präsentiert.'},
                {title: 'Vernetzung',   body: 'Termine mit Nachbarwehren teilen.'},
                {title: 'Erinnerungen', body: 'Was wichtig ist, kommt an.'},
            ],
        },
        hosting: {
            eyebrow: 'Drei Wege, anzufangen',
            headline: 'Hostet so, wie es zu eurer Wehr passt.',
            options: [
                {title: 'Selbst hosten',     body: 'Auf eurem Server. Volle Kontrolle, euch gehört alles.'},
                {title: 'Gemeinsam hosten',  body: 'Eine Instanz für mehrere Wehren. Geteilte Kosten, getrennte Daten.'},
                {title: 'Hosting durch uns', body: 'Wir nehmen euch Server und Updates ab. Loslegen ohne Setup.'},
            ],
        },
        replaces: {
            eyebrow: 'Was Ember ersetzt',
            headline: 'Statt vieler Werkzeuge - eins.',
            items: [
                {old: 'Excel-Anwesenheitsliste', neu: 'Anmelden, abhaken, auswerten.'},
                {old: 'WhatsApp-Termingruppe',   neu: 'Kalender mit Erinnerungen.'},
                {old: 'Karteikasten',            neu: 'Mitgliederakte digital.'},
                {old: 'Verstreute Notizen',      neu: 'Wiki und Wissensbasis.'},
            ],
        },
        federation: {
            eyebrow: 'Vernetzung',
            headline: 'Was eine Wehr allein nicht kann.',
            lede: 'Ember-Instanzen können sich miteinander verbinden. Termine, Material und Wissen lassen sich teilen - ohne dass jemand etwas doppelt eintragen muss. Jede Wehr behält ihre Daten, aber zusammen geht mehr.',
            benefits: [
                'Wettkampf bei der Nachbarwache? Steht im eigenen Kalender.',
                'Funkgeräte geliehen? Sichtbar bei beiden Wehren.',
                'Lehrmaterial einmal pflegen, alle nutzen.',
                'Gemeinsame Übungen ohne Doppel-Eintragerei.',
            ],
        },
        usability: {
            eyebrow: 'Bedienbarkeit',
            headline: 'Damit es niemand erklären muss.',
            lede: 'Ehrenamtliche haben nach dem Job wenig Zeit für Schulungen. Ember bringt die Hilfe direkt mit, passt sich der Wache an, und funktioniert auf dem Handy genauso wie am PC.',
            help: {
                aria: 'Beispiel: Eingebettete Hilfe',
                crumbRoot: 'Hilfe',
                crumbSection: 'Anwesenheit',
                eyebrow: 'Schritt für Schritt',
                title: 'So nimmst du Anwesenheit auf.',
                foot: 'Vollständiger Artikel',
                steps: [
                    'Öffne die Übung im Kalender.',
                    'Tippe auf jedes Mitglied: anwesend, abwesend, abgemeldet.',
                    'Bei Bedarf Check-in-Uhrzeit setzen.',
                    'Fertig - die Statistik schreibt sich selbst.',
                ],
            },
            points: [
                'Hilfe-Center: zu jeder Seite ein eigener Artikel.',
                'Mobil-first - zur Übung reicht das Handy.',
                'Themes pro Wache, bei Bedarf pro Mitglied.',
                'Dunkler Modus für späte Übungen und müde Augen.',
                'Tour beim ersten Login zeigt, wo was steht.',
            ],
        },
        facts: {
            items: [
                {label: 'Lizenz',    value: 'AGPL-3.0',                  note: 'Frei nutzbar. Wer Ember verändert, muss seine Änderungen ebenfalls offen teilen.'},
                {label: 'Hosting',   value: 'Selbst auf eurem Server',   note: 'Oder bei einem Partner - oder bei uns.'},
                {label: 'Quelltext', value: 'Offen auf GitHub',          note: 'Jeder darf reinschauen, prüfen, mitbauen.'},
                {label: 'Daten',     value: 'Bleiben bei euch',          note: 'Kein Tracking, keine Drittparteien, keine Profilbildung.'},
            ],
        },
        closing: {
            title: 'Bereit, wenn ihr es seid.',
            linkHelp: 'Hilfe-Center →',
        },
    },
    login: {
        title: 'Anmelden',
        identifier: 'E-Mail oder Benutzername',
        password: 'Passwort',
        submit: 'Anmelden',
        trustedDevice: 'Auf diesem Gerät angemeldet bleiben',
        trustedDeviceHint: 'Nur auf Geräten, die dir gehören.',
        forgotPassword: 'Passwort vergessen?',
        applyForStation: 'Neue Wache beantragen',
        storageDenied: 'Du musst der Speicherung zustimmen, um dich anzumelden.',
        incomplete: 'Unvollständig',
        withPasskey: 'Mit Passkey anmelden',
        passkeyFailed: 'Die Anmeldung mit Passkey hat nicht geklappt. Du kannst dich weiter mit deinem Passwort anmelden.',
        passkeyElsewhere: 'Mit einem anderen Gerät anmelden',
        passwordOff: 'Die Anmeldung mit Passwort ist für dieses Konto ausgeschaltet. Melde dich mit '
            + 'deinem Passkey an, oder setze dein Passwort zurück, um sie wieder einzuschalten.',
    },
    passkeys: {
        // Ein Wort, überall: Passkey. Der erklärende Satz ist immer derselbe.
        explainer: 'Ein Passkey ist eine Anmeldung ohne Passwort. Dein Gerät fragt dich nach Fingerabdruck, '
            + 'Gesicht oder deiner Geräte-PIN, und das war es.',
        defaultLabel: 'Passkey',
        errors: {
            notAllowed: 'Abgebrochen oder zu lange gewartet. Es hat sich nichts geändert.',
            alreadyRegistered: 'Auf diesem Gerät gibt es für dein Konto schon einen Passkey.',
            notSupported: 'Dieses Gerät kann keine Passkeys. Melde dich wie bisher mit deinem Passwort an.',
            security: 'Die Adresse dieser Seite passt nicht. Wende dich an deine Wehrführung.',
            aborted: 'Abgebrochen.',
            generic: 'Das hat nicht geklappt. Dein Passwort funktioniert weiter.',
            foreignCredential: 'Das war der Passkey eines anderen Kontos. Wähle beim Ausprobieren deinen eigenen.',
        },
        offer: {
            title: 'Beim nächsten Mal ohne Passwort anmelden',
            body: 'Dein Gerät fragt dich dann nach Fingerabdruck, Gesicht oder deiner Geräte-PIN. '
                + 'Das geht schneller und ist sicherer als ein Passwort.',
            bulletBiometrics: 'Dein Fingerabdruck bleibt auf deinem Gerät. Wir bekommen ihn nie zu sehen.',
            bulletPassword: 'Dein Passwort funktioniert weiter. Du kannst dich jederzeit wie bisher anmelden.',
            bulletRemovable: 'Du kannst den Passkey jederzeit wieder löschen.',
            accept: 'Passkey einrichten',
            later: 'Später',
            decline: 'Nein danke',
        },
        section: {
            title: 'Anmeldung',
            empty: 'Du hast noch keinen Passkey.',
            create: 'Passkey einrichten',
            labelPlaceholder: 'Name, z. B. "Mein Handy"',
            createdAt: 'Angelegt am {date}',
            lastUsedAt: 'zuletzt verwendet am {date}',
            notTried: 'noch nicht ausprobiert',
            staleUnused: 'Nie verwendet. Vielleicht wurde er auf einem Gerät angelegt, das du nicht mehr hast.',
            rename: 'Umbenennen',
            renameTitle: 'Passkey umbenennen',
            remove: 'Löschen',
            removeTitle: 'Passkey löschen',
            removeConfirm: 'Passkey "{label}" wirklich löschen? Auf dem Gerät, das ihn hält, wird er dann nicht mehr angeboten.',
            removeRefusedNoPassword: 'Das ist der einzige Weg in dein Konto. Lass dich zuerst neu onboarden, dann kannst du diesen Passkey löschen.',
            passwordReenabled: 'Dein letzter Passkey ist gelöscht. Die Anmeldung mit Passwort ist wieder eingeschaltet.',
            askWithPassword: 'Beim Anmelden mit Passwort zusätzlich nach dem Passkey fragen',
            askWithPasswordHint: 'Dein Passwort allein reicht dann nicht mehr. Nur einschalten, wenn du deinen Passkey immer dabei hast.',
            passwordLogin: 'Anmeldung mit Passwort',
            passwordLoginHint: 'Ausgeschaltet meldet dich nur noch dein Passkey an. Dein Passwort bleibt gespeichert; '
                + '"Passwort vergessen" schaltet die Anmeldung wieder ein.',
            passwordOffDone: 'Die Anmeldung mit Passwort ist ausgeschaltet. Dein Weg hinein ist jetzt dein Passkey.',
        },
        enroll: {
            title: 'Passkey einrichten',
            codePrompt: 'Gib den Code ein, der dir gezeigt oder geschickt wurde.',
            unknownCode: 'Diesen Code gibt es nicht oder er ist abgelaufen. Lass dir einen neuen geben.',
            confirm: 'Du richtest einen Passkey für das Konto von {name} ein.',
            setPasswordInstead: 'Stattdessen ein Passwort setzen',
        },
        code: {
            title: 'Passkey über Code einrichten',
            qrAlt: 'QR-Code zum Einrichten des Passkeys',
            hint: 'Der Code lebt fünf Minuten und ist einmal verwendbar. Er stirbt, wenn du diesen Bildschirm verlässt.',
            issue: 'Code anzeigen',
            guardianHint: 'Zeig den QR-Code dem Mitglied: Es scannt ihn mit dem eigenen Gerät, bestätigt, '
                + 'und der Passkey ist angelegt. Ohne Kamera tippt es den kurzen Code auf der Einrichtungsseite ein.',
            managerHint: 'Für ein Mitglied ohne eigene Adresse und ohne Erziehungsberechtigte: Zeig den QR-Code '
                + 'im Raum, das Mitglied scannt ihn und legt seinen Passkey an.',
        },
        onboardAgain: {
            title: 'Zugang',
            hint: 'Erneut onboarden schaltet alle Passkeys des Mitglieds ab, beendet alle Sitzungen und '
                + 'schickt einen frischen Einrichtungslink dorthin, wo Post über dieses Konto ohnehin hingeht.',
            button: 'Erneut onboarden',
            mailed: 'Der Einrichtungslink ist unterwegs.',
            unreachable: 'Niemand ist per Mail erreichbar. Der Weg ist der QR-Code hier im Raum.',
        },
        device: {
            title: 'Dieses Gerät freischalten',
            instruction: 'Scanne diesen QR-Code mit einem Gerät, auf dem du schon angemeldet bist. '
                + 'Dort wirst du nach der Zahl gefragt, die unten steht.',
            qrAlt: 'QR-Code zum Freischalt-Bildschirm',
            identifierLabel: 'E-Mail-Adresse oder Benutzername',
            identifierPlaceholder: "name{'@'}beispiel.de",
            identifierHint: 'Damit weiß das andere Gerät, für wen die Anfrage gilt. Nur dieses Konto kann '
                + 'sie freischalten.',
            numberIntro: 'Deine Zahl:',
            codeFallback: 'Falls du den QR-Code nicht scannen kannst, gib diesen Code ein:',
            waiting: 'Warte auf die Freischaltung ...',
            waitingThrottled: 'Es wurde gerade sehr oft nachgefragt. Die Anfrage läuft weiter, es dauert '
                + 'nur einen Moment länger.',
            rejected: 'Auf dem anderen Gerät wurde die falsche Zahl gewählt.',
            rejectedHint: 'Aus Sicherheitsgründen ist diese Anfrage damit beendet. Fordere eine neue an '
                + 'und achte darauf, die Zahl von diesem Bildschirm zu wählen.',
            signingIn: 'Du wirst angemeldet ...',
            signedInAs: 'Angemeldet als {name}.',
            expired: 'Die Anfrage ist abgelaufen oder wurde nicht freigeschaltet.',
            requestFailed: 'Die Anfrage konnte nicht erstellt werden. Versuche es später noch einmal.',
            signInFailed: 'Die Anmeldung hat nicht geklappt. Fordere einen neuen Code an.',
            retry: 'Neuen Code anfordern',
            backToLogin: 'Zurück zur Anmeldung',
            chooseIntro: 'Wie möchtest du dich auf diesem Gerät anmelden?',
            choosePasskey: 'Passkey auf diesem Gerät anlegen',
            choosePasskeyHint: 'Empfohlen für dein eigenes Gerät. Danach meldest du dich hier ohne ein '
                + 'zweites Gerät an.',
            chooseSignIn: 'Nur anmelden, nichts speichern',
            chooseSignInHint: 'Für ein geliehenes oder fremdes Gerät. Es bleibt nichts zurück, und die '
                + 'Anmeldung endet mit der Sitzung.',
            instructionSignIn: 'Scanne diesen QR-Code mit einem Gerät, auf dem du schon angemeldet bist. '
                + 'Dort wirst du nach der Zahl gefragt, die unten steht. Auf diesem Gerät wird nichts '
                + 'gespeichert.',
        },
        approve: {
            title: 'Neues Gerät freischalten',
            hint: 'Scanne den QR-Code des anderen Geräts, oder gib den Code hier ein. Danach siehst du, '
                + 'was du freischaltest, und wählst die Zahl, die auf dem anderen Gerät steht.',
            lookup: 'Code prüfen',
            numberTitle: 'Welche Zahl steht auf dem anderen Gerät?',
            numberHint: 'Sieh auf den Bildschirm des Geräts, das hereinkommen möchte, und wähle die Zahl, '
                + 'die dort steht.',
            numberWrongWarning: 'Wenn du die Zahl nicht sehen kannst, brich ab. Wer dir einen QR-Code '
                + 'geschickt hat, kann dir die Zahl nicht zeigen.',
            wrongNumber: 'Das war die falsche Zahl. Die Anfrage ist damit beendet; das andere Gerät muss '
                + 'eine neue stellen.',
            wrongNumberGuidance: 'Hier ist nichts mehr zu tun. Wenn du die Anfrage selbst gestellt hast, '
                + 'starte sie auf dem anderen Gerät neu und achte auf die Zahl, die dort steht.',
            unknownCode: 'Diesen Code gibt es nicht oder er ist abgelaufen.',
            unknownCodeGuidance: 'Prüfe den Code auf Tippfehler. Codes laufen nach kurzer Zeit ab, lass dir '
                + 'also notfalls auf dem anderen Gerät einen neuen geben.',
            unknownDevice: 'Unbekanntes Gerät',
            place: 'Ort: {place}',
            when: 'Angefragt am {when}',
            warning: 'Nur freischalten, wenn du gerade selbst an diesem Gerät sitzt.',
            approve: 'Freischalten',
            done: 'Freigeschaltet. Das andere Gerät macht jetzt weiter.',
            another: 'Weiteren Code eingeben',
            purposePasskey: 'Das Gerät legt danach einen Passkey für dein Konto an.',
            purposeSignIn: 'Das Gerät wird danach in deinem Namen angemeldet, bis die Sitzung dort '
                + 'beendet wird. Es wird nichts auf dem Gerät gespeichert.',
            purposeStepUp: 'Du bestätigst damit eine sensible Aktion auf dem anderen Gerät.',
            purposeStepUpCategory: 'Du bestätigst damit auf dem anderen Gerät: {category}',
            purposeStepUpFor: 'Diese Bestätigung gehört nicht zu deinem eigenen Konto, sondern zu {name}. Bestätige nur, wenn diese Person gerade neben dir darum gebeten hat.',
            signInFor: 'Wen meldest du an?',
        },
        create: {
            preparing: 'Gleich fragt dich dein Gerät nach Fingerabdruck, Gesicht oder PIN. Bestätige einfach.',
            cancelled: 'Kein Problem, es hat sich nichts geändert. Du kannst das jederzeit wieder versuchen.',
            trialPrompt: 'Und jetzt probieren wir ihn einmal aus.',
            trialRun: 'Ausprobieren',
            trialSkip: 'Später ausprobieren',
            trialOk: 'Passt. Beim nächsten Mal meldest du dich genau so an.',
            trialSkipped: 'Der Passkey ist angelegt, aber noch nicht ausprobiert. Dein Passwort funktioniert weiter.',
            done: 'Fertig',
        },
    },
    forgotPassword: {
        title: 'Passwort vergessen',
        hint: 'Gib deine E-Mail-Adresse oder deinen Benutzernamen ein und wir senden einen Link zum '
            + 'Zurücksetzen. Meldest du dich mit einem Benutzernamen an, geht der Link an deine '
            + 'Aufsichtsperson.',
        email: 'E-Mail oder Benutzername',
        submit: 'Link senden',
        sent: 'Falls es dazu ein Konto gibt, wurde ein Link zum Zurücksetzen gesendet.',
        backToLogin: 'Zurück zur Anmeldung',
    },
    changePassword: {
        newPasswordTooShort: 'Das neue Passwort muss mindestens 12 Zeichen lang sein.',
        newPasswordBreached: 'Das neue Passwort wurde in bekannten Datenlecks gefunden. Bitte wähle ein anderes.',
        noPasswordSet: 'Für dieses Konto ist kein Passwort hinterlegt. Melde dich mit deinem Passkey an, '
            + 'oder setze über "Passwort vergessen" eines.',
        currentPasswordWrong: 'Das aktuelle Passwort ist falsch.',
    },
    setPassword: {
        title: 'Neues Passwort festlegen',
        newPassword: 'Neues Passwort',
        confirmPassword: 'Passwort bestätigen',
        submit: 'Passwort festlegen',
        mismatch: 'Die Passwoerter stimmen nicht überein.',
        required: 'Passwort ist erforderlich.',
        passwordTooShort: 'Das Passwort muss mindestens 12 Zeichen lang sein.',
        passwordBreached: 'Dieses Passwort wurde in bekannten Datenlecks gefunden. Bitte wähle ein anderes.',
        tokenInvalid: 'Der Link ist ungültig oder abgelaufen. Logge dich erneut ein, um einen neuen Link zu bekommen.',
        tokenExpired: 'Dieser Link ist abgelaufen.',
        passwordlessMode: 'Diese Instanz vergibt keine Passwörter mehr. Dein Weg hinein ist ein Passkey; '
            + 'lass dich von deiner Wache erneut onboarden.',
        expiredTitle: 'Der Link ist abgelaufen',
        expiredText: 'Ein Einrichtungslink gilt nur eine begrenzte Zeit. Deiner ist inzwischen '
            + 'verfallen, dein Konto gibt es aber weiterhin. Du brauchst nur einen neuen Link.',
        unknownTitle: 'Der Link führt ins Leere',
        unknownText: 'Zu diesem Link gehört nichts mehr. Entweder wurde er bereits benutzt, oder er '
            + 'ist beim Kopieren unvollständig geblieben.',
        askYourStation: 'Wende dich an die Administration deiner Wache, sie kann dir eine neue '
            + 'Einrichtungs-Mail schicken.',
        askAgainYourself: 'Du kannst dir selbst einen neuen Link schicken lassen.',
        requestAnother: 'Neuen Link anfordern',
        backToLogin: 'Zur Anmeldung',
    },
    setAddress: {
        title: 'E-Mail-Adresse hinterlegen',
        hint: 'Dieses Konto verwaltet die Instanz, ist aber unter keiner Adresse erreichbar. Ohne '
            + 'eine erreichbare Adresse kommt weder eine Passwortzurücksetzung noch eine '
            + 'Sicherheitsmeldung bei dir an.',
        email: 'E-Mail-Adresse',
        placeholder: "name{'@'}beispiel.de",
        submit: 'Adresse hinterlegen und anmelden',
        malformed: 'Das ist keine gültige E-Mail-Adresse.',
        unreachable: 'An diese Adresse kann nichts zugestellt werden. Bitte hinterlege eine Adresse, '
            + 'unter der du wirklich erreichbar bist.',
        taken: 'Diese E-Mail-Adresse gehört bereits zu einem anderen Konto.',
        tokenInvalid: 'Der Vorgang ist ungültig. Melde dich erneut an, um es noch einmal zu versuchen.',
        tokenExpired: 'Der Vorgang ist abgelaufen. Melde dich erneut an, um es noch einmal zu versuchen.',
    },
    storageConsent: {
        title: 'Einwilligung zur Datenverarbeitung',
        description: 'Diese Anwendung speichert ein Sitzungs-Token und Einstellungen im lokalen Speicher deines Browsers, um dich angemeldet zu halten. Deine Daten werden ausschließlich zur Organisation deiner Jugendgruppe verwendet. Wir geben keine personenbezogenen Daten an Dritte weiter.',
        accept: 'Zustimmen',
        deny: 'Ablehnen',
        privacyPolicy: 'Datenschutzerklärung lesen',
        privacyPolicyTitle: 'Datenschutzerklärung',
        tos: 'Nutzungsbedingungen lesen',
        tosTitle: 'Nutzungsbedingungen',
        settingsTitle: 'Speicherung im Browser',
        settingsHint: 'Du entscheidest, was diese Anwendung auf deinem Gerät ablegen darf. '
            + 'Nimmst du eine Gruppe zurück, werden ihre Werte sofort gelöscht.',
        listLink: 'Alle gespeicherten Werte ansehen',
        scope: {
            always: 'Immer aktiv',
            REQUIRED: {
                title: 'Technisch erforderlich',
                hint: 'Anmeldung, Sitzung und die Einwilligung selbst. Ohne diese Werte kannst du dich '
                    + 'nicht anmelden.',
            },
            FUNCTIONAL: {
                title: 'Für einzelne Funktionen',
                hint: 'Entsteht erst, wenn du die jeweilige Funktion nutzt, etwa die Zugangsdaten für '
                    + 'die KI-Unterstützung. Ohne sie bleibt der Rest bedienbar.',
            },
            COMFORT: {
                title: 'Einstellungen der Ansicht',
                hint: 'Merkt sich Farbschema, eingeklappte Seitenleiste und Ansichtsoptionen. Ohne sie '
                    + 'startet die Anwendung jedes Mal mit ihren Vorgaben.',
            },
        },
    },
    reconsent: {
        title: 'Aktualisierte Bedingungen',
        description: 'Seit deiner letzten Zustimmung wurden die Datenschutzerklärung oder Nutzungsbedingungen aktualisiert. Bitte lies die Änderungen und stimme erneut zu, um fortzufahren.',
        privacyChanged: 'Datenschutzerklärung geändert',
        tosChanged: 'Nutzungsbedingungen geändert',
        whatChanged: 'Was hat sich geändert?',
        showFullText: 'Vollständigen Text anzeigen',
        hideFullText: 'Vollständigen Text ausblenden',
        consentNote: 'Mit der Zustimmung bestätigst du, dass du die aktualisierten Dokumente gelesen hast und ihnen zustimmst.',
        accept: 'Ich stimme zu',
    },
    adminNews: {
        intro: 'Eine Systemmeldung wird einmal geschrieben und in jeder Wache gelesen. Sie steht dort in der normalen Neuigkeitenliste, mit Ember als Absender.',
        create: 'Systemmeldung schreiben',
        createTitle: 'Neue Systemmeldung',
        editTitle: 'Systemmeldung bearbeiten',
        entryTitle: 'Titel',
        entryTitlePlaceholder: 'Titel der Systemmeldung',
        content: 'Inhalt',
        contentPlaceholder: 'Was alle Wachen wissen sollen …',
        enableBlocks: 'Mit dem Seiten-Editor schreiben',
        enableBlocksHint: 'Für längere Meldungen mit Bildern, Spalten und Hinweisboxen. Der Wechsel gilt nur in eine Richtung.',
        audience: 'Für wen',
        audienceHint: 'Ohne Auswahl liest sie jeder. Gruppen, Tags und einzelne Mitglieder gibt es nur innerhalb einer Wache, die Meldung steht aber in allen.',
        notify: 'Benachrichtigen',
        notifyHint: 'Ohne Häkchen finden die Mitglieder die Meldung beim nächsten Blick in die Neuigkeiten.',
        notifyEditHint: 'Eine Korrektur benachrichtigt nicht noch einmal.',
        publish: 'Veröffentlichen',
        draft: 'Entwurf',
        forEveryone: 'Für alle sichtbar',
        commentCount: '{count} Kommentare',
        retract: 'Zurückziehen',
        retractMessage: 'Soll „{title}“ aus allen Wachen zurückgezogen werden? Die Meldung und die Kommentare darunter verschwinden überall.',
        empty: 'Noch keine Systemmeldung geschrieben.',
    },
    pageModule: {
        attendance: 'Anwesenheit',
        boards: 'Boards',
        checklists: 'Checklisten',
        dashboard: 'Dashboard',
        discovery: 'Verzeichnis',
        events: 'Termine',
        federation: 'Föderation',
        forms: 'Umfragen',
        inventory: 'Inventar',
        knowledge: 'Wiki',
        lostAndFound: 'Fundbüro',
        manage: 'Verwalten',
        media: 'Medien',
        members: 'Mitglieder',
        monitoring: 'Monitoring',
        news: 'Neuigkeiten',
        pages: 'Öffentliche Seiten',
        procedures: 'Abläufe',
        profile: 'Profil',
        protocols: 'Prüfungsbögen',
        quiz: 'Quiz',
        requirements: 'Aufgaben',
        setup: 'Einrichtung',
        account: 'Konto',
        admin: 'Admin Panel',
        cluster: 'Verband',
        helpCenter: 'Hilfe',
        applications: 'Bewerbungen',
        stations: 'Wachen',
        storage: 'Speicher',
        team: 'Team',
    },
    sidebar: {
        collapse: 'Seitenleiste einklappen',
        expand: 'Seitenleiste ausklappen',
        openMenu: 'Menü öffnen',
        stationMoved: 'Wache verschoben',
        dashboard: 'Dashboard',
        overview: 'Übersicht',
        statistics: 'Statistiken',
        requirements: 'Aufgaben',
        profile: 'Profil',
        myInventory: 'Mein Inventar',
        managedProfiles: 'Verwaltete Profile',
        absences: 'Abwesenheit',
        station: 'Wache',
        manage: 'Verwalten',
        members: 'Mitglieder',
        create: 'Anlegen',
        list: 'Liste',
        detail: 'Details',
        edit: 'Bearbeiten',
        groups: 'Gruppen',
        changes: 'Änderungen',
        inventory: 'Inventar',
        inventoryManage: 'Inventare',
        inventoryFlows: 'Abläufe',
        inventorySettings: 'Einstellungen',
        inventoryLendingGroup: 'Verleih',
        inventoryStorage: 'Lager',
        inventoryBorrowed: 'Geliehenes',
        inventoryAssign: 'Zuweisen',
        inventoryRequirements: 'Benötigt',
        inventoryCheck: 'Prüfung',
        inventoryCheckMember: 'Mitglieder-Prüfung',
        inventoryCheckContainer: 'Behälter-Prüfung',
        inventoryCheckResult: 'Prüf-Ergebnis',
        inventoryMovements: 'Vorgänge',
        inventoryMembers: 'Mitglieder',
        inventoryMemberDetail: 'Mitglieder-Inventar',
        inventoryDetail: 'Inventar-Details',
        inventoryEdit: 'Inventar bearbeiten',
        inventoryProcurement: 'Beschaffung',
        inventoryLending: 'Ausleihe',
        inventoryLendingShares: 'Ausleih-Angebot',
        tags: 'Tags',
        documents: 'Dokumente',
        typePermissions: 'Typberechtigungen',
        formerMembers: 'Ehemalige',
        waitingLists: 'Wartelisten',
        attendance: 'Anwesenheit',
        newAttendance: 'Neue Anwesenheit',
        pastAttendance: 'Vergangene Anwesenheiten',
        attendanceSession: 'Erfassung',
        attendanceReport: 'Bericht',
        attendanceConfig: 'Anwesenheits Konfiguration',
        attendanceConfigEdit: 'Vorlage bearbeiten',
        membersConfig: 'Mitglieder Konfiguration',
        federation: 'Föderation',
        federationSettings: 'Einstellungen',
        discovery: 'Verzeichnis',
        membersImport: 'CSV Import',
        membersImportTeam: 'Team Import',
        events: 'Termine',
        upcomingEvents: 'Termine',
        pendingRegistrations: 'Anmeldungen',
        manageEvents: 'Verwalten',
        eventEdit: 'Termin bearbeiten',
        eventDetail: 'Termin-Details',
        eventCategories: 'Kategorien',
        eventTemplates: 'Vorlagen',
        eventBatch: 'Mehrfach erstellen',
        forms: 'Umfragen',
        formsCreate: 'Erstellen',
        formsEdit: 'Bearbeiten',
        formsFill: 'Ausfüllen',
        formsAnalytics: 'Auswertung',
        lostAndFound: 'Fundbüro',
        checklists: 'Checklisten',
        quiz: 'Quiz',
        quizAndProtocols: 'Quiz & Prüfungen',
        quizCatalogs: 'Fragenkataloge',
        quizCatalogDetail: 'Katalog-Details',
        quizTests: 'Tests',
        quizTestDetail: 'Test-Details',
        quizTraining: 'Training',
        quizCatalogGenerate: 'Fragen generieren',
        quizCatalogImport: 'Fragen importieren',
        quizTestCreate: 'Test erstellen',
        quizTestEdit: 'Test bearbeiten',
        quizTestTake: 'Test schreiben',
        quizTestEvaluate: 'Test auswerten',
        protocols: 'Prüfungsbögen',
        protocolsGroup: 'Prüfungen',
        protocolRuns: 'Prüfungsläufe',
        protocolDetail: 'Protokoll-Details',
        protocolRunDetail: 'Lauf-Details',
        protocolEvaluation: 'Auswertung',
        protocolGrade: 'Mitglied prüfen',
        quizAi: 'KI-Generierung',
        knowledgeBase: 'Wiki',
        knowledgeEditor: 'Texteditor',
        news: 'Neuigkeiten',
        newsEdit: 'Neuigkeit bearbeiten',
        settings: 'Einstellungen',
        theming: 'Erscheinungsbild',
        sessions: 'Sitzungen & Konto',
        notifications: 'Benachrichtigungen',
        security: 'Sicherheit',
        securityTokens: 'Tokens & Sitzungen',
        securityHibp: 'HIBP',
        securityTwoFactor: 'Zwei-Faktor',
        twoFactor: 'Zwei-Faktor verwalten',
        accountAvatar: 'Profil',
        accountTheming: 'Erscheinungsbild',
        accountSessions: 'Sitzungen',
        accountSecurity: 'Sicherheit',
        accountGdpr: 'DSGVO',
        stationTheme: 'Erscheinungsbild',
        stationMailing: 'E-Mail',
        stationMailImport: 'Post-Import',
        stationModules: 'Module',
        stationImport: 'Import / Export',
        stations: 'Wachen',
        manageStations: 'Wachen verwalten',
        clusters: 'Verbände',
        manageClusters: 'Verbände verwalten',
        stationCluster: 'Verband',
        applications: 'Anträge',
        systemNews: 'Systemmeldungen',
        general: 'Allgemein',
        mailing: 'E-Mail',
        legal: 'Rechtliches',
        storage: 'Speicher',
        storageDashboard: 'Speicher-Übersicht',
        monitoring: 'Monitoring',
        problemLog: 'Fehlerprotokoll',
        beacon: 'Beacon',
        beaconSettings: 'Beacon einrichten',
        beaconFaults: 'Fehlerprotokoll',
        beaconReports: 'Problemmeldungen',
        beaconMetrics: 'Zahlen der Instanzen',
        mailLog: 'Mailprotokoll',
        applicationLog: 'Anwendungslog',
        problemReports: 'Problemmeldungen',
        apiStatus: 'API-Status',
        feedMetrics: 'Feed-Telemetrie',
        adminTraffic: 'Traffic',
        stationTraffic: 'Traffic',
        stationInsights: 'Seiten-Statistik',
        stationFeeds: 'Feeds',
        stationSecurity: 'Sicherheit',
        adminDiscovery: 'Discovery-Netzwerk',
        maps: 'Karten',
        devTools: 'Dev-Tools',
        dataTracking: 'Data Tracking',
        boards: 'Boards',
        boardManage: 'Verwalten',
        procedures: 'Abläufe',
        procedureTemplates: 'Vorlagen',
        pages: 'Öffentliche Seiten',
        media: 'Medien',
        pagesForms: 'Kontaktformulare',
        pagesPolls: 'Umfragen',
        storageBackend: 'Speicher-Backend',
        storageAudit: 'Speicher-Audit',
    },
    pages: {
        'station-moved-delete': {
            title: 'Lokale Kopie löschen',
            subtitle: 'Schreibgeschützte Kopie dieser Wache endgültig entfernen',
        },
        'station-moved': {
            title: 'Wache verschoben',
            subtitle: 'Diese Wache ist auf eine andere Instanz umgezogen',
            headline: 'Diese Wache wurde verschoben',
            body: 'Die Daten dieser Wache wurden auf eine andere Ember-Instanz übertragen. Auf dieser Instanz ist die Wache schreibgeschützt und nicht mehr nutzbar.',
            action: 'Zur neuen Instanz wechseln',
            hint: 'Bitte melde dich auf der neuen Instanz erneut an. Dein Konto und deine Daten findest du dort wieder.',
            tabInfo: 'Information',
            tabDelete: 'Lokale Kopie löschen',
            deleteHeadline: 'Lokale Kopie löschen',
            deleteBody: 'Die produktiven Daten dieser Wache liegen jetzt auf der Zielinstanz. Auf dieser Instanz bleibt nur eine schreibgeschützte Kopie. Du kannst sie hier endgültig löschen.',
            deleteWarning: 'Diese Aktion ist nicht rückgängig zu machen.',
            deleteConfirm: 'Lokale Kopie endgültig löschen',
            deleteSuccess: 'Lokale Kopie gelöscht.',
            deleteError: 'Löschen fehlgeschlagen.',
            deleteModalTitle: 'Bist du sicher?',
            deleteModalBody: 'Die lokale Kopie wird sofort und unwiderruflich entfernt. Die produktiven Daten auf der Zielinstanz bleiben unverändert.',
            deleteModalTypeName: 'Gib zur Bestätigung den Wachnamen "{name}" ein.',
            deleteModalCancel: 'Abbrechen',
            deleteModalConfirm: 'Endgültig löschen',
        },
        'profile': {
            title: 'Mein Profil',
            subtitle: 'Deine Profildaten bearbeiten',
        },
        'profile-absences': {
            title: 'Abwesenheit',
            subtitle: 'Abwesenheiten verwalten',
        },
        'profile-managed': {
            title: 'Verwaltete Profile',
            subtitle: 'Profile deiner zugewiesenen Mitglieder bearbeiten',
        },
        'forms-list': {
            title: 'Umfragen',
            subtitle: 'Umfragen der Wache',
        },
        'forms-create': {
            title: 'Umfrage erstellen',
            subtitle: 'Neue Umfrage anlegen',
        },
        'forms-edit': {
            title: 'Umfrage bearbeiten',
            subtitle: 'Umfrage anpassen',
        },
        'forms-fill': {
            title: 'Umfrage ausfüllen',
            titleNamed: '{name} ausfüllen',
            subtitle: 'Deine Antworten eingeben',
        },
        'forms-analytics': {
            title: 'Auswertung',
            titleNamed: '{name} Auswertung',
            subtitle: 'Ergebnisse und Statistiken',
        },
        'procedure-list': {
            title: 'Abläufe',
            subtitle: 'Offene und abgeschlossene Abläufe',
        },
        'procedure-create': {
            title: 'Neuer Ablauf',
            subtitle: 'Ablauf erstellen',
        },
        'procedure-edit': {
            title: 'Ablauf bearbeiten',
            subtitle: 'Schritte und Zuweisungen anpassen',
        },
        'procedure-detail': {
            title: 'Ablauf-Details',
            subtitle: 'Checkliste und Fortschritt',
        },
        'procedure-template-list': {
            title: 'Ablauf-Vorlagen',
            subtitle: 'Vorlagen verwalten',
        },
        'procedure-template-edit': {
            title: 'Vorlage bearbeiten',
            subtitle: 'Vorlage konfigurieren',
        },
        'board-overview': {
            title: 'Planer',
            subtitle: 'Deine Boards',
        },
        'board-manage': {
            title: 'Boards verwalten',
            subtitle: 'Boards erstellen und verwalten',
        },
        'board-view': {
            title: 'Board',
            subtitle: '',
        },
        'board-backlog': {
            title: 'Backlog',
            titleNamed: '{name} Backlog',
            subtitle: '',
        },
        'board-archived': {
            title: 'Archiv',
            titleNamed: '{name} Archiv',
            subtitle: '',
        },
        'board-settings': {
            title: 'Board-Einstellungen',
            titleNamed: '{name} Einstellungen',
            subtitle: 'Board konfigurieren',
        },
        'ticket-detail': {
            title: 'Ticket',
            subtitle: '',
        },
        'dashboard-overview': {
            title: 'Übersicht',
            subtitle: 'Alle Informationen auf einen Blick',
        },
        'dashboard-statistics': {
            title: 'Statistiken',
            subtitle: 'Auswertungen und Analysen',
        },
        'station-manage': {
            title: 'Wache verwalten',
            subtitle: 'Einstellungen und Konfiguration',
        },
        'members-create': {
            title: 'Mitglied anlegen',
            subtitle: 'Neues Mitglied einladen',
        },
        'members-list': {
            title: 'Mitgliederliste',
            subtitle: 'Alle Mitglieder der Wache',
        },
        'members-detail': {
            title: 'Mitglied',
            subtitle: 'Detailansicht',
        },
        'members-edit': {
            title: 'Mitglied bearbeiten',
            titleNamed: '{name} bearbeiten',
            subtitle: 'Rollen und Profilfelder bearbeiten',
        },
        'members-groups': {
            title: 'Gruppen',
            subtitle: 'Mitgliedergruppen verwalten',
        },
        'members-type-permissions': {
            title: 'Typberechtigungen',
            subtitle: 'Berechtigungen pro Mitgliedstyp festlegen',
        },
        'members-changes': {
            title: 'Änderungen',
            subtitle: 'Offene Profiländerungen prüfen',
        },
        'members-tags': {
            title: 'Tags',
            subtitle: 'Mitglieder-Tags verwalten',
        },
        'members-former': {
            title: 'Ehemalige Mitglieder',
            subtitle: 'Ehemalige Mitglieder einsehen und reaktivieren',
        },
        'inventory-overview': {
            title: 'Inventar',
            subtitle: 'Übersicht aller Inventare',
        },
        'inventory-movements': {
            title: 'Vorgänge',
            subtitle: 'Was gerade unterwegs ist, und wer als Nächstes dran ist',
        },
        'inventory-procurement': {
            title: 'Beschaffung',
            subtitle: 'Ausrüstungsbeschaffung verwalten',
        },
        'inventory-lending': {
            title: 'Ausleihe',
            subtitle: 'Ausrüstungsausleihe zwischen Wachen',
        },
        'inventory-lending-request': {
            title: 'Ausleih-Anfrage',
            titleNamed: '{name} Ausleih-Anfrage',
            subtitle: 'Details und Kommunikation',
        },
        'inventory-lending-blocks': {
            title: 'Ausleih-Sperrzeiten',
            subtitle: 'Zeiträume für Ausleihe sperren',
        },
        'inventory-lending-shares': {
            title: 'Ausleih-Angebot',
            subtitle: 'Was diese Wache ihren Partnern anbietet',
        },
        'inventory-lending-browse': {
            title: 'Ausleih-Anfrage erstellen',
            subtitle: 'Inventar von Partnerwachen anfragen',
        },
        'inventory-lending-collect': {
            title: 'Ausrüstung zusammenstellen',
            titleNamed: '{name} Ausrüstung zusammenstellen',
            subtitle: 'Was ein Termin braucht, bei den Partnerwachen zusammensuchen',
        },
        'inventory-my': {
            title: 'Mein Inventar',
            subtitle: 'Zugewiesene Ausrüstung',
        },
        'inventory-member': {
            title: 'Mitglieder-Inventar',
            subtitle: 'Zugewiesene Ausrüstung eines Mitglieds',
        },
        'inventory-members': {
            title: 'Inventar Mitglieder',
            subtitle: 'Übersicht aller Mitglieder und deren Inventar',
        },
        'inventory-manage': {
            title: 'Inventar verwalten',
            subtitle: 'Inventare anlegen und bearbeiten',
        },
        'inventory-borrowed': {
            title: 'Geliehene Ausrüstung',
            subtitle: 'Ausrüstung, die einer Partnerwache gehört und gerade hier liegt.',
        },
        'inventory-storage': {
            title: 'Lager',
            subtitle: 'Räume, Schränke, Schubladen und Kisten als verschachtelte Behälter verwalten.',
        },
        'inventory-container-detail': {
            title: 'Behälter',
            subtitle: 'Inhalt, Position und Verlauf eines Behälters.',
        },
        'inventory-assign': {
            title: 'Items zuweisen',
            subtitle: 'Mitglied wählen und Items per Scan ausgeben oder zurücknehmen.',
        },
        'inventory-check-member-overview': {
            title: 'Mitglieder-Prüfung',
            subtitle: 'Items eines Mitglieds gegen die zugewiesenen Items abgleichen.',
        },
        'inventory-check-container-overview': {
            title: 'Behälter-Prüfung',
            subtitle: 'Den Inhalt eines Lagerbehälters per Scan abgleichen.',
        },
        'inventory-check-container-walk': {
            title: 'Behälter-Prüfung',
            titleNamed: '{name} Prüfung',
            subtitle: 'Erwartete Items per Scan bestätigen, Abweichungen markieren.',
        },
        'attendance-session': {
            title: 'Anwesenheit',
            subtitle: 'Anwesenheit erfassen und prüfen',
        },
        'attendance-new': {
            title: 'Neue Anwesenheit',
            subtitle: 'Anwesenheit erfassen',
        },
        'attendance-past': {
            title: 'Vergangene Anwesenheiten',
            subtitle: 'Anwesenheiten einsehen und verwalten',
        },
        'events': {
            title: 'Termine verwalten',
            subtitle: 'Veranstaltungen und Pausen verwalten',
        },
        'events-upcoming': {
            title: 'Termine',
            subtitle: 'Kommende Veranstaltungen',
        },
        'events-registrations': {
            title: 'Anmeldungen',
            subtitle: 'Offene Anmeldungen bearbeiten',
        },
        'news-list': {
            title: 'Neuigkeiten',
            subtitle: 'Aktuelle Nachrichten und Mitteilungen',
        },
        'news-create': {
            title: 'Neuigkeit erstellen',
            subtitle: 'Neue Nachricht verfassen',
        },
        'news-edit': {
            title: 'Neuigkeit bearbeiten',
            titleNamed: '{name} bearbeiten',
            subtitle: 'Nachricht bearbeiten',
        },
        'profile-settings': {
            title: 'Einstellungen',
            subtitle: 'Benachrichtigungen und Präferenzen',
        },
        'station-attendance-config': {
            title: 'Anwesenheits Konfiguration',
            subtitle: 'Anwesenheitsfelder und Einstellungen verwalten',
        },
        'station-members-documents': {
            title: 'Dokumente',
            subtitle: 'Die Dokumentenablage der Wache',
        },
        'station-members-config': {
            title: 'Mitglieder Konfiguration',
            subtitle: 'Mitgliederfelder und Einstellungen verwalten',
        },
        'station-attendance-config-edit': {
            title: 'Anwesenheits Vorlage',
            subtitle: 'Vorlage bearbeiten',
        },
        'admin-overview': {
            title: 'Übersicht',
            subtitle: 'Administration auf einen Blick',
        },
        'admin-statistics': {
            title: 'Statistiken',
            subtitle: 'Systemweite Auswertungen',
        },
        'admin-stations': {
            title: 'Wachen verwalten',
            subtitle: 'Wachen anlegen und bearbeiten',
        },
        'admin-station-edit': {
            title: 'Wache bearbeiten',
            subtitle: 'Wache konfigurieren',
        },
        'admin-storage': {
            title: 'Speicher-Übersicht',
            subtitle: 'Speichernutzung und Kontingente aller Wachen verwalten',
        },
        'station-storage': {
            title: 'Speicher',
            subtitle: 'Speichernutzung dieser Wache',
        },
        'admin-clusters': {
            title: 'Verbände',
            subtitle: 'Verbände anlegen und entfernen',
        },
        'cluster-overview': {
            title: 'Verband',
            subtitle: 'Übersicht über den Verband, für den du gerade arbeitest',
        },
        'cluster-settings': {
            title: 'Verbands-Einstellungen',
            subtitle: 'Name und Beschreibung des Verbands',
        },
        'cluster-stations': {
            title: 'Wachen',
            subtitle: 'Die Wachen, die zu diesem Verband gehören',
        },
        'cluster-applications': {
            title: 'Beitrittsanfragen',
            subtitle: 'Wachen, die dem Verband beitreten möchten',
        },
        'station-manage-cluster': {
            title: 'Verband',
            subtitle: 'Zu welchem Verband diese Wache gehört',
        },
        'cluster-modules': {
            title: 'Module',
            subtitle: 'Welche Module die Wachen des Verbands nutzen dürfen',
        },
        'cluster-look-and-feel': {
            title: 'Erscheinungsbild',
            subtitle: 'Wie die Wachen des Verbands aussehen',
        },
        'cluster-storage': {
            title: 'Speicherplatz',
            subtitle: 'Wie der Verband seinen Speicherplatz verteilt',
        },
        'cluster-storage-backend': {
            title: 'Eigener Speicher',
            subtitle: 'Wo die Dateien des Verbands und seiner Wachen liegen',
        },
        'cluster-members': {
            title: 'Mitglieder',
            subtitle: 'Wer für diesen Verband handelt',
        },
        'cluster-member-groups': {
            title: 'Gruppen',
            subtitle: 'Rechte an mehrere Mitglieder auf einmal geben',
        },
        'cluster-station-groups': {
            title: 'Wachgruppen',
            subtitle: 'Wachen zusammenfassen, die dieselbe Arbeit machen',
        },
        'cluster-member-management': {
            title: 'Mitglieder der Wachen',
            subtitle: 'Die Leute an allen Wachen des Verbands',
        },
        'cluster-member-detail': {
            title: 'Mitglied',
            subtitle: 'Was von dieser Person erfragt wird',
        },
        'cluster-fields': {
            title: 'Profilfelder',
            subtitle: 'Was der Verband über die Mitglieder seiner Wachen wissen möchte',
        },
        'cluster-inventory': {
            title: 'Material',
            subtitle: 'Was dem Verband gehört und wo es gerade ist',
        },
        'cluster-inventory-out': {
            title: 'Bei den Wachen',
            subtitle: 'Material des Verbands, das gerade nicht im eigenen Lager liegt',
        },
        'cluster-inventory-statistics': {
            title: 'Zahlen zum Material',
            subtitle: 'Wie viel der Verband hat und wo es steht',
        },
        'cluster-inventory-settings': {
            title: 'Materialeinstellungen',
            subtitle: 'Was der Verband über Material entscheidet',
        },
        'cluster-inventory-dispatch': {
            title: 'Material ausgeben',
            subtitle: 'Aus dem Lager des Verbands an eine seiner Wachen',
        },
        'cluster-movements': {
            title: 'Offene Schritte',
            subtitle: 'Vorgänge, die auf eine Antwort des Verbands warten',
        },
        'cluster-news': {
            title: 'Neuigkeiten',
            subtitle: 'Was der Verband seinen Wachen mitteilt',
        },
        'cluster-events': {
            title: 'Termine',
            subtitle: 'Termine des Verbands im Kalender aller Wachen',
        },
        'cluster-knowledge': {
            title: 'Wiki',
            subtitle: 'Was der Verband allen Wachen zur Verfügung stellt',
        },
        'admin-settings': {
            title: 'Allgemein',
            subtitle: 'Allgemeine Plattform-Einstellungen',
        },
        'admin-legal': {
            title: 'Rechtliche Dokumente',
            subtitle: 'Datenschutz, Nutzungsbedingungen und Impressum verwalten',
        },
        'admin-mailing': {
            title: 'E-Mail-Konfiguration',
            subtitle: 'E-Mail-Versand und SMTP-Einstellungen',
        },
        'admin-data-tracking': {
            title: 'Data Tracking',
            subtitle: 'Inspiziere und bearbeite data_tracking.json (nur Dev-Modus)',
        },
        'admin-station-import': {
            title: 'Wache importieren',
            titleNamed: '{name} importieren',
            subtitle: 'Wache von einer anderen Instanz übernehmen',
        },
        'admin-station-applications': {
            title: 'Wachen-Anträge',
            subtitle: 'Anträge auf neue Wachen prüfen',
        },
        'admin-security': {
            title: 'Sicherheit',
            subtitle: 'Sicherheitseinstellungen der Instanz',
        },
        'admin-security-tokens': {
            title: 'Token-Peppers',
            subtitle: 'Serverseitige Schlüssel für Token-Speicherung',
        },
        'admin-security-hibp': {
            title: 'Have I Been Pwned',
            subtitle: 'Passwortprüfung gegen bekannte Leaks',
        },
        'admin-security-two-factor': {
            title: 'Zwei-Faktor-Richtlinien',
            subtitle: 'Zwei-Faktor-Anforderungen konfigurieren',
        },
        'admin-two-factor': {
            title: 'Zwei-Faktor-Reset',
            subtitle: 'Zwei-Faktor-Authentifizierung für Konten zurücksetzen',
        },
        'admin-problems': {
            title: 'System-Log',
            subtitle: 'Warnungen und Fehler der laufenden Instanz',
        },
        'admin-beacon': {
            title: 'Beacon',
            subtitle: 'Wohin diese Instanz meldet, und was sie selbst entgegennimmt',
        },
        'admin-beacon-faults': {
            title: 'Fehlerprotokoll anderer Instanzen',
            subtitle: 'Warnungen und Fehler, die andere Instanzen hierher gemeldet haben',
        },
        'admin-beacon-reports': {
            title: 'Problemmeldungen anderer Instanzen',
            subtitle: 'Von Nutzern anderer Instanzen gemeldete Probleme',
        },
        'admin-beacon-figures': {
            title: 'Zahlen der Instanzen',
            subtitle: 'Was die Instanzen täglich an Zahlen schicken',
        },
        'admin-mail-log': {
            title: 'Mailprotokoll',
            subtitle: 'Was aus der Post geworden ist, und woran sie gerade steht',
        },
        'admin-log': {
            title: 'Anwendungslog',
            subtitle: 'Das Log der Instanz, durchsuchbar und nach Stufe filterbar',
        },
        'admin-problem-reports': {
            title: 'Problemmeldungen',
            subtitle: 'Von Nutzern gemeldete Probleme',
        },
        'admin-storage-backend': {
            title: 'Speicher-Backend',
            subtitle: 'Instanzweites Speicher-Backend konfigurieren',
        },
        'admin-storage-audit': {
            title: 'Speicher-Audit',
            subtitle: 'Änderungen am Speicher-Backend',
        },
        'admin-api-status': {
            title: 'API-Status',
            subtitle: 'Anfrage- und Fehlerstatistiken',
        },
        'admin-api-status-detail': {
            title: 'Endpunkt-Details',
            subtitle: 'Detaillierte Statistiken zu einem Endpunkt',
        },
        'admin-feed-metrics': {
            title: 'Feed-Metriken',
            subtitle: 'Nutzung der RSS- und iCal-Feeds',
        },
        'admin-traffic': {
            title: 'Traffic',
            subtitle: 'Anfrage- und Datenverkehr pro Wache',
        },
        'admin-discovery': {
            title: 'Discovery',
            subtitle: 'Bekannte Peer-Instanzen',
        },
        'admin-maps': {
            title: 'Karten-Konfiguration',
            subtitle: 'Tile-Provider und Kartenoptionen',
        },
        'waiting-lists': {
            title: 'Wartelisten',
            subtitle: 'Wartelisten verwalten',
        },
        'waiting-list-detail': {
            title: 'Warteliste',
            subtitle: 'Warteliste verwalten',
        },
        'waiting-list-fields': {
            title: 'Wartelisten-Felder',
            titleNamed: '{name} Felder',
            subtitle: 'Felder konfigurieren',
        },
        'waiting-list-entry': {
            title: 'Wartelisten-Eintrag',
            subtitle: 'Eintrag bearbeiten',
        },
        'quiz-catalogs': {
            title: 'Fragenkataloge',
            subtitle: 'Fragen verwalten und organisieren',
        },
        'quiz-catalog-detail': {
            title: 'Fragenkatalog',
            subtitle: 'Fragen und Kategorien bearbeiten',
        },
        'quiz-tests': {
            title: 'Tests',
            subtitle: 'Tests erstellen und verwalten',
        },
        'quiz-test-create': {
            title: 'Test erstellen',
            subtitle: 'Neuen Test anlegen',
        },
        'quiz-test-detail': {
            title: 'Test',
            subtitle: 'Test-Details und Ergebnisse',
        },
        'quiz-test-edit': {
            title: 'Test bearbeiten',
            subtitle: 'Test-Konfiguration anpassen',
        },
        'quiz-test-take': {
            title: 'Test schreiben',
            titleNamed: '{name} schreiben',
            subtitle: 'Fragen beantworten',
        },
        'quiz-test-evaluate': {
            title: 'Test bewerten',
            titleNamed: '{name} bewerten',
            subtitle: 'Antworten durchgehen und Punkte vergeben',
        },
        'quiz-catalog-generate': {
            title: 'Fragen generieren',
            titleNamed: '{name} Fragen generieren',
            subtitle: 'Fragen mit KI erstellen',
        },
        'quiz-training': {
            title: 'Training',
            subtitle: 'Fragen üben und Wissen testen',
        },
        'kb-browse': {
            title: 'Wiki',
            subtitle: 'Dateien und Ordner durchsuchen',
        },
        'kb-file': {
            title: 'Datei',
            subtitle: 'Datei anzeigen',
        },
        'federated-kb-file': {
            title: 'Datei einer Partnerwache',
            subtitle: 'Geteilte Datei anzeigen',
        },
        'federated-quiz-catalog': {
            title: 'Fragenkatalog einer Partnerwache',
            subtitle: 'Geteilten Katalog ansehen',
        },
        'federated-protocol': {
            title: 'Prüfungsbogen einer Partnerwache',
            subtitle: 'Geteilten Prüfungsbogen ansehen',
        },
        'kb-versions': {
            title: 'Versionen',
            subtitle: 'Versionsverlauf',
        },
        'account-avatar': {
            title: 'Profil',
            subtitle: 'Profilbild und Kontodaten',
        },
        'account-theming': {
            title: 'Erscheinungsbild',
            subtitle: 'Design und Farbschema',
        },
        'account-sessions': {
            title: 'Sitzungen',
            subtitle: 'Aktive Anmeldungen verwalten',
        },
        'account-security': {
            title: 'Sicherheit',
            subtitle: 'Passwort, Passkeys und Zwei-Faktor-Authentifizierung',
        },
        'account-unlock-device': {
            title: 'Neues Gerät freischalten',
            subtitle: 'Einem Gerät ohne Passkey den Weg hinein geben',
        },
        'account-gdpr': {
            title: 'DSGVO',
            subtitle: 'Datenexport und Kontolöschung',
        },
        'reconsent': {
            title: 'Einwilligung erneuern',
            subtitle: 'Aktualisierte Nutzungsbedingungen zustimmen',
        },
        'imprint': {
            title: 'Impressum',
            subtitle: 'Rechtliche Angaben',
        },
        'privacy': {
            title: 'Datenschutz',
            subtitle: 'Datenschutzhinweise',
        },
        'terms': {
            title: 'Nutzungsbedingungen',
            subtitle: 'Bedingungen für die Nutzung',
        },
        'patch-notes': {
            title: 'Änderungsprotokoll',
            subtitle: 'Neuigkeiten und Änderungen',
        },
        'post-login-requirements': {
            title: 'Pflichtangaben',
            subtitle: 'Fehlende Angaben ergänzen',
        },
        'style': {
            title: 'Style Guide',
            subtitle: 'Übersicht aller Basis-Komponenten',
        },
        'style-badges': {
            title: 'Badges',
            subtitle: 'Badge-Varianten',
        },
        'style-buttons': {
            title: 'Buttons',
            subtitle: 'Button-Varianten',
        },
        'style-containers': {
            title: 'Container',
            subtitle: 'Container-Varianten',
        },
        'style-feedback': {
            title: 'Feedback',
            subtitle: 'Alerts, Spinner und Modals',
        },
        'style-inputs': {
            title: 'Eingabefelder',
            subtitle: 'Input-Varianten',
        },
        'style-layered-logo': {
            title: 'Layered Logo',
            subtitle: 'Logo-Komponente',
        },
        'style-pride-text': {
            title: 'Pride Text',
            subtitle: 'Farbverlauf-Text',
        },
        'style-scanner': {
            title: 'Scanner',
            subtitle: 'Barcode- und QR-Scanner',
        },
        'style-table': {
            title: 'Tabelle',
            subtitle: 'Tabellen-Komponenten',
        },
        'style-typography': {
            title: 'Typografie',
            subtitle: 'Überschriften und Text',
        },
        'waitlist-status': {
            title: 'Status der Warteliste',
            subtitle: 'Deinen Wartelistenstatus einsehen',
        },
        'waitlist-register': {
            title: 'Warteliste beitreten',
            subtitle: 'Für die Warteliste anmelden',
        },
        'not-found': {
            title: 'Seite nicht gefunden',
            subtitle: '',
        },
        'public-blog': {
            title: 'Blog',
            subtitle: 'Neuigkeiten der Wache',
        },
        'public-blog-detail': {
            title: 'Beitrag',
            subtitle: '',
        },
        'public-form-submit': {
            title: 'Formular',
            subtitle: 'Formular ausfüllen',
        },
        'public-kb-file': {
            title: 'Datei',
            subtitle: '',
        },
        'public-knowledge-base': {
            title: 'Wiki',
            subtitle: 'Öffentliches Wiki',
        },
        'public-station-calendar': {
            title: 'Kalender',
            subtitle: 'Öffentliche Veranstaltungen',
        },
        'public-waitlist': {
            title: 'Warteliste',
            subtitle: 'Auf die Warteliste setzen',
        },
        'public-discovery': {
            title: 'Wachen entdecken',
            subtitle: 'Wachen in deiner Nähe finden',
        },
        'station-requirements': {
            title: 'Offene Aufgaben',
            subtitle: 'Noch ausstehende Aufgaben erledigen',
        },
        'attendance-report': {
            title: 'Anwesenheitsbericht',
            subtitle: 'Zeiten auswerten und exportieren',
        },
        'ticket-create': {
            title: 'Ticket erstellen',
            titleNamed: '{name} Neues Ticket',
            subtitle: 'Neues Ticket anlegen',
        },
        'checklist-list': {
            title: 'Checklisten',
            subtitle: 'Nachfolge-Aufgaben verwalten',
        },
        'checklist-detail': {
            title: 'Checkliste',
            subtitle: 'Aufgaben abarbeiten',
        },
        'station-discovery-network': {
            title: 'Wachen-Netzwerk',
            subtitle: 'Andere Ember-Instanzen entdecken',
        },
        'event-detail': {
            title: 'Termin',
            subtitle: 'Details und Anmeldungen',
        },
        'event-detail-date': {
            title: 'Termin',
            subtitle: 'Details und Anmeldungen',
        },
        'event-new': {
            title: 'Neuer Termin',
            subtitle: 'Termin anlegen',
        },
        'event-edit': {
            title: 'Termin bearbeiten',
            titleNamed: '{name} bearbeiten',
            subtitle: 'Termin anpassen',
        },
        'event-categories': {
            title: 'Termin-Kategorien',
            subtitle: 'Kategorien verwalten',
        },
        'event-templates': {
            title: 'Termin-Vorlagen',
            subtitle: 'Vorlagen verwalten',
        },
        'event-template-edit': {
            title: 'Termin-Vorlage bearbeiten',
            titleNamed: '{name} bearbeiten',
            subtitle: 'Vorlage anpassen',
        },
        'event-batch': {
            title: 'Termine im Stapel anlegen',
            subtitle: 'Mehrere Termine auf einmal erstellen',
        },
        'federated-event-detail': {
            title: 'Termin einer Partnerwache',
            subtitle: 'Details ansehen',
        },
        'federated-boards': {
            title: 'Föderierte Boards',
            subtitle: 'Boards von Partnerwachen',
        },
        'federated-board-view': {
            title: 'Föderiertes Board',
            subtitle: 'Tickets einer Partnerwache',
        },
        'federated-ticket-detail': {
            title: 'Ticket einer Partnerwache',
            subtitle: 'Details ansehen',
        },
        'federated-news-detail': {
            title: 'Neuigkeit einer Partnerwache',
            subtitle: 'Details ansehen',
        },
        'pages-forms-submissions': {
            title: 'Einreichungen',
            titleNamed: '{name} Einreichungen',
            subtitle: 'Kontakt-Einreichungen ansehen',
        },
        'inventory-edit': {
            title: 'Inventar bearbeiten',
            titleNamed: '{name} bearbeiten',
            subtitle: 'Bestand anpassen',
        },
        'inventory-move': {
            title: 'Gegenstände verschieben',
            titleNamed: '{name} umlagern',
            subtitle: 'Ein Inventar aufteilen, ohne die Gegenstände neu anzulegen',
        },
        'inventory-tidy': {
            title: 'Arten aufräumen',
            titleNamed: '{name} aufräumen',
            subtitle: 'Gleiche Dinge unter eine Art bringen und Tippfehler geraderücken',
        },
        'inventory-detail': {
            title: 'Inventar-Details',
            subtitle: 'Details ansehen',
        },
        'inventory-requirements': {
            title: 'Ausrüstungs-Anforderungen',
            subtitle: 'Pflichtausrüstung pro Mitgliedstyp festlegen',
        },
        'inventory-checks': {
            title: 'Prüfungen',
            subtitle: 'Übersicht aller Prüfungen',
        },
        'inventory-check-member': {
            title: 'Mitglieder-Prüfung',
            titleNamed: '{name} Prüfung',
            subtitle: 'Items eines Mitglieds abgleichen',
        },
        'inventory-check-result': {
            title: 'Prüfungsergebnis',
            titleNamed: '{name} Prüfungsergebnis',
            subtitle: 'Abweichungen ansehen',
        },
        'inventory-self-check': {
            title: 'Eigene Ausrüstung prüfen',
            titleNamed: '{name} Selbstauskunft',
            subtitle: 'Sag uns, was du noch hast',
        },
        'inventory-self-check-review': {
            title: 'Selbstauskunft prüfen',
            titleNamed: '{name} Selbstauskunft',
            subtitle: 'Angabe für Angabe entscheiden',
        },
        'inventory-lending-create': {
            title: 'Ausleih-Anfrage',
            subtitle: 'Neue Ausleihe anfragen',
        },
        'inventory-lending-blocks-create': {
            title: 'Sperrzeit anlegen',
            subtitle: 'Neuen Sperrzeitraum erstellen',
        },
        'inventory-item-detail': {
            title: 'Item-Details',
            subtitle: 'Details ansehen',
        },
        'lost-and-found': {
            title: 'Fundsachen',
            subtitle: 'Verlorene und gefundene Sachen',
        },
        'station-theme': {
            title: 'Design',
            subtitle: 'Farben und Look der Wache',
        },
        'station-mailing': {
            title: 'E-Mail-Versand',
            subtitle: 'E-Mail-Einstellungen der Wache',
        },
        'station-mail-import': {
            title: 'Post-Import',
            subtitle: 'Anhänge aus einem Postfach automatisch als Dokumente ablegen',
        },
        'station-modules': {
            title: 'Module',
            subtitle: 'Aktive Module verwalten',
        },
        'station-traffic': {
            title: 'Traffic',
            subtitle: 'Anfragen und Datenverkehr der Wache',
        },
        'station-insights': {
            title: 'Einblicke',
            subtitle: 'Nutzungs-Statistiken',
        },
        'station-feeds': {
            title: 'Feeds',
            subtitle: 'Wer Kalender und Benachrichtigungen abonniert hat',
        },
        'station-import': {
            title: 'Wache übertragen',
            subtitle: 'Wache auf eine andere Instanz übertragen',
        },
        'station-storage-backend': {
            title: 'Speicher-Backend',
            subtitle: 'Speicherort für Dateien konfigurieren',
        },
        'station-security': {
            title: 'Sicherheit',
            subtitle: 'Sicherheitseinstellungen der Wache',
        },
        'station-federation': {
            title: 'Föderation',
            subtitle: 'Partnerwachen verwalten',
        },
        'station-federation-settings': {
            title: 'Föderations-Einstellungen',
            subtitle: 'Einstellungen der Föderation',
        },
        'station-federation-partner': {
            title: 'Partnerwache',
            subtitle: 'Details und Konfiguration',
        },
        'station-discovery': {
            title: 'Discovery',
            subtitle: 'Bekannte Peer-Instanzen',
        },
        'members-import': {
            title: 'Mitglieder importieren',
            subtitle: 'Mitglieder aus einer Datei laden',
        },
        'members-import-team': {
            title: 'Team importieren',
            subtitle: 'Ganzes Team aus Datei laden',
        },
        'waiting-list-create-entry': {
            title: 'Neuer Wartelisten-Eintrag',
            titleNamed: '{name} Neuer Eintrag',
            subtitle: 'Eintrag anlegen',
        },
        'news-detail': {
            title: 'Neuigkeit',
            subtitle: 'Details ansehen',
        },
        'pages-list': {
            title: 'Seiten',
            subtitle: 'Öffentliche Seiten verwalten',
        },
        'station-media': {
            title: 'Medien',
            subtitle: 'Bilder und Dateien der Wache verwalten',
        },
        'pages-forms': {
            title: 'Formulare',
            subtitle: 'Öffentliche Formulare verwalten',
        },
        'pages-polls': {
            title: 'Umfragen',
            subtitle: 'Öffentliche Umfragen verwalten',
        },
        'pages-polls-analytics': {
            title: 'Auswertung',
            subtitle: 'Ergebnisse und Statistiken',
        },
        'page-editor': {
            title: 'Seite bearbeiten',
            subtitle: 'Seiten-Editor',
        },
        'protocol-list': {
            title: 'Protokolle',
            subtitle: 'Prüfungs-Protokolle verwalten',
        },
        'protocol-detail': {
            title: 'Protokoll',
            subtitle: 'Protokoll-Details',
        },
        'protocol-run-list': {
            title: 'Prüfungen',
            subtitle: 'Absolvierte Prüfungen',
        },
        'protocol-run-detail': {
            title: 'Prüfung',
            subtitle: 'Prüfungs-Details',
        },
        'protocol-grade': {
            title: 'Bewertung',
            titleNamed: '{name} Bewertung',
            subtitle: 'Prüfung eines Mitglieds bewerten',
        },
        'protocol-evaluation': {
            title: 'Auswertung',
            titleNamed: '{name} Auswertung',
            subtitle: 'Prüfungs-Auswertung',
        },
        'quiz-catalog-mc-fill': {
            title: 'MC-Antworten ergänzen',
            titleNamed: '{name} MC-Antworten ergänzen',
            subtitle: 'Multiple-Choice-Antworten mit KI ergänzen',
        },
        'quiz-catalog-import': {
            title: 'Fragen importieren',
            titleNamed: '{name} Fragen importieren',
            subtitle: 'Fragen aus CSV-Datei importieren',
        },
        'profile-inventory': {
            title: 'Mein Inventar',
            subtitle: 'Zugewiesene Ausrüstung',
        },
        'profile-notifications': {
            title: 'Benachrichtigungen',
            subtitle: 'Präferenzen für Benachrichtigungen',
        },
        'station-setup': {
            title: 'Einrichtungs-Assistent',
            subtitle: 'Wache einrichten',
        },
    },
    helpCenter: {
        link: 'Hilfe',
        federation: {
            publicWaitlistTitle: 'Wartelisten teilen',
            publicWaitlistText: 'Du kannst öffentliche Wartelisten für Partnerwachen sichtbar machen. So können Bewerber bei einer befreundeten Wache auf der Liste landen, wenn bei euch kein Platz mehr frei ist.',
            publicBlogTitle: 'Blog teilen',
            publicBlogText: 'Aktiviere die Blog-Föderation, damit eure Neuigkeiten auf den Wachenseiten der Partner erscheinen.',
        },
    },
    header: {
        login: 'Anmelden',
        logout: 'Abmelden',
        adminPanel: 'Admin Panel',
        clusterPanel: 'Verband',
        stationPanel: 'Wach Panel',
        accountSettings: 'Einstellungen',
    },
    quickSearch: {
        openLabel: 'Schnellsuche öffnen (Strg+K)',
        shortcut: 'Strg + K',
        placeholder: 'Seite, Mitglied, Termin, Inventar oder Wiki suchen…',
        startTyping: 'Tippe einen Suchbegriff, um Ergebnisse zu sehen.',
        noResults: 'Keine Treffer.',
        loading: 'Lädt…',
        sectionPages: 'Seiten',
        sectionMembers: 'Mitglieder',
        sectionEvents: 'Termine',
        sectionInventories: 'Inventare',
        sectionKnowledge: 'Wiki',
        hintNavigate: 'Navigieren',
        hintOpen: 'Öffnen',
        hintClose: 'Schließen',
    },
    checkNotes: {
        birthdayToday: 'Hat heute Geburtstag',
        birthdayYesterday: 'Hatte gestern Geburtstag',
        birthdayDaysAgo: 'Hatte vor {days} Tagen Geburtstag',
        replacementNotChosen: 'Ersatzteil ist noch nicht ausgewählt',
        foundItem: 'Fundsache abzuholen: {description}',
        signOffFound: 'Übergeben',
        openNone: 'Hinweis',
        openOne: '1 offener Vorgang',
        openMany: '{count} offene Vorgänge',
        dropSwapConfirm: 'Den Vorgang zu {piece} wirklich löschen? Das Teil wurde noch nicht übergeben, der Vorgang wird ersatzlos entfernt.',
    },
    footer: {
        privacy: 'Datenschutz',
        terms: 'Nutzungsbedingungen',
        imprint: 'Impressum',
        copyright: '\u00a9 2026 RainbowDashLabs and Contributor',
        madeWith: 'Made with \u2764\ufe0f and \ud83c\udff3\ufe0f\u200d\ud83c\udf08',
        license: 'Licensed under AGPL-3.0',
        updateAvailable: 'Version {version} ist verfügbar.',
        updateLink: 'Release ansehen',
    },
    patchNotes: {
        title: 'Versionshinweise',
        fetchError: 'Versionshinweise konnten nicht geladen werden.',
        noReleases: 'Keine Versionshinweise verfügbar.',
        installed: 'Installiert',
        compare: 'Änderungen auf GitHub ansehen',
    },
    adminStations: {
        create: 'Neue Wache',
        createTitle: 'Wache erstellen',
        editTitle: 'Wache bearbeiten',
        name: 'Name',
        namePlaceholder: 'Name der Wache',
        createSubmit: 'Erstellen',
        save: 'Speichern',
        cancel: 'Abbrechen',
        deleteConfirm: 'Wache "{name}" wirklich löschen? Alle zugehörigen Daten werden ebenfalls gelöscht.',
        back: 'Zurück',
        managerEmail: 'Manager E-Mail',
        managerEmailPlaceholder: 'E-Mail-Adresse des Managers',
        managerEmailHint: 'Falls kein Konto mit dieser E-Mail existiert, wird eine Einladung versendet.',
        accountReady: 'Konto aktiv',
        accountPending: 'Einrichtung ausstehend',
        cancelTransfer: 'Abbrechen',
        importStation: 'Wache importieren',
        importHint: 'Importiere eine Wache von einer anderen Ember-Instanz. Der Manager der Quell-Wache erstellt einen Transfer-Code; die Quell-URL ist im Code enthalten.',
        importToken: 'Transfer-Code',
        importTokenPlaceholder: 'Code vom Quell-Manager',
        importStart: 'Import starten',
        importStarting: 'Verbinde...',
    },
    attendanceConfig: {
        create: 'Neue Vorlage',
        empty: 'Noch keine Anwesenheitsvorlagen vorhanden.',
        duplicate: 'Duplizieren',
        deleteConfirm: 'Vorlage "{name}" wirklich löschen? Alle zugehörigen Sitzungen werden ebenfalls gelöscht.',
        cancel: 'Abbrechen',
        back: 'Zurück',
        editTitle: 'Vorlage bearbeiten',
        createTitle: 'Neue Vorlage erstellen',
        name: 'Name',
        namePlaceholder: 'Name der Vorlage',
        save: 'Speichern',
        createSubmit: 'Erstellen',
        fields: 'Felder',
        addField: 'Feld hinzufügen',
        noFields: 'Noch keine Felder vorhanden.',
        editField: 'Feld bearbeiten',
        fieldName: 'Feldname',
        fieldNamePlaceholder: 'Name des Feldes',
        fieldType: 'Feldtyp',
        groups: 'Gruppen',
        groupsHint: 'Wähle die Mitgliedergruppen aus, für die Anwesenheit erfasst werden soll.',
        noGroups: 'Noch keine Gruppen zugewiesen.',
        addGroup: 'Gruppe hinzufügen',
        fieldGroup: 'Gruppe',
        fieldGroupPlaceholder: 'Gruppe auswählen',
        fieldRequired: 'Pflichtfeld',
        fieldAutoAttend: 'Automatisch zur Anwesenheit hinzufügen',
        fieldAutoAttendHint: 'Mitglieder in diesem Feld werden automatisch als anwesend in der Sitzung erfasst.',
        fieldHasDefault: 'Standardwert',
        fieldDefaultValuePlaceholder: 'Standardwert eingeben...',
        fieldDefaultValueHint: 'Wird beim Erstellen einer neuen Sitzung automatisch eingetragen.',
        fieldDefaultDateHint: 'Aktiviert: Heutiges Datum wird automatisch eingetragen.',
        fieldEnumOptions: 'Optionen',
        fieldPosition: 'Position',
        deleteFieldConfirm: 'Feld "{name}" wirklich löschen?',
        fieldTypeLabels: {
            STRING: 'Text',
            NUMBER: 'Zahl',
            TIME: 'Uhrzeit',
            DATE: 'Datum',
            BOOLEAN: 'Ja/Nein',
            ENUM: 'Auswahl',
            URL: 'Link',
            TEXTAREA: 'Mehrzeiliger Text',
            MEMBER: 'Mitglied',
            MEMBER_LIST: 'Mitgliederliste',
            MEMBER_OF_GROUP: 'Mitglied aus Gruppe',
            MEMBER_LIST_OF_GROUP: 'Mitgliederliste aus Gruppe',
        },
        fieldTypeDescriptions: {
            STRING: 'Ein einfaches Textfeld zur freien Eingabe.',
            TIME: 'Eingabefeld für eine Uhrzeit.',
            DATE: 'Eingabefeld für ein Datum.',
            BOOLEAN: 'Ein Ja/Nein-Schalter.',
            ENUM: 'Auswahl aus vordefinierten Optionen.',
            MEMBER: 'Auswahl eines einzelnen Mitglieds aus allen Mitgliedern der Wache.',
            MEMBER_LIST: 'Auswahl mehrerer Mitglieder aus allen Mitgliedern der Wache.',
            MEMBER_OF_GROUP: 'Auswahl eines einzelnen Mitglieds aus einer bestimmten Gruppe.',
            MEMBER_LIST_OF_GROUP: 'Auswahl mehrerer Mitglieder aus einer bestimmten Gruppe.',
        },
    },
    profile: {
        documentsOf: 'Dokumente von {name}',
        avatar: 'Profilbild',
        uploadAvatar: 'Bild hochladen',
        avatarHint: 'PNG, JPEG oder WebP.',
        manageInAccount: 'Name, E-Mail, Passwort und 2FA werden im',
        manageInAccountLink: 'Kontobereich verwaltet.',
        accountTitle: 'Kontodaten',
        firstName: 'Vorname',
        lastName: 'Nachname',
        email: 'E-Mail',
        username: 'Benutzername',
        usernameHint: 'Optional. Damit kannst du dich statt mit deiner E-Mail-Adresse anmelden; beide '
            + 'funktionieren. Erlaubt sind 3 bis 32 Zeichen aus Buchstaben, Ziffern, Punkt, Bindestrich '
            + 'und Unterstrich.',
        saveAccount: 'Kontodaten speichern',
        nicknameTitle: 'Spitzname',
        nickname: 'Spitzname',
        nicknameHint: 'Der Name, mit dem dich deine Wache anspricht. Er ersetzt deinen Vornamen auf den '
            + 'Bildschirmen, in Benachrichtigungen und in Mails. Auf Anwesenheitslisten, Ausdrucken und '
            + 'Exporten steht weiterhin dein Name aus dem Register. Leer lassen, um wieder beim Vornamen '
            + 'genannt zu werden.',
        nicknamePreview: 'So stehst du in Mitgliederlisten',
        emailChangePending:
            'Eine Bestätigungs-E-Mail wurde an die neue Adresse gesendet. Die E-Mail-Adresse ändert sich erst nach der Bestätigung.',
        passwordTitle: 'Passwort ändern',
        currentPassword: 'Aktuelles Passwort',
        newPassword: 'Neues Passwort',
        confirmPassword: 'Passwort bestätigen',
        changePassword: 'Passwort ändern',
        passwordMismatch: 'Die Passwörter stimmen nicht überein.',
        passwordTooShort: 'Das neue Passwort muss mindestens {count} Zeichen lang sein.',
        passwordHint: 'Mindestens {count} Zeichen.',
        passwordError: 'Das Passwort konnte nicht geändert werden.',
        title: 'Profilfelder',
        noFields: 'Keine Profilfelder konfiguriert.',
        incompleteTitle: 'Profil unvollständig',
        incompleteHint: 'Bitte fülle die folgenden Pflichtfelder aus:',
        save: 'Speichern',
        readonlyHint: 'nur lesbar',
        calculatedHint: 'berechnet',
        chooseOne: 'Bitte auswählen',
        absenceEmpty: 'Keine Abwesenheiten eingetragen.',
        absenceAdd: 'Abwesenheit eintragen',
        absenceFrom: 'Von',
        absenceUntil: 'Bis',
        absenceReason: 'Grund',
        absenceReasonPlaceholder: 'Optionaler Grund...',
        absenceCreated: 'Abwesenheit eingetragen.',
        absenceActive: 'Aktiv',
        absenceUpcoming: 'Geplant',
        absenceExpired: 'Abgelaufen',
        absenceFor: 'Abwesend melden für',
        absenceMyself: 'Mich selbst',
        absenceInvalidRange: 'Das Enddatum darf nicht vor dem Startdatum liegen.',
        inventory: 'Mein Inventar',
        myInventorySelf: 'Mein Inventar',
        noInventory: 'Dir ist aktuell kein Inventar zugewiesen.',
        lostSince: 'Verloren seit',
        requestExchange: 'Tausch anfragen',
        exchangeReasonPlaceholder: 'Grund für den Tausch...',
        exchangeExtraPlaceholder: 'Noch etwas dazu? (freiwillig)',
        submitExchange: 'Tausch anfragen',
        exchangeCreated: 'Tausch-Anfrage erstellt.',
        reportLost: 'Als vermisst melden',
        reportLostHint: 'Das Teil gilt ab sofort als vermisst. Taucht es wieder auf, kann die Wache das zurücknehmen.',
        lostNotePlaceholder: 'Was ist passiert? (optional)',
        lostNoteRequired: 'Was ist passiert?',
        submitLost: 'Als vermisst melden',
        lostReported: 'Als vermisst gemeldet.',
    },
    profileManaged: {
        title: 'Verwaltete Profile',
        selectMember: 'Mitglied auswählen',
        selectMemberPlaceholder: 'Mitglied auswählen',
        noManaged: 'Du verwaltest keine Mitglieder.',
        fields: 'Profilfelder',
        access: {
            title: 'Zugang',
            hint: 'Du legst fest, unter welcher E-Mail-Adresse dieses Mitglied erreichbar ist, mit welchem '
                + 'Benutzernamen es sich anmeldet und ob es sich überhaupt anmelden darf.',
            email: 'E-Mail-Adresse',
            emailPlaceholder: "name{'@'}example.org",
            emailHint: 'Die Änderung gilt sofort. Bestehende Anmeldungen dieses Mitglieds werden beendet; '
                + 'war bereits eine Adresse hinterlegt, werden alte und neue Adresse benachrichtigt.',
            emailSaved: 'Adresse gespeichert.',
            username: 'Benutzername',
            usernamePlaceholder: 'z. B. lena.sommer',
            usernameHint: 'Damit kann sich das Mitglied ohne eigene E-Mail-Adresse anmelden. Alles, was '
                + 'Ember sonst an das Mitglied schreiben würde, geht dann an dich. Erlaubt sind 3 bis 32 '
                + 'Zeichen aus Buchstaben, Ziffern, Punkt, Bindestrich und Unterstrich.',
            usernameSaved: 'Benutzername gespeichert.',
            password: 'Passwort',
            passwordPlaceholder: 'Neues Passwort',
            passwordHint: 'Weil dieses Mitglied keine eigene E-Mail-Adresse hat, vergibst du das Passwort '
                + 'hier selbst und gibst es zusammen mit dem Benutzernamen weiter. Offene Anmeldungen des '
                + 'Mitglieds werden dabei beendet.',
            passwordSaved: 'Passwort gesetzt. Gib es dem Mitglied zusammen mit dem Benutzernamen weiter.',
            login: 'Anmeldung erlauben',
            loginHint: 'Das Mitglied wird ein paar Minuten später per E-Mail informiert. Wer noch kein '
                + 'Passwort hat, erhält stattdessen die Einladung, eines zu setzen.',
            loginNeedsEmail: 'Dafür wird zuerst eine E-Mail-Adresse oder ein Benutzername benötigt.',
            loginOn: 'Anmeldung erlaubt. Das Mitglied wird in wenigen Minuten per E-Mail informiert.',
            loginOff: 'Anmeldung entzogen; offene Sitzungen wurden beendet. Das Mitglied wird in wenigen '
                + 'Minuten per E-Mail informiert.',
        },
    },
    memberEdit: {
        accountSavedRestNot: 'Name und Adresse wurden gespeichert, die Antworten und der Spitzname aber nicht. Versuche es gleich noch einmal: Name und Adresse sind schon in Ordnung und werden dabei nur erneut geschrieben.',
        nickname: 'Spitzname',
        nicknameHint: 'Der Name, mit dem die Wache dieses Mitglied anspricht. Er ersetzt den Vornamen auf den '
            + 'Bildschirmen, in Benachrichtigungen und in Mails; auf Anwesenheitslisten, Ausdrucken und Exporten '
            + 'steht weiterhin der Name aus dem Register. Leer lassen, um wieder den Vornamen zu verwenden.',
        back: 'Zurück',
        tabProfile: 'Profil',
        tabPermissions: 'Berechtigungen',
        tabRelations: 'Zuordnungen',
        tabNotes: 'Notizen',
        relations: {
            guardians: 'Erziehungsberechtigte',
            managedMembers: 'Zugeordnete Mitglieder',
            noGuardians: 'Keine Erziehungsberechtigten zugeordnet.',
            noManagedMembers: 'Keine Mitglieder zugeordnet.',
            addGuardian: 'Erziehungsberechtigten zuordnen',
            addManagedMember: 'Mitglied zuordnen',
            selectMember: 'Mitglied auswählen...',
            saved: 'Zuordnung gespeichert.',
        },
        baseFields: 'Kontodaten',
        firstName: 'Vorname',
        lastName: 'Nachname',
        email: 'E-Mail',
        username: 'Benutzername',
        usernameHint: 'Optional. Damit meldet sich das Mitglied statt mit seiner E-Mail-Adresse an; beides '
            + 'funktioniert. Leer bedeutet: Es meldet sich mit seiner Adresse an.',
        emailConfirmationPending: 'Deine eigene Adresse ändert sich erst, wenn du die Links bestätigst, die an '
            + 'die alte und die neue Adresse geschickt wurden.',
        emailHint: 'Die Änderung gilt sofort. Das Mitglied wird abgemeldet und beide Adressen werden benachrichtigt.',
        permissions: 'Berechtigungen',
        userType: 'Mitgliedstyp',
        joinDate: 'Beigetreten am',
        joinDateHint: 'Datum, an dem das Mitglied der Wache beigetreten ist. Vorbelegt auf den Tag der Anlage.',
        userTypeMember: 'Mitglied',
        userTypeGuardian: 'Erziehungsberechtigter',
        userTypeTeam: 'Team',
        userTypeManager: 'Manager',
        userTypeTrial: 'Probe',
        fields: 'Profilfelder',
        fieldName: 'Feld',
        fieldValue: 'Wert',
        fieldFromCluster: 'Verband',
        groups: 'Gruppen',
        noGroups: 'Keine Gruppen vorhanden.',
        tags: 'Tags',
        noTags: 'Keine Tags vorhanden.',
    },
    tableFilter: {
        by: 'Filtern: {column}',
        sortBy: 'Sortieren: {column}',
        selectAll: 'Alle',
        selectNone: 'Keine',
        empty: 'Kein Wert (leer)',
        noValues: 'Keine Werte vorhanden.',
        apply: 'Anwenden',
        columns: 'Spalten',
        range: 'Zeitraum',
        from: 'Ab (einschließlich)',
        before: 'Vor (ausschließlich)',
        currentAge: 'Aktuelles Alter',
        calendarAge: 'Kalenderalter',
        calendarAgeHint: 'Das Kalenderalter ist das Alter am 31. Dezember des laufenden Jahres.',
        ageAtLeast: 'Mindestens',
        ageBelow: 'Unter',
        expandYear: 'Monate anzeigen',
        expandMonth: 'Tage anzeigen',
        numberRange: 'Bereich',
        atLeast: 'Mindestens',
        atMost: 'Höchstens',
        sortLabel: 'Sortieren nach',
        unsorted: 'Unsortiert',
        direction: 'Reihenfolge umkehren',
        filterColumns: 'Spalten filtern',
    },
    setupMailChoice: {
        label: 'Einrichtungs-Mail sofort senden',
        hintNow: 'Die Mail mit dem Einrichtungs-Link geht direkt hinaus.',
        hintLater: 'Es geht keine Mail hinaus. Du versendest sie später über den Knopf in der '
            + 'Mitgliederliste, der dabei einen frischen Link erzeugt.',
        noMailServer: 'Diese Instanz hat keinen Mailserver eingerichtet und versendet keine '
            + 'Einrichtungs-Mails. Gib den Einrichtungs-Link selbst weiter.',
    },
    memberTable: {
        menu: 'Ansicht',
        asTable: 'Als Tabelle anzeigen',
        asCards: 'Als Karten anzeigen',
        export: 'Exportieren',
        exportHint: 'Es wird genau das ausgegeben, was hier steht.',
        format: 'Format',
        csv: 'Tabelle (CSV)',
        pdf: 'Blatt (PDF)',
        nobody: 'Auf dieser Liste steht niemand.',
    },
    membersList: {
        savedFiltersUnreadable: 'Deine gespeicherten Filter konnten nicht geladen werden. Es steht deshalb keiner zur Auswahl, auch wenn du welche gespeichert hast.',
        managersUnreadable: 'Es konnte nicht gelesen werden, wer dieses Mitglied betreut. Die aufgeklappte Zeile zeigt deshalb niemanden an, auch wenn jemand eingetragen ist.',
        accountPending: 'Konto noch nicht eingerichtet - die Person hat sich noch kein Passwort gesetzt.',
        accountPendingExpires: 'Der zugesendete Link ist gültig bis {date}.',
        accountPendingExpired: 'Der zugesendete Link ist am {date} abgelaufen und funktioniert nicht '
            + 'mehr. Sende die Einrichtungs-Mail erneut, damit ein neuer Link entsteht.',
        setupLinkExpired: 'Link abgelaufen',
        accountPendingResend: 'Einrichtungs-Mail erneut senden',
        setupMailToMember: 'Die Mail geht an die Adresse des Mitglieds.',
        setupMailToGuardians: 'Das Mitglied hat keine eigene Adresse, die Mail geht an die '
            + 'Erziehungsberechtigten.',
        setupMailToNobody: 'Es gibt niemanden, dem die Mail zugestellt werden könnte: weder das '
            + 'Mitglied noch eine erziehungsberechtigte Person hat eine erreichbare Adresse. Trage '
            + 'eine Adresse ein oder ordne eine erziehungsberechtigte Person mit Adresse zu.',
        resendConfirm: 'Die Einrichtungs-Mail an {name} wird erneut versendet. Fortfahren?',
        resendAction: 'Erneut senden',
        resendSuccess: 'Einrichtungs-Mail wurde erneut versendet.',
        tabAll: 'Alle',
        tabTrial: 'Probe',
        tabMember: 'Mitglieder',
        tabMemberManager: 'Erziehungsberechtigter',
        tabTeam: 'Team',
        tabManager: 'Leitung',
        filter: 'Suchen...',
        noExtraColumns: 'Keine weiteren Felder verfügbar.',
        colName: 'Name',
        colRole: 'Typ',
        colUserType: 'Typ',
        colEmail: 'E-Mail',
        colGroups: 'Gruppen',
        colTags: 'Tags',
        detail: 'Details',
        incomplete: 'Unvollständig',
        empty: 'Keine Mitglieder gefunden.',
        managers: 'Verwalter',
        savedFilters: 'Gespeicherte Filter',
        saveFilter: 'Filter speichern',
        saveFilterSubmit: 'Speichern',
        filterName: 'Filtername',
        clearFilters: 'Zurücksetzen',
        export: {
            button: 'Exportieren',
            cancel: 'Export abbrechen',
            selected: '{count} ausgewählt',
            continue: 'Weiter',
            title: 'Export',
            hint: '{count} Mitglieder ausgewählt',
            selectColumns: 'Spalten auswählen',
            format: 'Format',
            formatCsv: 'CSV-Datei',
            formatValues: 'Semikolongetrennte Werte',
            formatPdf: 'Blatt zum Mitnehmen (PDF)',
            valuesHint: 'Nur bei einer einzelnen Spalte verfügbar.',
            submit: 'Exportieren',
            colFirstName: 'Vorname',
            colLastName: 'Nachname',
            colEmail: 'E-Mail',
            colGroups: 'Gruppen',
        },
    },
    mailImport: {
        mailboxes: 'Postfächer',
        addMailbox: 'Postfach hinzufügen',
        noMailboxes: 'Noch kein Postfach eingerichtet. Ein Postfach verbindest du einmal, danach werden Anhänge daraus von selbst zu Dokumenten.',
        switchedOffForInstance: 'Das Lesen von Postfächern ist auf dieser Instanz abgeschaltet. Wende dich an den Betreiber, wenn du es nutzen möchtest.',
        noEncryptionKey: 'Auf dieser Instanz ist kein Schlüssel für Zugangsdaten hinterlegt. Ohne ihn kann ein Postfach-Kennwort nicht sicher gespeichert werden, deshalb lässt sich kein Postfach anlegen.',
        floorHint: 'Frühestens alle {minutes} Minuten. Kürzer erlaubt der Betreiber nicht, damit kein Anbieter das Konto sperrt.',
        rules: 'Regeln',
        addRule: 'Regel hinzufügen',
        noRules: 'Noch keine Regel. Ohne Regel wird aus diesem Postfach nichts übernommen.',
        log: 'Protokoll',
        logHint: 'Jede angesehene Nachricht steht hier, auch die, aus denen nichts übernommen wurde. Wenn eine Regel scheinbar nichts tut, steht hier der Grund.',
        logEmpty: 'Noch nichts angesehen.',
        logPruned: 'Ältere Angaben wurden entfernt',
        pageOf: 'Seite {page} von {pages}',
        testConnection: 'Verbindung testen',
        testConnected: 'Verbindung steht. Es wurde nichts gesendet.',
        testFolders: 'Gefundene Ordner: {folders}',
        testFolderMissing: 'Den Ordner „{folder}" gibt es dort nicht. Schreibe ihn genau so, wie der Anbieter ihn nennt.',
        runNow: 'Jetzt abrufen',
        resume: 'Wieder aufnehmen',
        cycleSummary: '{looked} Nachrichten angesehen, {imported} Dokumente abgelegt, {refused} abgelehnt.',
        lastChecked: 'Zuletzt geprüft: {when}',
        neverChecked: 'Noch nie geprüft',
        dkimChecked: 'Absendersignatur wird geprüft',
        state: {
            on: 'Aktiv',
            off: 'Aus',
            suspended: 'Ausgesetzt',
        },
        security: {
            SSL: 'SSL',
            STARTTLS: 'STARTTLS',
            NONE: 'Unverschlüsselt',
        },
        titleSource: {
            SUBJECT: 'Betreff der Nachricht',
            FILE_NAME: 'Name der Datei',
        },
        action: {
            NOTHING: 'Nachricht unverändert lassen',
            MARK_SEEN: 'Nachricht als gelesen markieren',
            FLAG: 'Nachricht markieren',
            MOVE: 'Nachricht in einen Ordner verschieben',
        },
        outcome: {
            IMPORTED: 'Abgelegt',
            DUPLICATE: 'Schon vorhanden',
            SENDER_NOT_ALLOWED: 'Absender nicht erlaubt',
            NO_RULE_MATCHED: 'Keine Regel passt',
            TYPE_NOT_ALLOWED: 'Dateiart nicht erlaubt',
            TOO_LARGE: 'Zu groß',
            TOO_SMALL: 'Zu klein',
            NO_ATTACHMENT: 'Kein Anhang',
            QUOTA_EXCEEDED: 'Kein Speicher frei',
            AUTHENTICATION_FAILED: 'Absenderprüfung fehlgeschlagen',
            FAILED: 'Fehlgeschlagen',
        },
        field: {
            name: 'Bezeichnung',
            namePlaceholder: 'Wofür dieses Postfach da ist',
            host: 'Server',
            port: 'Port',
            security: 'Verschlüsselung',
            username: 'Benutzer',
            password: 'Kennwort',
            passwordUnchanged: 'Unverändert lassen',
            folder: 'Ordner',
            enabled: 'Postfach regelmäßig abrufen',
            verifyDkim: 'Absendersignatur prüfen (DKIM)',
            verifyDkimHint: 'Das geht nur, wenn deine Absender ihre Mail signieren. Ist die Prüfung an, wird eine Nachricht ohne gültige Signatur der Domain, aus der sie zu kommen behauptet, abgelehnt statt abgelegt.',
            interval: 'Abrufen alle (Minuten)',
            intervalHint: 'Mindestens {minutes} Minuten.',
            importFrom: 'Nachrichten ab',
            importFromHint: 'Ältere Nachrichten werden nicht angesehen. So holt der erste Abruf kein ganzes Archiv herein.',
        },
        rule: {
            name: 'Bezeichnung',
            enabled: 'Regel aktiv',
            senders: 'Erlaubte Absender',
            sendersHint: "Eine Adresse, oder alle Adressen einer Domain als *{'@'}domain.de. Genau das entscheidet, wer Dateien in die Ablage legen darf, also halte die Liste eng.",
            senderInvalid: "Das ist weder eine Adresse noch *{'@'}domain.de",
            subjectFilter: 'Betreff enthält',
            attachmentNameFilter: 'Dateiname enthält',
            types: 'Erlaubte Dateiarten',
            minSize: 'Mindestgröße (KB)',
            minSizeHint: 'Hält kleine Bilder wie Signatur-Logos aus der Ablage.',
            titleSource: 'Titel des Dokuments aus',
            action: 'Danach mit der Nachricht',
            moveToFolder: 'Zielordner',
            tags: 'Tags',
            readSubjectForMember: 'Namen im Betreff auswerten',
            readSubjectHint: 'Wer scannt, tippt einen Namen in den Betreff. Passt der Betreff auf mehrere Personen, wird das Dokument keinem zugeordnet: ein Dokument bei der falschen Person ist schlimmer als eines bei keiner.',
            includeInline: 'Eingebettete Bilder mitnehmen',
            includeInlineHint: 'Aus: sonst landet das Wappen aus jeder Signatur in der Ablage. An, wenn Handyfotos eingebettet ankommen.',
            hiddenLabel: 'Dokumente vor dem Mitglied verbergen',
            keepOnArchive: 'Dokumente behalten, wenn das Mitglied ausscheidet',
            readsSubject: 'Betreff wird ausgewertet',
            hidden: 'Verborgen',
            from: 'Von: {senders}',
            takes: 'Nimmt: {types}',
            named: 'Dateiname enthält „{name}"',
            subject: 'Betreff enthält „{subject}"',
        },
    },

    documents: {
        title: 'Dokumente',
        none: 'Noch keine Dokumente abgelegt.',
        upload: 'Hochladen',
        onlyUnbound: 'Nur ohne Mitglied',
        uploadTitle: 'Dokument hochladen',
        file: 'Datei',
        titlePlaceholder: 'Wie das Dokument heißen soll',
        tags: 'Tags',
        tagsPlaceholder: 'Vertrag, Einverständnis',
        tagsHint: 'Mit Komma getrennt. Neue Tags entstehen beim Speichern.',
        keepOnArchive: 'Bei Archivierung behalten',
        keepOnArchiveHint: 'Bleibt erhalten, wenn das Mitglied als ehemalig markiert wird. Für rechtlich bindende Dokumente.',
        kept: 'Wird behalten',
        hide: 'Vor dem Mitglied verbergen',
        hideHint: 'Nur wer Mitgliederdokumente einsehen darf, sieht das Dokument dann noch.',
        hidden: 'Verborgen',
        boundMembers: 'Gehört zu',
        boundToNobody: 'Keinem Mitglied zugeordnet.',
        bind: 'Zuordnen',
        bindPlaceholder: 'Mitglied wählen',
        search: 'Suche',
        searchPlaceholder: 'Titel oder Inhalt',
        searchOwnPlaceholder: 'Titel oder Tag',
        searchHint: 'Durchsucht Titel und, wo lesbar, den Inhalt der Dokumente.',
        filterMember: 'Mitglied',
        allMembers: 'Alle Dokumente',
        pageOf: 'Seite {page} von {pages}',
    },
    memberDetail: {
        back: 'Zurück',
        edit: 'Bearbeiten',
        tabProfile: 'Profil',
        tabPermissions: 'Berechtigungen',
        tabGuardians: 'Erziehungsberechtigte',
        tabManagedMembers: 'Verwaltete Mitglieder',
        tabDocuments: 'Dokumente',
        tabAbsences: 'Abwesenheiten',
        tabInventory: 'Inventar',
        tabNotes: 'Notizen',
        absences: {
            title: 'Abwesenheiten',
            empty: 'Keine Abwesenheiten eingetragen.',
            add: 'Abwesenheit eintragen',
            from: 'Von',
            until: 'Bis',
            reason: 'Grund',
            reasonPlaceholder: 'Optional',
            active: 'Aktiv',
            upcoming: 'Geplant',
            expired: 'Abgelaufen',
            saved: 'Abwesenheit gespeichert.',
        },
        userType: 'Mitgliedstyp',
        permissions: 'Berechtigungen',
        groups: 'Gruppen',
        tags: 'Tags',
        managedMembers: 'Verwaltete Mitglieder',
        noManagedMembers: 'Keine Mitglieder zugewiesen.',
        linkManaged: 'Mitglied zuweisen',
        selectManaged: 'Mitglied auswählen',
        selectManagedPlaceholder: 'Mitglied auswählen',
        noRelations: 'Keine Zuordnungen für diesen Mitgliedstyp.',
        fields: 'Profilfelder',
        noFields: 'Keine Felder für diesen Nutzer.',
        managers: 'Verwalter',
        noManagers: 'Keine Verwalter zugewiesen.',
        changesUnreadable: 'Der Änderungsverlauf konnte nicht geladen werden. Er steht deshalb leer da, was nicht heißt, dass an diesem Profil nie etwas geändert wurde.',
        inventoryUnreadable: 'Die Ausrüstung dieses Mitglieds konnte nicht geladen werden. Die Liste ist deshalb leer, was nicht heißt, dass das Mitglied nichts hat.',
        managerDetailsUnreadable: 'Zu mindestens einer betreuenden Person konnten die Angaben nicht geladen werden. Ihre Felder stehen deshalb leer da, auch wenn etwas eingetragen ist.',
        invitedButNotLinked: 'Die Person wurde eingeladen, konnte aber nicht als betreuende Person eingetragen werden. Lade die Seite neu und trage sie über „Verwalter zuweisen" ein, statt sie noch einmal einzuladen.',
        invitedButNotLinkedGuidance: 'Die Einladung ist raus, das Konto gibt es also schon. Ein zweiter Versuch über diesen Knopf wird an der bereits vergebenen Adresse scheitern.',
        inventory: 'Inventar',
        requestExchange: 'Tausch anfragen',
        linkManager: 'Verwalter zuweisen',
        createManager: 'Neuen Verwalter',
        selectManager: 'Verwalter auswählen',
        selectManagerPlaceholder: 'Mitglied auswählen',
        assign: 'Zuweisen',
        createManagerTitle: 'Neuen Verwalter erstellen',
        firstName: 'Vorname',
        lastName: 'Nachname',
        email: 'E-Mail',
        createManagerSubmit: 'Erstellen & zuweisen',
        changeHistory: 'Änderungsverlauf',
        noChanges: 'Keine Änderungen vorhanden.',
        changedBy: 'Geändert von',
        acknowledged: 'Bestätigt',
        notAcknowledged: 'Nicht bestätigt',
        acknowledge: 'Bestätigen',
        acknowledgeAll: 'Alle bestätigen',
        acknowledgeWithComment: 'Bestätigen mit Kommentar',
        commentPlaceholder: 'Optionaler Kommentar...',
        submitAcknowledge: 'Bestätigen',
        markFormer: 'Als ehemalig markieren',
        markFormerTitle: 'Mitglied als ehemalig markieren',
        markFormerConfirm: '„{name}" wird als ehemaliges Mitglied markiert. Die Person kann sich nicht mehr anmelden und wird aus allen Gruppen entfernt.',
        markFormerHint: 'Tausch-Anfragen werden gelöscht, Verwalter-Zuweisungen entfernt und alle Rollen entzogen. Die Profildaten bleiben erhalten.',
        formerSuccess: 'Mitglied wurde als ehemalig markiert.',
        deleteData: 'Daten löschen',
        deleteTitle: 'Mitglied löschen',
        deleteText: 'Möchtest du "{name}" wirklich löschen? Alle Daten des Mitglieds werden anonymisiert und können nicht wiederhergestellt werden.',
        deleteHint: 'Dies entspricht einer Löschung nach DSGVO. Profilfelder, Gruppenzugehörigkeiten, Benachrichtigungen und Kommentare werden entfernt.',
        deleteConfirmAction: 'Endgültig löschen',
        deleteConfirmTitle: 'Bist du sicher?',
        deleteConfirmText: 'Diese Aktion kann nicht rückgängig gemacht werden!',
        deleteConfirmFinal: 'Ja, Mitglied löschen',
        deleteSuccess: 'Mitglied wurde gelöscht und alle Daten anonymisiert.',
        assignItem: 'Gegenstand zuweisen',
        unassignItem: 'Zuweisung aufheben',
        reassignItem: 'Umzuweisen',
        scanAlreadyAssigned: '"{name}" ist bereits einem Mitglied zugewiesen.',
        scanAlreadyHere: '"{name}" ist diesem Mitglied bereits zugewiesen.',
        noInventory: 'Keine Gegenstände zugewiesen.',
        missingRequirements: 'Fehlende Ausstattung',
        missingRequirementsHint: 'Diese Gegenstände sind für das Mitglied vorgesehen, aber noch nicht zugewiesen.',
        missingCount: '({count} fehlt)',
        pickStockItem: 'Vorhandenen Gegenstand wählen',
        pickSize: 'Größe wählen',
        handOut: 'Zuweisen',
        createAndHandOut: 'Neu anlegen und zuweisen',
        nothingInStock: 'Kein freier Gegenstand im Lager.',
        selectTargetMember: 'Neues Mitglied',
        selectTargetPlaceholder: 'Mitglied auswählen',
        formerBlocked: 'Dieses Mitglied kann derzeit nicht als ehemalig markiert werden:',
        formerBlockInventory: 'Dem Mitglied sind noch {count} Gegenstände zugewiesen.',
        formerBlockRole: 'Erziehungsberechtigter, Manager und Admins können nicht als ehemalig markiert werden.',
    },
    formerMembers: {
        title: 'Ehemalige Mitglieder',
        empty: 'Keine ehemaligen Mitglieder.',
        colFormerAt: 'Ausgetreten am',
        reactivate: 'Reaktivieren',
        reactivateTitle: 'Mitglied reaktivieren',
        reactivateConfirm: '„{name}" wird als aktives Mitglied reaktiviert und erhält die Rollen Login und Mitglied.',
        reactivated: 'Mitglied wurde reaktiviert.',
        count: 'ehemalige Mitglieder',
    },
    memberChanges: {
        noChanges: 'Keine offenen Änderungen vorhanden.',
        noHistory: 'Keine vergangenen Änderungen vorhanden.',
        lastChange: 'Letzte Änderung',
        pending: 'offen',
        toProfile: 'Zum Profil',
        tabPending: 'Offene Änderungen',
        tabHistory: 'Verlauf',
        page: 'Seite {current} von {total}',
        next: 'Weiter',
    },
    membersCreate: {
        createdButIncomplete: 'Das Konto wurde angelegt, aber nicht alles daran konnte gespeichert werden. Öffne das Mitglied in der Liste und ergänze den Rest dort, statt es hier noch einmal anzulegen.',
        stepRole: 'Rolle auswählen',
        stepRoleHint: 'Welche Rolle soll das neue Konto haben?',
        roleTrial: 'Probe',
        roleTrialHint: 'Probemitglied ohne Standardrechte - schnuppert hinein.',
        roleMember: 'Mitglied',
        roleMemberHint: 'Reguläres Mitglied der Wache.',
        roleMemberManager: 'Erziehungsberechtigter',
        roleMemberManagerHint: 'Kann zugewiesene Mitglieder verwalten.',
        roleTeam: 'Team',
        roleTeamHint: 'Teammitglied mit erweiterten Rechten.',
        stepIdentity: 'Persönliche Daten',
        firstName: 'Vorname',
        firstNamePlaceholder: 'Vorname',
        lastName: 'Nachname',
        lastNamePlaceholder: 'Nachname',
        canLogin: 'Kann sich anmelden',
        canLoginHint: 'Deaktivieren für Mitglieder ohne eigenen Zugang (z.B. Kinder).',
        email: 'E-Mail',
        emailPlaceholder: 'E-Mail-Adresse',
        emailHint: 'Eine Einladung wird an diese Adresse gesendet.',
        stepFields: 'Profilfelder',
        noFields: 'Keine Profilfelder für diesen Typ konfiguriert.',
        requiredFields: 'Pflichtfelder',
        optionalFields: 'Optionale Felder',
        stepGroups: 'Gruppen',
        stepGroupsHint: 'Zu welchen Gruppen soll das Mitglied gehören?',
        noGroups: 'Keine Gruppen vorhanden.',
        stepManager: 'Erziehungsberechtigter zuweisen',
        stepManagerHint: 'Wähle einen bestehenden Verwalter aus oder erstelle einen neuen.',
        existingManagers: 'Bestehende Verwalter',
        createNewManager: 'Neuen Verwalter erstellen',
        createNewManagerHint: 'Erstelle ein neues Erziehungsberechtigter-Konto und weise es direkt zu.',
        addManager: 'Verwalter erstellen',
        next: 'Weiter',
        back: 'Zurück',
        create: 'Konto erstellen',
        done: 'Konto erstellt',
        doneHint: 'Die Einladung wurde versendet. Das Mitglied kann sich nun registrieren.',
        createAnother: 'Weiteres Konto erstellen',
        toList: 'Zur Mitgliederliste',
    },
    eventDetail: {
        tabInfo: 'Informationen',
        yourAnswer: 'Deine Antwort',
        updateAnswer: 'Antworten aktualisieren',
        answerMissing: 'Antwort erforderlich',
        tabRegistrations: 'Anmeldungen',
        tabEquipment: 'Ausrüstung',
        tabAttendance: 'Anwesenheit',
        registrations: 'Anmeldungen',
        attendanceTitle: 'Abmeldungen',
        attendanceHint: 'Zu diesem Termin sind alle eingeplant. Hier stehen die, die sich abgemeldet haben.',
        noSignOffs: 'Bisher hat sich niemand abgemeldet.',
        absentMembers: 'Abwesend',
        noAbsences: 'Keine Abwesenheiten gemeldet.',
        noAnswerYet: 'Noch keine Antwort',
        myRegistration: 'Meine Anmeldung',
        federatedRegistrations: 'Föderierte Anmeldungen',
        federatedPartnerDecides: 'Die Partnerwache entscheidet selbst',
        manualRegister: 'Mitglied anmelden',
        selectMember: 'Mitglied wählen',
        editAnswers: 'Antworten bearbeiten',
    },
    eventEquipment: {
        title: 'Was der Termin braucht',
        empty: 'Für diesen Termin ist noch nichts eingetragen.',
        noDate: 'Für diesen Termin steht kein Datum fest, deshalb lässt sich auch nichts vormerken.',
        addLine: 'Zeile hinzufügen',
        covered: 'Gedeckt',
        missing: 'Es fehlen {count}',
        split: 'Eigene: {own}, geliehen: {borrowed}, angefragt: {outstanding}',
        away: 'Weg von {from} bis {to}',
        overClaim: 'Am selben Wochenende auch eingeplant für: {names}',
        thisDateOnly: 'Nur dieser Termin',
        thisDateOnlyLabel: 'Gilt nur für diesen Termin',
        thisDateOnlyHint: 'Ohne Haken gilt die Zeile für jeden Termin der Reihe.',
        lead: 'Stunden vorher weg',
        leadHint: 'Wie lange vor dem Termin die Ausrüstung schon aus dem Regal ist. Wer sie am Vorabend holt, trägt hier 24 ein.',
        trail: 'Stunden nachher zurück',
        borrowMissing: 'Fehlende {count} bei Partnerwachen suchen',
    },
    lendingCollect: {
        offersTitle: 'Was die Partnerwachen anbieten',
        listTitle: 'Zusammengestellt',
        listEmpty: 'Noch nichts ausgewählt.',
        add: 'Übernehmen',
        freeCount: '{count} frei',
        onlyLeft: 'Nur noch {count} frei',
        somethingChanged: 'Bei {count} Zeilen hat sich etwas geändert. Du kannst trotzdem fragen oder die Zeile anpassen.',
        send: 'Anfragen senden ({count})',
        sentCount: '{count} Anfragen sind unterwegs.',
        sentPartly: '{count} Anfragen sind bereits raus, die restlichen nicht. Frage nur die fehlenden Wachen noch einmal an, sonst bekommen die ersten dieselbe Anfrage zweimal.',
        nothingShared: 'Keine Partnerwache bietet dieser Wache im Moment etwas an.',
        nothingFree: 'Angeboten wird etwas, aber in diesem Zeitraum ist nichts davon frei.',
        forOccasion: 'Für {occasion} am {date}',
        noDate: 'Ohne Termin lässt sich nichts anfragen. Ruf die Seite aus dem Ausrüstungs-Reiter eines Termins auf, dann steht der Termin fest.',
    },
    signupLists: {
        menu: 'Aus den Anmeldungen',
        menuIntro: 'Zugesagt am {date}: {count}',
        menuNobody: 'Für diesen Termin hat niemand einen Platz.',
        checklist: 'Checkliste aus den Anmeldungen',
        checklistTitle: 'Checkliste aus den Anmeldungen',
        firstColumn: 'Erste Spalte',
        firstColumnHelp: 'Eine Checkliste ohne Spalte lässt sich nicht anlegen, deshalb wird hier gleich die erste gesetzt. Weitere kommen später auf der Liste dazu.',
        namePrefill: '{event} am {date}',
        setSummary: 'Übernommen werden {count} Personen, die am {date} einen Platz haben.',
        guestsLeftOut: 'Dazu kommen {count} Zusagen von Partnerwachen. Sie sind hier keine Mitglieder und können deshalb nicht übernommen werden.',
        formerLeftOut: '{count} Personen mit Zusage gehören nicht mehr zur Wache. Sie werden nicht übernommen.',
        kind: 'Art der Liste',
        kindSnapshot: 'Namen jetzt übernehmen',
        kindSnapshotHelp: 'Die Liste hält fest, wer heute einen Platz hat. Später Angemeldete fügst du von Hand hinzu.',
        kindFollowing: 'Dem Termin folgen',
        kindFollowingHelp: 'Die Liste merkt sich den Termin statt der Namen und holt beim Auffrischen alle nach, die inzwischen zugesagt haben.',
        snapshot: 'Die Liste wird jetzt übernommen. Wer sich später anmeldet, kommt nicht von selbst dazu.',
        following: 'Die Liste bleibt an diesen Termin gebunden. Wer sich später anmeldet, kommt beim nächsten Auffrischen dazu; wer absagt, bleibt stehen und wird als nicht mehr passend gekennzeichnet.',
        createError: 'Anlegen fehlgeschlagen.',
        loadError: 'Die Vorlagen konnten nicht geladen werden.',
        checklistCreated: 'Checkliste mit {count} Personen angelegt.',
        survey: 'Umfrage aus den Anmeldungen',
        surveyTitle: 'Umfrage aus den Anmeldungen',
        surveyName: 'Titel der Umfrage',
        surveyDraft: 'Die Umfrage wird als Entwurf angelegt und auf diese Personen beschränkt. Sie erreicht niemanden, bevor du die Fragen geschrieben und sie veröffentlicht hast.',
        surveyCreated: 'Entwurf angelegt, beschränkt auf {count} Personen. Er erreicht sie erst nach dem Veröffentlichen.',
        procedure: 'Ablauf aus den Anmeldungen',
        procedureTitle: 'Ablauf aus den Anmeldungen',
        procedureTemplate: 'Vorlage',
        procedureTemplatePlaceholder: 'Vorlage wählen',
        procedureTemplateHelp: 'Die Schritte kommen aus der Vorlage. Ohne Vorlage hat der Ablauf nichts zu tun, deshalb ist die Auswahl nötig.',
        procedureName: 'Name des Ablaufs',
        procedureDescription: 'Beschreibung',
        procedureDue: 'Fällig am',
        procedureDueHelp: 'Vorbelegt mit dem Termin selbst, weil die Vorbereitung bis dahin stehen muss.',
        procedureNoTemplates: 'Diese Wache hat noch keine Ablaufvorlage. Lege zuerst eine unter Abläufe > Vorlagen an, dann steht sie hier zur Auswahl.',
        procedureAssignees: 'Der Ablauf geht an {count} Personen. Jede von ihnen bekommt eine Benachrichtigung. Alle teilen sich dieselben Schritte: ein Haken zählt für alle.',
        procedureExists: 'Für den {date} gibt es bereits den Ablauf „{name}“. Ein zweiter daneben hilft niemandem.',
        procedureOpen: 'Vorhandenen Ablauf öffnen',
        procedureAnyway: 'Trotzdem neu anlegen',
        procedureCreated: 'Ablauf für {count} Personen angelegt.',
    },
    eventsRegistrations: {
        title: 'Offene Anmeldungen',
        empty: 'Keine offenen Anmeldungen.',
        accept: 'Bestätigen',
        deny: 'Ablehnen',
        pending: 'offen',
        accepted: 'bestätigt',
        denied: 'abgelehnt',
        declined: 'abgemeldet',
        withdrawn: 'zurückgezogen',
        expired: 'Frist abgelaufen',
        deadline: 'Frist',
        limit: 'Limit',
        member: 'Mitglied',
        score: 'Fairness',
        acceptedCol: 'Bestätigt',
        deniedCol: 'Abgelehnt',
        date: 'Datum',
        groupPending: 'Ausstehend',
        groupAccepted: 'Bestätigt',
        groupDenied: 'Abgelehnt',
        groupDeclined: 'Abgemeldet',
        groupWithdrawn: 'Zurückgezogen',
    },
    eventsUpcoming: {
        today: 'Heute',
        noToday: 'Keine Termine heute.',
        search: 'Suche',
        searchPlaceholder: 'Titel, Beschreibung oder Feldinhalt...',
        allCategories: 'Alle Kategorien',
        needsAction: 'Anmeldung offen',
        tabUpcoming: 'Kommend',
        tabPast: 'Vergangen',
        upcoming: 'Kommende Termine',
        past: 'Vergangene Termine',
        noUpcoming: 'Keine kommenden Termine.',
        noPast: 'Keine vergangenen Termine.',
        federated: 'Termine von Partnerwachen',
        attendance: 'Anwesenheit',
        registration: 'Anmeldung',
        registrationRequired: 'Anmeldung erforderlich',
        register: 'Anmelden',
        deadline: 'Frist',
        statusAccepted: 'Bestätigt',
        statusPending: 'Ausstehend',
        statusDenied: 'Abgelehnt',
        myself: 'Mich selbst',
        selectMember: 'Mitglied wählen',
        registerFor: '{name} anmelden',
        remove: 'Entfernen',
        unregister: 'Abmelden',
        heldByField: 'Über ein Feld des Termins eingetragen',
        heldByNamedField: 'Über das Feld "{field}" eingetragen',
        undoDecline: 'Abmeldung zurücknehmen',
        decline: 'Abmelden',
        declineFor: '{name} abmelden',
        statusDeclined: 'Abgemeldet',
        statusWithdrawn: 'Zurückgezogen',
        signOffConfirmTitle: 'Wirklich abmelden?',
        signOffConfirmBody: 'Der Platz wird wieder freigegeben. Wer später doch teilnehmen möchte, meldet sich neu an, und bei einem vollen Termin kann der Platz dann weg sein.',
        signedOff: 'Abgemeldet.',
        placesUncapped: 'Eure Wache entscheidet selbst, wer von euch mitkommt.',
        placesGiven: 'Eure Wache entscheidet selbst, wer von euch mitkommt: {total} Plätze stehen euch zu.',
        noPlacesLeft: 'Die ausrichtende Wache hat keine Plätze mehr für euch frei.',
        undoSignOff: 'Rückgängig',
        undoTooLate: 'Das lässt sich jetzt nicht mehr rückgängig machen. Melde dich neu an, solange der Termin Anmeldungen annimmt.',
        registrationClosedAskLead: 'Die Anmeldung ist geschlossen. Wer den Termin leitet, kann dich noch auf die Liste setzen.',
        listNotLoaded: 'Die Liste konnte nicht neu geladen werden. Was hier steht, ist möglicherweise nicht mehr aktuell.',
        moreNotLoaded: 'Weitere Termine konnten nicht geladen werden.',
        accepted: 'zugesagt',
        pendingCount: 'ausstehend',
        declinedCount: 'abgemeldet',
        loadMore: 'Weitere laden',
        viewList: 'Liste',
        viewCalendar: 'Kalender',
        calendarPrev: 'Vorheriger Monat',
        calendarNext: 'Nächster Monat',
        calendarToday: 'Heute',
        multiDay: 'Mehrtägiger Termin',
        registrationAudience: 'Teilnahme nur für: {audience}',
        audienceMemberOne: 'ein einzeln ausgewähltes Mitglied',
        audienceMemberMany: '{n} einzeln ausgewählte Mitglieder',
        audienceAnd: ' und ',
        audienceOr: ' oder ',
    },
    federatedEventDetail: {
        partnerEvent: 'Partnerwache',
        recurring: 'Wiederkehrend',
        oneTime: 'Einmalig',
        fields: 'Felder',
    },
    events: {
        attachments: {
            title: 'Dateien',
            hint: 'Dateien aus der Mediathek, die zu diesem Termin gehören: Laufzettel, Formulare, Pläne.',
            add: 'Datei anhängen',
            empty: 'Noch keine Dateien angehängt.',
            labelPlaceholder: 'Anzeigename (optional)',
            internal: 'Intern',
            internalHint: 'Intern: nur für alle mit dem Recht, interne Termindaten zu sehen. Sonst für alle, die den Termin sehen, Partnerwachen eingeschlossen.',
            remove: 'Datei entfernen',
            loadFailed: 'Die Dateien zu diesem Termin konnten nicht geladen werden.',
        },
        register: 'Anmelden',
        answerFor: 'Für wen zusagen?',
        declineFor: 'Für wen abmelden?',
        answerForHint: 'Wähle aus, für wen die Antwort gilt.',
        answerForFieldsHint: 'Fragen des Termins werden je Person gestellt.',
        answerForAll: 'Für alle zusagen',
        declineForAll: 'Für alle abmelden',
        registrationFields: {
            title: 'Angaben zur Anmeldung',
            sectionTitle: 'Fragen bei der Anmeldung',
            sectionHint: 'Diese Fragen werden jedem gestellt, der sich für den Termin anmeldet. Die Antworten stehen anschließend neben dem Namen in der Anmeldeliste.',
            addField: 'Frage hinzufügen',
            fieldName: 'Frage',
            fieldNamePlaceholder: 'z.B. Begleitpersonen',
            fieldType: 'Antworttyp',
            required: 'Pflichtangabe',
            defaultValue: 'Vorgabewert',
            defaultValueHint: 'Wird beim Öffnen vorausgefüllt und kann geändert werden.',
            options: 'Auswahlmöglichkeiten',
            min: 'Minimum',
            max: 'Maximum',
            overview: 'In der Anmeldeliste anzeigen',
            managersOnly: 'Antwort nur für die Terminverwaltung',
            managersOnlyHint: 'Die Frage wird allen gestellt, die Antwort sieht nur, wer den Termin betreut.',
            noFields: 'Noch keine Fragen hinterlegt.',
            range: 'Erlaubter Bereich: {min} bis {max}',
            missing: 'Bitte ausfüllen: {fields}',
            missingShort: 'fehlt',
        },
        today: 'Heute',
        manageAttendance: 'Anwesenheit erfassen',
        allEvents: 'Termine',
        tabCurrent: 'Aktuell',
        tabPast: 'Vergangen',
        upcomingDates: 'Einmalige Termine',
        pastDates: 'Vergangene Termine',
        series: 'Serientermine',
        endedSeries: 'Beendete Serien',
        nextOn: 'Nächster Termin: {date}',
        previouslyOn: 'Zuletzt am {date}',
        dateFrom: 'Von',
        dateTo: 'Bis',
        addCategory: 'Kategorie',
        manageCategories: 'Kategorien',
        editCategory: 'Kategorie bearbeiten',
        categoryName: 'Name',
        categoryNamePlaceholder: 'Kategoriename',
        category: 'Kategorie',
        noCategory: 'Keine Kategorie',
        breaks: 'Pausen & Ferien',
        addEvent: 'Neuer Termin',
        editEvent: 'Termin bearbeiten',
        announceAsNews: 'Als Neuigkeit ankündigen',
        takeAttendance: 'Anwesenheit erfassen',
        openAttendance: 'Erfassung öffnen',
        noEvents: 'Keine Termine vorhanden.',
        noPastEvents: 'Keine vergangenen Termine vorhanden.',
        loadMore: 'Weitere laden',
        addBreak: 'Neue Pause',
        editBreak: 'Pause bearbeiten',
        noBreaks: 'Keine Pausen definiert.',
        name: 'Name',
        namePlaceholder: 'Name des Termins',
        description: 'Beschreibung',
        descriptionPlaceholder: 'Optionale Beschreibung',
        type: 'Art',
        typeRecurring: 'Wöchentlich',
        typeRecurringShort: 'Wiederkehrend',
        typeOneTime: 'Einmalig',
        typeMonthlyFirst: 'Monatlich (1. Wochentag)',
        typeQuarterly: 'Vierteljährlich (1. Wochentag)',
        typeYearly: 'Jährlich',
        dayOfWeek: 'Wochentag',
        repeatEnd: 'Wiederholung endet',
        repeatEndNever: 'Nie',
        repeatEndOnDate: 'An einem Datum',
        repeatEndAfterCount: 'Nach einer Anzahl',
        repeatEndCountHint: 'Gezählt ab dem ersten Termin der Reihe.',
        repeatUntilLabel: 'Läuft bis {date}',
        repeatCountLabel: 'Läuft {count} Mal',
        startTime: 'Beginn',
        endTime: 'Ende',
        endBeforeStart: 'Das Ende darf nicht vor dem Beginn liegen.',
        template: 'Anwesenheitsvorlage',
        noTemplate: 'Keine Vorlage',
        templateHint: 'Verknüpft diesen Termin mit einer Anwesenheitsvorlage für schnellen Zugriff.',
        breakName: 'Name',
        breakNamePlaceholder: 'z.B. Sommerferien',
        breakStart: 'Von',
        breakEnd: 'Bis',
        requiresRegistration: 'Anmeldung erforderlich',
        requiresConfirmation: 'Anmeldung muss bestätigt werden',
        requiresConfirmationHint: 'Anmeldungen müssen von einem Verwalter bestätigt werden.',
        registrationLimit: 'Teilnehmerlimit',
        registrationLimitHint: 'Leer = unbegrenzt',
        hasDeadline: 'Anmeldefrist setzen',
        registrationDeadline: 'Anmeldefrist',
        eventFields: 'Termin-Felder',
        noFields: 'Keine Termin-Felder definiert.',
        export: 'Export',
        exportPdf: 'PDF exportieren',
        exportPeriod: 'Zeitraum',
        exportYear: 'Jahr',
        exportMonth: 'Monat',
        exportCategories: 'Kategorien',
        exportColumns: 'Spalten',
        exportColName: 'Termin',
        exportColType: 'Art',
        exportColDay: 'Wochentag',
        exportColDate: 'Datum',
        exportColTime: 'Zeit',
        exportColDescription: 'Beschreibung',
        exportNoColumns: 'Füge mindestens eine Spalte hinzu.',
        exportExtraFields: 'Termin-Felder',
        federation: 'Föderations-Freigabe',
        federationShareHint: 'Wenn aktiviert, können Mitglieder von Partnerwachen diesen Termin sehen und sich dafür anmelden.',
        partnerPlacesTitle: 'Plätze für Partnerwachen',
        partnerPlacesHint: 'Standardmäßig entscheidet ihr über jede Anmeldung einer Partnerwache. Ihr könnt einer Wache die Entscheidung überlassen und ihr dafür eine feste Zahl an Plätzen je Termin geben.',
        partnerPlacesLetPartnerDecide: 'Wache entscheidet selbst',
        partnerPlacesCap: 'Auf eine Platzzahl begrenzen',
        partnerPlacesHostDecides: 'Ihr entscheidet',
        partnerPlacesPartnerDecides: 'Die Wache entscheidet, ohne Begrenzung',
        partnerPlacesCapped: '{count} Plätze, die Wache entscheidet',
        federationNoPermission: 'Du benötigst die Berechtigung „Termine föderieren", um die Föderationseinstellungen zu ändern.',
        fieldDefaults: 'Feld-Vorbelegung',
        fieldDefaultsHint: 'Felder der Anwesenheitsvorlage mit Standardwerten oder Termin-Eigenschaften vorbelegen.',
        noDefault: 'Keine Vorbelegung',
        staticValue: 'Fester Wert',
        defaultValuePlaceholder: 'Wert eingeben...',
        defaultSources: {
            EVENT_NAME: 'Terminname',
            EVENT_DESCRIPTION: 'Beschreibung',
            EVENT_START_TIME: 'Startzeit',
            EVENT_END_TIME: 'Endzeit',
        },
        general: 'Allgemein',
        registration: 'Anmeldung',
        restrictions: 'Einschränkungen',
        restrictToRoles: 'Anmeldung einschränken',
        restrictToRolesHint: 'Wer sich anmelden darf. Ohne Auswahl alle. Wer nicht dazugehört, sieht den Termin '
            + 'weiterhin im Kalender und kann ihn nur nicht beantworten.',
        restrictVisibility: 'Sichtbarkeit einschränken',
        restrictVisibilityHint: 'Wer überhaupt von diesem Termin erfahren darf. Ohne Auswahl alle. Für alle '
            + 'anderen taucht er nirgends auf, weder im Kalender noch in einer Benachrichtigung. Ein so '
            + 'eingeschränkter Termin kann weder öffentlich stehen noch mit Partnerwachen geteilt werden.',
        notOpenToYou: 'Für dich ist hier keine Anmeldung möglich.',
        notOpenToHousehold: 'Weder du noch die Personen, die du verwaltest, können sich hier anmelden.',
        deleteEventConfirm: 'Termin "{name}" wirklich löschen?',
        deleteBreakConfirm: 'Pause "{name}" wirklich löschen?',
        importHolidays: 'Ferien importieren',
        importHolidaysTitle: 'Schulferien importieren',
        importHolidaysHint: 'Importiere Schulferien eines Bundeslandes als Pausen.',
        holidayState: 'Bundesland',
        holidayYear: 'Jahr',
        importHolidaysSubmit: 'Importieren',
        importHolidaysUnreachable: 'Das Ferienverzeichnis ist gerade nicht erreichbar.',
        cancelEvent: 'Veranstaltung absagen',
        cancelReason: 'Begründung (optional)',
        cancelConfirm: 'Diese Veranstaltung wirklich absagen? Alle angemeldeten Teilnehmer werden benachrichtigt.',
        minRegistrations: 'Mindestanzahl Anmeldungen',
        thresholdDate: 'Stichtag für Mindestanzahl',
        thresholdHint: 'Wird die Mindestanzahl bis zu diesem Datum nicht erreicht, wird die Veranstaltung automatisch abgesagt.',
        registrationCloseDays: 'Anmeldefrist (Tage vorher)',
        registrationCloseDaysHint: 'Offene Anmeldungen werden automatisch abgelehnt, wenn die Frist abgelaufen ist.',
        cancelled: 'Abgesagt',
    },
    eventEdit: {
        reminders: 'Erinnerungen',
        remindersHint: 'Mitglieder werden X Tage vor dem Termin benachrichtigt.',
        addReminder: 'Erinnerung hinzufügen',
        daysBefore: 'Tage vorher',
        noReminders: 'Keine Erinnerungen konfiguriert.',
    },
    attendancePast: {
        empty: 'Keine vergangenen Anwesenheiten vorhanden.',
        untitled: 'Ohne Titel',
        present: 'Anwesend',
        absent: 'Abwesend',
        declined: 'Abgesagt',
        unconfirmed: 'Offen',
    },
    attendanceSession: {
        exportFailed: 'Der Anwesenheitsbogen konnte nicht als PDF erzeugt werden.',
        detailsUnsaved: 'Titel und Zeiten des Bogens konnten nicht gespeichert werden.',
        fieldUnsaved: 'Dieses Feld konnte nicht gespeichert werden.',
        back: 'Zurück',
        openEvent: 'Termin öffnen',
        sync: 'Synchronisieren',
        export: 'PDF Export',
        exportOptions: {
            title: 'PDF exportieren',
            signature: 'Zum Unterschreiben',
            signatureHint:
                'Jede Zeile bekommt ein Feld für die Unterschrift. Der in Ember erfasste Status '
                + 'steht dann nicht auf dem Blatt: unterschrieben wird auf Papier, und was dort steht, gilt.',
            documentTitle: 'Titel des Dokuments',
            documentTitlePlaceholder: 'Titel der Erfassung',
            documentTitleHint: 'Leer lassen, um stattdessen eine Linie zum Ausfüllen zu drucken.',
            blankRows: 'Leerzeilen',
            blankRowsHint: 'Zusätzliche leere Zeilen für alle, die nicht auf der Liste stehen.',
            instanceUrl: 'Adresse der Instanz drucken',
            instanceUrlHint: 'Gilt nur für dieses Dokument. Die Voreinstellung steht in den Einstellungen der Wache.',
        },
        delete: 'Erfassung löschen',
        deleteConfirm: 'Diese Anwesenheitserfassung wirklich löschen? Alle erfassten Anwesenheiten gehen dabei verloren.',
        resetTimes: 'Zeiten zurücksetzen',
        checkMode: 'Anwesenheit prüfen',
        unconfirmed: 'Offen',
        present: 'Anwesend',
        absent: 'Abwesend',
        declined: 'Abgesagt',
        skip: 'Überspringen',
        endCheck: 'Prüfung beenden',
        allChecked: 'Alle Einträge geprüft!',
        addMember: 'Mitglied hinzufügen...',
        add: 'Hinzufügen',
        title: 'Titel',
        startTime: 'Beginn',
        endTime: 'Ende',
        countedHours: 'Gewertete Stunden',
        countedHoursPlaceholder: 'Nach Zeiten',
        countedHoursHint: 'Leer lassen, damit Beginn und Ende die Stunden bestimmen.',
        eventTimes: 'Der Termin läuft von {start} bis {end}.',
        takeEventTimes: 'Zeiten des Termins übernehmen',
        fields: 'Felder',
        otherMembers: 'Weitere Mitglieder',
        noEntry: 'Kein Eintrag',
        notesTitle: 'Offen für dieses Mitglied',
        beforeJoining: 'Noch nicht beigetreten',
        frozen: 'Diese Anwesenheit ist abgeschlossen und kann nicht mehr bearbeitet werden.',
        frozenForManager: 'Diese Anwesenheit ist abgeschlossen. Du kannst sie wieder öffnen, um sie zu bearbeiten.',
        reopen: 'Wieder öffnen',
        close: 'Abschließen',
        total: 'Gesamt',
    },
    attendanceReport: {
        presetSaveFailed: 'Der Filter konnte nicht gespeichert werden.',
        filters: 'Filter',
        userTypes: 'Mitgliedstypen',
        groups: 'Gruppen',
        selectUserTypes: 'Typen wählen',
        selectGroups: 'Gruppen wählen',
        period: 'Zeitraum',
        year: 'Jahr',
        month: 'Monat',
        quarter: 'Quartal',
        week: 'Kalenderwoche',
        rounding: 'Rundung',
        preview: 'Vorschau',
        exportPdf: 'PDF exportieren',
        savePreset: 'Filter speichern',
        presetName: 'Name der Vorlage',
        summary: 'Gesamtübersicht',
        monthlySummary: 'Monatsübersicht',
        presentOfExpected: 'Anwesend: {present} von {expected} erwartet',
        monthReport: 'Monatsbericht {month}',
        name: 'Name',
        sessions: 'Termine',
        present: 'Anwesend',
        hours: 'Stunden',
        from: 'Von',
        to: 'Bis',
        countedAs: 'Gewertet als {hours} Std.',
    },
    attendanceNew: {
        todayEvents: 'Heutige Termine',
        fromTemplate: 'Aus Vorlage erstellen',
        noTemplates: 'Keine Anwesenheitsvorlagen vorhanden.',
        template: 'Vorlage',
        create: 'Erstellen',
        creating: 'Anwesenheit wird erstellt...',
        modalTitle: 'Zeitraum der Erfassung',
        modalHint: 'Beginn und Ende bestimmen die Stunden, die gezählt werden. '
            + 'Ein Zeltlager darf über mehrere Tage laufen.',
        entersGroups: 'Trägt ein: {groups}',
        entersNobody: 'Trägt niemanden ein',
        asksFor: 'Fragt nach: {fields}',
        asksNothing: 'Fragt nichts ab',
        withoutTemplate: 'Ohne passende Vorlage',
        emptyTitle: 'Leere Anwesenheit',
        emptyHint: 'Im nächsten Schritt wählst du aus, wer eingetragen wird.',
        audienceTitle: 'Wer wird eingetragen?',
        audienceHint: 'Mitgliedstypen und Gruppen ergänzen sich: Wer einem davon entspricht, steht auf der Liste.',
        userTypes: 'Mitgliedstypen',
        groups: 'Gruppen',
        noGroups: 'Diese Wache hat noch keine Gruppen.',
        fieldsFrom: 'Felder übernehmen von',
        fieldsFromHint: 'Die Liste braucht Felder. Die Gruppen dieser Vorlage werden dabei nicht übernommen.',
    },
    membersConfig: {
        roles: {
            TRIAL: 'Schnupperer',
            MEMBER: 'Mitglieder',
            GUARDIAN: 'Erziehungsberechtigte',
            TEAM: 'Team',
            MANAGER: 'Leitung',
        },
        everyStation: 'Alle Wachen',
        audiences: {
            title: 'Wer wird gefragt',
            hint: 'Wem "{name}" gestellt wird, und wie es jeweils gestellt wird.',
            selectAQuestion: 'Wähle links eine Frage aus.',
            noneYet: 'Diese Frage wird noch niemandem gestellt.',
            none: 'Niemand',
            unknownGroup: 'Unbekannte Gruppe',
            add: 'Weitere Zielgruppe',
            addPlaceholder: 'Zielgruppe auswählen',
            kindRoles: 'Mitgliedsarten',
            kindGroups: 'Gruppen',
            inherit: 'Wie die Frage ({value})',
        },
        orderEmpty: 'Diese Zielgruppe wird noch nichts gefragt.',
        typeLabels: {
            TEXT: 'Text',
            NUMBER: 'Zahl',
            DATE: 'Datum',
            BIRTH_DATE: 'Geburtsdatum',
            BOOLEAN: 'Ja/Nein',
            ENUM: 'Auswahl',
            AGE: 'Alter (berechnet)',
            SECTION: 'Überschrift',
            SPACER: 'Abstand',
        },
        colWidth: 'Breite',
        widthShortFull: '1/1',
        widthShortHalf: '1/2',
        widthShortThird: '1/3',
        fieldWidth: 'Breite',
        fieldWidthHint: 'Schmale Felder stehen nebeneinander, solange eine Zeile sie fasst.',
        widthFull: 'Ganze Zeile',
        widthHalf: 'Halbe Zeile',
        widthThird: 'Drittel Zeile',
        previewTitle: 'Formular',
        previewHint: 'So sieht das Formular beim Ausfüllen aus. Felder lassen sich hier verschieben und '
            + 'an der rechten Kante breiter oder schmaler ziehen. Beides gilt nur für diese Zielgruppe.',
        preview: {
            moveHint: 'Zum Verschieben ziehen',
            resizeHint: 'Breite ziehen',
            spacer: 'Abstand ({width})',
        },
        previewUnnamed: 'Ohne Namen',
        fields: 'Profilfelder',
        addField: 'Feld hinzufügen',
        questionsHint: 'Jede Frage wird einmal beschrieben. Wem sie gestellt wird, steht daneben.',
        batch: {
            toggleRow: 'Frage auswählen',
            chosen: '{count} Fragen ausgewählt',
            assignPlaceholder: 'Allen eine Zielgruppe geben',
            clear: 'Auswahl aufheben',
        },
        noFields: 'Noch keine Felder definiert.',
        colName: 'Name',
        colType: 'Typ',
        templates: 'Schnellvorlagen',
        editField: 'Feld bearbeiten',
        fieldName: 'Feldname',
        fieldNamePlaceholder: 'Name des Feldes',
        fieldType: 'Feldtyp',
        fieldDescription: 'Erklärung',
        fieldDescriptionPlaceholder: 'Wobei hilft diese Angabe?',
        fieldDescriptionHint: 'Steht beim Ausfüllen unter dem Feldnamen. Optional.',
        fieldAgeSource: 'Datumsfeld',
        fieldAgeSourcePlaceholder: 'Datumsfeld auswählen',
        fieldAgeSourceHint: 'Das Datumsfeld, aus dem das Alter berechnet wird.',
        fieldAgeMode: 'Berechnung',
        fieldAgeModeNow: 'Aktuelles Alter',
        fieldAgeModeEndOfYear: 'Alter am Jahresende',
        fieldEnumOptions: 'Auswahlmöglichkeiten',
        fieldDefault: 'Standardwert',
        fieldDefaultPlaceholder: 'Kein Standardwert',
        fieldDefaultDateHint: 'Heutiges Datum als Standardwert verwenden.',
        fieldRequired: 'Pflichtfeld',
        fieldRequiredHint: 'Gilt für alle, die gefragt werden. Einzelne Zielgruppen können es anders halten.',
        fieldReadonly: 'Nur lesen',
        fieldReadonlyHint: 'Nur Nutzer mit Mitgliederverwaltung können dieses Feld bearbeiten. '
            + 'Gilt für alle, die gefragt werden.',
        writability: {
            column: 'Antwort änderbar von',
            hint: 'Wer die Antwort auf diese Frage ändern darf',
            EVERYONE: 'Allen',
            NOT_MEMBER: 'Wache und Verband',
            OWNER_ONLY: 'Nur Verband',
            station: {
                EVERYONE: 'Dieser Zielgruppe selbst',
                NOT_MEMBER: 'Nur der Mitgliederverwaltung',
            },
        },
        fieldNotifyOnChange: 'Bei Änderung benachrichtigen',
        fieldNotifyOnChangeHint: 'Mitgliederverwalter werden benachrichtigt, wenn dieses Feld geändert wird.',
        fieldShowAge: 'Alter hinter dem Datum anzeigen',
        fieldShowAgeHint: 'Das Geburtsdatum steht mit dem heutigen Alter dahinter, etwa 03.11.2019 (6). '
            + 'Schalte es aus, wenn das Alter bereits als eigenes Feld gefragt wird.',
        fieldOverview: 'In Übersicht anzeigen',
        fieldKeepOnArchive: 'Bei Archivierung behalten',
        fieldKeepOnArchiveHint: 'Dieses Feld bleibt bei ehemaligen Mitgliedern erhalten.',
        deleteConfirm: 'Feld "{name}" wirklich löschen? Alle gespeicherten Werte gehen verloren.',
    },
    memberSelect: {
        choose: 'Mitglied wählen',
        search: 'Nach Name oder E-Mail suchen',
        anyUserType: 'Alle Arten',
        nobody: 'Nicht zugewiesen',
        nobodyMatches: 'Niemand passt dazu',
        chosenCount: '{count} gewählt',
        remove: '{name} entfernen',
        showMore: '+{count} weitere',
        showFewer: 'Weniger zeigen',
    },
    memberGroups: {
        title: 'Gruppen',
        create: 'Neue Gruppe',
        empty: 'Noch keine Gruppen vorhanden.',
        createTitle: 'Gruppe erstellen',
        editTitle: 'Gruppe bearbeiten',
        name: 'Gruppenname',
        namePlaceholder: 'Name der Gruppe',
        save: 'Speichern',
        cancel: 'Abbrechen',
        delete: 'Löschen',
        deleteConfirm: 'Gruppe "{name}" wirklich löschen?',
        selectHint: 'Wähle eine Gruppe aus, um Mitglieder zu verwalten.',
        searchMembers: 'Nach Name oder E-Mail suchen',
        anyUserType: 'Alle Mitgliedstypen',
        showList: 'Liste anzeigen',
        hideList: 'Liste ausblenden',
        nobodyMatches: 'Niemand passt zur Suche.',
        currentMembers: 'Mitglieder',
        noMembers: 'Keine Mitglieder in dieser Gruppe.',
        addMembers: 'Mitglied hinzufügen',
        removeMember: 'Mitglied entfernen',
        allAdded: 'Alle Mitglieder sind bereits in dieser Gruppe.',
        noPermissions: 'Keine Berechtigungen zugewiesen. Mitglieder dieser Gruppe erhalten keine zusätzlichen Berechtigungen.',
        noPermissionAccess: 'Du hast keine Berechtigung, Gruppenberechtigungen zu bearbeiten.',
        tabMembers: 'Mitglieder',
        tabPermissions: 'Berechtigungen',
        convertToTag: 'In Tag umwandeln',
        convertToTagConfirm: 'Gruppe "{name}" in einen Tag umwandeln? Die Mitglieder bleiben erhalten, aber Rollen und Berechtigungen der Gruppe gehen verloren.',
        color: 'Farbe',
        colorHint: 'Optionale Farbe für Mitgliedernamen',
    },
    clusterStationGroups: {
        selectHint: 'Wähle eine Wachgruppe aus, um ihre Wachen zu verwalten.',
        currentStations: 'Wachen',
        noStations: 'Keine Wachen in dieser Gruppe.',
        addStations: 'Wache hinzufügen',
        removeStation: 'Wache entfernen',
        allAdded: 'Alle Wachen sind bereits in dieser Gruppe.',
        searchStations: 'Nach Wachnamen suchen',
        nothingMatches: 'Keine Wache passt dazu',
    },
    permissions: {
        partial: 'teilweise',
        loading: 'Berechtigungen werden geladen…',
        grantedBy: 'Durch „{name}" gewährt',
        grantedByUserType: 'Durch den Mitgliedstyp gewährt',
        grantedByGroup: 'Durch Gruppe „{name}" gewährt',
        lockedByUserType: 'Pflicht des Mitgliedstyps',
        LOGIN: {
            label: 'Login',
            desc: 'Erlaubt das Einloggen auf der Station.',
        },
        USER: {
            label: 'Nutzer',
            desc: 'Allgemeiner Zugriff auf die Station als eingeloggter Nutzer.',
        },
        ATTENDANCE_MANAGER: {
            label: 'Anwesenheit',
            desc: 'Vollzugriff auf alle Anwesenheitsfunktionen.',
        },
        ATTENDANCE_READ: {
            label: 'Anwesenheit einsehen',
            desc: 'Kann vergangene Anwesenheiten einsehen.',
        },
        ATTENDANCE_EDIT: {
            label: 'Anwesenheit erfassen',
            desc: 'Kann Anwesenheiten erstellen und Teilnehmer eintragen.',
        },
        ATTENDANCE_CONFIGURE: {
            label: 'Anwesenheit konfigurieren',
            desc: 'Kann Anwesenheitsvorlagen anlegen und bearbeiten.',
        },
        ATTENDANCE_EXPORT: {
            label: 'Anwesenheit exportieren',
            desc: 'Kann Anwesenheitsdaten als Datei exportieren.',
        },
        INVENTORY_MANAGER: {
            label: 'Inventar',
            desc: 'Vollzugriff auf alle Inventarfunktionen.',
        },
        INVENTORY_CREATE: {
            label: 'Gegenstände anlegen',
            desc: 'Kann neue Inventargegenstände erstellen.',
        },
        INVENTORY_CREATE_EXTERNAL: {
            label: 'Externe Gegenstände',
            desc: 'Kann Gegenstände anlegen, die dem Träger gehören.',
        },
        INVENTORY_CREATE_INTERNAL: {
            label: 'Interne Gegenstände',
            desc: 'Kann Gegenstände anlegen, die der Station gehören.',
        },
        INVENTORY_READ: {
            label: 'Inventar einsehen',
            desc: 'Kann die Inventarlisten aller Mitglieder einsehen.',
        },
        INVENTORY_EDIT: {
            label: 'Inventar bearbeiten',
            desc: 'Kann Gegenstände zuweisen, bearbeiten und als verloren markieren.',
        },
        INVENTORY_ASSIGN: {
            label: 'Gegenstände ausgeben',
            desc: 'Kann Gegenstände an Mitglieder ausgeben und Rückgaben annehmen.',
        },
        INVENTORY_STORAGE: {
            label: 'Lagerorte verwalten',
            desc: 'Kann Lagerorte anlegen, umbenennen, verschieben und Gegenstände einlagern.',
        },
        INVENTORY_PROCUREMENT: {
            label: 'Beschaffung',
            desc: 'Kann Beschaffungsanfragen erstellen und verwalten.',
        },
        INVENTORY_MOVEMENTS: {
            label: 'Vorgänge verwalten',
            desc: 'Kann alle Vorgänge der Wache einsehen und bearbeiten.',
        },
        INVENTORY_CHECK: {
            label: 'Inventur',
            desc: 'Kann eine Inventur bei Mitgliedern durchführen.',
        },
        INVENTORY_LENDING_REQUEST: {
            label: 'Ausleihe anfragen',
            desc: 'Kann Ausleih-Anfragen erstellen und eigene verwalten.',
        },
        INVENTORY_LENDING_MANAGER: {
            label: 'Ausleihe verwalten',
            desc: 'Kann alle Ausleih-Anfragen genehmigen oder ablehnen.',
        },
        EVENT_MANAGER: {
            label: 'Termine',
            desc: 'Vollzugriff auf alle Terminfunktionen.',
        },
        EVENT_EDIT: {
            label: 'Termine erstellen',
            desc: 'Kann Termine erstellen und bearbeiten.',
        },
        EVENT_INTERNAL: {
            label: 'Interne Termindaten',
            desc: 'Kann das benötigte Material und die internen Dateien eines Termins lesen, aber nichts ändern.',
        },
        EVENT_REGISTRATION: {
            label: 'Anmeldungen verwalten',
            desc: 'Kann Anmeldungen zu Terminen bestätigen oder ablehnen.',
        },
        EVENT_MANAGE_TEMPLATE: {
            label: 'Terminvorlagen',
            desc: 'Kann Vorlagen für wiederkehrende Termine verwalten.',
        },
        EVENT_MANAGE_CATEGORY: {
            label: 'Kategorien',
            desc: 'Kann Terminkategorien anlegen und bearbeiten.',
        },
        EVENTS_FEDERATE: {
            label: 'Termine föderieren',
            desc: 'Kann Termine mit Partnerstationen teilen.',
        },
        MEMBER_MANAGER: {
            label: 'Mitglieder',
            desc: 'Vollzugriff auf alle Mitgliederfunktionen.',
        },
        DOCUMENT_MANAGER: {
            label: 'Dokumente',
            desc: 'Vollzugriff auf die gesamte Ablage.',
        },
        DOCUMENT_READ: {
            label: 'Dokumente einsehen',
            desc: 'Kann die Ablage der Wache einsehen, also die Dokumente, die keinem Mitglied zugeordnet sind.',
        },
        DOCUMENT_READ_MEMBER: {
            label: 'Mitgliederdokumente einsehen',
            desc: 'Kann die Dokumente einsehen, die einem Mitglied zugeordnet sind.',
        },
        DOCUMENT_EDIT: {
            label: 'Dokumente verwalten',
            desc: 'Kann Dokumente der Wache ablegen, taggen und wieder entfernen.',
        },
        DOCUMENT_EDIT_MEMBER: {
            label: 'Mitgliederdokumente verwalten',
            desc: 'Kann festlegen, welche Mitglieder ein Dokument betrifft, und deren Dokumente ablegen und entfernen.',
        },
        MEMBER_READ: {
            label: 'Mitglieder einsehen',
            desc: 'Kann die Daten aller Mitglieder einsehen.',
        },
        MEMBER_SELF_UPLOAD: {
            label: 'Eigene Dokumente hochladen',
            desc: 'Kann Dokumente im eigenen Profil ablegen. Lesen darf sie jeder im eigenen Profil ohnehin.',
        },
        MEMBER_EDIT: {
            label: 'Mitglieder bearbeiten',
            desc: 'Kann Mitglieder anlegen und ihre Daten ändern.',
        },
        MEMBER_NOTES: {
            label: 'Notizen',
            desc: 'Kann interne Notizen zu Mitgliedern erstellen und lesen.',
        },
        MEMBER_GUARDIAN: {
            label: 'Erziehungsberechtigter',
            desc: 'Kann die Daten der eigenen zugeordneten Mitglieder verwalten.',
        },
        MEMBER_CHANGES: {
            label: 'Änderungen prüfen',
            desc: 'Wird über Änderungen an Mitgliederdaten informiert und kann diese bestätigen.',
        },
        MEMBER_MANAGE_GROUP: {
            label: 'Gruppen verwalten',
            desc: 'Kann Mitgliedergruppen anlegen, bearbeiten und zuordnen.',
        },
        MEMBER_MANAGE_TAGS: {
            label: 'Tags verwalten',
            desc: 'Kann Tags anlegen, bearbeiten und Mitgliedern zuweisen.',
        },
        MEMBER_FIELDS: {
            label: 'Profilfelder konfigurieren',
            desc: 'Kann die Profilfelder der Mitglieder anlegen und ändern.',
        },
        MEMBER_EXPORT: {
            label: 'Mitglieder exportieren',
            desc: 'Kann Mitgliederdaten als CSV exportieren.',
        },
        WAITLIST_MANAGER: {
            label: 'Warteliste',
            desc: 'Vollzugriff auf die Warteliste.',
        },
        WAITLIST_READ: {
            label: 'Warteliste einsehen',
            desc: 'Kann die Einträge auf der Warteliste sehen.',
        },
        WAITLIST_ADD: {
            label: 'Zur Warteliste hinzufügen',
            desc: 'Kann neue Einträge auf die Warteliste setzen.',
        },
        WAITLIST_EDIT: {
            label: 'Warteliste bearbeiten',
            desc: 'Kann Einträge auf der Warteliste bearbeiten und verwalten.',
        },
        NEWS_MANAGER: {
            label: 'Neuigkeiten',
            desc: 'Vollzugriff auf alle Neuigkeiten-Funktionen.',
        },
        NEWS_EDIT: {
            label: 'Neuigkeiten bearbeiten',
            desc: 'Kann Beiträge erstellen, bearbeiten und löschen.',
        },
        NEWS_FEDERATE: {
            label: 'Neuigkeiten föderieren',
            desc: 'Kann Neuigkeiten mit Partnerstationen teilen.',
        },
        POLL_MANAGER: {
            label: 'Umfragen',
            desc: 'Vollzugriff auf alle Umfrage-Funktionen.',
        },
        POLL_CREATE: {
            label: 'Umfragen erstellen',
            desc: 'Kann neue Umfragen anlegen.',
        },
        POLL_VIEW_RESULTS: {
            label: 'Ergebnisse ansehen',
            desc: 'Kann die Antworten und Ergebnisse von Umfragen einsehen.',
        },
        LOST_AND_FOUND_MANAGER: {
            label: 'Fundbüro',
            desc: 'Vollzugriff auf das Fundbüro.',
        },
        LOST_AND_FOUND_CREATE: {
            label: 'Fundstücke eintragen',
            desc: 'Kann neue Fundstücke erfassen.',
        },
        LOST_AND_FOUND_MANAGE: {
            label: 'Fundstücke verwalten',
            desc: 'Kann Fundstücke bearbeiten und als erledigt markieren.',
        },
        CHECKLIST_MANAGER: {
            label: 'Checklisten',
            desc: 'Vollzugriff auf alle Checklistenfunktionen.',
        },
        CHECKLIST_READ: {
            label: 'Checklisten einsehen',
            desc: 'Kann Checklisten öffnen und Einträge einsehen.',
        },
        CHECKLIST_MANAGE: {
            label: 'Checklisten verwalten',
            desc: 'Kann Checklisten erstellen, bearbeiten und Einträge pflegen.',
        },
        TEST_MANAGER: {
            label: 'Tests verwalten',
            desc: 'Vollzugriff auf alle Testfunktionen.',
        },
        TEST_CATALOG_VIEW: {
            label: 'Testkatalog ansehen',
            desc: 'Kann verfügbare Tests und Fragen einsehen.',
        },
        TEST_CATALOG_EDIT: {
            label: 'Testkatalog bearbeiten',
            desc: 'Kann Tests und Fragen erstellen und ändern.',
        },
        TEST_CONFIGURE: {
            label: 'Tests konfigurieren',
            desc: 'Kann neue Testdurchläufe einrichten.',
        },
        TEST_RESULT_READ: {
            label: 'Testergebnisse einsehen',
            desc: 'Kann die Ergebnisse von Testdurchläufen einsehen.',
        },
        TEST_REVIEW: {
            label: 'Tests bewerten',
            desc: 'Kann Testdurchläufe bewerten und benoten.',
        },
        PROTOCOL_MANAGER: {
            label: 'Protokolle verwalten',
            desc: 'Vollzugriff auf alle Protokollfunktionen.',
        },
        PROTOCOL_TESTER: {
            label: 'Protokoll ausfüllen',
            desc: 'Kann als Prüfer an einem Protokoll teilnehmen.',
        },
        PROTOCOL_CREATE: {
            label: 'Protokoll erstellen',
            desc: 'Kann neue Prüfprotokolle anlegen und starten.',
        },
        PROTOCOL_CONFIGURE: {
            label: 'Protokolle konfigurieren',
            desc: 'Kann Protokollvorlagen anlegen und bearbeiten.',
        },
        BOARD_MANAGER: {
            label: 'Boards',
            desc: 'Vollzugriff auf alle Board-Funktionen.',
        },
        BOARD_USE: {
            label: 'Boards nutzen',
            desc: 'Kann Boards und Tickets sehen, erstellen und bearbeiten.',
        },
        BOARD_EDIT: {
            label: 'Boards verwalten',
            desc: 'Kann Boards erstellen, Spalten und Einstellungen ändern.',
        },
        BOARD_FEDERATE: {
            label: 'Boards föderieren',
            desc: 'Kann Boards mit Partnerstationen teilen.',
        },
        KNOWLEDGE_MANAGER: {
            label: 'Wiki',
            desc: 'Vollzugriff auf das Wiki.',
        },
        KNOWLEDGE_EDIT: {
            label: 'Artikel bearbeiten',
            desc: 'Kann Artikel im Wiki erstellen und ändern.',
        },
        KNOWLEDGE_FEDERATE: {
            label: 'Wissen föderieren',
            desc: 'Kann Artikel mit Partnerstationen teilen.',
        },
        STATION_ADMINISTRATOR: {
            label: 'Stationsadministrator',
            desc: 'Vollzugriff auf alle Funktionen und Einstellungen der Station.',
        },
        CLUSTER_ADMINISTRATOR: {
            label: 'Verbandsleitung',
            desc: 'Vollzugriff auf alles, was der Verband entscheidet, und auf seine Mitglieder.',
        },
        CLUSTER_MANAGER: {
            label: 'Verbandsverwaltung',
            desc: 'Alles verwalten, was der Verband seinen Wachen vorgibt und für sie bereitstellt.',
        },
        CLUSTER_GENERAL: {
            label: 'Verband bearbeiten',
            desc: 'Name, Beschreibung und Vernetzung des Verbands ändern.',
        },
        CLUSTER_LOOK_AND_FEEL: {
            label: 'Erscheinungsbild',
            desc: 'Farben, Thema, Form und Logo für alle Wachen des Verbands vorgeben oder festsetzen.',
        },
        CLUSTER_FEDERATION: {
            label: 'Vernetzung',
            desc: 'Entscheiden, ob die Wachen des Verbands untereinander verbunden werden.',
        },
        CLUSTER_MODULES: {
            label: 'Module',
            desc: 'Module für alle Wachen des Verbands sperren.',
        },
        CLUSTER_STORAGE: {
            label: 'Speicherplatz',
            desc: 'Den Speicher des Verbands auf seine Wachen verteilen.',
        },
        CLUSTER_STATIONS: {
            label: 'Wachen',
            desc: 'Wachen anlegen, Beitrittsanfragen beantworten und Wachen wieder entlassen.',
        },
        CLUSTER_MEMBER_MANAGER: {
            label: 'Mitgliederverwaltung',
            desc: 'Die Mitglieder aller Wachen des Verbands suchen und bearbeiten.',
        },
        CLUSTER_MEMBER_READ: {
            label: 'Mitglieder sehen',
            desc: 'Die Mitglieder der Wachen des Verbands einsehen.',
        },
        CLUSTER_MEMBER_EDIT: {
            label: 'Mitglieder bearbeiten',
            desc: 'Angaben zu Mitgliedern der Wachen des Verbands ändern.',
        },
        CLUSTER_MEMBER_FIELDS: {
            label: 'Felder der Mitglieder',
            desc: 'Die Antworten auf die Felder des Verbands bearbeiten.',
        },
        CLUSTER_MEMBER_EXPORT: {
            label: 'Mitglieder exportieren',
            desc: 'Mitgliederlisten des Verbands herunterladen.',
        },
        CLUSTER_INVENTORY_MANAGER: {
            label: 'Materialverwaltung',
            desc: 'Das Material des Verbands vollständig verwalten.',
        },
        CLUSTER_INVENTORY_READ: {
            label: 'Material sehen',
            desc: 'Sehen, was der Verband besitzt und wo es gerade ist.',
        },
        CLUSTER_INVENTORY_EDIT: {
            label: 'Material bearbeiten',
            desc: 'Material des Verbands anlegen und ändern.',
        },
        CLUSTER_INVENTORY_TRANSFER: {
            label: 'Material ausgeben',
            desc: 'Material an eine Wache schicken und wieder zurückholen.',
        },
        CLUSTER_INVENTORY_MOVEMENTS: {
            label: 'Schritte bestätigen',
            desc: 'Die Schritte einer Bewegung bestätigen, die nur der Verband bestätigen kann.',
        },
        CLUSTER_FIELD_MANAGER: {
            label: 'Felderverwaltung',
            desc: 'Die Felder verwalten, die der Verband in jedem Profil abfragt.',
        },
        CLUSTER_FIELD_EDIT: {
            label: 'Felder bearbeiten',
            desc: 'Felder des Verbands anlegen und ändern.',
        },
        CLUSTER_KNOWLEDGE_MANAGER: {
            label: 'Wikiverwaltung',
            desc: 'Das Wiki des Verbands vollständig verwalten.',
        },
        CLUSTER_KNOWLEDGE_EDIT: {
            label: 'Wiki bearbeiten',
            desc: 'Artikel des Verbands schreiben und ändern.',
        },
        CLUSTER_NEWS_MANAGER: {
            label: 'Neuigkeitenverwaltung',
            desc: 'Die Neuigkeiten des Verbands vollständig verwalten.',
        },
        CLUSTER_NEWS_EDIT: {
            label: 'Neuigkeiten schreiben',
            desc: 'Neuigkeiten des Verbands schreiben und zurückziehen.',
        },
        CLUSTER_EVENT_MANAGER: {
            label: 'Terminverwaltung',
            desc: 'Die Termine des Verbands vollständig verwalten.',
        },
        CLUSTER_EVENT_EDIT: {
            label: 'Termine anlegen',
            desc: 'Termine des Verbands anlegen und absagen.',
        },
        STATION_MANAGER: {
            label: 'Stationseinstellungen',
            desc: 'Vollzugriff auf alle Stationseinstellungen.',
        },
        STATION_LOOK_AND_FEEL: {
            label: 'Erscheinungsbild',
            desc: 'Kann Farben, Logo und Design der Station ändern.',
        },
        STATION_GENERAL: {
            label: 'Allgemeine Einstellungen',
            desc: 'Kann den Namen und allgemeine Einstellungen der Station ändern.',
        },
        STATION_MAIL: {
            label: 'E-Mail-Einstellungen',
            desc: 'Kann die E-Mail-Konfiguration der Station ändern.',
        },
        STATION_FEDERATION: {
            label: 'Föderationseinstellungen',
            desc: 'Kann Partnerstationen verwalten und Föderationsoptionen konfigurieren.',
        },
        STATION_MODULES: {
            label: 'Module',
            desc: 'Kann festlegen, welche Funktionen auf der Station aktiviert sind.',
        },
        STATION_IMPORT_EXPORT: {
            label: 'Import & Export',
            desc: 'Kann Stationsdaten importieren und exportieren.',
        },
        STATION_STATISTICS: {
            label: 'Statistiken',
            desc: 'Kann die Stationsstatistiken einsehen.',
        },
        PAGE_MANAGER: {
            label: 'Seiten verwalten',
            desc: 'Vollzugriff auf öffentliche Seiten: veröffentlichen, löschen, Startseite festlegen.',
        },
        PAGE_EDIT: {
            label: 'Seiten bearbeiten',
            desc: 'Kann öffentliche Seiten erstellen und bearbeiten.',
        },
        PAGE_FORMS_VIEW: {
            label: 'Formulareinsendungen einsehen',
            desc: 'Kann Einsendungen von Kontaktformularen auf Seiten einsehen und bestätigen.',
        },
        PAGE_POLLS_VIEW: {
            label: 'Umfrageergebnisse einsehen',
            desc: 'Kann Ergebnisse und Antworten von Umfragen auf Seiten einsehen.',
        },
        PROCEDURE_MANAGER: {
            label: 'Abläufe verwalten',
            desc: 'Vollzugriff auf Abläufe und Vorlagen.',
        },
        PROCEDURE_EDIT: {
            label: 'Abläufe bearbeiten',
            desc: 'Kann Abläufe erstellen, bearbeiten und abhaken.',
        },
        PROCEDURE_READ: {
            label: 'Abläufe einsehen',
            desc: 'Kann zugewiesene Abläufe und deren Status einsehen.',
        },
    },
    userTypePermissions: {
        loadFailed: 'Die Berechtigungen der Mitgliedstypen konnten nicht geladen werden. Was hier steht, ist nicht der aktuelle Stand.',
        title: 'Typberechtigungen',
        description: 'Lege zusätzliche Berechtigungen fest, die alle Mitglieder eines bestimmten Typs auf dieser Station erhalten. Dies ergänzt die Standardberechtigungen des Typs.',
        selectHint: 'Wähle einen Mitgliedstyp aus, um die Berechtigungen zu bearbeiten.',
        noPermissions: 'Keine zusätzlichen Berechtigungen. Mitglieder dieses Typs erhalten nur die Standardberechtigungen.',
    },
    labelSelect: {
        placeholder: 'Auswählen...',
        search: 'Suchen...',
        searchOrCreate: 'Suchen oder erstellen...',
        create: '„{name}“ erstellen',
        empty: 'Nichts gefunden',
        draft: 'neu',
    },
    userTags: {
        title: 'Tags',
        create: 'Tag erstellen',
        createTitle: 'Neues Tag',
        editTitle: 'Tag bearbeiten',
        name: 'Name',
        namePlaceholder: 'Tag-Name eingeben',
        save: 'Speichern',
        cancel: 'Abbrechen',
        delete: 'Löschen',
        color: 'Farbe',
        visible: 'Als Badge anzeigen',
        visibleHint: 'Zeigt das Tag als Badge hinter dem Mitgliedernamen',
        selectHint: 'Tag auswählen, um Mitglieder zu verwalten',
        currentMembers: 'Mitglieder',
        noMembers: 'Keine Mitglieder',
        addMembers: 'Mitglieder hinzufügen',
        allAdded: 'Alle Mitglieder sind bereits zugeordnet',
        convertToGroup: 'In Gruppe umwandeln',
        empty: 'Keine Tags vorhanden.',
        removeMember: 'Entfernen',
        convertConfirmDetail: 'Soll der Tag "{name}" in eine Gruppe konvertiert werden? Der Tag wird dabei gelöscht.',
        deleteConfirmDetail: 'Soll der Tag "{name}" wirklich gelöscht werden?',
    },
    inventory: {
        storage: {
            title: 'Lager',
            searchPlaceholder: 'Behälter suchen…',
            scanNoMatch: 'Kein Behälter zu "{scan}" gefunden.',
            newContainer: 'Neuer Behälter',
            treeTitle: 'Behälter',
            searchResults: 'Suchergebnisse',
            searchKindContainer: 'Behälter',
            searchKindItem: 'Gegenstand',
            itemPathUnassigned: 'Nicht zugeordnet',
            empty: 'Noch keine Behälter angelegt.',
            noResults: 'Keine Treffer.',
            loadError: 'Behälter konnten nicht geladen werden.',
            contents: 'Inhalt',
            recursive: 'Verschachtelt anzeigen',
            addChild: 'Behälter hinzufügen',
            addChoice: {
                title: 'Behälter hinzufügen',
                intro: 'Was soll als Unter-Behälter hinzugefügt werden?',
                existing: 'Vorhandenen verschieben',
                existingHint: 'Einen bestehenden Behälter aus der Station unter diesen verschieben.',
                new: 'Neuen anlegen',
                newHint: 'Einen frischen Behälter erstellen und direkt hier ablegen.',
            },
            addExisting: {
                title: 'Vorhandenen Behälter hinzufügen',
                intro: 'Bestehenden Behälter unter den geöffneten verschieben. Mehrere Klicks nacheinander funktionieren ohne Zwischenklick.',
                searchPlaceholder: 'Behälter suchen…',
                empty: 'Keine passenden Behälter gefunden - entweder gibt es keine mehr, alle sind bereits hier, oder sie sind Unter-Behälter dieses Behälters.',
                moveHere: 'Hierher verschieben',
                moved: 'Verschoben',
                moveFailed: 'Verschieben fehlgeschlagen.',
                scanNotFound: 'Kein Behälter zu "{scan}" gefunden.',
                scanIneligible: '"{name}" kann nicht hierher verschoben werden.',
            },
            addItems: {
                button: 'Gegenstände hinzufügen',
                title: 'Gegenstände hinzufügen',
                intro: 'Gegenstände scannen oder suchen, auswählen und die Auswahl gemeinsam in diesen Behälter legen.',
                searchPlaceholder: 'Gegenstände suchen…',
                onlyUnstored: 'Nur Gegenstände ohne Lagerplatz',
                empty: 'Keine passenden Gegenstände gefunden.',
                scanNotFound: 'Kein Gegenstand zu "{scan}" gefunden.',
                addFailed: 'Gegenstände konnten nicht hinzugefügt werden.',
                submit: 'Hinzufügen ({count})',
            },
            childContainers: 'Behälter',
            containedItems: 'Gegenstände',
            contentsEmpty: 'Dieser Behälter ist leer.',
            history: 'Verlauf',
            historyEmpty: 'Keine Verlaufseinträge.',
            editTitle: 'Behälter bearbeiten',
            deleteTitle: 'Behälter löschen',
            deleteWarning: '"{name}" wirklich löschen? Untergeordnete Behälter werden zu Wurzelelementen, Items verlieren ihre Lagerzuordnung.',
            fields: {
                name: 'Name',
                namePlaceholder: 'z. B. Schrank A',
                parent: 'Übergeordneter Behälter',
                parentNone: '- Keine -',
                kind: 'Typ',
                kindPlaceholder: 'Typ suchen oder neu anlegen…',
                kindAdd: '＋ "{label}" anlegen',
                kindIconPickerHint: 'Symbol für "{label}" wählen - wird neben dem Typ angezeigt.',
                kindCreateFailed: 'Typ konnte nicht angelegt werden.',
                internalId: 'Interner Code',
                internalIdPlaceholder: 'optional, für Scanner',
                description: 'Beschreibung',
            },
            events: {
                CREATED: 'Erstellt',
                RENAMED: 'Umbenannt',
                MOVED: 'Verschoben',
                DELETED: 'Gelöscht',
            },
            scan: {
                itemAlreadyHere: '"{name}" ist bereits hier.',
                containerAlreadyHere: '"{name}" ist bereits untergeordnet.',
                selfTarget: 'Ein Behälter kann sich nicht selbst enthalten.',
            },
            errors: {
                nameRequired: 'Bitte einen Namen angeben.',
                createFailed: 'Behälter konnte nicht angelegt werden.',
                updateFailed: 'Speichern fehlgeschlagen.',
                deleteFailed: 'Löschen fehlgeschlagen.',
                cycle: 'Ein Behälter kann nicht unter einen seiner Unter-Behälter verschoben werden.',
                nameSlash: 'Der Name darf keinen Schrägstrich enthalten.',
            },
        },
        unknownScan: {
            title: 'Neues Item anlegen',
            introContainer: 'Dieser Code ist noch nicht registriert. Lege ein neues Item an - es wird direkt in diesem Behälter abgelegt.',
            introMember: 'Dieser Code ist noch nicht registriert. Lege ein neues Item an - es wird direkt diesem Mitglied zugewiesen.',
            code: 'Gescannter Code',
            targetInventory: 'Inventar',
            createNewInventory: '＋ Neues Inventar anlegen',
            newInventoryName: 'Name des neuen Inventars',
            newInventoryPlaceholder: 'z. B. Handschuhe',
            newInventoryType: 'Inventar-Typ',
            itemName: 'Bezeichnung',
            itemNamePlaceholder: 'z. B. Helm M-12',
            itemOwner: 'Eigentümer',
            create: 'Anlegen und ablegen',
            types: {
                INTERNAL: 'Eigentum der Wache',
                EXTERNAL: 'Eigentum des Trägers',
                MIXED: 'Gemischt - pro Item wählbar',
            },
            owners: {
                STATION: 'Wache',
                CLUSTER: 'Träger',
            },
            newInventoryHasSizes: 'Inventar hat Größen',
            newInventorySizes: 'Größen',
            newInventorySizesHint: 'Trage die verfügbaren Größen ein - eine pro Zeile.',
            newInventorySizePlaceholder: 'z. B. 152 oder M',
            addSize: 'Größe hinzufügen',
            itemSize: 'Größe',
            pickSize: '- Größe wählen -',
            errors: {
                loadFailed: 'Inventare konnten nicht geladen werden.',
                nameRequired: 'Bitte eine Bezeichnung angeben.',
                inventoryNameRequired: 'Bitte einen Namen für das neue Inventar angeben.',
                sizesRequired: 'Bitte mindestens eine Größe eintragen.',
                sizesDuplicate: 'Die Größe "{label}" ist doppelt eingetragen.',
                sizeRequired: 'Bitte eine Größe wählen.',
                fieldRequired: 'Bitte "{label}" ausfüllen.',
                createFailed: 'Anlegen fehlgeschlagen.',
                inventoryMadeButNotTheItem: 'Das neue Inventar wurde angelegt, der Gegenstand darin aber nicht. Wähle es unten aus und lege den Gegenstand dort an, statt das Inventar noch einmal zu erstellen.',
            },
        },
        memberInventory: {
            scanTitle: 'Per Scan zuweisen',
            scanHint: 'Code scannen, um ein Item diesem Mitglied zuzuweisen. Mehrere Scans nacheinander funktionieren ohne Zwischenklick.',
            scanPlaceholder: 'Code eingeben oder scannen…',
            alreadyHere: '"{name}" ist bereits diesem Mitglied zugewiesen.',
        },
        itemPicker: {
            placeholder: 'Item suchen oder Code scannen…',
            emptyNoMatch: 'Keine passenden Items.',
            heldBy: 'Bei {name}',
            storedAt: 'In {path}',
            unassigned: 'Im Inventar',
            lost: 'Verloren',
            scanNotFound: 'Kein Item zu "{scan}" gefunden.',
            unknownInventory: 'Inventar #{id}',
            unknownMember: 'Mitglied #{id}',
            badgeMember: 'Bei Mitglied',
            badgeStorage: 'Lager',
            badgeLost: 'Verloren',
            badgeFree: 'Frei',
        },
        artPicker: {
            placeholder: 'Art suchen…',
            empty: 'Keine passende Art.',
        },
        inventoryPicker: {
            placeholder: 'Inventar suchen…',
            empty: 'Kein passendes Inventar.',
        },
        iconPicker: {
            label: 'Symbol',
            placeholder: 'Symbol suchen…',
            empty: 'Kein passendes Symbol.',
            none: 'Ohne Symbol',
            color: 'Farbe',
            noColor: 'Ohne Farbe',
            preview: 'So wird es gezeichnet',
            groups: {
                protection: 'Schutzkleidung',
                equipment: 'Gerät',
                medical: 'Sanität',
                youth: 'Jugendarbeit',
                general: 'Allgemein',
            },
        },
        icons: {
            'helmet-safety': 'Helm',
            shirt: 'Jacke',
            't-shirt': 'T-Shirt',
            hoodie: 'Pullover',
            pants: 'Hose',
            vest: 'Weste',
            'vest-patches': 'Einsatzweste',
            'baseball-cap': 'Kappe',
            mitten: 'Handschuhe',
            boot: 'Stiefel',
            sneaker: 'Turnschuhe',
            'shoe-prints': 'Sohlen',
            sock: 'Socke',
            socks: 'Socken',
            mask: 'Maske',
            'head-side-mask': 'Atemschutzmaske',
            glasses: 'Schutzbrille',
            radio: 'Funkgerät',
            'walkie-talkie': 'Handfunkgerät',
            toolbox: 'Werkzeugkasten',
            headphones: 'Kopfhörer',
            'screwdriver-wrench': 'Werkzeug',
            wrench: 'Schlüssel',
            hammer: 'Hammer',
            'fire-extinguisher': 'Feuerlöscher',
            ladder: 'Leiter',
            'water-ladder': 'Poolleiter',
            axe: 'Axt',
            shovel: 'Schaufel',
            flashlight: 'Handlampe',
            lightbulb: 'Lampe',
            'battery-full': 'Akku',
            plug: 'Ladegerät',
            'gas-can': 'Kanister',
            'traffic-cone': 'Pylone',
            lifebuoy: 'Rettungsring',
            binoculars: 'Fernglas',
            'fire-truck': 'Fahrzeug',
            siren: 'Blaulicht',
            broom: 'Besen',
            trowel: 'Kelle',
            'kit-medical': 'Erste Hilfe',
            'suitcase-medical': 'Sanitätskoffer',
            'truck-medical': 'Rettungsdienst',
            dice: 'Würfel',
            'dice-d20': 'Spielwürfel',
            book: 'Buch',
            'book-open': 'Unterlagen',
            tent: 'Zelt',
            campfire: 'Lagerfeuer',
            flag: 'Fahne',
            compass: 'Kompass',
            'map-trifold': 'Landkarte',
            ruler: 'Lineal',
            camera: 'Kamera',
            bullhorn: 'Megafon',
            'graduation-cap': 'Ausbildung',
            box: 'Kiste',
            'box-open': 'Offene Kiste',
            'boxes-stacked': 'Kisten',
            cube: 'Gegenstand',
            warehouse: 'Lager',
            suitcase: 'Koffer',
            backpack: 'Rucksack',
            'bag-shopping': 'Tasche',
            briefcase: 'Aktentasche',
            'layer-group': 'Sammlung',
            folder: 'Ordner',
            key: 'Schlüssel',
            stapler: 'Tacker',
            'hand-holding': 'Geliehenes',
        },
        stock: {
            pieces: '{count} Stück',
            available: 'Vorhanden: {count} Stück',
            none: 'Davon ist gerade nichts an der Wache.',
            short: 'Verlangt sind {requested}, an der Wache sind {available}. Eintragen lässt sich das trotzdem, gedeckt ist es nicht.',
        },
        assign: {
            selectMember: 'Mitglied',
            pickMember: '- Bitte wählen -',
            scanItem: 'Scannen',
            bulkMode: 'Schnellmodus',
            bulkModeHint: 'Scans werden direkt verarbeitet - ideal für mehrere Items in Folge.',
            assigned: '"{name}" zugewiesen.',
            returned: '"{name}" zurückgenommen.',
            undone: 'Letzte Aktion rückgängig gemacht.',
            confirmReassign: 'Dieses Item ist aktuell {previous} zugewiesen. Trotzdem neu zuweisen?',
            recent: 'Verlauf dieser Sitzung',
            recentHint: 'Aufzeichnung im Browser - bei Neuladen verschwindet die Liste.',
            recentEmpty: 'Noch keine Aktionen.',
            undo: 'Letztes rückgängig',
            loadError: 'Mitgliederliste konnte nicht geladen werden.',
            errors: {
                pickMember: 'Bitte zuerst ein Mitglied auswählen.',
                failed: 'Aktion fehlgeschlagen.',
                memberLookupFailed: 'Mitglied konnte nicht zugeordnet werden. Bitte Seite neu laden.',
            },
        },
        handOut: {
            label: 'Ausgabe',
            now: 'Jetzt aushändigen',
            planned: 'Ausgabe planen',
            hintNow: 'Das Teil wechselt sofort den Besitzer und ist dem Mitglied zugewiesen.',
            hintPlanned: 'Es wird ein Vorgang angelegt. Das Teil bleibt im Lager und ist für das Mitglied '
                + 'vorgemerkt, bis die Übergabe bestätigt ist.',
            plannedFlash: '"{name}" für die Ausgabe vorgemerkt.',
            plannedFailed: 'Der Vorgang konnte nicht angelegt werden.',
        },
        checkContainer: {
            title: 'Behälter-Prüfung',
            searchPlaceholder: 'Behälter suchen…',
            loadError: 'Behälter konnten nicht geladen werden.',
            noContainers: 'Noch keine Behälter zum Prüfen verfügbar.',
            deep: 'Auch Unter-Behälter einbeziehen',
            walkPosition: 'Behälter {current} von {total}',
            walkPrev: 'Vorheriger',
            walkNext: 'Nächster Behälter',
            walkFinishHint: 'Letzter Behälter - du kannst die Prüfung jetzt abschließen.',
            walkConfirmedElsewhere: '"{name}" bestätigt - gehört in: {path}',
            walkEmptySubtree: 'In diesem Bereich sind keine Items eingetragen.',
            scanTitle: 'Scannen',
            scanPlaceholder: 'Code eingeben oder scannen…',
            scanNoMatch: 'Kein Item zu "{scan}" gefunden.',
            expected: 'Erwartet',
            expectedEmpty: 'Es ist nichts in diesem Behälter eingetragen.',
            extra: 'Zusätzlich gefunden',
            statusPending: 'Offen',
            statusConfirmed: 'Bestätigt',
            statusMissing: 'Fehlt',
            statusLost: 'Verloren',
            statusExtra: 'Zusätzlich',
            lastChecked: 'Zuletzt geprüft am {date} von {by}',
            lastCheckedNever: 'Noch nie geprüft',
            previousResult: {
                CONFIRMED: 'Bestätigt',
                NOT_IN_POSSESSION: 'Fehlte',
                LOST: 'Verloren',
            },
            markConfirmed: 'Als bestätigt markieren',
            markMissing: 'Als fehlend markieren',
            markLost: 'Als verloren markieren',
            borrowedHint: 'Gehört einer Partnerwache. Fehlt das Stück, wird es hier vermerkt, aber nicht '
                + 'als verloren gemeldet. Sagen Sie es der Wache über die Ausleih-Anfrage.',
            reset: 'Zurücksetzen',
            removeExtra: 'Entfernen',
            finish: 'Prüfung abschließen',
            completed: 'Prüfung abgeschlossen. Das Ergebnis ist gespeichert.',
            backToOverview: 'Zur Übersicht',
            completeError: 'Abschluss fehlgeschlagen.',
        },
        fields: {
            title: 'Benutzerdefinierte Felder',
            intro: 'Zusätzliche Datenfelder für alle Gegenstände dieses Inventars (Datum, Auswahl, Text, Zahl, Ja/Nein). '
                + 'Felder einer Art stehen an der Art, Felder eines einzelnen Gegenstands am Gegenstand.',
            introArt: 'Zusätzliche Datenfelder für alle Gegenstände dieser Art, zum Beispiel eine Auswahl „Farbe“ '
                + 'am Funkgerät. Die Werte bleiben bei jedem Gegenstand einzeln.',
            introItem: 'Zusätzliche Datenfelder für genau diesen Gegenstand und keinen anderen.',
            add: 'Feld hinzufügen',
            edit: 'Feld bearbeiten',
            empty: 'Noch keine benutzerdefinierten Felder.',
            label: 'Anzeigename',
            labelPlaceholder: 'z. B. Letzte Wartung',
            numberBelowMin: 'Der Wert muss mindestens {min} sein.',
            numberAboveMax: 'Der Wert darf höchstens {max} sein.',
            numberInvalid: 'Der Wert für "{label}" liegt außerhalb des erlaubten Bereichs.',
            key: 'Schlüssel',
            keyPlaceholder: 'z. B. last_service',
            type: 'Typ',
            sortOrder: 'Sortierung',
            required: 'Pflichtfeld',
            confirmDelete: 'Feld "{label}" wirklich löschen?',
            types: {
                DATE: 'Datum',
                ENUM: 'Auswahl',
                TEXT: 'Text',
                NUMBER: 'Zahl',
                BOOLEAN: 'Ja/Nein',
            },
            text: {
                multiline: 'Mehrzeilig',
                maxLength: 'Maximale Länge',
            },
            number: {
                min: 'Minimum',
                max: 'Maximum',
                step: 'Schritt',
                unit: 'Einheit',
            },
            boolean: {
                trueLabel: 'Bezeichnung Ja',
                falseLabel: 'Bezeichnung Nein',
            },
            enum: {
                options: 'Auswahlmöglichkeiten',
                value: 'Wert',
                label: 'Anzeige',
                add: 'Option hinzufügen',
            },
            errors: {
                loadFailed: 'Felder konnten nicht geladen werden.',
                saveFailed: 'Speichern fehlgeschlagen.',
            },
        },
        scan: {
            button: {
                label: 'Barcode scannen',
            },
            modal: {
                title: 'Barcode scannen',
                hint: 'Halte die Kamera ruhig über den Strichcode oder QR-Code. Die Erkennung läuft automatisch.',
                cancel: 'Abbrechen',
                requesting: 'Kamera wird angefragt...',
                scanning: 'Suche nach Code...',
            },
            error: {
                denied: 'Kamerazugriff verweigert. Erlaube den Zugriff in den Browser-Einstellungen, um den Scanner zu nutzen.',
                unsupported: 'Dieses Gerät oder dieser Browser unterstützt das Scannen von Barcodes nicht.',
                insecureContext: 'Scannen erfordert eine sichere Verbindung (HTTPS). Diese Seite wird über HTTP geladen - wechsle zu HTTPS oder rufe die App während der Entwicklung über localhost auf.',
                notFound: 'Keine Kamera gefunden.',
            },
            continuous: {
                confirmed: 'Erkannt',
            },
        },
        member: {
            returnEverything: 'Alles zurückfordern',
            returnEverythingDone: 'Für {count} Teile wurde die Rückgabe angefordert.',
        },
        overview: {
            lost: 'Verlorene Gegenstände',
            noLost: 'Keine Einträge.',
            exchanges: 'Offene Tausch-Anfragen',
            procurement: 'Offene Beschaffungen',
            colItem: 'Gegenstand',
            colOwner: 'Zugewiesen an',
            colLostSince: 'Verloren seit',
            colStatus: 'Status',
            colNotes: 'Notizen',
            colType: 'Typ',
            colUpdated: 'Aktualisiert',
            colRequested: 'Angefragt',
            updatedAt: 'Aktualisiert',
        },
        manage: {
            title: 'Inventarverwaltung',
            create: 'Inventar erstellen',
            createdWithoutSizes: 'Das Inventar wurde angelegt, aber nicht alle Größen konnten gespeichert werden. Trage die fehlenden in den Einstellungen des Inventars nach, statt es hier noch einmal anzulegen.',
            scanLabel: 'Gegenstand suchen',
            scanPlaceholder: 'Interne ID eingeben oder scannen...',
            scanSubmit: 'Öffnen',
            scanNotFound: 'Kein Gegenstand mit dieser internen ID gefunden.',
            sharesUnreadable: 'Es konnte nicht gelesen werden, welche Inventare ihr anderen Wachen anbietet. Die Karten unten zeigen deshalb kein Angebot an, auch wenn eines eingetragen ist.',
            empty: 'Noch kein Inventar angelegt.',
            name: 'Bezeichnung',
            namePlaceholder: 'z.B. Schutzkleidung',
            typeLabel: 'Typ',
            typeHint: 'Intern = Eigentum der Wache, Extern = Eigentum des Trägers, Gemischt = beides.',
            type: {
                INTERNAL: 'Intern',
                EXTERNAL: 'Extern',
                MIXED: 'Gemischt',
            },
            hasSizes: 'Größen verwenden',
            hasSizesHint: 'Aktiviere diese Option, wenn die Gegenstände in verschiedenen Größen vorhanden sind.',
            kindLabel: 'Was das Inventar enthält',
            kindStockName: 'Einheitlich',
            kindCollectionName: 'Sammlung',
            kindStockOption: 'Einheitlich: eine Sache in vielen Exemplaren',
            kindCollectionOption: 'Sammlung: verschiedene Dinge, die zusammengehören',
            kindStockHint:
                'Das Regal voller Blousons. Jedes Stück ist dasselbe Ding, nur ein weiteres Exemplar. ' +
                'Nur ein einheitliches Inventar kennt Größen, Vorgaben, Beschaffungen und Tausche.',
            kindCollectionHint:
                'Die Kiste mit zwölf Funkgeräten, einer Ladestation und einer Antenne. Die Stücke sind ' +
                'verschieden: gleiche Stücke fasst eine Art zusammen, alles andere liegt lose in der Sammlung.',
            artCount: '{count} Arten',
            artCountOne: 'Eine Art',
            withSizes: 'mit Größen',
            quickSizes: 'Schnellauswahl',
            quickSizesHint: 'Felder anklicken oder mit gedrückter Maustaste überstreichen. Eingefügt wird in der Reihenfolge der Anzeige.',
            quickSizesPicked: '{count} ausgewählt',
            quickSizesAdd: 'Übernehmen',
            sizesHint: 'Definiere die verfügbaren Größen für dieses Inventar.',
            sizeLabel: 'Größe (z.B. S, M, L)',
            next: 'Weiter',
            back: 'Zurück',
            itemCount: '{count} Gegenstände',
            lostCount: '{count} verloren',
            lentOutCount: '{count} verliehen',
            procurementCount: '{count} in Beschaffung',
            deleteConfirm: 'Inventar "{name}" wirklich löschen? Alle Gegenstände werden ebenfalls gelöscht.',
            dispatch: 'An eine Wache geben',
        },
        lossSettings: {
            title: 'Vermisstes Material',
            description:
                'Mitglieder können Material, das ihnen zugewiesen ist, selbst als vermisst melden. '
                + 'Eine Betreuung kann das für die Person übernehmen, die sie betreut.',
            noteRequired: 'Notiz beim Melden verlangen',
        },
        intake: {
            open: 'Bestand aufnehmen',
            title: 'Bestand aufnehmen',
            titleNamed: '{name} aufnehmen',
            subtitle: 'Vorhandene Ausrüstung eintragen und direkt zuweisen.',
            who: 'Wer kommt in die Liste',
            whoHint: 'Wähle, mit welchen Mitgliedern die Tabelle startet. Einzelne Zeilen kannst du danach '
                + 'streichen, und wer fehlt, lässt sich nachtragen.',
            whoAll: 'Alle Mitglieder',
            whoUserType: 'Nach Mitgliedstyp',
            whoGroup: 'Nach Gruppe',
            userType: 'Mitgliedstyp',
            group: 'Gruppe',
            chooseGroup: 'Gruppe wählen',
            load: 'Liste laden',
            member: 'Mitglied',
            size: 'Größe',
            number: 'Nummer',
            numberPlaceholder: 'optional',
            bulkSize: 'Größe für alle',
            chooseSize: 'Größe wählen',
            applyToEmpty: 'In leere Zeilen eintragen',
            removeLine: 'Zeile streichen',
            addByHand: 'Fehlt jemand? Hier nachtragen.',
            addByHandPlaceholder: 'Mitglied suchen',
            nobodyYet: 'Noch niemand in der Liste.',
            filled: '{count} Zeilen mit Angaben',
            owner: 'Eigentümer',
            ownerStation: 'Wache',
            ownerCluster: 'Träger',
            sortBy: {
                firstName: 'Vorname',
                lastName: 'Nachname',
            },
            askedFor: 'Aufnehmen',
            askedForHint: 'Nimmt den Gegenstand auch dann auf, wenn nichts einzutragen ist. '
                + 'Zeilen mit Größe, Nummer oder Feldern werden ohnehin aufgenommen.',
            save: 'Bestand eintragen',
        },
        detail: {
            edit: 'Inventar bearbeiten',
            total: 'Gesamt',
            free: 'Verfügbar',
            assigned: 'Zugewiesen',
            lost: 'Verloren',
            bySize: 'Nach Größe',
            size: 'Größe',
            noSize: 'Ohne Größe',
            shareBarLabel: '{free} verfügbar, {assigned} zugewiesen, {away} verliehen oder verloren',
            shareBarEmpty: 'Kein Bestand in dieser Größe',
            lostItems: 'Verlorene Gegenstände',
            freeItems: 'Verfügbare Gegenstände',
            item: 'Gegenstand',
            owner: 'Zugewiesen an',
            lostSince: 'Verloren seit',
            notes: 'Notizen',
            assign: 'Zuweisen',
            assignTo: 'Zuweisen an:',
            selectMember: 'Mitglied auswählen',
            procurement: 'Offene Beschaffungen',
            createProcurement: 'Beschaffung erstellen',
            anySize: 'Beliebige Größe',
            procurementNotes: 'Notizen zur Beschaffung...',
            lentOut: 'Verliehen',
            lentToStation: 'Verliehen an',
            lentQuantity: 'Anzahl',
            lentUntil: 'Bis',
            lentStatus: 'Status',
            statusLent: 'Ausgeliehen',
            statusApproved: 'Genehmigt',
            viewRequest: 'Anfrage anzeigen',
            lentTo: 'Verliehen an',
        },
        borrowed: {
            empty: 'Zurzeit liegt hier nichts, was einer Partnerwache gehört.',
            loadError: 'Die geliehene Ausrüstung konnte nicht geladen werden.',
            snapshotNote: 'Diese Angaben wurden bei der Übergabe kopiert und werden nicht nachgeführt. '
                + 'Sie zeigen die Ausrüstung so, wie sie übergeben wurde. Ändern, verleihen und löschen '
                + 'darf nur die Wache, der sie gehört.',
            colName: 'Bezeichnung',
            colId: 'Kennung',
            colOwner: 'Gehört',
            colDue: 'Zurück am',
            noDueDate: 'Kein Datum vereinbart',
        },
        edit: {
            back: 'Zurück zur Übersicht',
            backToStock: 'Zum Bestand',
            moveItems: 'Gegenstände verschieben',
            settings: 'Einstellungen',
            settingsSaved: 'Einstellungen gespeichert.',
            typeSwitchHint: 'Beim Umstellen auf Intern oder Extern werden die vorhandenen Gegenstände auf diesen '
                + 'Eigentümer umgestellt. Geliehene Gegenstände behalten ihren Eigentümer.',
            kindRefused: 'Das lässt sich noch nicht umstellen. Folgendes steht im Weg:',
            blocker: {
                REQUIREMENT: 'Vorgabe für {name}',
                PROCUREMENT: 'Offene Beschaffung für {name}',
                EXCHANGE: 'Laufender Tausch von {name}',
                SIZE: 'Größe {name}',
            },
            blockerUnnamed: {
                REQUIREMENT: 'Eine Vorgabe',
                PROCUREMENT: 'Eine offene Beschaffung',
                EXCHANGE: 'Ein laufender Tausch',
                SIZE: 'Eine Größe',
            },
            sizesTitle: 'Größen',
            addSize: 'Größe hinzufügen',
            editSize: 'Größe bearbeiten',
            noSizes: 'Noch keine Größen definiert.',
            sizeLabel: 'Bezeichnung',
            sizeNote: 'Notiz',
            sizeNotePlaceholder: 'Optionale Notiz zur Größe',
            itemsTitle: 'Gegenstände',
            addItem: 'Gegenstand hinzufügen',
            editItem: 'Gegenstand bearbeiten',
            noItems: 'Noch keine Gegenstände in diesem Inventar.',
            colName: 'Bezeichnung',
            colId: 'Kennung',
            colSize: 'Größe',
            colAssigned: 'Zugewiesen an',
            colOwner: 'Eigentümer',
            ownerStation: 'Wache',
            ownerCluster: 'Träger',
            ownerPartner: 'Partnerwache',
            filterAssigned: 'Zugewiesen',
            filterNotAssigned: 'Nicht zugewiesen',
            filterInStorage: 'Im Lager',
            filterNotInStorage: 'Nicht im Lager',
            itemName: 'Bezeichnung',
            itemNamePlaceholder: 'z.B. Helm',
            itemInternalId: 'Interne Kennung',
            itemInternalIdPlaceholder: 'z.B. HLM-001',
            itemSize: 'Größe',
            itemLocation: 'Lagerort',
            itemLocationNone: '- Kein Lagerort -',
            assign: 'Zuweisen',
            reassign: 'Umzuweisen',
            unassign: 'Zurücknehmen',
            assignTitle: 'Gegenstand zuweisen',
            assignHint: '"{name}" einem Mitglied zuweisen.',
            selectMember: 'Mitglied auswählen',
            assignSubmit: 'Zuweisen',
            historyTitle: 'Verlauf',
            noHistory: 'Noch kein Verlauf vorhanden.',
            givenOut: 'Ausgegeben',
            returned: 'Zurückgegeben',
            corrected: 'Korrektur',
            currentlyAssigned: 'Aktuell zugewiesen',
            deleteConfirm: 'Gegenstand "{name}" wirklich löschen?',
            markLost: 'Verloren',
            lost: 'Verloren',
            markFound: 'Ersetzt',
            itemQuantity: 'Anzahl',
            itemQuantityHint: 'Erstellt mehrere Gegenstände ohne individuelle Kennung.',
            quickAssign: 'Ausgeben',
            quickAssignHint: 'Erstellt automatisch einen Gegenstand und weist ihn dem Mitglied zu.',
            quickAssignLeftFree: 'Der Gegenstand wurde angelegt, konnte aber niemandem zugewiesen werden. Er steht jetzt frei im Bestand: weise ihn dort zu, statt hier einen zweiten anzulegen.',
            reloadFailed: 'Die Änderung wurde gespeichert, aber die Seite konnte danach nicht aktualisiert werden.',
            externalItemsHint: 'Externe Gegenstände werden automatisch erstellt, wenn sie einem Mitglied zugewiesen werden. Nutze "Ausgeben" um Gegenstände zuzuweisen.',
            searchItems: 'Gegenstände durchsuchen...',
        },
        art: {
            title: 'Arten',
            intro: 'Eine Art fasst gleiche Dinge in dieser Sammlung zusammen, zum Beispiel alle blauen '
                + 'Funkgeräte. Die Bezeichnung eines Gegenstands bleibt daneben stehen: „Pager 01“ ist '
                + 'ein Stück der Art „Pager“. Stücke ohne Art, etwa die Ladestation, liegen einfach '
                + 'lose in der Sammlung.',
            field: 'Art',
            fieldHint: 'Darf leer bleiben. Die meisten Gegenstände haben keine Art.',
            add: 'Art anlegen',
            edit: 'Art bearbeiten',
            name: 'Name',
            namePlaceholder: 'z. B. Funkgerät blau',
            note: 'Notiz',
            notePlaceholder: 'Optional',
            none: 'Keine Art',
            none2: 'Für dieses Inventar gibt es noch keine Arten.',
            empty: 'Keine Art gefunden',
            pieces: '{pieces} Stück, davon {free} frei',
            groupCount: '{count} Stück',
            looseTitle: 'Ohne Art',
            fieldsAfterSave: 'Felder lassen sich vergeben, sobald die Art angelegt ist.',
            deleteConfirm: 'Art „{name}“ löschen? Die Gegenstände bleiben erhalten und verlieren nur ihre Art.',
            onlyForDrawers: 'Arten gibt es nur in einer Sammlung.',
            backToInventory: 'Zurück zum Inventar',
            tidyLink: 'Aufräumen',
            tidyIntro: 'Hier stehen die Bezeichnungen, die auf den Gegenständen dieses Inventars stehen, '
                + 'mit ihrer Anzahl. So fällt auf, wo dasselbe zweimal geschrieben wurde. Wähle die '
                + 'Bezeichnungen aus, die dasselbe meinen, und gib der Gruppe eine Art.',
            nothingToTidy: 'In diesem Inventar liegt nichts, was sich aufräumen ließe.',
            nameCount: '{pieces} Stück, davon {unassigned} ohne Art',
            existing: 'Vorhandene Art',
            newOne: 'Neue Art anlegen',
            newName: 'Name der Art',
            newNameHint: 'Die Art wird erst angelegt, wenn du unten auf eine Schaltfläche klickst.',
            willRename: '{count} Gegenstände werden der Art „{name}“ zugeordnet.',
            mergeSubmit: 'Zusammenführen und umbenennen',
            assignSubmit: 'Nur Art setzen',
            assignHint: 'Beim Zusammenführen bekommen die Gegenstände den Namen der Art. Das lässt sich '
                + 'nicht rückgängig machen. „Nur Art setzen“ lässt die Bezeichnungen unverändert.',
            tidied: '{count} Gegenstände aufgeräumt.',
        },
        tag: {
            title: 'Tags',
            intro: 'Ein Tag sagt, wozu ein Gegenstand gehört, und gilt über alle Inventare hinweg. '
                + 'Die Funkgeräte, die Ladestation und die Antenne tragen so dasselbe Wort, obwohl sie '
                + 'nicht dasselbe sind.',
            field: 'Tags',
            fieldHint: 'Darf leer bleiben. Ein neues Wort wird erst beim Speichern angelegt.',
            column: 'Tags',
            add: 'Tag anlegen',
            edit: 'Tag bearbeiten',
            name: 'Wort',
            namePlaceholder: 'z. B. Funk',
            color: 'Farbe',
            none: 'Keine Tags',
            none2: 'Diese Abteilung verwendet noch keine Tags.',
            empty: 'Kein Tag gefunden',
            itemCount: '{count} Gegenstände',
            deleteConfirm: 'Tag „{name}“ löschen? Die Gegenstände bleiben erhalten und verlieren nur '
                + 'dieses Wort.',
            recommended: 'Vom Verband empfohlen',
            recommendedHint: 'Der Verband schlägt diese Wörter vor. Ein Wort, das ihr schon verwendet, ist '
                + 'bereits dasselbe Wort und muss nicht angelegt werden.',
            recommendedAdopted: '{name} (bereits in Verwendung)',
        },
        move: {
            back: 'Zurück zum Inventar',
            explainer:
                'Die Gegenstände bleiben dieselben: Kennung, Verlauf und wer sie gerade hat, ziehen mit um. '
                + 'Nur die Größe gehört zum alten Inventar. Gibt es im Ziel eine Größe mit demselben Namen, '
                + 'behält der Gegenstand sie, sonst kommt er ohne Größe an.',
            target: 'Ziel',
            targetLabel: 'Inventar',
            selectTarget: 'Inventar auswählen',
            selectAll: 'Alle auswählen',
            nothingToMove: 'In diesem Inventar liegt nichts, was sich verschieben ließe.',
            sizeKept: 'Größe {size} bleibt',
            sizeCleared: 'Größe {size} entfällt',
            submit: '{count} Gegenstände verschieben',
            moved: '{count} Gegenstände verschoben.',
        },
        request: {
            title: 'Material anfragen',
            hint: 'Ihr könnt {name} um ein Teil bitten, das ihr nicht habt. Der Verband schickt eines oder '
                + 'lehnt mit einer Begründung ab; ihr seht beides in den Bewegungen.',
            ask: 'Beim Verband anfragen',
        },
        requirements: {
            title: 'Benötigt',
            hint: 'Definiere, welche Inventargegenstände Mitglieder mit einer bestimmten Rolle oder Gruppe besitzen '
                + 'sollen. Nur ein einheitliches Inventar kommt dafür in Frage, denn in einer Sammlung gibt es nichts, '
                + 'wovon jeder eines braucht.',
            empty: 'Noch kein benötigter Bestand definiert.',
            add: 'Anforderung hinzufügen',
            addItem: 'Hinzufügen',
            userType: 'Benutzertyp',
            group: 'Gruppe',
            targetType: 'Zuweisungstyp',
            stationGroup: 'Gilt für',
            everyStation: 'Alle Wachen',
            stationGroupBadge: 'Nur {name}',
            selectUserType: 'Benutzertyp auswählen',
            selectGroup: 'Gruppe auswählen',
            inventory: 'Inventar',
            selectInventory: 'Inventar auswählen',
            quantity: 'Anzahl',
            byUserType: 'Nach Benutzertyp',
            byGroup: 'Nach Gruppe',
        },
        line: {
            lineKind: 'Art der Zeile',
            kindItem: 'Ein bestimmtes Stück',
            kindArt: 'Eine Anzahl einer Art',
            kindCount: 'Eine Anzahl aus einem einheitlichen Inventar',
            item: 'Gegenstand',
            art: 'Art',
            noArts: 'In den Sammlungen ist noch keine Art angelegt.',
            inventory: 'Inventar',
            quantity: 'Anzahl',
        },
        check: {
            exchange: 'Tausch',
            inExchange: '(davon {count} im Tausch)',
            exchangeReasonTooSmall: 'Zu klein',
            exchangeReasonDamaged: 'Kaputt',
            exchangeReasonOther: 'Eigene Angabe',
            exchangeReasonTooSmallText: 'Passt nicht, eine Größe größer nötig',
            exchangeReasonDamagedText: 'Kaputt, Ersatz in gleicher Größe nötig',
            exchangeOwnReason: 'Was ist mit dem Stück?',
            exchangeNewSize: 'Gewünschte Größe',
            exchangeReason: 'Grund',
            exchangeHandedIn: 'Wurde der Gegenstand abgegeben?',
            exchangeHandedInYes: 'Abgegeben',
            exchangeHandedInNo: 'Bleibt beim Mitglied',
            exchangeCreate: 'Tausch anfragen',
            onTheMove: 'Für dieses Stück läuft bereits ein Vorgang, ein Tausch ist erst danach wieder möglich.',
            procurementNoted: 'Beschaffung vorgemerkt.',
            title: 'Inventarprüfung',
            tabTeam: 'Team',
            tabMember: 'Mitglieder',
            start: 'Prüfung starten',
            lastResult: 'Letzte Prüfung',
            showLastCheck: 'Ergebnis',
            backToOverview: 'Zurück',
            noLastCheck: 'Keine Prüfung vorhanden.',
            resultConfirmed: 'Vorhanden',
            resultNotInPossession: 'Nicht im Besitz',
            resultLost: 'Verloren',
            complete: 'Prüfung abschließen',
            completePartial: 'Geprüftes speichern',
            cancel: 'Abbrechen',
            noMembers: 'Keine Mitglieder vorhanden.',
            member: 'Mitglied',
            lastChecked: 'Zuletzt geprüft',
            checkedBy: 'Geprüft von',
            reportedBy: 'Angegeben von',
            status: 'Status',
            neverChecked: 'Noch nie geprüft',
            locked: 'Wird geprüft von',
            lockedByOther: 'Diese Prüfung hat gerade jemand anderes offen. Zwei Leute können dasselbe Mitglied nicht gleichzeitig prüfen.',
            recordedButNoNext: 'Die Prüfung ist gespeichert. Wer als Nächstes dran ist, konnte aber nicht ermittelt werden, du bleibst deshalb auf dieser Seite.',
            lockedByMe: 'Von dir gestartet',
            continue: 'Fortsetzen',
            confirmed: 'Vorhanden',
            notInPossession: 'Nicht im Besitz',
            lost: 'Verloren',
            notePlaceholder: 'Optionale Notiz...',
            createProcurement: 'Beschaffung',
            procurementCreated: 'Beschaffung erstellt',
            emptySlot: 'Kein Gegenstand zugewiesen',
            selectItem: 'Gegenstand auswählen...',
            assign: 'Zuweisen',
            unassignConfirm: 'Gegenstand wirklich zurücknehmen? Er wird dem Mitglied entzogen.',
            selectSize: 'Größe auswählen...',
            create: 'Erstellen & zuweisen',
            markAll: 'Alle bestätigen',
            submitting: 'Wird gespeichert...',
            rapidCheck: 'Schnellprüfung',
            rapidProgress: '{current} Gegenstände verbleibend',
            rapidDone: 'Alle Gegenstände wurden geprüft.',
            rapidScanNoMatch: 'Dieser Code passt zu keinem offenen Gegenstand der Prüfung.',
            missingItem: 'Fehlender Gegenstand',
            backToList: 'Zurück zur Übersicht',
            skip: 'Überspringen',
            previousItem: 'Zurück',
            correct: {
                action: 'Korrigieren',
                title: 'Eintrag korrigieren',
                intro: 'Das Mitglied hält etwas anderes als eingetragen ist. Hier wird der Eintrag richtiggestellt, bewegt wird nichts.',
                oldToStation: 'Der bisher eingetragene Gegenstand geht zurück ins Lager der Wache.',
                oldToCluster: 'Der bisher eingetragene Gegenstand geht zurück an den Träger.',
                oldVanishes: 'Der bisher eingetragene Gegenstand wird entfernt. Der Träger führt hier kein Lager, in das er zurückgehen könnte.',
                source: 'Was hält das Mitglied?',
                fromStock: 'Einen vorhandenen Gegenstand',
                newPiece: 'Einen neuen Gegenstand',
                pick: 'Aus dem Bestand',
                owner: 'Eigentümer',
                ownerStation: 'Wache',
                ownerCluster: 'Träger',
                number: 'Nummer',
                numberPlaceholder: 'optional',
                confirm: 'Korrektur speichern',
            },
        },
    },
    stationManage: {
        nameTitle: 'Wachname',
        name: 'Name',
        namePlaceholder: 'Name der Wache',
        timezone: 'Zeitzone',
        locale: 'Sprache',
        pdfHidesInstanceUrl: 'Adresse der Instanz auf PDFs ausblenden',
        pdfHidesInstanceUrlHint:
            'In der Fußzeile jedes exportierten PDFs steht die Adresse dieser Instanz. '
            + 'Ausgeblendet bleibt alles andere erhalten: Logo, Wachname, wer das Dokument erstellt '
            + 'hat und die Seitenzahl. Beim Export lässt sich das für ein einzelnes Dokument abweichend wählen.',
        nicknamesEnabled: 'Spitznamen verwenden',
        nicknamesEnabledHint:
            'Mitglieder können einen Spitznamen hinterlegen, der auf den Bildschirmen, in '
            + 'Benachrichtigungen und in Mails an die Stelle ihres Vornamens tritt. Auf '
            + 'Anwesenheitslisten, Ausdrucken und Exporten steht weiterhin der Name aus dem Register. '
            + 'Ausgeschaltet bleibt jeder hinterlegte Spitzname erhalten, wird aber nirgends gelesen.',
        save: 'Speichern',
        saved: 'Gespeichert',
        logoTitle: 'Logo',
        noLogo: 'Kein Logo hochgeladen.',
        uploadLogo: 'Logo hochladen',
        changeLogo: 'Logo ändern',
        logoUploaded: 'Logo wurde hochgeladen.',
        logoDeleted: 'Logo wurde entfernt.',
        logoTooLarge: 'Das Logo darf maximal 2 MB groß sein.',
        logoHint: 'Erlaubte Formate: PNG, JPEG, WebP, SVG.',
        mailTitle: 'E-Mail-Versand',
        mailHint: 'Konfiguriere einen eigenen E-Mail-Dienst für diese Wache. Ohne Konfiguration wird der globale Dienst verwendet.',
        mailProvider: 'Anbieter',
        mailProviderNone: 'Nicht konfiguriert',
        mailProviderCustomSmtp: 'Eigener SMTP-Server',
        mailProviderName: 'Anbieter-Name',
        mailProviderNamePlaceholder: 'z.B. Eigener Server',
        mailProviderUrl: 'Datenschutz-URL des Anbieters',
        mailSenderAddress: 'Absender-Adresse',
        mailSenderName: 'Absender-Name',
        mailServerTitle: 'Serveradresse',
        mailSmtpHost: 'SMTP-Host',
        mailSmtpPort: 'SMTP-Port',
        mailSmtpUser: 'SMTP-Benutzer',
        mailSmtpPassword: 'SMTP-Passwort',
        mailPasswordPlaceholder: 'Unverändert lassen',
        mailApiKeyPlaceholder: 'Gespeichert - leer lassen für keine Änderung',
        mailTest: 'Verbindung testen',
        mailTestSuccess: 'Verbindung erfolgreich!',
        mailTestFailed: 'Verbindungstest fehlgeschlagen',
        mailTestMail: 'Test-Mail an mich senden',
        mailTestMailSent: 'Test-Mail wurde versendet. Prüfe dein Postfach.',
        mailLimits: 'Sendelimits',
        mailDailyLimit: 'Tägliches Limit',
        mailMonthlyLimit: 'Monatliches Limit',
        mailSentToday: '{count} / {limit} heute gesendet',
        mailSentMonth: '{count} / {limit} diesen Monat gesendet',
        mailSaved: 'E-Mail-Konfiguration gespeichert.',
        mailClear: 'Konfiguration löschen',
        mailClearConfirm: 'Die E-Mail-Konfiguration dieser Wache wird vollständig entfernt. Fortfahren?',
        mailCleared: 'E-Mail-Konfiguration entfernt.',
        mailSaveFailed: 'Speichern fehlgeschlagen: {error}',
        modulesTitle: 'Module',
        modulesHint: 'Aktiviere oder deaktiviere Module für diese Wache. Deaktivierte Module werden im Menü und auf dem Dashboard ausgeblendet.',
        moduleClusterLocked: 'Vom Verband {cluster} abgeschaltet',
        lookClusterLocked: 'Der Verband {cluster} gibt Teile des Erscheinungsbilds vor. Was er festgelegt hat, lässt sich hier nicht ändern.',
        moduleInventory: 'Inventar',
        moduleNews: 'Neuigkeiten',
        moduleEvents: 'Termine',
        moduleAttendance: 'Anwesenheit',
        moduleForms: 'Umfragen',
        moduleLostAndFound: 'Fundbüro',
        moduleWaitingList: 'Warteliste',
        moduleQuiz: 'Quiz',
        moduleTestProtocol: 'Prüfprotokolle',
        moduleKnowledgeBase: 'Wiki',
        moduleBoards: 'Boards',
        moduleProcedures: 'Abläufe',
        moduleDocuments: 'Dokumente',
        ownerBadge: 'Eigentümer',
        transferOwnershipTitle: 'Eigentümer übertragen',
        transferOwnershipHint: 'Übertrage die Eigentümerschaft dieser Wache an ein anderes Mitglied mit der Manager-Rolle.',
        transferOwnershipSubmit: 'Übertragen',
        transferOwnershipSuccess: 'Eigentümerschaft wurde übertragen.',
        transferOwnershipSelect: 'Neuen Eigentümer auswählen',
        deleteTitle: 'Wache löschen',
        deleteHint: 'Lösche diese Wache und alle zugehörigen Daten unwiderruflich. Du erhältst eine Bestätigungs-E-Mail.',
        deleteRequest: 'Löschung beantragen',
        deleteRequesting: 'Wird gesendet...',
        deleteRequested: 'Bestätigungs-E-Mail wurde gesendet. Prüfe deinen Posteingang.',
        deleteDone: 'Die Wache wurde gelöscht. Diese Instanz kann keine E-Mails versenden, daher gab es '
            + 'keine Rückfrage per E-Mail.',
        importTitle: 'Wache importieren',
        importHint: 'Importiere Daten von einer anderen Ember-Instanz in diese Wache. Der Manager der Quell-Wache erstellt einen Transfer-Code; die Quell-URL ist im Code enthalten.',
        importToken: 'Transfer-Code',
        importTokenPlaceholder: 'Code vom Quell-Manager',
        importStart: 'Import starten',
        importStarting: 'Verbinde...',
        importProgress: 'Importiere: {table} ({completed}/{total})',
        importCompleted: 'Import abgeschlossen!',
        importFailed: 'Import fehlgeschlagen: {error}',
        importConfirmTitle: 'Daten überschreiben?',
        importConfirmText: 'Die importierten Daten werden in deine aktuelle Wache eingefügt. Bestehende Daten bleiben erhalten, aber es können Duplikate entstehen. Möchtest du fortfahren?',
        importConfirmAction: 'Import starten',
        ownerHandoverTitle: 'Eigentümer übertragen',
        ownerHandoverHint: 'Übertrage die Eigentümerschaft an ein anderes Mitglied mit der Manager-Rolle. Nur der aktuelle Eigentümer kann dies tun.',
        ownerHandoverSelect: 'Neuen Eigentümer auswählen',
        ownerHandoverSubmit: 'Übertragen',
        ownerHandoverSuccess: 'Eigentümerschaft wurde übertragen.',
        ownerHandoverNone: 'Keine weiteren Manager vorhanden.',
        ownerHandoverLoadFailed: 'Die Manager dieser Wache konnten nicht geladen werden.',
        transferTitle: 'Wach-Transfer',
        transferHint: 'Erstelle einen Transfer-Code, um diese Wache auf eine andere Ember-Instanz zu übertragen. Der Code ist 24 Stunden gültig.',
        transferCreate: 'Transfer-Code erstellen',
        transferCreating: 'Wird erstellt...',
        transferTokenLabel: 'Transfer-Code',
        transferTokenCopied: 'In die Zwischenablage kopiert!',
        publicCalendar: {
            title: 'Öffentlicher Kalender',
            hint: 'Mache den Terminkalender ohne Login öffentlich zugänglich.',
            enabled: 'Öffentlicher Kalender aktiviert',
            publicUrl: 'Öffentliche URL',
            categoriesHint: 'Wenn eine Kategorie als öffentlich markiert ist, sind alle Termine darin automatisch öffentlich. Einzelne Termine können auch direkt als öffentlich markiert werden. In beiden Fällen können einzelne Termine ausgeblendet werden.',
        },
        publicPages: {
            title: 'Öffentliche Seiten',
            hint: 'Erlaube die Veröffentlichung von Seiten, die ohne Login sichtbar sind.',
            enabled: 'Öffentliche Seiten aktiviert',
            publicUrl: 'Öffentliche URL',
        },
        publicSlug: {
            title: 'Öffentliche URL-Kennung',
            hint: 'Vergib eine kurze, lesbare URL für die öffentliche Seite deiner Wache. Erlaubt sind Kleinbuchstaben, Zahlen und Bindestriche.',
            slug: 'URL-Kennung',
            placeholder: 'z. B. jugendfeuerwehr-musterstadt',
            taken: 'Diese URL-Kennung ist bereits vergeben. Bitte wähle eine andere.',
        },
        publicKb: {
            title: 'Öffentliches Wiki',
            hint: 'Mache Inhalte des Wikis ohne Login öffentlich zugänglich.',
            enabled: 'Öffentliches Wiki aktiviert',
            mode: 'Modus',
            modeAllowAll: 'Alles öffentlich, einzeln ausschließen',
            modeDenyAll: 'Nichts öffentlich, einzeln freigeben',
            publicUrl: 'Öffentliche URL',
            urlCopied: 'URL in die Zwischenablage kopiert!',
            saved: 'Öffentliches Wiki gespeichert.',
            visibilityTitle: 'Öffentliche Sichtbarkeit',
            visibleByDefault: 'Öffentlich sichtbar (Standard)',
            hiddenByDefault: 'Nicht öffentlich (Standard)',
            overrideVisible: 'Öffentlich sichtbar (Überschrieben)',
            overrideHidden: 'Nicht öffentlich (Überschrieben)',
            setVisible: 'Öffentlich machen',
            setHidden: 'Verbergen',
            resetToDefault: 'Standard wiederherstellen',
        },
    },
    stationSelect: {
        title: 'Wache auswählen',
        subtitle: 'Bitte wähle eine Wache aus, um fortzufahren.',
        clusterHint: 'Oder weiter zu einem Verband:',
    },
    stationSwitcher: {
        title: 'Wache wechseln',
        noStation: 'Keine Wache',
        allStations: 'Alle Wachen',
    },
    crossStation: {
        administration: 'Verwaltung',
        clusters: 'Verbände',
        title: 'Übersicht',
        subtitle: 'Alle Wachen auf einen Blick',
        notifications: 'Benachrichtigungen',
        requirements: 'Offene Aufgaben',
        recentNotifications: 'Letzte Benachrichtigungen',
        noNotifications: 'Keine neuen Benachrichtigungen',
    },
    twoFactor: {
        title: 'Zwei-Faktor-Authentifizierung',
        active: 'Aktiv',
        inactive: 'Nicht eingerichtet',
        authenticator: 'Authenticator-App',
        setup: {
            description: 'Schütze dein Konto mit einer Authenticator-App (z. B. Google Authenticator, Authy).',
            begin: 'Einrichten',
            scanQr: 'QR-Code scannen',
            codePlaceholder: '6-stelliger Code',
            verify: 'Code bestätigen',
            invalidCode: 'Ungültiger Code. Bitte versuche es erneut.',
            passwordPrompt: 'Bestätige zur Sicherheit dein aktuelles Passwort, um die Zwei-Faktor-Authentifizierung einzurichten.',
            passwordPlaceholder: 'Aktuelles Passwort',
            passwordRequired: 'Bitte gib dein aktuelles Passwort ein.',
            passwordWrong: 'Das Passwort ist falsch.',
        },
        verify: {
            title: 'Zwei-Faktor-Verifizierung',
            totpHint: 'Gib den 6-stelligen Code aus deiner Authenticator-App ein.',
            backupHint: 'Gib einen deiner Wiederherstellungscodes ein.',
            submit: 'Bestätigen',
            invalidCode: 'Ungültiger Code. Bitte versuche es erneut.',
            useBackupCode: 'Wiederherstellungscode verwenden',
            useAuthenticator: 'Authenticator-App verwenden',
            rememberDevice: 'Diesem Gerät vertrauen',
        },
        backup: {
            title: 'Wiederherstellungscodes',
            label: 'Wiederherstellungscodes',
            remaining: '{count} verbleibend',
            saveWarning: 'Speichere diese Codes an einem sicheren Ort. Sie werden nur einmal angezeigt.',
            done: 'Codes gespeichert',
            regenerate: 'Neue Codes',
        },
        remove: {
            title: '2FA deaktivieren',
            warning: 'Die Zwei-Faktor-Authentifizierung wird deaktiviert. Dein Konto ist dann nur noch durch dein Passwort geschützt.',
            confirm: 'Deaktivieren',
        },
        webauthn: {
            title: 'Sicherheitsschlüssel',
            add: 'Schlüssel hinzufügen',
            empty: 'Noch kein Sicherheitsschlüssel registriert.',
            added: 'Hinzugefügt',
            labelPrompt: 'Schlüssel benennen',
            labelHint: 'Gib dem Schlüssel einen wiedererkennbaren Namen, z. B. „Yubikey 5C" oder „Backup-Stick".',
            labelPlaceholder: 'Mein Sicherheitsschlüssel',
            continue: 'Weiter',
            waiting: 'Warte auf Schlüssel...',
            unsupported: 'Dieser Browser unterstützt keine Sicherheitsschlüssel.',
            unsupportedGuidance: 'Das liegt am Browser und nicht an Ember. Nimm einen aktuellen Browser, oder richte stattdessen eine Authenticator-App ein.',
            cancelled: 'Vorgang abgebrochen.',
            cancelledGuidance: 'Du hast die Abfrage deines Geräts abgebrochen oder sie ist abgelaufen. Starte sie noch einmal, wenn du fortfahren möchtest.',
            failed: 'Registrierung des Schlüssels fehlgeschlagen.',
            renamePrompt: 'Schlüssel umbenennen',
            useKey: 'Sicherheitsschlüssel verwenden',
        },
        trustedDevices: {
            title: 'Vertraute Geräte',
            description: 'Geräte, auf denen du beim Login „Diesem Gerät vertrauen" gewählt hast. Solange das Vertrauen gilt, wird auf diesen Geräten kein zweiter Faktor mehr abgefragt.',
            empty: 'Keine vertrauten Geräte. Beim nächsten Login kannst du dieses Gerät als vertraut markieren.',
            revokeAll: 'Alle widerrufen',
            unknownDevice: 'Unbekanntes Gerät',
            lastSeen: 'Zuletzt gesehen',
            expires: 'Läuft ab',
        },
        stepUp: {
            title: 'Sicherheitsbestätigung erforderlich',
            description: 'Diese Aktion ist sensibel. Bitte bestätige sie mit deinem zweiten Faktor.',
            categoryAccountSecurity: 'Du bestätigst eine Änderung an deinen Sicherheits­einstellungen.',
            categoryFederation: 'Du bestätigst eine Änderung an der Vernetzung mit anderen Wachen.',
            categoryInstanceConfig: 'Du bestätigst eine Änderung an der Instanz­konfiguration.',
            categoryRoleChange: 'Du bestätigst eine Änderung an Berechtigungen oder Rollen.',
            /** Named for the approving screen, which says which kind of action it is confirming. */
            category: {
                ACCOUNT_SECURITY: 'eine Änderung an den Sicherheitseinstellungen',
                FEDERATION: 'eine Änderung an der Vernetzung mit anderen Wachen',
                INSTANCE_CONFIG: 'eine Änderung an der Instanzkonfiguration',
                ROLE_CHANGE: 'eine Änderung an Berechtigungen oder Rollen',
            },
            anotherDevice: {
                start: 'Auf einem anderen Gerät bestätigen',
                hint: 'Öffne auf einem Gerät, auf dem du schon angemeldet bist, Konto, Sicherheit, '
                    + '"Neues Gerät freischalten" und gib dort diesen Code ein:',
                waiting: 'Warte auf die Bestätigung ...',
                expired: 'Die Anfrage ist abgelaufen oder wurde nicht bestätigt.',
                stopped: 'Das Warten auf die Bestätigung wurde abgebrochen. Fordere einen neuen Code an.',
                again: 'Neuen Code anfordern',
            },
            submit: 'Bestätigen',
            cancel: 'Abbrechen',
            invalidCode: 'Ungültiger Code. Bitte versuche es erneut.',
            usePasskey: 'Mit Passkey bestätigen',
            passkeyFailed: 'Das hat nicht geklappt. Versuche es noch einmal oder nutze einen anderen Weg.',
            passwordPlaceholder: 'Dein Passwort',
            wrongPassword: 'Passwort falsch. Bitte versuche es erneut.',
            tryAgainGuidance: 'Gib es hier noch einmal ein. Du bist weiterhin angemeldet, es geht nur um diese eine Bestätigung.',
            usePassword: 'Passwort verwenden',
            noProofs: 'Für dieses Konto ist gerade keine Bestätigung möglich. Melde dich neu an und versuche es dann noch einmal.',
            stillRequired: 'Die Bestätigung wurde angenommen, die Aktion aber trotzdem abgelehnt. '
                + 'Melde dich bitte neu an und versuche es noch einmal.',
        },
        admin: {
            policiesTitle: 'Pflicht je Benutzertyp',
            policiesHint: 'Aktivierte Typen müssen 2FA einrichten. Rollen-basierte Pflicht (Admin, Manager) wird automatisch angewandt.',
            required: 'Pflicht',
            optional: 'Optional',
            membersTitle: 'Mitglieder­übersicht',
            statusEnrolled: 'Eingerichtet',
            statusMandatedGap: 'Pflicht - fehlt',
            statusOptional: 'Optional',
            enrolledCount: '{n} von {total} eingerichtet',
            mandatedCount: '{n} verpflichtet',
            gapCount: '{n} Pflicht-Lücken',
            noMembers: 'Keine Mitglieder gefunden.',
            col: {
                name: 'Name',
                email: 'E-Mail',
                userType: 'Typ',
                status: 'Status',
                actions: 'Aktionen',
            },
            reset: '2FA zurücksetzen',
            resetTitle: 'Konto-2FA zurücksetzen',
            resetHint: 'Setzt alle 2FA-Faktoren, Wiederherstellungscodes, vertrauten Geräte und aktiven Sitzungen eines Kontos zurück. Das Konto wird beim nächsten Login zur erneuten Einrichtung aufgefordert.',
            resetAccountLabel: 'Konto',
            accountSearchPlaceholder: 'Nach Name oder E-Mail suchen…',
            resetSuccess: '2FA für {name} zurückgesetzt.',
            resetConfirmTitle: 'Wirklich zurücksetzen?',
            resetConfirmText: 'Alle 2FA-Faktoren, Wiederherstellungscodes, vertrauten Geräte und aktiven Sitzungen von {name} werden entfernt.',
            resetWarning: 'Dieser Vorgang kann nicht rückgängig gemacht werden. Das betroffene Konto wird per E-Mail benachrichtigt.',
            userTypes: {
                MEMBER: 'Mitglied',
                GUARDIAN: 'Erziehungsberechtigter',
                TEAM: 'Team',
                MANAGER: 'Manager',
                TRIAL: 'Probe',
            },
            instanceHint: 'Diese Einstellungen gelten in jeder Wache der Instanz.',
            audit: {
                title: 'Audit-Protokoll',
                empty: 'Noch keine Einträge.',
                col: {
                    when: 'Wann',
                    account: 'Konto',
                    actor: 'Auslöser',
                    event: 'Ereignis',
                    factor: 'Faktor',
                    country: 'Land',
                },
                loadMore: 'Weitere laden',
                filterAccount: 'Nach Konto-ID filtern',
                events: {
                    ENROLLED: 'Faktor eingerichtet',
                    REMOVED: 'Faktor entfernt',
                    LOGIN_VERIFIED: 'Anmeldung bestätigt',
                    STEPUP_VERIFIED: 'Erneute Bestätigung erfolgreich',
                    BACKUP_CODE_USED: 'Wiederherstellungscode verwendet',
                    BACKUP_CODE_REGENERATED: 'Wiederherstellungscodes neu erzeugt',
                    ADMIN_RESET: 'Durch Administration zurückgesetzt',
                    TRUSTED_DEVICE_ADDED: 'Vertrautes Gerät hinzugefügt',
                    TRUSTED_DEVICE_REVOKED: 'Vertrautes Gerät entfernt',
                    POLICY_CHANGED: 'Richtlinie geändert',
                    PASSKEY_SIGN_IN: 'Anmeldung mit Passkey',
                    PASSKEY_ENROLLED_VIA_DEVICE_CODE: 'Passkey über Gerätecode eingerichtet',
                    PASSKEY_CODE_ISSUED: 'Passkey-Code ausgestellt',
                    PASSWORD_LOGIN_DISABLED: 'Anmeldung mit Passwort abgeschaltet',
                    PASSWORD_LOGIN_ENABLED: 'Anmeldung mit Passwort eingeschaltet',
                    PASSWORD_RETIRED: 'Passwort stillgelegt',
                    STEPUP_FAILED: 'Erneute Bestätigung fehlgeschlagen',
                    SIGNED_IN_VIA_DEVICE_CODE: 'Über anderes Gerät angemeldet',
                    STEPUP_VIA_DEVICE_CODE: 'Über anderes Gerät bestätigt',
                    DEVICE_REQUEST_APPROVED: 'Geräteanfrage freigegeben',
                },
                factors: {
                    TOTP: 'Authenticator-App',
                    WEBAUTHN: 'Sicherheitsschlüssel',
                    BACKUP_CODES: 'Wiederherstellungscodes',
                },
            },
        },
    },
    apply: {
        title: 'Wache beantragen',
        hint: 'Beantrage die Erstellung einer neuen Wache auf der Plattform. Dein Antrag wird von einem Administrator geprüft.',
        firstName: 'Vorname',
        firstNamePlaceholder: 'Max',
        lastName: 'Nachname',
        lastNamePlaceholder: 'Mustermann',
        email: 'E-Mail',
        emailPlaceholder: "max{'@'}feuerwehr-musterstadt.de",
        emailHint: 'Idealerweise eine E-Mail-Adresse der Feuerwehr.',
        stationName: 'Name der Wache',
        stationNamePlaceholder: 'Freiwillige Feuerwehr Musterstadt',
        introduction: 'Kurze Vorstellung',
        introductionPlaceholder: 'Erzähle kurz etwas über dich und die Wache...',
        introductionHint: 'Eine kurze Vorstellung hilft bei der Bearbeitung deines Antrags.',
        submit: 'Antrag absenden',
        allFieldsRequired: 'Bitte fülle alle Pflichtfelder aus.',
        success: 'Dein Antrag wurde eingereicht. Bitte bestätige deine E-Mail-Adresse über den Link in der E-Mail, die wir dir gesendet haben.',
        backToLogin: 'Zurück zum Login',
        disabled: 'Die Registrierung neuer Wachen ist derzeit deaktiviert.',
    },
    applyVerify: {
        success: 'Deine E-Mail-Adresse wurde bestätigt. Dein Antrag wird nun von einem Administrator geprüft. Du erhältst eine E-Mail, sobald eine Entscheidung getroffen wurde.',
        error: 'Der Bestätigungslink ist ungültig oder wurde bereits verwendet.',
    },
    confirmEmailChange: {
        committed: 'Deine E-Mail-Adresse wurde geändert. Melde dich ab jetzt mit der neuen Adresse an.',
        waiting: 'Diese Bestätigung ist angekommen. Die Änderung wird wirksam, sobald auch die andere Adresse bestätigt hat.',
        error: 'Der Bestätigungslink ist ungültig, abgelaufen oder wurde bereits verwendet.',
        backToLogin: 'Zur Anmeldung',
    },
    adminDiscovery: {
        subtitle: 'Andere Ember-Instanzen finden und öffentliche Wachen aus dem Netzwerk anzeigen.',
        identity: 'Diese Instanz',
        instanceId: 'Instance-ID',
        publicKey: 'Public Key',
        baseUrl: 'Basis-URL',
        settings: 'Einstellungen',
        enabled: 'Discovery aktiviert',
        enabledHelp: 'Sendet ausgehende Pings und beantwortet öffentliche Listenanfragen.',
        maxDepth: 'Maximale Tiefe',
        maxDepthHelp: 'Wie viele Schritte sich ein Ping ausbreiten darf (0–10).',
        pingInterval: 'Ping-Intervall (Minuten)',
        pingIntervalHelp: 'Wie oft bekannte Peers automatisch angefragt werden. Mindestens 60 Minuten.',
        peers: 'Bekannte Instanzen',
        peersEmpty: 'Noch keine bekannten Instanzen.',
        addPeer: 'Instanz hinzufügen',
        addPeerHelp: 'Trage die Basis-URL einer bekannten Ember-Instanz ein. Der Public Key wird vom Server abgefragt.',
        baseUrlPlaceholder: 'z. B. https://ember.example.org',
        expectedKey: 'Erwarteter Public Key (optional)',
        probe: 'Verbindung testen',
        probeOk: 'Erreichbar - Public Key bekannt.',
        probeFailed: 'Keine Antwort vom Discovery-Endpunkt.',
        add: 'Hinzufügen',
        added: 'Instanz hinzugefügt.',
        block: 'Blockieren',
        unblock: 'Freischalten',
        upvote: 'Vertrauen erhöhen',
        downvote: 'Vertrauen senken',
        pingNow: 'Jetzt anfragen',
        discoverNow: 'Sofort durchsuchen',
        discoverNowResult: '{pings} Anfragen ausgelöst, {stations} Wachen aktualisiert.',
        seedFederation: 'Bekannte Föderations-Partner übernehmen',
        seedFederationResult: '{count} neue Peers übernommen.',
        peerTable: {
            reachable: 'Erreichbar',
            blocked: 'Blockiert',
            lastSeen: 'Zuletzt gesehen',
        },
        sourceBootstrap: 'Föderation',
        sourceGossip: 'Gossip',
        sourceManual: 'Manuell',
        no: 'Nein',
        blocklist: 'Blockliste',
        blocklistEmpty: 'Keine Einträge auf der Blockliste.',
        blocklistAdd: 'Eintrag hinzufügen',
        blocklistValue: 'Basis-URL oder Public Key',
        blocklistKindBaseUrl: 'Basis-URL',
        blocklistKindPublicKey: 'Public Key',
        blocklistNote: 'Notiz (optional)',
        blocklistRemove: 'Eintrag entfernen',
        confirmDeletePeer: 'Diese Instanz wirklich aus der Liste entfernen?',
    },
    stationDiscovery: {
        empty: 'Es wurden noch keine öffentlichen Wachen aus dem Discovery-Netz geladen.',
        searchPlaceholder: 'Suche nach Name, Ort oder Tag…',
        listTab: 'Liste',
        mapTab: 'Karte',
        nearMeOnly: 'Nur in meiner Nähe',
        radiusLabel: 'Radius (km)',
        noCoordinatesForFilter: 'Setze zuerst die Koordinaten deiner Wache, um nach Entfernung zu filtern.',
    },
    geolocation: {
        sectionTitle: 'Standort',
        sectionSubtitle: 'Adresse und Koordinaten dieser Wache. Wird auf der Discovery-Karte und im Ausleih-Suchergebnis genutzt.',
        addressLine: 'Straße und Hausnummer',
        addressLinePlaceholder: 'z. B. Hauptstraße 1',
        postalCode: 'Postleitzahl',
        postalCodePlaceholder: 'z. B. 80331',
        city: 'Ort',
        cityPlaceholder: 'z. B. München',
        country: 'Land',
        countryPlaceholder: '– bitte wählen –',
        latitude: 'Breitengrad',
        latitudePlaceholder: 'z. B. 48.137154',
        longitude: 'Längengrad',
        longitudePlaceholder: 'z. B. 11.576124',
        pickHint: 'Klicke auf die Karte oder ziehe den Marker, um den Standort zu setzen.',
        clearPin: 'Marker entfernen',
        useMyPosition: 'Aktuellen Standort verwenden',
        locating: 'Standort wird ermittelt …',
        geolocationUnsupported: 'Dein Browser unterstützt keine Standortabfrage.',
        geolocationDenied: 'Standortzugriff verweigert.',
        geolocationFailed: 'Standort konnte nicht ermittelt werden.',
        saved: 'Standort gespeichert.',
        clear: 'Standort entfernen',
        cleared: 'Standort entfernt.',
    },
    lendingDistance: {
        distanceKm: '{distance} km entfernt',
        distanceUnknown: 'Entfernung unbekannt',
    },
    lendingShare: {
        overview: 'Ausleih-Angebot',
        overviewHint: 'Nichts wird angeboten, bis es hier steht. Der engste Eintrag entscheidet: ein Gegenstand schlägt seine Art, eine Art schlägt ihr Inventar.',
        offered: 'Angeboten',
        withheld: 'Zurückgehalten',
        nothingOffered: 'Diese Wache bietet ihren Partnern derzeit nichts an.',
        levelInventory: 'Inventar',
        levelArt: 'Art',
        levelItem: 'Gegenstand',
        noPartnersNamed: 'Keine Partnerwache benannt',
        panelTitle: 'Ausleihe an Partner',
        edit: 'Angebot ändern',
        title: 'Ausleih-Angebot festlegen',
        loading: 'Angebot wird geladen...',
        loadError: 'Fehler beim Laden des Angebots.',
        saveError: 'Fehler beim Speichern des Angebots.',
        save: 'Speichern',
        clear: 'Eintrag entfernen',
        grant: 'Entscheidung',
        grantValues: {
            GRANT: 'Anbieten',
            WITHHOLD: 'Zurückhalten',
        },
        grantHint: {
            inventory: 'Angeboten heißt: alles in diesem Inventar, außer wo eine Art oder ein einzelner Gegenstand etwas anderes sagt.',
            art: 'Eine Art schlägt ihr Inventar, in beide Richtungen, und wird ihrerseits von einem einzelnen Gegenstand geschlagen.',
            item: 'Ein Gegenstand schlägt seine Art und sein Inventar, in beide Richtungen.',
        },
        scope: 'Für wen',
        scopeValues: {
            ALL_PARTNERS: 'Alle Partnerwachen',
            SPECIFIC: 'Nur bestimmte Partnerwachen',
        },
        partners: 'Partnerwachen',
        partnersPlaceholder: 'Partnerwache hinzufügen',
        partnersHint: 'Ohne Auswahl erreicht der Eintrag niemanden, hebt aber weiterhin auf, was darüber steht.',
        stateUnshared: 'Nicht angeboten',
        stateAllPartners: 'Allen Partnerwachen angeboten',
        stateNamedPartners: 'Bestimmten Partnerwachen angeboten',
        stateWithheld: 'Zurückgehalten',
        stillPromisedHint: 'Bereits genehmigte Anfragen laufen zu Ende, auch wenn hier nichts mehr angeboten wird.',
    },
    adminMaps: {
        subtitle: 'Tile-Anbieter, API-Schlüssel und Cache der Karten-Funktion.',
        tilesSection: 'Tile-Anbieter',
        provider: 'Anbieter',
        providerHelp: 'OSM Raw ist kostenlos, aber für hohe Last nicht zugelassen. Stadia, Mapbox, MapTiler und Thunderforest benötigen einen API-Schlüssel.',
        apiKey: 'API-Schlüssel',
        apiKeyPlaceholder: 'Vom Anbieter erhaltener Schlüssel',
        urlTemplate: 'Eigene URL-Vorlage',
        urlTemplatePlaceholder: 'z. B. https://tiles.example.org/{z}/{x}/{y}.png?key={apiKey}',
        attribution: 'Attributierung (optional)',
        attributionPlaceholder: 'Standardwert des Anbieters wird genutzt',
        minZoom: 'Min. Zoom',
        maxZoom: 'Max. Zoom',
        geocodingSection: 'Geocoding (optional)',
        geocodingProvider: 'Geocoder',
        contactEmail: 'Kontakt-E-Mail (Nominatim-Pflicht)',
        cacheSection: 'Tile-Cache',
        cacheMaxMb: 'Maximale Größe (MB)',
        cacheStatsLabel: 'Belegung',
        cacheStatsValue: '{tiles} Kacheln · {used} MB / {max} MB',
        cachePurge: 'Cache leeren',
        cachePurgeConfirm: 'Soll der Tile-Cache wirklich geleert werden? Beim nächsten Aufruf werden alle Kacheln neu vom Anbieter geladen.',
        cachePurged: 'Cache geleert.',
        testTile: 'Testkachel anfordern',
        testTileSuccess: 'Anbieter antwortet (HTTP {status}).',
        testTileFailure: 'Keine Antwort vom Anbieter (HTTP {status}).',
    },
    adminApplications: {
        title: 'Wachanträge',
        pending: 'Ausstehend',
        all: 'Alle',
        empty: 'Keine Anträge vorhanden.',
        name: 'Name',
        email: 'E-Mail',
        station: 'Wache',
        date: 'Eingereicht',
        status: 'Status',
        pendingBadge: 'Ausstehend',
        unverified: 'Unbestätigt',
        accepted: 'Angenommen',
        denied: 'Abgelehnt',
        accept: 'Annehmen',
        deny: 'Ablehnen',
        denyTitle: 'Antrag ablehnen',
        denyReasonLabel: 'Begründung',
        denyReasonPlaceholder: 'Grund für die Ablehnung...',
    },
    theme: {
        custom: 'Eigenes Design',
        title: 'Erscheinungsbild',
        selectTheme: 'Farbschema',
        selectThemeHint: 'Wähle ein Farbschema für die Anwendung.',
        darkMode: 'Modus',
        darkModeSystem: 'System',
        darkModeLight: 'Hell',
        darkModeDark: 'Dunkel',
        themeCustom: 'Benutzerdefiniert',
        controlledByStation: 'Das Farbschema wird von der Wache festgelegt.',
        stationTheme: 'Wach-Farbschema',
        stationDefaultTheme: 'Standard-Farbschema',
        stationDefaultThemeHint: 'Dieses Farbschema wird für alle Mitglieder verwendet, die kein eigenes gewählt haben.',
        allowUserTheme: 'Mitglieder dürfen eigenes Farbschema wählen',
        customColors: 'Eigene Farben',
        customColorsHint: 'Definiere eigene Farben für ein benutzerdefiniertes Farbschema.',
        colorPrimary: 'Primärfarbe',
        colorPrimaryAccent: 'Primär-Akzent',
        colorSecondary: 'Sekundärfarbe',
        colorSecondaryAccent: 'Sekundär-Akzent',
        colorSuccess: 'Erfolg',
        colorError: 'Fehler',
        colorInfo: 'Info',
        colorInfoAccent: 'Info-Akzent',
        colorBgLight: 'Hintergrund Hell',
        colorBgLightAccent: 'Hintergrund Hell Akzent',
        colorBgDark: 'Hintergrund Dunkel',
        colorBgDarkAccent: 'Hintergrund Dunkel Akzent',
        feel: 'Stil',
        feelROUNDED: 'Abgerundet',
        feelCORNERS: 'Eckig',
        feelControlledByStation: 'Der Stil wird von der Wache festgelegt.',
        stationDefaultFeel: 'Standard-Stil',
        allowUserFeel: 'Mitglieder dürfen eigenen Stil wählen',
        loadPreset: 'Vorlage laden',
        selectPreset: 'Farbschema als Vorlage wählen...',
        applyPreset: 'Übernehmen',
        preview: 'Vorschau',
        saved: 'Farbschema gespeichert.',
        lightModeColors: 'Hellmodus-Farben',
        darkModeColors: 'Dunkelmodus-Farben',
        backgrounds: 'Hintergrundfarben',
    },
    notFound: {
        title: 'Seite nicht gefunden',
        text: 'Die Seite, die du suchst, existiert nicht oder wurde verschoben.',
        home: 'Startseite',
        login: 'Anmelden',
    },
    errorPage: {
        reportSummary: 'Die Seite konnte nicht geladen werden (Status {status}).',
        title: 'Da ist etwas schiefgelaufen',
        text: 'Die Seite konnte nicht geladen werden. Versuche es später noch einmal.',
    },
    applicationLog: {
        disabled: 'Das Log wird derzeit nicht in der Datenbank gespeichert, hier steht deshalb '
            + 'nichts oder nur, was vor dem Abschalten anfiel. Einschalten lässt sich das unter '
            + 'Einstellungen. Console und Logdatei auf dem Server enthalten unabhängig davon alles.',
        dropped: '{count} Zeile(n) sind seit dem Start verworfen worden, weil sie schneller anfielen '
            + 'als sie geschrieben werden konnten. Das Log hat also Lücken; in der Logdatei stehen sie.',
        kept: 'Gespeichert wird ab {level}, {days} Tage lang.',
        searchPlaceholder: 'In Nachricht oder Logger suchen',
        doSearch: 'Suchen',
        levels: 'Stufen',
        filtered: 'Es sind nicht alle Stufen eingeschaltet, es fehlt also möglicherweise Kontext.',
        empty: 'Keine Zeilen, auf die das passt.',
        loadMore: 'Weiter zurück laden',
        showTrace: 'Stacktrace anzeigen',
        hasTrace: 'Stacktrace',
        byLogger: 'Klasse',
        byThread: 'Thread',
        facetSearch: 'Suchen...',
        facetEmpty: 'Nichts gefunden.',
        narrowedTo: 'Eingegrenzt auf:',
        clearNarrowing: 'Eingrenzung aufheben',
    },
    mailDashboard: {
        title: 'Zustellung',
        hint: 'Was aus der Post geworden ist: was noch wartet, an welchem Anbieter es wartet, und '
            + 'was die Anbieter über die Mails zurückgemeldet haben, die sie übernommen haben.',
        pending: 'Wartet',
        sending: 'In Zustellung',
        sent: 'Übergeben',
        failed: 'Gescheitert',
        stuck: 'Hängengeblieben',
        stuckWarning: '{count} Mail(s) stehen seit über zehn Minuten in der Zustellung. Das '
            + 'bedeutet, dass ein Arbeitsprozess während des Versands abgebrochen ist. Sie werden '
            + 'von selbst nicht erneut versucht.',
        stuckTitle: 'Hängengebliebene Mails',
        stuckHint: 'Diese Mails hat niemand mehr in der Hand. Wieder einreihen versendet sie erneut. '
            + 'Hatte der Anbieter sie schon angenommen, bevor der Prozess abbrach, kommt sie doppelt an.',
        requeueAll: 'Alle wieder einreihen',
        requeueOne: 'Wieder einreihen',
        requeued: '{count} Mail(s) wieder eingereiht.',
        oldestPending: 'Die längste wartende Mail wurde am {when} geschrieben.',
        providersTitle: 'Anbieter heute',
        noProviders: 'Kein Anbieter eingetragen, es kann also nichts versendet werden.',
        waitingHere: '{count} warten hier',
        exhausted: 'Tageslimit erreicht',
        sentOfLimit: '{sent} von {limit} heute versendet',
        sentNoLimit: '{sent} heute versendet, kein Limit',
        attempts: '{count} Versuche je Mail',
        recentTitle: 'Letzte Mails',
        searchPlaceholder: 'Nach Empfänger oder Betreff suchen',
        deliveryFilter: 'Nach Zustellstatus filtern',
        allDeliveryStates: 'Alle Zustellstatus',
        noMails: 'Keine Mails, auf die das passt.',
        colRecipient: 'Empfänger',
        colSubject: 'Betreff',
        colWhen: 'Zeitpunkt',
        colStatus: 'Warteschlange',
        colDelivery: 'Zustellstatus',
        colProvider: 'Anbieter',
        colAttempts: 'Versuche',
        colDetail: 'Rückmeldung',
        delivery: {
            UNKNOWN: 'Keine Rückmeldung',
            DELIVERED: 'Zugestellt',
            SOFT_BOUNCE: 'Vorübergehend abgewiesen',
            HARD_BOUNCE: 'Dauerhaft abgewiesen',
            BLOCKED: 'Absender abgelehnt',
            SPAM: 'Als Spam markiert',
            DEFERRED: 'Zurückgestellt',
            ERROR: 'Fehler beim Anbieter',
        },
        unreachable: 'Kein Anbieter kann diese Mail derzeit zustellen. Sie wartet nicht, sie steckt fest.',
        blocksTitle: 'Abgelehnte Anbieter',
        blocksHint: 'Diese Empfängerdomains haben unseren Absendeserver abgelehnt, nicht die Nachricht. '
            + 'Der betroffene Anbieter wird dort übersprungen, statt jedes Mal ein Kontingent zu '
            + 'verbrauchen. Die Sperre läuft von selbst wieder ab.',
        blockRow: '{provider} wird von {domain} abgelehnt',
        blockUntil: 'Wird ab {when} wieder versucht.',
        liftBlock: 'Jetzt wieder versuchen',
    },
    mailChain: {
        title: 'Mailanbieter',
        hint: 'Mail wird von oben nach unten durch diese Liste versucht. Jeder Anbieter bekommt eine '
            + 'eigene Anzahl Versuche und ein eigenes Tageslimit; ist eines davon aufgebraucht, '
            + 'übernimmt der nächste. Lehnt die Gegenstelle nicht die Nachricht, sondern den '
            + 'Absendeserver ab, etwa weil er auf einer Sperrliste steht, wird sofort gewechselt, '
            + 'weil ein weiterer Versuch über denselben Weg genauso abgelehnt würde.',
        empty: 'Noch kein Anbieter eingetragen. Bis dahin bleibt jede Mail liegen.',
        position: '{position}. Anbieter',
        firstBadge: 'Wird zuerst versucht',
        provider: 'Anbieter',
        ownServer: 'Eigener Server',
        serverTitle: 'Serveradresse',
        attempts: 'Versuche',
        attemptsHint: 'So oft wird dieser Anbieter versucht, bevor der nächste übernimmt.',
        dailyLimit: 'Tageslimit',
        dailyLimitHint: 'So viele Mails darf dieser Anbieter am Tag versenden. 0 bedeutet kein Limit.',
        host: 'Server',
        port: 'Port',
        ssl: 'SSL statt STARTTLS',
        user: 'Benutzername',
        password: 'Passwort',
        apiKey: 'API-Schlüssel',
        senderAddress: 'Absender-Adresse',
        senderName: 'Absender-Name',
        providerName: 'Anzeigename für Mitglieder',
        providerUrl: 'Website des Anbieters',
        add: 'Anbieter hinzufügen',
        clearAll: 'Alle entfernen',
        cleared: 'Alle Anbieter entfernt. Bis ein neuer eingetragen ist, bleibt jede Mail liegen.',
        saved: 'Anbieter gespeichert.',
        loadFailed: 'Die Anbieterliste konnte nicht geladen werden. Lade die Seite neu, bevor du '
            + 'etwas speicherst, sonst würde eine leere Liste über die gespeicherte geschrieben.',
        test: 'Testmail senden',
        testRecipient: 'Testmail an',
        testHint: 'Geprüft wird der gespeicherte Stand dieses Anbieters, nicht das, was gerade im '
            + 'Formular steht. Die Adresse muss nicht deine eigene sein.',
        testOk: 'Der {position}. Anbieter hat die Mail an {recipient} angenommen.',
        testFailed: 'Der {position}. Anbieter hat abgelehnt: {error}',
    },
    adminSettings: {
        title: 'Einstellungen',
        stationRegistration: 'Wachenregistrierung',
        stationRegistrationHint: 'Erlaubt neuen Nutzern, eine Wache über das Antragsformular zu beantragen.',
        saved: 'Einstellungen gespeichert.',
        forcePrideFlag: 'Pride-Flagge erzwingen',
        forcePrideFlagHint: 'Zeigt die Regenbogen-Flagge im Logo ganzjährig an. Im Juni und Juli wird sie automatisch angezeigt.',
        mailLocale: 'Sprache für System-E-Mails',
        mailLocaleHint: 'Gilt für Konten, die zu keiner Wache gehören - Selbstregistrierung und das '
            + 'beim ersten Start angelegte Administratorkonto. Konten, die aus einer Wache heraus '
            + 'angelegt wurden, bekommen weiterhin die Sprache dieser Wache. Zur Auswahl steht, '
            + 'wofür diese Instanz E-Mail-Vorlagen mitbringt.',
        mailLocaleNames: {
            de: 'Deutsch',
            en: 'Englisch',
        },
        theme: {
            title: 'Instanz-Erscheinungsbild',
            hint: 'Standard-Farbschema und -Stil für alle Nutzer der Instanz. Wird von Wach- und Benutzer-Einstellungen überschrieben.',
            defaultTheme: 'Standard-Farbschema',
            defaultFeel: 'Standard-Stil',
            lockFeel: 'Stil sperren',
            lockFeelHint: 'Verhindert, dass Wachen und Nutzer den Stil ändern.',
        },
        auth: {
            title: 'Authentifizierung',
            tokenBytes: 'Token-Länge (Bytes)',
            tokenBytesHint: 'Anzahl der Zufallsbytes für neue Token. Empfohlen: 32.',
            verifyTokenHours: 'Gültigkeitsdauer Verifizierungs-Token (Stunden)',
            verifyTokenHoursHint: 'Wie lange ein Link zur E-Mail-Bestätigung gültig bleibt.',
            passwordTokenHours: 'Gültigkeitsdauer Passwort-Reset-Token (Stunden)',
            passwordTokenHoursHint: 'Wie lange ein Link zum Zurücksetzen des Passworts gültig bleibt.',
            sessionMinutes: 'Sitzungsdauer auf vertrauten Geräten (Minuten)',
            sessionMinutesHint: 'Gilt, wenn beim Anmelden „Auf diesem Gerät angemeldet bleiben" '
                + 'angekreuzt wurde. Bis zu 30 Tage sind möglich: häufiges Anmelden frustriert mehr, '
                + 'als es schützt.',
            untrustedSessionMinutes: 'Sitzungsdauer sonst (Minuten)',
            untrustedSessionMinutesHint: 'Gilt ohne dieses Häkchen, also auf geteilten oder '
                + 'geliehenen Rechnern. Darf nicht länger sein als die Dauer für vertraute Geräte.',
        },
        logging: {
            title: 'Log',
            hint: 'Console und Logdatei auf dem Server bekommen immer alles. Hier wird entschieden, '
                + 'was zusätzlich in die Datenbank geht, damit es unter Monitoring durchsucht werden '
                + 'kann. In der Datenbank zu speichern ist nie die einzige Ablage: der Ausfall, den '
                + 'sie nicht abdecken kann, ist ihr eigener.',
            enabled: 'Log in der Datenbank speichern',
            level: 'Ab welcher Stufe',
            levelHint: 'Alles darunter erreicht weiterhin Console und Datei, wird aber nicht gespeichert.',
            retentionDays: 'Aufbewahrung in Tagen',
            retentionHint: 'Ältere Zeilen werden stündlich entfernt.',
            stored: 'Derzeit sind {count} Zeilen gespeichert.',
            saved: 'Log-Einstellungen gespeichert.',
        },
        mailing: {
            title: 'E-Mail',
            instanceTitle: 'Einstellungen der Instanz',
            digestInterval: 'Benachrichtigungsintervall (Minuten)',
            digestIntervalHint: 'Intervall für die Zusammenfassung von Benachrichtigungen.',
            clear: 'Alle Anbieter entfernen',
            clearConfirm: 'Die Anbieterliste der Instanz wird vollständig geleert. Bis ein neuer '
                + 'Anbieter eingetragen ist, bleibt jede Mail liegen. Fortfahren?',
            saveFailed: 'Speichern fehlgeschlagen: {error}',
            testMail: 'Test-Mail an mich senden',
            testMailSent: 'Test-Mail wurde versendet. Prüfe dein Postfach.',
        },
        legal: {
            title: 'Rechtliche Dokumente',
            privacy: 'Datenschutz',
            tos: 'Nutzungsbedingungen',
            consent: 'Einwilligung',
            imprint: 'Impressum',
            save: 'Speichern',
            contentPlaceholder: 'Markdown-Inhalt hier eingeben...',
            version: 'Version',
            preview: 'Vorschau',
            edit: 'Bearbeiten',
            addLocale: 'Sprache hinzufügen',
            addLocaleTitle: 'Neue Sprache hinzufügen',
            localeCodePlaceholder: 'Sprachcode (z.B. en, fr)',
            addFile: 'Datei hinzufügen',
            addFileTitle: 'Neue Datei erstellen',
            fileNamePlaceholder: 'Dateiname (z.B. 03-cookies.md)',
            deleteFileTitle: 'Datei löschen',
            deleteFileConfirm: 'Möchtest du die Datei „{name}" wirklich löschen?',
            noFiles: 'Keine Dateien vorhanden. Erstelle eine neue Datei.',
            moveUp: 'Nach oben',
            moveDown: 'Nach unten',
            generatedName: 'Speicherung im Browser (automatisch)',
            generatedHint: 'Dieser Abschnitt wird aus der Anwendung erzeugt und bleibt dadurch aktuell. '
                + 'Du bestimmst nur, ob und an welcher Stelle er erscheint.',
            loadTemplate: 'Vorlage laden',
            loadTemplateTitle: 'Mitgelieferte Vorlage laden',
            loadTemplateHint: 'Wähle die Abschnitte, die aus Embers Vorlage übernommen werden sollen. '
                + 'Ein Abschnitt mit gleichem Namen wird überschrieben, alle anderen bleiben unberührt. '
                + 'Die Änderung greift erst beim Speichern. Als „standardmäßig aus" markierte '
                + 'Abschnitte sind Alternativen, etwa je Mailanbieter einer; von ihnen gehört in der '
                + 'Regel höchstens einer ins Dokument, weshalb „Alle auswählen" sie ausspart.',
            selectAllTemplates: 'Alle auswählen',
            selectNoTemplates: 'Auswahl aufheben',
            loadSelected: 'Auswahl laden',
            selectedCount: '{count} von {total} ausgewählt',
            templateReplaces: 'Ersetzt vorhandenen Abschnitt',
            templateOptional: 'Standardmäßig aus',
            templatePreview: 'Inhalt anzeigen',
            noTemplates: 'Für diese Sprache liefert Ember keine Vorlage mit.',
            import: 'Dokument importieren',
            importTitle: 'Dokument importieren',
            importHint: 'Übernimm ein anderswo geschriebenes Dokument. Ember schneidet es in Abschnitte, '
                + 'nimmt die Nummern aus den Überschriften und setzt Verweise wie „§ 12" auf den jeweiligen '
                + 'Abschnitt um. Danach nummeriert sich das Dokument selbst. Gespeichert wird erst, wenn du '
                + 'im Editor speicherst.',
            importPastePlaceholder: 'Oder den Text hier einfügen…',
            importPaste: 'Eingefügten Text übernehmen',
            importApply: 'In den Editor übernehmen',
            importFailed: 'Das Dokument konnte nicht gelesen werden.',
            importNoTitle: 'Ohne Titel',
            importFound: '{sections} Abschnitte, {references} Verweise umgesetzt',
            importUnmatched: 'Nicht zugeordnete Nummern',
            importUnmatchedHint: 'Diese Stellen sehen aus wie Verweise, zeigen aber auf keinen Abschnitt '
                + 'dieses Dokuments. Sie bleiben unverändert stehen - sieh sie dir an, bevor du speicherst.',
            placeholderTitle: 'Platzhalter',
            placeholderHint: 'Ein Name in doppelten geschweiften Klammern wird beim Anzeigen durch seinen Wert '
                + 'ersetzt. Ember erkennt jeden Platzhalter selbst und setzt ihn in allen Dokumenten und allen '
                + 'Sprachen gleich ein. Ein Platzhalter ohne Wert bleibt im Text stehen, damit er nicht unbemerkt '
                + 'verschwindet.',
            placeholderShow: 'Anzeigen',
            placeholderHide: 'Einklappen',
            placeholderNone: 'In den Dokumenten steht bisher kein Platzhalter.',
            placeholderUnused: 'In keinem Dokument mehr verwendet',
            placeholderMissing: '{count} ohne Wert',
            placeholderValuePlaceholder: 'Wert eingeben',
        },
    },
    adminSecurity: {
        hub: {
            hint: 'Konfigurationen und operative Werkzeuge rund um die Sicherheit der Instanz.',
            configTitle: 'Konfiguration',
            operationsTitle: 'Operative Werkzeuge',
        },
        tokens: {
            title: 'Tokens & Sitzungen',
            hint: 'Längen und Gültigkeiten für Sitzungstokens, Verifizierungslinks und Passwort-Reset-Codes.',
            tokenBytes: 'Token-Bytes',
            tokenBytesHint: 'Anzahl an Bytes für neu generierte Tokens (16–256). Empfohlen: 32.',
            verifyTokenHours: 'Verifizierungstoken (Stunden)',
            verifyTokenHoursHint: 'Gültigkeitsdauer des E-Mail-Verifizierungstokens (1–720).',
            passwordTokenHours: 'Passworttoken (Stunden)',
            passwordTokenHoursHint: 'Gültigkeitsdauer des Passwort-Reset-Tokens (1–720).',
            setupTokenDays: 'Einrichtungslink (Tage)',
            setupTokenDaysHint: 'Wie lange der Link aus der Einrichtungsmail gültig bleibt (1 bis 30). '
                + 'In Tagen statt Stunden, weil eine Einladung auf die Ferien oder den nächsten Blick '
                + 'ins Postfach wartet. Ein Link, der über ein langes Wochenende verfällt, führt dazu, '
                + 'dass Einladungen zweimal verschickt werden.',
            sessionMinutes: 'Sitzungsdauer auf vertrauten Geräten (Minuten)',
            sessionMinutesHint: 'Gilt, wenn beim Anmelden „Auf diesem Gerät angemeldet bleiben" '
                + 'angekreuzt wurde (5 bis 43200, also bis zu 30 Tage). Lange Sitzungen ersparen '
                + 'häufiges Anmelden; wer sich abmeldet oder sein Passwort ändert, ist unabhängig '
                + 'davon sofort ausgeloggt.',
            untrustedSessionMinutes: 'Sitzungsdauer sonst (Minuten)',
            untrustedSessionMinutesHint: 'Gilt ohne dieses Häkchen, also auf geteilten oder '
                + 'geliehenen Rechnern. Darf nicht länger sein als die Dauer für vertraute Geräte.',
            pepper: {
                title: 'Token-Pepper',
                hint: 'Server-seitiges Geheimnis, das in den HMAC der gespeicherten Tokens einfließt. Ohne ihn kann ein reiner Datenbankleak Tokens nicht wiederherstellen.',
                configured: 'Konfiguriert',
                missing: 'Nicht konfiguriert',
                generate: 'Pepper generieren',
                restartWarning: 'Hinweis: Änderungen am Pepper werden erst nach einem Anwendungsneustart wirksam. Ein vorhandener Pepper kann hier nicht ersetzt werden, da dies alle Sitzungen ungültig macht.',
            },
        },
        hibp: {
            title: 'Have I Been Pwned',
            hint: 'Prüft Passwörter beim Setzen und im Hintergrund gegen die HIBP-Datenbank kompromittierter Passwörter.',
            enabled: 'HIBP-Prüfung aktiviert',
            enabledHint: 'Schaltet sowohl die synchrone Set-Zeit-Prüfung als auch den Hintergrund-Worker ein.',
            endpoint: 'Endpunkt-URL',
            endpointHint: 'Range-Endpunkt der HIBP-API. Muss mit Schrägstrich enden. Standard: https://api.pwnedpasswords.com/range/',
            staleAfterDays: 'Erneut prüfen nach (Tage)',
            staleAfterDaysHint: 'Wartezeit bis zur erneuten Hintergrund-Prüfung eines bereits geprüften Passworts (1–365).',
            timeoutSeconds: 'Timeout (Sekunden)',
            timeoutSecondsHint: 'Verbindungs- und Lese-Timeout pro Anfrage (1–30).',
        },
        twoFactor: {
            title: 'Zwei-Faktor-Authentifizierung',
            hint: 'Globaler Schalter und Schwellenwerte für die 2FA-Logik.',
            enabled: '2FA aktiviert',
            enabledHint: 'Schaltet die 2FA-Funktionalität für die gesamte Instanz ein.',
            stepUpFreshnessSeconds: 'Step-Up Frische (Sekunden)',
            stepUpFreshnessSecondsHint: 'Maximales Alter einer 2FA-Bestätigung für sensible Aktionen (60–3600).',
            trustedDeviceMaxDays: 'Vertrauenswürdige Geräte (Tage)',
            trustedDeviceMaxDaysHint: 'Wie lange ein als vertrauenswürdig markiertes Gerät 2FA überspringen darf (1–30).',
            enrollmentGraceDays: 'Einrichtungsfrist (Tage)',
            enrollmentGraceDaysHint: 'Wie lange ein Nutzer mit erzwungener 2FA Zeit zur Einrichtung hat (1–7).',
            secretKey: {
                title: 'Verschlüsselungsschlüssel',
                hint: '32-Byte-Schlüssel, der die gespeicherten TOTP-Secrets und Backup-Codes verschlüsselt. Ohne ihn startet die Anwendung in Produktion nicht.',
                configured: 'Konfiguriert',
                missing: 'Nicht konfiguriert',
                generate: 'Schlüssel generieren',
                restartWarning: 'Hinweis: Änderungen am Schlüssel werden erst nach einem Anwendungsneustart wirksam. Ein vorhandener Schlüssel kann hier nicht ersetzt werden, da dies alle bestehenden 2FA-Einrichtungen unbrauchbar macht.',
            },
        },
        totp: {
            title: 'TOTP',
            hint: 'Parameter für zeitbasierte Einmalpasswörter (z.B. Google Authenticator).',
            digits: 'Stellen',
            digitsHint: 'Anzahl der angezeigten Ziffern (4–8). Standard: 6.',
            periodSeconds: 'Periode (Sekunden)',
            periodSecondsHint: 'Gültigkeitsdauer eines Codes (15–60). RFC 6238 empfiehlt 30.',
            algorithm: 'Hash-Algorithmus',
            algorithmHint: 'HMAC-Algorithmus. SHA1 ist mit allen Authenticator-Apps kompatibel.',
            driftWindow: 'Drift-Fenster',
            driftWindowHint: 'Wie viele Perioden vor und nach jetzt akzeptiert werden (0–3). 1 ist üblich.',
            issuer: 'Aussteller',
            issuerHint: 'Name, der in der Authenticator-App angezeigt wird.',
        },
        backupCodes: {
            title: 'Backup-Codes',
            hint: 'Einmal verwendbare Wiederherstellungscodes, wenn ein 2FA-Faktor verloren geht.',
            count: 'Anzahl',
            countHint: 'Wie viele Codes pro Einrichtung erzeugt werden (5–20). Standard: 10.',
        },
        webauthn: {
            title: 'WebAuthn',
            hint: 'Hardware-Schlüssel und Passkeys als 2FA-Faktor.',
            rpId: 'Relying-Party-ID',
            rpIdHint: 'Domäne der Anwendung (z.B. ember.example.com). Leer lassen, um automatisch aus der Basis-URL abzuleiten.',
            rpName: 'Relying-Party-Name',
            rpNameHint: 'Angezeigter Name in der Browser-Anfrage. Leer lassen für „Ember".',
            attestation: 'Attestation-Modus',
            attestationHint: 'Wie streng das Authenticator-Modell beim Registrieren geprüft wird.',
            attestationOptions: {
                none: 'Keine',
                indirect: 'Indirekt',
                direct: 'Direkt',
            },
            timeoutSeconds: 'Timeout (Sekunden)',
            timeoutSecondsHint: 'Wie lange der Browser auf eine Antwort wartet (10–300).',
        },
        passkeys: {
            title: 'Passkeys',
            hint: 'Wie weit diese Instanz mit Passkeys geht. Ein Passkey ist eine Anmeldung ohne Passwort: '
                + 'Das Gerät fragt nach Fingerabdruck, Gesicht oder der Geräte-PIN, und das war es.',
            mode: 'Modus',
            modes: {
                OFF: 'Aus',
                OPTIONAL: 'Optional',
                ENCOURAGED: 'Angeboten',
                PREFERRED: 'Bevorzugt',
                PASSWORDLESS: 'Passwortlos',
            },
            modeHints: {
                OFF: 'Passkeys existieren auf dieser Instanz nicht. Sicherheitsschlüssel als zweiter Faktor sind nicht betroffen.',
                OPTIONAL: 'Passkeys funktionieren und können im Konto angelegt werden. Niemandem wird etwas vorgeschlagen.',
                ENCOURAGED: 'Zusätzlich bietet die Anmeldeseite den Passkey-Weg an, und Mitglieder werden einmalig nach der Anmeldung gefragt.',
                PREFERRED: 'Zusätzlich darf ein Konto mit funktionierendem Passkey seine eigene Passwort-Anmeldung abschalten.',
                PASSWORDLESS: 'Neue Konten werden ganz ohne Passwort angelegt. Verlangt nachweislich funktionierenden Mailversand.',
            },
            heldAtOff: 'Die WebAuthn-Domäne fiel auf "{rpId}" zurück, darum sind Passkeys wirksam abgeschaltet. '
                + 'Setze die Relying-Party-ID oder die Basis-URL auf einen echten Hostnamen.',
            readinessTitle: 'Bereitschaft',
            rpIdLine: 'Passkeys binden an die Domäne "{rpId}".',
            lastMail: 'Zuletzt ging am {when} erfolgreich eine E-Mail hinaus.',
            mailStale: 'Das ist lange her; sende zur Sicherheit eine Test-Mail.',
            noMailYet: 'Es ist noch nie eine E-Mail erfolgreich hinausgegangen. Der passwortlose Modus bleibt verweigert, bis eine Test-Mail ankam.',
            dependentAccounts: '{count} Konto/Konten kommen ohne Passkey nicht mehr hinein; unter "Angeboten" lässt sich der Modus darum nicht senken.',
            readinessLimit: 'Geprüft wird nur die eigene Konfiguration der Instanz, nicht, was ein Browser tatsächlich sieht.',
            figureTried: 'Konten mit erprobtem Passkey',
            figurePassword: 'Konten mit Passwort',
            figureStuck: 'Konten mit Passwort und ohne Passkey',
            loadReport: 'Bericht vor dem Umstellen laden',
            reportKeepPassword: '{count} Konto/Konten behalten ihr Passwort.',
            reportWithoutPasskey: 'Davon halten {count} noch keinen Passkey.',
            reportQrOnly: 'Davon sind {count} weder per Mail noch über Erziehungsberechtigte erreichbar, nur über den QR-Code im Raum.',
            reportDormant: '{count} Konto/Konten haben sich seit über einem Jahr nicht angemeldet.',
            saveFailed: 'Der Modus konnte nicht gespeichert werden.',
            loadResidue: 'Wer kann noch nicht?',
            retireAll: 'Alle qualifizierten Passwörter stilllegen',
            retireAllResult: '{retired} Passwort/Passwörter stillgelegt, {passedOver} übergangen: dort steht noch kein erprobter Passkey daneben.',
            residueTitle: '{count} Konto/Konten können noch nicht umziehen',
            residueHint: 'Passwort vorhanden, aber kein erprobter Passkey. Drei Antworten, alle von Menschen: erneut onboarden, so lassen, oder den Zugang nehmen.',
            residueReachable: 'per Mail erreichbar',
            residueGuardian: 'über Erziehungsberechtigte erreichbar',
            residueQrOnly: 'nur über den QR-Code im Raum erreichbar',
            residueLastSeen: 'zuletzt angemeldet am {date}',
            residueNeverSeen: 'noch nie angemeldet',
        },
    },
    waitingList: {
        title: 'Wartelisten',
        createList: 'Neue Warteliste',
        create: 'Erstellen',
        createTitle: 'Neue Warteliste erstellen',
        name: 'Name',
        namePlaceholder: 'z.B. Warteliste 2026',
        description: 'Beschreibung',
        descriptionPlaceholder: 'Beschreibung der Warteliste...',
        scoringFormula: 'Bewertungsformel',
        scoringFormulaPlaceholder: 'z.B. [Alter] * ("[Erfahrung]" == "fortgeschritten" ? 2 : 1)',
        scoringFormulaHint: 'Verwende [Feldname] als Platzhalter. Für Textvergleiche: "[Feldname]" == "wert". Automatische Felder: [wartezeit_monate], [wartezeit_quartale], [wartezeit_jahre]. Alter berechnen: age([Datumsfeld]).',
        confirmInterval: 'Bestätigungsintervall (Tage)',
        noLists: 'Noch keine Wartelisten vorhanden.',
        entries: 'Einträge',
        invites: 'Einladungslinks',
        noEntries: 'Noch keine Einträge.',
        firstname: 'Vorname',
        lastname: 'Nachname',
        firstnamePlaceholder: 'Vorname',
        lastnamePlaceholder: 'Nachname',
        parentName: 'Name der Eltern',
        email: 'E-Mail',
        status: 'Status',
        score: 'Punkte',
        notes: 'Notizen',
        notesPlaceholder: 'Anmerkungen...',
        // New statuses
        status_PENDING: 'Ausstehend',
        status_WAITING: 'Wartend',
        status_INVITED: 'Eingeladen',
        status_TESTING: 'Probezeitraum',
        status_WITHDRAWN: 'Ausgeschieden',
        status_JOINED: 'Beigetreten',
        // State transition actions
        invite: 'Einladen',
        backToWaiting: 'Zurück in die Warteliste',
        answer_COMING: 'Kommt',
        answer_DATE_DOES_NOT_SUIT: 'Termin passt nicht',
        answer_NOT_INTERESTED: 'Kein Interesse',
        answerNote: 'Nachricht zur Antwort',
        startTesting: 'Probezeit starten',
        join: 'Aufnehmen',
        withdraw: 'Ausscheiden',
        inviteTitle: 'Zum Reinschnuppern einladen',
        inviteText: '{name} bekommt eine Einladung per E-Mail. Ein Mitglied wird dabei noch nicht angelegt.',
        inviteAppointment: 'Termin',
        invitePickAppointment: 'Termin suchen…',
        inviteNoAppointments: 'Keine anstehenden Termine gefunden.',
        inviteAppointmentHint: 'Der Termin steht nur in der E-Mail. Angemeldet wird dafür niemand.',
        inviteArrival: 'Bitte da sein um',
        inviteArrivalHint: 'Meist etwas früher als alle anderen, damit jemand Zeit für die Begrüßung hat.',
        inviteSend: 'Einladung senden',
        // Transition confirmation modal
        transitionConfirmTitle: 'Statuswechsel bestätigen',
        transitionBackToWaitingText: 'Soll {name} zurück in die Warteliste? Die Einladung wird damit hinfällig.',
        transitionTestingText: 'Soll für {name} die Probezeit gestartet werden? Erst damit wird das Mitglied angelegt.',
        transitionJoinText: 'Soll {name} endgültig aufgenommen werden?',
        transitionApproveText: 'Soll {name} freigeschaltet und in die Warteliste verschoben werden?',
        transitionRejectText: 'Soll {name} abgelehnt werden?',
        transitionWithdrawText: 'Soll {name} ausgeschieden werden?',
        transitionConfirmAction: 'Bestätigen',
        // Sections
        sectionWaiting: 'Wartend & Eingeladen',
        sectionTesting: 'Im Probezeitraum',
        sectionFinished: 'Beigetreten / Ausgeschieden',
        noTestingEntries: 'Keine Einträge im Probezeitraum.',
        noFinishedEntries: 'Keine abgeschlossenen Einträge.',
        // Group/PermissionGrant config
        testingGroup: 'Probegruppe',
        joinGroup: 'Beitrittsgruppe',
        attendanceThreshold: 'Anwesenheiten bis Aufnahme',
        attendanceCount: 'Anwesenheiten',
        noGroup: 'Keine Gruppe',
        // Guardians
        guardian: 'Erziehungsberechtigter',
        guardians: 'Erziehungsberechtigte',
        addGuardian: 'Erziehungsberechtigten hinzufügen',
        guardianNamePlaceholder: 'Vor- und Nachname',
        guardianEmailPlaceholder: 'E-Mail-Adresse',
        guardianPhonePlaceholder: 'Telefonnummer',
        noGuardians: 'Keine Erziehungsberechtigten hinterlegt.',
        // Timestamps
        invitedAt: 'Eingeladen am',
        testingAt: 'Probezeit seit',
        joinedAt: 'Beigetreten am',
        withdrawnAt: 'Ausgeschieden am',
        addEntry: 'Eintrag hinzufügen',
        saved: 'Gespeichert.',
        createInvite: 'Neuen Link erstellen',
        noInvites: 'Keine Einladungslinks vorhanden.',
        fields: 'Felder',
        addField: 'Feld hinzufügen',
        fieldName: 'Feldname',
        fieldType: 'Typ',
        noFields: 'Noch keine Felder konfiguriert.',
        typeText: 'Text',
        typeNumber: 'Zahl',
        typeDate: 'Datum',
        typeBoolean: 'Ja/Nein',
        typeEnum: 'Auswahl',
        typeBirthDate: 'Geburtsdatum',
        minAgeRegister: 'Mindestalter zur Anmeldung',
        minAgeRegisterHint: 'Leer lassen für keine Grenze. Wirkt nur, wenn die Liste ein Geburtsdatumsfeld hat.',
        minAgeJoin: 'Alter für den Beitritt',
        minAgeJoinHint: 'Wer jünger ist, bleibt auf der Liste und wird gekennzeichnet.',
        belowJoinAge: 'Zu jung',
        belowJoinAgeHint: 'Wartet auf das Beitrittsalter, nicht auf den Platz.',
        hideBelowJoinAge: 'Einträge unter dem Beitrittsalter ausblenden',
        position: 'Position',
        confirmedAt: 'Bestätigt am',
        createdAt: 'Eingetragen am',
        register: {
            title: 'Wartelisten-Anmeldung',
            noCode: 'Kein Einladungscode angegeben.',
            invalidCode: 'Ungültiger oder abgelaufener Einladungscode.',
            requiredFields: 'Bitte fülle alle Pflichtfelder aus.',
            requiredField: 'Das Feld "{name}" ist ein Pflichtfeld.',
            firstnamePlaceholder: 'Vorname',
            lastnamePlaceholder: 'Nachname',
            notesPlaceholder: 'Anmerkungen...',
            submit: 'Anmelden',
            success: 'Deine Anmeldung war erfolgreich! Bitte speichere den folgenden Link, um deinen Status einzusehen.',
            saveLink: 'Dein persönlicher Status-Link',
            saveLinkHint: 'Speichere diesen Link, um deinen Wartelisten-Status einzusehen und dein Interesse zu bestätigen.',
            backToLogin: 'Zurück zum Login',
        },
        publicStatus: {
            title: 'Wartelisten-Status',
            noToken: 'Kein Zugangstoken angegeben.',
            invalidToken: 'Ungültiger oder abgelaufener Zugangstoken.',
            confirmed: 'Interesse wurde bestätigt.',
            savedButStale: 'Deine Antwort ist angekommen. Nur der Stand hier auf der Seite konnte danach nicht aktualisiert werden, du musst nichts noch einmal senden.',
            removed: 'Du wurdest von der Warteliste entfernt.',
            confirmInterest: 'Interesse bestätigen',
            removeFromList: 'Von Warteliste entfernen',
            removeTitle: 'Von Warteliste entfernen?',
            removeConfirm: 'Möchtest du dich wirklich von der Warteliste entfernen?',
            removeHint: 'Diese Aktion kann nicht rückgängig gemacht werden.',
            backToLogin: 'Zurück zum Login',
            waitingSince: 'Auf der Liste seit',
            lastConfirmation: 'Letzte Bestätigung',
            nextConfirmation: 'Nächste Bestätigung bis',
            positionHint: 'Die Position ist nur ein grober Anhaltspunkt und entspricht nicht unbedingt der tatsächlichen Aufnahme­reihenfolge.',
            invitationTitle: 'Deine Einladung',
            invitationIntro: 'Wir würden uns freuen, dich kennenzulernen. Sag uns bitte kurz Bescheid, ob es dir passt.',
            invitationAppointment: 'Termin',
            invitationDate: 'Datum',
            invitationTime: 'Uhrzeit',
            invitationArrival: 'Bitte da sein um',
            invitationLocation: 'Wo',
            answerNote: 'Möchtest du uns noch etwas mitteilen?',
            answerNotePlaceholder: 'Optional',
            answerComing: 'Ich komme',
            answerDate: 'Der Termin passt mir nicht',
            answerNotInterested: 'Kein Interesse',
            answered: 'Danke, deine Antwort ist angekommen.',
            answerGiven: {
                COMING: 'Du hast zugesagt. Wir freuen uns auf dich.',
                DATE_DOES_NOT_SUIT: 'Du hast uns gesagt, dass der Termin nicht passt. Wir melden uns mit einem anderen.',
                NOT_INTERESTED: 'Du hast abgesagt. Wir werden dich nicht wieder anfragen.',
            },
        },
        back: 'Zurück',
        backToList: 'Zurück zur Liste',
        overview: 'Übersicht',
        customFields: 'Eigene Felder',
        fieldsHint: 'Felder definieren, die bei der Anmeldung ausgefüllt werden.',
        manageFields: 'Felder verwalten',
        editField: 'Feld bearbeiten',
        deleteList: 'Warteliste löschen',
        deleteListTitle: 'Warteliste wirklich löschen?',
        deleteListConfirm: 'Soll die Warteliste "{name}" wirklich gelöscht werden? Alle Einträge gehen verloren.',
        deleteEntryTitle: 'Eintrag wirklich löschen?',
        deleteEntryConfirm: 'Soll der Eintrag "{name}" wirklich gelöscht werden?',
        deleteFieldTitle: 'Feld wirklich löschen?',
        deleteFieldConfirm: 'Soll das Feld "{name}" wirklich gelöscht werden? Alle gespeicherten Werte gehen verloren.',
        copyLink: 'Link kopieren',
        linkCopied: 'Link kopiert!',
        uses: 'Nutzungen',
        maxUses: 'Max. Nutzungen',
        maxUsesHint: 'Wie oft kann der Link benutzt werden?',
        maxUsesPlaceholder: '1',
        expiresAt: 'Ablaufdatum',
        expiresAtHint: 'Optional: Link nach diesem Datum ungültig.',
        days: 'Tage',
        confirmIntervalHint: 'Alle wie viele Tage müssen Einträge bestätigt werden?',
        entryDetails: 'Eintrag-Details',
        entryNotFound: 'Eintrag nicht gefunden.',
        fieldNamePlaceholder: 'Name des Feldes',
        enumOptions: 'Auswahloptionen',
        required: 'Pflichtfeld',
        options: 'Optionen',
        selectOption: 'Bitte wählen',
        moveUp: 'Nach oben',
        moveDown: 'Nach unten',
        isPublic: 'Öffentlich',
        isPublicHint: 'Warteliste für öffentliche Anmeldungen freigeben.',
        sendsMail: 'E-Mails senden',
        sendsMailHint:
            'Aus: Diese Warteliste verschickt keine E-Mails. Anmeldungen müssen nicht bestätigt werden, '
            + 'Einladungen und Erinnerungen gehen nicht raus, und niemand wird wegen einer fehlenden '
            + 'Bestätigung entfernt. Für Wartelisten, die nur intern geführt werden.',
        fieldPublic: 'Im öffentlichen Formular',
        publicBadge: 'Öffentlich',
        sectionPending: 'Ausstehende Anmeldungen',
        approve: 'Annehmen',
        reject: 'Ablehnen',
        publicRegistration: {
            selectList: 'Warteliste auswählen',
            noLists: 'Aktuell sind keine Wartelisten für die öffentliche Anmeldung verfügbar.',
            firstname: 'Vorname',
            lastname: 'Nachname',
            email: 'E-Mail-Adresse',
            emailHint: 'Zur Bestätigung wird ein Link an diese Adresse gesendet.',
            notes: 'Anmerkungen',
            notesPlaceholder: 'Optionale Anmerkungen...',
            submit: 'Anmeldung absenden',
            successTitle: 'Fast geschafft!',
            successText: 'Wir haben dir eine E-Mail zur Bestätigung gesendet. Bitte klicke auf den Link in der E-Mail, um deine Anmeldung abzuschließen.',
            successHint: 'Der Link ist 24 Stunden gültig.',
            receivedTitle: 'Anmeldung eingegangen',
            receivedText: 'Deine Anmeldung liegt vor und wird von einem Betreuer geprüft. '
                + 'Eine Bestätigung per E-Mail ist nicht nötig.',
            verifyTitle: 'E-Mail bestätigt',
            verifyText: 'Deine E-Mail-Adresse wurde bestätigt. Deine Anmeldung wird nun von einem Betreuer geprüft.',
            verifyError: 'Der Bestätigungslink ist ungültig oder abgelaufen.',
            verifyErrorGuidance: 'Vielleicht hast du ihn schon einmal benutzt, dann ist deine Anmeldung bereits bestätigt. Sonst melde dich noch einmal an, dann bekommst du einen neuen Link.',
            verifyPageTitle: 'E-Mail-Bestätigung',
            verifyPageDescription: 'Bestätigung der E-Mail-Adresse für eine Anmeldung auf einer Warteliste.',
        },
    },
    csvImport: {
        uploadTitle: 'CSV-Datei hochladen',
        chooseFile: 'Datei auswählen',
        separator: 'Trennzeichen',
        tab: 'Tab',
        rows: 'Zeilen',
        next: 'Weiter',
        preview: 'Vorschau',
        import: 'Importieren',
        startOver: 'Weiteren Import starten',
        fileTooLarge: 'Die Datei ist zu groß. Maximale Größe: {size} MB.',
    },
    memberImport: {
        title: 'Mitglieder importieren',
        previewTitle: '{count} Mitglieder erkannt',
        mappingTitle: 'Spalten zuordnen',
        mappingHint: 'Ordne jede CSV-Spalte einem Ziel zu. Nicht benötigte Spalten auf "Überspringen" setzen.',
        helpTitle: 'Hilfe: Zusammenführen, Aufteilen & Werte zuordnen',
        helpMergeTitle: 'Felder zusammenführen',
        helpMerge: 'Um zwei CSV-Spalten in ein Feld zu kombinieren (z.B. „Straße" und „Hausnummer" → „Straße"), weise beiden Spalten dasselbe Zielfeld zu. Es erscheint ein Zusammenführungs-Hinweis mit Reihenfolge und Trennzeichen. Die Reihenfolge bestimmt, welche Spalte zuerst kommt (niedrigere Zahl = weiter vorne). Das Trennzeichen (Standard: Leerzeichen) wird zwischen den Werten eingefügt.',
        helpSplitTitle: 'Felder aufteilen',
        helpSplit: 'Um eine CSV-Spalte auf mehrere Felder aufzuteilen (z.B. „Max Mustermann" → Vorname + Nachname), weise dieselbe Spalte beiden Zielfeldern zu. Es erscheint eine Aufteilen-Konfiguration mit Trennzeichen (Standard: Leerzeichen) und Auswahl, welcher Teil verwendet wird (Erster, Zweiter, Letzter).',
        helpValueMapTitle: 'Werte zuordnen',
        helpValueMap: 'Bei Ja/Nein-Feldern und Auswahlfeldern müssen die CSV-Werte oft auf die Zielwerte abgebildet werden. Klicke auf „Werte zuordnen" neben dem Feld, um eine Zuordnung zu definieren (z.B. „Ja" → „true", „Nein" → „false" oder „m" → „männlich"). Werte ohne Zuordnung werden unverändert übernommen.',
        managerCountLabel: 'Betreuer pro Mitglied',
        targetSkip: 'Überspringen',
        targetFirstName: 'Vorname',
        targetLastName: 'Nachname',
        targetEmail: 'E-Mail',
        targetGroup: 'Gruppe',
        groupMember: 'Mitglied',
        groupManager: 'Betreuer {n}',
        managerFirstName: 'Vorname',
        managerLastName: 'Nachname',
        managerPhone: 'Telefon',
        managerEmail: 'E-Mail',
        splitAction: 'Spalte aufteilen',
        addSplitPart: 'Weiteren Teil hinzufügen',
        unsplit: 'Aufteilung aufheben',
        splitChar: 'Trennzeichen',
        splitPart: 'Teil',
        scopeAsked: 'Felder dieser Zielgruppe',
        scopeOther: 'Weitere Felder',
        mergedWith: 'Zusammengeführt',
        order: 'Reihenfolge',
        sep: 'Trenner',
        addMap: 'Werte zuordnen',
        editMap: 'Zuordnung bearbeiten',
        valueMapTitle: 'Werte zuordnen',
        valueMapHint: 'CSV-Werte auf Zielwerte abbilden (z.B. "Ja" → "true", "m" → "männlich").',
        csvValue: 'CSV-Wert',
        targetValue: 'Zielwert',
        targetValueNone: 'Nicht übernehmen',
        valueYes: 'Ja',
        valueNo: 'Nein',
        rowIgnore: 'Zeile überspringen',
        rowInclude: 'Doch übernehmen',
        addRow: 'Zeile hinzufügen',
        name: 'Name',
        email: 'E-Mail',
        group: 'Gruppe',
        fields: 'Profilfelder',
        contacts: 'Kontakte',
        done: 'Import abgeschlossen',
        doneTitle: 'Import abgeschlossen',
        doneImported: '{count} Mitglieder importiert.',
        doneSkipped: '{count} Zeilen übersprungen.',
        doneGoToList: 'Zur Mitgliederliste',
        doneStartOver: 'Weiteren Import starten',
        importAnother: 'Weiteren Import',
        toList: 'Zur Mitgliederliste',
        membersCreated: 'Mitglieder',
        managersCreated: 'Betreuer erstellt',
        managersLinked: 'Zuordnungen',
        groupsAssigned: 'Gruppen',
        fieldsSet: 'Profilfelder',
        linkFromCreate: 'Importiere von CSV',
        modeMembers: 'Mitglieder mit Erziehungsberechtigten',
        modeMembersHint: 'Importiere Mitglieder mit optionalen Erziehungsberechtigtern (Eltern/Betreuer), Gruppen und Profilfeldern.',
    },
    teamImport: {
        title: 'Teammitglieder importieren',
        modeTeam: 'Teammitglieder',
        modeTeamHint: 'Importiere Teammitglieder (Betreuer, Trainer) mit der Team-Rolle, Gruppen und Profilfeldern.',
        groupTeam: 'Teammitglied',
        previewTitle: '{count} Teammitglieder erkannt',
        membersCreated: 'Teammitglieder',
    },
    statistics: {
        members: 'Mitglieder',
        groups: 'Gruppen',
        totalSessions: 'Termine (12 Mon.)',
        totalItems: 'Inventar Artikel',
        groupDistribution: 'Mitglieder nach Gruppen',
        attendanceOverTime: 'Anwesenheit nach Monat',
        sessionsPerMonth: 'Termine pro Monat',
        inventoryStatus: 'Inventar Status',
        eventRegistrations: 'Anmeldungen',
        present: 'Anwesend',
        absent: 'Abwesend',
        declined: 'Abgemeldet',
        assigned: 'Zugewiesen',
        unassigned: 'Verfügbar',
        lost: 'Verloren',
        accepted: 'Angenommen',
        pending: 'Ausstehend',
        declinedReg: 'Abgelehnt',
    },
    adminStorageBackend: {
        linkUsage: 'Speicher-Nutzung',
        linkAudit: 'Speicher-Audit',
        linkFromUsage: 'Instanz-Backend verwalten →',
        summary: {
            title: 'Aktive Konfiguration',
            current: 'Die Instanz nutzt derzeit ein {type}-Backend.',
        },
        banner: {
            inFlight: 'Eine Speicher-Migration läuft gerade. Schreibvorgänge auf der gesamten Instanz werden mit 503 abgewiesen, bis sie abgeschlossen ist.',
        },
        form: {
            title: 'Neuen Standard festlegen',
            hint: 'Speichern schreibt die Werte in die Konfiguration, ohne Dateien zu kopieren. Mit "Migrieren" werden alle Dateien ohne eigenen Override auf das neue Ziel verschoben.',
            type: 'Backend-Typ',
            local: {
                root: 'Verzeichnis (Pfad relativ zum Arbeitsverzeichnis)',
                hint: 'Üblicher Wert: data',
            },
            types: {
                local: 'LOCAL (Lokales Verzeichnis)',
                s3: 'S3 / S3-kompatibel',
                smb: 'SMB',
                sftp: 'SFTP',
            },
        },
        actions: {
            probeLive: 'Aktuelles Backend testen',
            probeConfig: 'Verbindung testen',
            probing: 'Teste …',
            apply: 'Speichern und übernehmen',
            applying: 'Übernehme …',
        },
        probe: {
            ok: 'Verbindung erfolgreich getestet.',
            failed: 'Verbindung fehlgeschlagen: {reason}',
        },
        confirm: {
            title: 'Instanz-weit verschieben?',
            body: 'Während der Verschiebung sind sämtliche Schreibvorgänge auf der Instanz pausiert. Nach erfolgreichem Kopieren werden die alten Dateien standardmäßig gelöscht.',
            keepSource: 'Quell-Dateien nach erfolgreicher Übernahme behalten',
            cancel: 'Abbrechen',
            confirm: 'Übernehmen',
        },
        feedback: {
            applied: 'Übernommen. {copied} Dateien kopiert, {skipped} übersprungen, {deleted} an der Quelle gelöscht.',
        },
        errors: {
            loadFailed: 'Konnte Speicher-Konfiguration nicht laden.',
            probeFailed: 'Verbindungstest fehlgeschlagen.',
            applyFailed: 'Übernahme fehlgeschlagen.',
        },
    },
    adminStorageAudit: {
        title: 'Speicher-Audit',
        linkBackend: 'Zur Backend-Konfiguration',
        filters: {
            title: 'Filter',
            hint: 'Optional nach Wache oder Zeitpunkt eingrenzen.',
            stationUid: 'Wachen-UID (optional)',
            before: 'Vor (ISO 8601, optional)',
            limit: 'Maximale Einträge',
        },
        actions: {
            apply: 'Suchen',
            loading: 'Lade …',
        },
        results: {
            title: 'Einträge',
        },
        errors: {
            loadFailed: 'Konnte Audit-Einträge nicht laden.',
        },
    },
    stationStorageBackend: {
        backToUsage: '← Zur Nutzungsübersicht',
        linkFromUsage: 'Speicher-Backend verwalten →',
        summary: {
            title: 'Aktive Konfiguration',
            inherit: 'Die Wache nutzt aktuell den Instanz-Standard ({type}).',
            override: 'Die Wache nutzt aktuell ein eigenes {type}-Backend.',
            cluster: 'Die Dateien der Wache liegen auf dem {type}-Speicher von {cluster}.',
            instanceDefault: 'Instanz-Standard: {type}',
        },
        lockedByCluster: '{cluster} legt fest, wo die Dateien dieser Wache liegen. Diese Seite ist deshalb nur zum Lesen.',
        form: {
            title: 'Backend wählen',
            hint: 'LOCAL übernimmt den Instanz-Standard, jede andere Auswahl überschreibt diesen für diese Wache. Zugangsdaten werden vor der Speicherung verschlüsselt.',
            type: 'Backend-Typ',
            localHint: 'Mit LOCAL wird der eigene Override entfernt und die Wache nutzt wieder den Instanz-Standard.',
            clusterHint: 'Die Dateien der Wache werden auf den Speicher ihres Verbands verschoben. Wohin genau, entscheidet der Verband.',
            types: {
                local: 'LOCAL (Instanz-Standard übernehmen)',
                cluster: 'Speicher des Verbands',
                s3: 'S3 / S3-kompatibel',
                smb: 'SMB',
                sftp: 'SFTP',
            },
            s3: {
                endpoint: 'Endpoint',
                endpointHint: 'Vollständige URL inklusive http:// oder https://. Bei AWS z. B. https://s3.eu-central-1.amazonaws.com.',
                endpointInvalid: 'Endpoint muss mit http:// oder https:// beginnen.',
                region: 'Region',
                regionHint: 'Regions-Bezeichner. Bei AWS z. B. eu-central-1. Andere Anbieter erwarten oft einen Platzhalter wie us-east-1.',
                bucket: 'Bucket',
                bucketHint: 'Name des Buckets, in dem die Dateien liegen sollen. Muss bereits existieren.',
                basePath: 'Basis-Pfad',
                basePathHint: 'Optionaler Präfix innerhalb des Buckets. Leer = Bucket-Wurzel.',
                sseAlgorithm: 'Serverseitige Verschlüsselung',
                sseAlgorithmHint: 'Leer lassen, wenn nicht benötigt. AWS unterstützt z. B. AES256.',
                pathStyle: 'Path-Style-URLs verwenden',
                pathStyleHint: 'Aktivieren bei MinIO, RustFS und den meisten Nicht-AWS-Anbietern. AWS S3 funktioniert auch ohne.',
                accessKey: 'Access Key',
                accessKeyHint: 'Statischer Access-Key, der den Bucket lesen und schreiben darf.',
                secretKey: 'Secret Key',
                secretKeyHint: 'Zugehöriger Secret-Key. Wird verschlüsselt gespeichert.',
            },
            smb: {
                host: 'Host',
                hostHint: 'Hostname oder IP des Servers. Ohne Schema - smb:// nicht voranstellen.',
                hostInvalid: 'Host darf kein Schema (smb://, http://, …) enthalten.',
                port: 'Port',
                portHint: 'Standardmäßig 445.',
                share: 'Share',
                shareHint: 'Name der freigegebenen Ressource ohne führende Backslashes oder Schrägstriche.',
                domain: 'Domain (optional)',
                domainHint: 'Windows-Domäne oder Workgroup. Leer lassen, wenn nicht benötigt.',
                basePath: 'Basis-Pfad',
                basePathHint: 'Optionaler Unterpfad innerhalb der Freigabe.',
                seal: 'SMB-Versiegelung',
                sealHint: 'SMB3-Verschlüsselung der Verbindung. Empfohlen, wenn der Server sie unterstützt.',
                dfs: 'DFS-Auflösung',
                dfsHint: 'Nur aktivieren, wenn der Server DFS-Verweise auflösen soll.',
                username: 'Benutzername',
                usernameHint: 'Konto mit Schreibrecht auf der Freigabe.',
                password: 'Passwort',
                passwordHint: 'Wird verschlüsselt gespeichert.',
            },
            sftp: {
                host: 'Host',
                hostHint: 'Hostname oder IP des Servers. Ohne Schema - sftp:// oder ssh:// nicht voranstellen.',
                hostInvalid: 'Host darf kein Schema (sftp://, ssh://, …) enthalten.',
                port: 'Port',
                portHint: 'Standardmäßig 22.',
                username: 'Benutzername',
                usernameHint: 'Konto mit Schreibrecht im Zielverzeichnis.',
                knownHostsFingerprint: 'Known-Hosts-Fingerprint',
                knownHostsFingerprintHint: 'Optionaler SHA-256-Fingerprint des Hostkeys im Format SHA256:…. Leer = jeden Hostkey akzeptieren (nur in Testumgebungen sinnvoll).',
                basePath: 'Basis-Pfad',
                basePathHint: 'Verzeichnis innerhalb des SFTP-Stamms. Leer = Heimatverzeichnis des Benutzers.',
                password: 'Passwort',
                passwordHint: 'Wird verschlüsselt gespeichert.',
                privateKey: 'Privater Schlüssel',
                privateKeyHint: 'OpenSSH-formatierter privater Schlüssel. Wird verschlüsselt gespeichert.',
                authMode: {
                    password: 'Mit Passwort anmelden',
                    privateKey: 'Mit privatem Schlüssel anmelden',
                    hint: 'Umschalten zwischen Passwort- und Schlüssel-Anmeldung. Es wird immer genau eine der beiden Varianten verwendet.',
                },
            },
        },
        actions: {
            probeConfig: 'Verbindung testen',
            probeLive: 'Aktuelles Backend testen',
            probing: 'Teste …',
            apply: 'Speichern und übernehmen',
            applying: 'Übernehme …',
        },
        probe: {
            ok: 'Verbindung erfolgreich getestet.',
            failed: 'Verbindung fehlgeschlagen: {reason}',
        },
        confirm: {
            title: 'Backend jetzt übernehmen?',
            body: 'Während der Übernahme werden Schreibvorgänge auf dieser Wache mit 503 abgewiesen. Schon vorhandene Dateien mit gleichem Hash werden übersprungen.',
            cancel: 'Abbrechen',
            confirm: 'Übernehmen',
        },
        feedback: {
            applied: 'Übernommen. {copied} Dateien kopiert, {skipped} übersprungen, {deleted} an der Quelle gelöscht.',
        },
        audit: {
            title: 'Letzte Änderungen',
            empty: 'Noch keine Einträge.',
            timestamp: 'Zeitpunkt',
            action: 'Aktion',
            outcome: 'Ergebnis',
            detail: 'Details',
        },
        errors: {
            loadFailed: 'Konnte Speicher-Konfiguration nicht laden.',
            probeFailed: 'Verbindungstest fehlgeschlagen.',
            applyFailed: 'Übernahme fehlgeschlagen.',
        },
    },
    storageMonitoring: {
        totalUsage: 'Gesamtverbrauch',
        totalStations: 'Wachen gesamt',
        stationsWarning: 'Wachen mit Warnung',
        stationsFull: 'Wachen voll',
        recalculateAll: 'Alle neu berechnen',
        presets: 'Kontingent-Vorlagen',
        createPreset: 'Vorlage erstellen',
        editPreset: 'Vorlage bearbeiten',
        deletePreset: 'Vorlage löschen',
        confirmDeletePreset: 'Vorlage löschen?',
        confirmDeletePresetText: 'Die Vorlage wird gelöscht. Bereits zugewiesene Wachen behalten ihre aktuellen Kontingente.',
        presetName: 'Name',
        total: 'Gesamt',
        board: 'Board',
        images: 'Bilder',
        pages: 'Seiten',
        perFile: 'Pro Datei',
        perImage: 'Pro Bild',
        actions: 'Aktionen',
        apply: 'Anwenden',
        applyPreset: 'Vorlage anwenden',
        selectStationsToApply: 'Wähle die Wachen aus, auf die die Vorlage angewendet werden soll.',
        noPresets: 'Keine Vorlagen vorhanden.',
        stationOverview: 'Wachen-Übersicht',
        stationName: 'Wache',
        usage: 'Nutzung',
        quota: 'Kontingent',
        status: 'Status',
        ok: 'OK',
        warning: 'Warnung',
        full: 'Voll',
        categoryBreakdown: 'Kategorie-Aufschlüsselung',
        categoryDetails: 'Kategorie-Details',
        category: 'Kategorie',
        used: 'Belegt',
        files: 'Dateien',
        trackedOnly: 'Nur erfasst',
        readOnlyNote: 'Die Speicherkontingente können nur von der Instanz-Administration angepasst werden.',
        preset: 'Vorlage',
        defaultQuota: 'Standard',
        ownBackendNotice: 'Diese Wache nutzt ein eigenes Speicher-Backend. Instanz-Kontingente gelten nicht - die Dateien liegen außerhalb des Instanz-Speichers.',
        ownBackendBadge: 'Eigenes Backend',
        ownStoreBadge: 'Lager des Verbands',
        clusterGovernedBadge: 'Vom Verband vergeben',
        clusterGovernedHint: 'Diese Wache gehört einem Verband. Was sie nutzen darf, vergibt der Verband aus seinem Kontingent.',
        origin: {
            CLUSTER_GRANT: 'Vom Verband vergeben',
            CLUSTER_DEFAULT: 'Standard des Verbands',
            INSTANCE_OVERRIDE: 'Von der Instanz gesetzt',
            INSTANCE_DEFAULT: 'Standard der Instanz',
            UNLIMITED: 'Ohne Grenze',
        },
        categories: {
            kbFiles: 'Wiki-Dateien',
            boardAttachments: 'Board-Anhänge',
            mediaFiles: 'Mediathek-Dateien',
            mediaImages: 'Mediathek-Bilder',
            instanceMediaFiles: 'Mediathek-Dateien (Instanz)',
            documents: 'Mitglieder-Dokumente',
            movementDocuments: 'Inventar-Belege',
            stationLogo: 'Wachen-Logos',
            images: 'Bilder (gesamt)',
            avatars: 'Profilbilder',
            lostAndFound: 'Fundsachen-Bilder',
            quizQuestion: 'Quiz-Bilder',
            kbIcon: 'Wiki-Ordnersymbole',
            kbImage: 'Wiki-Bilder',
            logoFragment: 'Logo-Bausteine',
            document: 'Rechtsdokumente',
            discoveryKey: 'Discovery-Schlüssel',
            mapTileCache: 'Karten-Cache',
            demoAvatar: 'Demo-Avatare',
        },
    },
    adminStats: {
        emailSection: 'E-Mail System',
        sentToday: 'Heute gesendet',
        queued: 'Warteschlange',
        sending: 'Wird gesendet',
        totalSent: 'Gesamt gesendet',
        failed: 'Fehlgeschlagen',
        providerBlocks: 'Sperren',
        emailHistory: 'E-Mails pro Tag',
        emailStatus: 'E-Mail Status',
        platformSection: 'Plattform',
        stations: 'Wachen',
        accounts: 'Konten',
        members: 'Mitglieder',
        activeSessions: 'Aktive Sitzungen (7T)',
        groups: 'Gruppen',
        twoFactorAccounts: 'Mit Zwei-Faktor',
        dataSection: 'Datenbestand',
        events: 'Termine',
        eventsUpcoming: 'Kommende Termine',
        eventRegistrations: 'Anmeldungen',
        attendanceSessions: 'Anwesenheiten',
        sessionsThisMonth: 'Anwesenheiten (Monat)',
        attendanceEntries: 'Einträge',
        inventoryItems: 'Inventar Artikel',
        profileFields: 'Profilfelder',
        emailStatusLabels: {
            PENDING: 'Warteschlange',
            SENDING: 'Wird gesendet',
            SENT: 'Gesendet',
            FAILED: 'Fehlgeschlagen',
        },
        attendanceStatusLabels: {
            PRESENT: 'Anwesend',
            ABSENT: 'Abwesend',
            EXCUSED: 'Entschuldigt',
            DECLINED: 'Abgesagt',
        },
        growthSection: 'Aktivität',
        sessionsGrowth: 'Neue Sitzungen pro Tag (30 T)',
        registrationsGrowth: 'Neue Anmeldungen pro Tag (30 T)',
        topStations: 'Top-Wachen nach Mitglieder',
        healthSection: 'Status',
        verificationStatus: 'E-Mail-Bestätigung',
        attendanceStatus: 'Anwesenheitsstatus',
        setupStatus: 'Einrichtungsstatus',
        verified: 'Bestätigt',
        unverified: 'Unbestätigt',
        setupComplete: 'Eingerichtet',
        setupPending: 'Einrichtung offen',
    },
    adminOverview: {
        attentionSection: 'Erfordert Aufmerksamkeit',
        allClear: 'Alles erledigt',
        allClearHint: 'Aktuell gibt es nichts, was sofortige Aufmerksamkeit braucht.',
        recentApplicationsSection: 'Offene Wachen-Anträge',
        recentReportsSection: 'Offene Problemmeldungen',
        noApplications: 'Keine offenen Anträge.',
        noReports: 'Keine offenen Meldungen.',
        cards: {
            emailFailed: 'Fehlgeschlagene E-Mails',
            emailStuck: 'Hängende E-Mails',
            emailPending: 'E-Mails in der Warteschlange',
            applicationsPending: 'Wachen-Anträge offen',
            stationsSetupPending: 'Wachen ohne Einrichtung',
            accountsUnverified: 'Unbestätigte Konten',
            federationPending: 'Föderationsanfragen offen',
            discoveryUnreachable: 'Unerreichbare Peers',
            problemReports: 'Offene Problemmeldungen',
        },
    },
    news: {
        loadMoreFailed: 'Weitere Neuigkeiten konnten nicht nachgeladen werden.',
        savedPartly: 'Die Neuigkeit ist angelegt, aber nicht alles daran konnte gespeichert werden. Drücke noch einmal auf Speichern: sie wird dann nicht doppelt angelegt, sondern fertig gespeichert.',
        systemBadge: 'System',
        readMore: 'Weiterlesen',
        create: 'Neuigkeit erstellen',
        empty: 'Keine Neuigkeiten vorhanden.',
        titleField: 'Titel',
        titlePlaceholder: 'Titel der Neuigkeit',
        content: 'Inhalt',
        contentPlaceholder: 'Schreibe hier deinen Text in Markdown...',
        markdownHint: 'Du kannst Markdown verwenden: **fett**, *kursiv*, ## Überschriften, - Listen, [Links](url).',
        showPreview: 'Vorschau',
        preview: 'Vorschau',
        restrictToGroups: 'Sichtbarkeit einschränken',
        restrictHint: 'Ohne Auswahl ist die Neuigkeit für alle Mitglieder sichtbar.',
        announcement: {
            title: 'Aus einem Termin geschrieben',
            occurrence: 'Angekündigt wird der Termin am {date}.',
            noOccurrence: 'Dieser Termin hat kein Datum, deshalb steht im Entwurf keins.',
            when: 'Wann:',
            until: 'bis',
            linkLabel: 'Zum Termin',
            snapshot: 'Der Entwurf ist eine Abschrift. Ändert sich der Termin später, ändert sich diese Neuigkeit nicht mit.',
            linkInternal: 'Der Link am Ende führt zum Termin und funktioniert nur für Mitglieder dieser Wache.',
            noLink: 'Ein Link zum Termin steht nicht im Entwurf: Partnerwachen und Leser der öffentlichen Seite haben keine Adresse dafür.',
            restricted: 'Der Termin ist auf eine Auswahl beschränkt, deshalb startet die Neuigkeit mit derselben Auswahl. Solange sie steht, erreicht die Neuigkeit weder Partnerwachen noch die öffentliche Seite.',
            widening: 'Diese Neuigkeit verlässt die Wache.',
            carries: 'Sie nimmt diese Angaben des Termins mit: {fields}.',
            notPublic: 'Am Termin sind davon nicht öffentlich: {fields}.',
        },
        deleteConfirm: 'Neuigkeit "{title}" wirklich löschen?',
        commentsCount: '{count} Kommentare',
        addComment: 'Kommentieren',
        deleteConfirmTitle: 'Neuigkeit löschen',
        deleteConfirmMessage: 'Möchtest du diese Neuigkeit wirklich löschen?',
        views: {
            openModal: 'Wer hat es gesehen?',
            title: 'Aufrufe',
            seen: 'Gesehen',
            unseen: 'Noch nicht gesehen',
            noneSeen: 'Noch niemand hat diese Neuigkeit gesehen.',
            allSeen: 'Alle berechtigten Mitglieder haben diese Neuigkeit gesehen.',
        },
        federation: 'Föderation',
        federationShareHint: 'Wenn aktiviert, können Mitglieder von Partnerwachen diese Neuigkeit sehen und kommentieren.',
        federationNoPermission: 'Du benötigst die Berechtigung „Neuigkeiten föderieren", um die Föderationseinstellungen zu ändern.',
        federationVisibility: 'Sichtbarkeit bei Partnern',
        visibilityAllMembers: 'Alle Mitglieder',
        visibilityTeam: 'Team',
        visibilityManager: 'Leitung',
        partnerNews: 'Partner-Neuigkeiten',
        publicBlog: 'Blog-Beitrag',
        publicBlogHint: 'Wenn aktiviert, wird diese Neuigkeit auf der öffentlichen Seite als Blog-Beitrag angezeigt.',
        enableBlocks: 'Mit dem Seiten-Editor schreiben',
        enableBlocksHint: 'Der Text wandert in einen Textblock, den du danach aufteilst. Zurück geht es nicht: der gespeicherte Text wird von da an aus den Blöcken erzeugt.',
        attachments: 'Anhänge',
        attachmentsHint: 'Dateien, die die Neuigkeit mitgibt - das Protokoll, das Formular, der Flyer. Sie stehen unter dem Text und reisen in Feeds und zu Partnerwachen mit.',
        attachmentsEmpty: 'Noch keine Anhänge.',
        attachmentAdd: 'Datei anhängen',
        attachmentRemove: 'Anhang entfernen',
        attachmentLabelPlaceholder: 'Anzeigename (optional)',
        publicBlogBadge: 'Blog',
    },
    publicForm: {
        submit: 'Absenden',
        submitting: 'Wird gesendet …',
        thanksTitle: 'Danke für deine Antwort!',
        thanksText: 'Deine Antwort wurde übermittelt.',
        alreadyAnswered: 'Dieses Formular hast du bereits ausgefüllt.',
        alreadyAnsweredGuidance: 'Deine erste Antwort ist angekommen und zählt. Ein zweites Mal nimmt dieses Formular niemanden auf.',
        rateLimited: 'Du hast in kurzer Zeit zu viele Antworten gesendet. Bitte versuche es später noch einmal.',
        rateLimitedGuidance: 'Warte ein paar Minuten und sende deine Antwort dann noch einmal. Deine Eingaben bleiben so lange stehen.',
        notFound: 'Dieses Formular ist nicht verfügbar.',
        notFoundGuidance: 'Es wurde geschlossen oder gelöscht, oder der Link ist unvollständig. Frag dort nach einem aktuellen Link, wo du diesen bekommen hast.',
        unreachableHere: 'Diese Umfrage ist von außen nicht mehr erreichbar und kann auf dieser Seite nicht beantwortet werden. Sie ist nur noch über ihren Link zugänglich oder wurde gelöscht.',
        cellPollUnpicked: 'Noch keine Umfrage ausgewählt.',
        cellContactUnpicked: 'Noch kein Kontaktformular ausgewählt.',
        notPublishedYet: 'Dieses Formular ist noch nicht freigegeben. Sobald es geöffnet wird, kannst du hier antworten.',
        notOpenYet: 'Dieses Formular ist noch nicht geöffnet. Schau später noch einmal vorbei.',
        closed: 'Dieses Formular nimmt keine Antworten mehr an.',
        closedSince: 'Dieses Formular nimmt seit dem {when} keine Antworten mehr an.',
        closedWhileOpen: 'Dieses Formular wurde geschlossen, während du es ausgefüllt hast. Deine Antwort konnte nicht mehr gespeichert werden.',
        closedWhileOpenGuidance: 'Daran lässt sich hier nichts ändern. Wenn deine Antwort trotzdem gebraucht wird, wende dich an die Wache, von der das Formular kommt.',
    },
    shareLink: {
        gone: 'Dieser Link führt nirgendwo mehr hin.',
        goneGuidance: 'Er wurde zurückgezogen, oder das, worauf er zeigt, gibt es nicht mehr. Frag dort nach einem aktuellen Link, wo du diesen bekommen hast.',
        hint: 'Wer diesen Link hat, kann antworten. Er steht in keinem Menü und wird von Suchmaschinen nicht erfasst.',
        copy: 'Link kopieren',
        copied: 'Kopiert',
        replace: 'Neuen Link erzeugen',
        replaceWarning: 'Der bisherige Link hört sofort auf zu funktionieren. Wer ihn bereits bekommen hat, kommt damit nicht mehr weiter.',
        replaceConfirm: 'Neuen Link erzeugen',
        replaceConflict: 'Der Link wurde zwischenzeitlich geändert. Öffne die Ansicht erneut, um den aktuellen zu sehen.',
        none: 'Diese Seite hat noch keinen Link.',
        formNone: 'Dieses Formular hat noch keinen Link zum Versenden.',
        formTitle: 'Link zum Versenden',
        formPublicHint: 'Dieses Formular ist öffentlich erreichbar und hat damit eine feste Adresse. Wenn du stattdessen einen Link möchtest, der sich wieder zurückziehen lässt, stelle oben „Öffentlich erreichbar" ab.',
        sentYou: 'hat dir das hier geschickt',
        activeOnSave: 'Der Link steht bereit. Was er öffnet, ist das zuletzt gespeicherte Formular: Speichere, damit deine Änderungen darin ankommen.',
        create: 'Link erzeugen',
        createFailed: 'Der Link konnte nicht erzeugt werden.',
    },
    userSettings: {
        title: 'Einstellungen',
        saved: 'Einstellungen gespeichert.',
        emailTitle: 'E-Mail-Versand',
        emailEnabled: 'E-Mail-Benachrichtigungen aktivieren',
        emailEnabledHint: 'Aktiviere den E-Mail-Versand, um Benachrichtigungen per E-Mail zu erhalten.',
        emailConsent: 'Mit der Aktivierung stimmst du zu, dass deine E-Mail-Adresse an den Anbieter „{provider}" weitergegeben wird.',
        mailProviderInfo: 'E-Mails werden über den Anbieter „{provider}" versendet.',
        mailProviderPrivacy: 'Datenschutzhinweise des Anbieters',
        mailProviderUnknown: 'Unbekannt',
        mailNotConfigured: 'E-Mail-Versand ist für diese Wache nicht konfiguriert.',
        notifications: 'Benachrichtigungen',
        notificationsHint: 'Wähle, welche Benachrichtigungen du in der App und per E-Mail erhalten möchtest.',
        columnApp: 'App',
        columnEmail: 'E-Mail',
        columnFeed: 'Feed',
        notifyNews: 'Neuigkeiten',
        notifyNewsHint: 'Neue Neuigkeiten auf der Pinnwand.',
        notifyComments: 'Kommentare',
        notifyCommentsHint: 'Antworten auf deine Beiträge und Kommentare.',
        notifyMentions: 'Erwähnungen',
        notifyMentionsHint: 'Wenn jemand dich in einem Kommentar erwähnt.',
        notifyEvents: 'Neue Termine',
        notifyEventsHint: 'Neue Termine werden erstellt.',
        notifyEventStatus: 'Anmeldungen',
        notifyEventStatusHint: 'Änderungen an Termin-Anmeldungen.',
        notifyExchanges: 'Tausch-Anfragen',
        notifyExchangesHint: 'Neue Tausch-Anfragen und Statusänderungen.',
        notifyGroups: 'Gruppen',
        notifyGroupsHint: 'Änderungen an deinen Gruppenmitgliedschaften.',
        notifyProfile: 'Profil',
        notifyProfileHint: 'Änderungen an Profildaten deiner verwalteten Mitglieder.',
        notifyProcurement: 'Beschaffung',
        notifyProcurementHint: 'Beschaffungsanfragen und -abschlüsse.',
        sessions: 'Aktive Sitzungen',
        currentSession: 'Aktuell',
        lastActive: 'Letzte Aktivität',
        created: 'Erstellt',
        location: 'Standort',
        unknownLocation: 'Unbekannt',
        invalidateAll: 'Alle abmelden',
        invalidateAllTitle: 'Alle Sitzungen beenden',
        invalidateAllWarning: 'Alle Sitzungen werden beendet. Du wirst ausgeloggt und musst dich neu anmelden.',
        unknownDevice: 'Unbekanntes Gerät',
        unknownBrowser: 'Unbekannter Browser',
        justNow: 'Gerade eben',
        minutesAgo: 'Vor {n} Min.',
        hoursAgo: 'Vor {n} Std.',
        daysAgo: 'Vor {n} Tagen',
        feedTitle: 'RSS / iCal-Feeds',
        feedHint: 'Generiere einen persönlichen Feed-Token, um deine Benachrichtigungen als RSS/Atom-Feed oder deinen Terminkalender als iCal-Abo in externen Apps zu nutzen.',
        feedIcal: 'iCal-Kalender',
        feedRss: 'RSS-Feed',
        feedAtom: 'Atom-Feed',
        feedCreate: 'Feed-Token erstellen',
        feedHelp: 'Hilfe',
        feedIcalHint: 'Abonniere deine Termine direkt in deinem Kalender (Apple Kalender, Google Kalender, Thunderbird, Outlook). Aktualisiert sich automatisch - neue oder geänderte Termine erscheinen ohne Zutun.',
        feedAtomRecommended: 'Empfohlen',
        feedAtomHint: 'Dein persönlicher Benachrichtigungsstrom für News, Anmeldungen, Lost & Found und mehr. Atom ist das moderne, voll unterstützte Format mit Autor, Kategorien und MediaRSS-Vorschaubildern. Funktioniert in Thunderbird, NetNewsWire, Reeder, Feedly und nahezu allen modernen Readern.',
        feedRssFallback: 'RSS-Notfall-Fallback anzeigen',
        feedRssHint: 'Nur verwenden, wenn dein Reader kein Atom unterstützt. Bietet weniger Metadaten als Atom.',
        feedPresetLabel: 'Inhalt der Feed-Einträge',
        feedPresetRich: 'Ausführlich',
        feedPresetCompact: 'Kompakt',
        feedPresetMinimal: 'Minimal',
        feedPresetHint: {
            rich: 'Volle Details inkl. Statuslisten, Felder und eingebetteter Bilder.',
            compact: 'Nur Überschrift und Link - kein Detailblock. Bilder bleiben sichtbar.',
            minimal: 'Nur Überschrift und Link - keine Detailblöcke, keine eingebetteten Bilder.',
        },
        feedRegenerate: 'Neu generieren',
        feedRegenerateConfirmTitle: 'Token neu generieren?',
        feedRegenerateConfirmBody: 'Der bisherige Token wird sofort ungültig. Alle Reader (Thunderbird, Apple Kalender, …), die ihn schon abonniert haben, müssen die neue URL übernehmen.',
        feedRevoke: 'Widerrufen',
        feedRevokeConfirm: 'Token widerrufen',
        feedRevokeConfirmTitle: 'Token widerrufen?',
        feedRevokeConfirmBody: 'Der Token wird gelöscht und alle abonnierten Reader liefern keine neuen Inhalte mehr aus. Du kannst jederzeit einen neuen Token erstellen.',
        gdprTitle: 'Datenauskunft (DSGVO)',
        gdprHint: 'Lade eine vollständige Kopie aller deiner gespeicherten Daten herunter. Das ZIP-Archiv enthält dein Konto, deine Mitgliedschaft in jeder Station, sowie Anwesenheiten, Termine, Inventar, Umfragen und Wiki-Dateien aus allen Stationen.',
        gdprExport: 'Vollständige Datenauskunft als ZIP herunterladen',
        gdprExportManaged: 'Daten von {name} als ZIP herunterladen',
        gdprManaged: 'Daten verwalteter Mitglieder',
        deleteTitle: 'Konto löschen',
        deleteWarning: 'Dein Konto und alle persönlichen Daten werden unwiderruflich gelöscht. Deine Mitgliedschaft wird anonymisiert.',
        deleteAccount: 'Konto endgültig löschen',
        deleteConfirmWarning: 'Bist du sicher? Dein Konto wird gelöscht, alle Sitzungen beendet, dein Name anonymisiert und alle persönlichen Daten entfernt. Diese Aktion kann nicht rückgängig gemacht werden.',
    },
    dashboard: {
        sessionLoadFailed: 'Deine Sitzungsdaten konnten nicht geladen werden. Bitte versuche es erneut.',
        sessionRetry: 'Erneut versuchen',
        notifications: 'Benachrichtigungen',
        notificationSettings: 'Benachrichtigungs-Einstellungen',
        acknowledge: 'Gelesen',
        acknowledgeAll: 'Alle gelesen',
        noNotifications: 'Keine neuen Benachrichtigungen.',
        feedSetupHint: 'Benachrichtigungen per RSS/Atom-Feed in deinem Feedreader empfangen.',
        feedInactiveHint: 'Dein Benachrichtigungs-Feed wurde seit über 24 Stunden nicht abgerufen.',
        feedSetup: 'Feed einrichten',
        feedIcalSetupHint: 'Termine direkt in deinen Kalender abonnieren (iCal).',
        feedIcalInactiveHint: 'Dein Kalender-Feed wurde seit über 24 Stunden nicht abgerufen.',
        exchanges: 'Tausch-Anfragen',
        noExchanges: 'Keine offenen Tausch-Anfragen.',
        exchangeStatus: {
            ANNOUNCED: 'Angekündigt',
            RECEIVED: 'Empfangen',
            SHIPPED: 'Versendet',
            ARRIVED: 'Angekommen',
            DONE: 'Erledigt',
            CANCELLED: 'Abgebrochen',
            DECLINED: 'Abgelehnt',
        },
        registrations: 'Meine Anmeldungen',
        registrationsManaged: 'Anmeldungen',
        awaitingAnswer: 'Antwort ausstehend',
        selfChecks: 'Ausrüstung prüfen',
        awaitingSoon: 'Läuft ab',
        awaitingDaysLeft: 'noch {days} Tage',
        noAwaitingAnswer: 'Alles beantwortet.',
        noRegistrations: 'Keine aktuellen Anmeldungen.',
        upcomingEvents: 'Nächste Termine',
        showAll: 'Alle anzeigen',
        registrationRequired: 'Anmeldung nötig',
        profileIncomplete: 'Dein Profil ist unvollständig',
        profileIncompleteHint: 'Bitte fülle alle Pflichtfelder in deinem Profil aus.',
        completeProfile: 'Profil vervollständigen',
        registrationStatus: {
            PENDING: 'Ausstehend',
            ACCEPTED: 'Bestätigt',
            DENIED: 'Abgelehnt',
            DECLINED: 'Abgemeldet',
            WITHDRAWN: 'Zurückgezogen',
        },
        lendingStatus: {
            REQUESTED: 'Angefragt',
            APPROVED: 'Genehmigt',
            DECLINED: 'Abgelehnt',
            LENT: 'Ausgeliehen',
            RETURNED: 'Zurückgegeben',
            CLOSED: 'Abgeschlossen',
        },
    },
    notification: {
        newNews: '{title} - von {author}',
        newsComment: '{author} hat „{newsTitle}" kommentiert',
        commentMention: '{author} hat dich in „{entityTitle}" erwähnt',
        movementAdvanced: '{inventoryName}: {stepLabel}',
        movementDeclined: '{inventoryName}: Bewegung abgelehnt. {reason}',
        movementCancelled: '{inventoryName}: Vorgang für {itemName} abgebrochen. {reason}',
        movementCancelledAway: '{inventoryName}: Vorgang für {itemName} abgebrochen. {reason} Das Stück ist beim Eigentümer geblieben.',
        movementRaised: '{memberName}: Vorgang für {inventoryName}',
        newEvent: 'Neuer Termin: {title}',
        newEventsBatch: '{count} neue Termine wurden angelegt',
        eventRegistrationStatus: '{memberName} - {eventName}: {status}',
        memberAddedToGroup: 'Du wurdest zur Gruppe {groupName} hinzugefügt',
        profileFieldChanged: '{memberName} hat das Feld {fieldName} geändert',
        procurementRequested: 'Beschaffung angefragt: {inventoryName}',
        procurementFulfilled: 'Beschaffung abgeschlossen: {inventoryName}',
        newForm: 'Neue Umfrage: {title}',
        lostAndFoundNew: 'Neuer Fundgegenstand: {description}',
        lostAndFoundClaimed: '{name} hat „{description}" beansprucht',
        lendingNewRequest: 'Neue Leihanfrage von {stationName}: {itemSummary}',
        lendingStatusChange: 'Leihanfrage von {stationName}: {status}',
        lendingNewMessage: 'Neue Nachricht von {senderName} ({stationName})',
        boardTicketUpdate: '{ticketKey} auf „{boardName}": {changeDescription}',
        waitlistNewEntry: '{childName} wurde auf die Warteliste {listName} gesetzt',
        waitlistPublicRegistration: 'Öffentliche Anmeldung: {childName} auf Warteliste {listName}',
        waitlistInvitationAnswered: '{childName} hat auf die Einladung geantwortet: {answer} (Warteliste {listName})',
        storageWarning: 'Speicher bei {usedPercent}% ({usedFormatted} von {quotaFormatted})',
        mailboxSuspended: 'Postfach „{mailbox}" wird nicht mehr abgerufen: {reason}',
        mailImportUnbound: '{count} per Mail eingegangene Dokumente sind noch keinem Mitglied zugeordnet',
        registrationDeadlineExpired: '{eventName}: {pendingCount} offene Anmeldungen nach Fristende',
        eventCancelled: '{eventName} wurde abgesagt',
        eventReminder: '{eventName} findet in {daysBefore} Tagen statt ({eventDate})',
        registrationClosing: 'Die Anmeldung für {eventName} endet in {daysBefore} Tagen. Für {memberName} steht noch keine Antwort.',
        registrationAnswerMissing: '{eventName} am {eventDate}: Es gibt eine neue Frage, {memberName} hat sie noch nicht beantwortet.',
        procedureAssigned: 'Neuer Ablauf: {procedureName} - zugewiesen von {assignedByName}',
        procedureResolved: 'Ablauf abgeschlossen: {procedureName}',
        procedureReopened: 'Ablauf wieder geöffnet: {procedureName}',
        procedureItemChecked: '{procedureName}: „{itemTitle}" erledigt von {checkedByName}',
        selfCheckAssigned: '{handedOutBy} bittet dich, die Ausrüstung von {memberName} zu prüfen (bis {dueOn})',
        selfCheckSubmitted: '{answeredBy} hat die Ausrüstungsangaben für {memberName} abgegeben',
        selfCheckRowRefused: 'Deine Angabe zu {itemName} für {memberName} kam zurück: {reason}',
        clusterApplicationSubmitted: '{stationName} möchte dem Verband beitreten',
        clusterApplicationApproved: 'Eure Wache gehört jetzt zu {clusterName}',
        clusterApplicationDenied: '{clusterName} hat die Anfrage abgelehnt: {reason}',
        clusterApplicationWithdrawn: '{stationName} hat die Beitrittsanfrage zurückgezogen',
        clusterStationReleased: '{clusterName} hat eure Wache entlassen',
        clusterModuleDenied: '{clusterName} hat {module} für eure Wache abgeschaltet. Eure Daten dazu bleiben erhalten.',
        clusterQuotaChanged: '{clusterName} hat euren Speicherplatz auf {quota} gesetzt',
        clusterItemIssued: '{clusterName} schickt euch {itemName}',
        clusterItemLost: '{itemName} wird bei {stationName} vermisst',
        clusterMemberRoleChanged: 'Deine Rechte im Verband {clusterName} haben sich geändert',
        clusterFieldValueChanged: '{clusterName} hat in deinem Profil {fields} geändert',
        typeLabel: {
            NEW_NEWS: 'Neuigkeit',
            NEWS_COMMENT: 'Kommentar',
            COMMENT_MENTION: 'Erwähnung',
            EVENT_REGISTRATION_STATUS: 'Anmeldung',
            MOVEMENT_ADVANCED: 'Vorgang weiter',
            MOVEMENT_RAISED: 'Neuer Vorgang',
            MOVEMENT_DECLINED: 'Bewegung abgelehnt',
            MOVEMENT_CANCELLED: 'Vorgang abgebrochen',
            NEW_EVENT: 'Neuer Termin',
            NEW_EVENTS_BATCH: 'Neue Termine',
            MEMBER_ADDED_TO_GROUP: 'Gruppenänderung',
            PROFILE_FIELD_CHANGED: 'Profiländerung',
            PROCUREMENT_REQUESTED: 'Beschaffung',
            PROCUREMENT_FULFILLED: 'Beschaffung abgeschlossen',
            NEW_FORM: 'Neue Umfrage',
            LOST_AND_FOUND_NEW: 'Fundbüro',
            LOST_AND_FOUND_CLAIMED: 'Fundsache beansprucht',
            LENDING_NEW_REQUEST: 'Neue Leihanfrage',
            LENDING_STATUS_CHANGE: 'Leihanfrage aktualisiert',
            LENDING_NEW_MESSAGE: 'Neue Nachricht',
            BOARD_TICKET_UPDATE: 'Ticket-Update',
            WAITLIST_NEW_ENTRY: 'Neue Wartelisten-Anmeldung',
            WAITLIST_PUBLIC_REGISTRATION: 'Öffentliche Anmeldung',
            WAITLIST_INVITATION_ANSWERED: 'Einladung beantwortet',
            STORAGE_WARNING: 'Speicherwarnung',
            MAILBOX_SUSPENDED: 'Postfach ausgesetzt',
            MAIL_IMPORT_UNBOUND: 'Dokumente ohne Zuordnung',
            REGISTRATION_DEADLINE_EXPIRED: 'Anmeldefrist abgelaufen',
            EVENT_CANCELLED: 'Veranstaltung abgesagt',
            EVENT_REMINDER: 'Termin-Erinnerung',
            REGISTRATION_CLOSING: 'Anmeldefrist endet',
            REGISTRATION_ANSWER_MISSING: 'Antwort nachgefragt',
            PROCEDURE_ASSIGNED: 'Ablauf zugewiesen',
            PROCEDURE_RESOLVED: 'Ablauf abgeschlossen',
            PROCEDURE_REOPENED: 'Ablauf wieder geöffnet',
            PROCEDURE_ITEM_CHECKED: 'Schritt erledigt',
            SELF_CHECK_ASSIGNED: 'Ausrüstungsprüfung angefragt',
            SELF_CHECK_SUBMITTED: 'Ausrüstungsangaben abgegeben',
            SELF_CHECK_ROW_REFUSED: 'Angabe zurückgegeben',
            CLUSTER_APPLICATION_SUBMITTED: 'Neue Beitrittsanfrage',
            CLUSTER_APPLICATION_APPROVED: 'Beitritt bestätigt',
            CLUSTER_APPLICATION_DENIED: 'Beitritt abgelehnt',
            CLUSTER_APPLICATION_WITHDRAWN: 'Anfrage zurückgezogen',
            CLUSTER_STATION_RELEASED: 'Aus dem Verband entlassen',
            CLUSTER_MODULE_DENIED: 'Modul abgeschaltet',
            CLUSTER_QUOTA_CHANGED: 'Speicherplatz geändert',
            CLUSTER_ITEM_ISSUED: 'Material unterwegs',
            CLUSTER_ITEM_LOST: 'Material vermisst',
            CLUSTER_MEMBER_ROLE_CHANGED: 'Rechte geändert',
            CLUSTER_FIELD_VALUE_CHANGED: 'Profil vom Verband geändert',
        },
    },
    comments: {
        title: 'Kommentare',
        placeholder: 'Kommentar schreiben...',
        post: 'Absenden',
        reply: 'Antworten',
        edit: 'Bearbeiten',
        delete: 'Löschen',
        save: 'Speichern',
        edited: 'bearbeitet',
        deleted: 'Dieser Kommentar wurde gelöscht.',
        replyPlaceholder: 'Antwort schreiben...',
        mentionGroup: 'Gruppe',
        mentionEvent: 'Alle Teilnehmer',
        mentionRegistered: 'Angemeldet',
        mentionDeclined: 'Abgelehnt',
        mentionHint: "Tippe {'@'}, um Mitglieder oder Gruppen zu erwähnen.",
        newest: 'Neueste zuerst',
        oldest: 'Älteste zuerst',
        highlightMissing: 'Der Kommentar aus der Meldung ist nicht mehr da.',
    },
    notes: {
        title: 'Notizen',
        placeholder: 'Notiz schreiben (Markdown)...',
        history: 'Verlauf',
        versionHistory: 'Änderungsverlauf',
        noVersions: 'Noch keine Änderungen.',
        loadFailed: 'Die vorhandene Notiz konnte nicht geladen werden. Speichere jetzt nichts, sonst überschreibst du sie.',
        versionsFailed: 'Der Änderungsverlauf konnte nicht geladen werden.',
    },
    flows: {
        title: 'Abläufe',
        intro: 'Ein Ablauf ist eine Kette von Schritten. Jeder Schritt sagt, wer ihn bestätigt, um welches Teil es geht und wo das Teil danach ist.',
        serves: {
            owner: 'Eigentümer',
            purpose: 'Zweck',
            party: 'Gegenseite',
        },
        restore: 'Auf Vorgabe zurücksetzen',
        restoreTitle: 'Kette zurücksetzen',
        restoreConfirm: 'Diese Kette wird durch die vorbereitete Kette für ihre Kombination ersetzt. '
            + 'Eigene Schritte gehen dabei verloren.',
        restoreMovementsHint: 'Auf dieser Kette sind noch Bewegungen unterwegs. Jeder Schritt, auf dem welche '
            + 'stehen, braucht einen Schritt der neuen Kette, auf dem sie weitergehen.',
        restoreCurrentStep: 'Bisheriger Schritt',
        restoreTargetStep: 'Geht weiter auf',
        restoreAffects: 'Betroffene Bewegungen: {count}',
        restoreStepGone: 'Ohne Schritt',
        restoreChooseStep: 'Schritt wählen',
        restoreUncertain: 'Bitte prüfen',
        restoreUncertainHint: 'Für die markierten Schritte ließ sich kein Schritt der neuen Kette sicher '
            + 'zuordnen. Wähle selbst, wo es weitergeht.',
        restoreAction: 'Zurücksetzen',
        newFlow: 'Neuer Ablauf',
        newFlowPlaceholder: 'z. B. Tausch, kurz',
        purpose: 'Zweck',
        create: 'Anlegen',
        addStep: 'Schritt hinzufügen',
        stepLabel: 'Bezeichnung',
        stepLabelPlaceholder: 'z. B. An den Träger geschickt',
        stepActor: 'Bestätigt von',
        stepSubject: 'Betrifft',
        stepCustody: 'Danach',
        picksItem: 'Legt das Ersatzteil fest',
        stepCount: '{count} Schritte',
        toggleSteps: 'Schritte anzeigen',
        archiveStep: 'Schritt stilllegen',
        archiveFlow: 'Ablauf stilllegen',
        archived: 'Stillgelegt',
        ownedByCluster: 'Vom Träger vorgegeben',
        editStep: 'Schritt bearbeiten',
        moveUp: 'Nach oben',
        moveDown: 'Nach unten',
        actor: 'Bestätigt von',
        subject: 'Betrifft',
        custodyAfter: 'Danach',
        party: {
            STORE: 'Lager',
            MEMBER: 'Mitglied',
        },
        problem: {
            TOO_SHORT: 'Eine Kette braucht mindestens zwei Schritte: einen, der das Teil anfordert, und einen, der den Erhalt bestätigt.',
            ENDS_IN_TRANSIT: 'Der letzte Schritt lässt das Teil unterwegs. Beende die Kette dort, wo jemand es in der Hand hat: beim Träger, auf der Wache oder beim Mitglied.',
            OUTGOING_NAMES_ITEM: 'Der Schritt „{detail}" legt das ankommende Teil fest, muss also auch das ankommende Teil betreffen.',
            EXCHANGE_NEEDS_BOTH_DIRECTIONS: 'Bei einem Tausch geht ein Teil raus und eins kommt rein. Die Kette braucht für beides einen Schritt.',
            ARRIVAL_UNNAMED: 'In dieser Kette kommt etwas an. Ein Schritt muss festlegen, welches Teil das ist.',
            FLOW_IN_USE: 'Eine Bewegung läuft noch durch diese Kette. Solange können ihre Schritte sich nicht ändern.',
            FLOW_NAME_REQUIRED: 'Ein Ablauf braucht einen Namen.',
            STEP_LABEL_REQUIRED: 'Ein Schritt braucht eine Bezeichnung.',
            ILLEGAL_STEP_CUSTODY: 'Ein Schritt darf das Teil nicht in diesem Zustand hinterlassen.',
            ONLY_ARRIVAL_NAMES_ITEM: 'Nur ein Schritt, der das ankommende Teil betrifft, darf es festlegen.',
            ITEM_ALREADY_NAMED: 'Ein anderer Schritt dieser Kette legt das ankommende Teil bereits fest.',
            ORDER_MUST_NAME_EVERY_STEP: 'Eine Reihenfolge nennt jeden Schritt der Kette genau einmal.',
        },
    },
    movements: {
        chain: 'Ablauf',
        lossReport: {
            title: 'Verlustmeldung',
            hint: 'Der Gegenstand gilt als vermisst. Angefragt ist ein Ersatz, den der Verband schicken oder ablehnen kann.',
            managerNote: 'Von der Wache:',
            memberNote: 'Vom Mitglied beim Melden:',
            download: 'Beleg herunterladen',
        },
        detailSubtitle: 'Bewegung #{id}',
        detailSubtitlePurpose: '{purpose}, Bewegung #{id}',
        member: 'Mitglied: ',
        inventory: 'Inventar: ',
        reason: 'Grund: ',
        closeReason: 'Abgelehnt bzw. abgebrochen mit: ',
        stateLabel: 'Status: ',
        acknowledge: 'Bestätigen',
        force: 'Schritt erzwingen',
        forceHint: 'Dieser Schritt gehört einer anderen Partei. Erzwingen trägt ihn mit deinem Namen und einer Notiz dauerhaft ein.',
        decline: 'Ablehnen',
        cancel: 'Vorgang abbrechen',
        note: 'Notiz',
        notePlaceholder: 'z. B. per Post verschickt',
        pickItem: 'Ersatzteil',
        pickItemPlaceholder: 'Teil auswählen',
        pickItemNotAllowed: 'Welches Teil es wird, trägt die Wache ein. Du siehst es, sobald es eingetragen ist.',
        newSize: 'Gewünschte Größe',
        selectNewSize: 'Größe auswählen',
        noSize: 'Keine Größe',
        recordArrival: 'Neues Teil erfassen',
        recordArrivalHint: 'Der Träger führt sein Lager nicht hier, also hält die Wache fest, was angekommen ist. Eigentümer und Inventar stehen fest.',
        recordOwnHint: 'Das Teil ist neu und steht noch nicht im Lager? Leg es hier an. Eigentümer und Inventar stehen fest.',
        arrivalName: 'Bezeichnung',
        arrivalInternalId: 'Kennzeichnung',
        arrivalSize: 'Größe',
        arrivalNoSize: 'Ohne Größe',
        mine: 'Meine Bewegungen',
        mineHint: 'Sobald ein Teil bei dir ist, bestätige den Erhalt. Erst dann ist der Ablauf abgeschlossen.',
        confirmReceipt: 'Erhalt bestätigen',
        callOff: 'Zurücknehmen',
        callOffReason: 'Vom Mitglied zurückgenommen',
        waitingFor: 'Wartet auf: {party}',
        openDetail: 'Ablauf ansehen',
        ownedBy: 'Eigentümer: {owner}',
        owner: {
            station: 'Wache',
            above: 'Träger',
        },
        place: {
            WITH_OWNER: 'Träger',
            STORE: 'Lager',
            AT_STATION: 'Wache',
            WITH_MEMBER: 'Mitglied',
            WITH_PARTNER: 'Verliehen',
            IN_TRANSIT: 'Unterwegs',
            LOST: 'Verloren',
        },
        outgoing: 'Geht zurück',
        incoming: 'Kommt dafür',
        rechain: {
            action: 'Ablauf wechseln',
            title: 'Auf den richtigen Ablauf umstellen',
            hint: 'Diese Bewegung läuft auf einem Ablauf, der nicht zu dem Teil gehört. Beim Umstellen '
                + 'läuft sie auf dem richtigen Ablauf weiter, ohne dass jemand Schritte erzwingen oder '
                + 'die Bewegung abbrechen muss.',
            currentFlow: 'Läuft jetzt auf',
            standingOn: 'Steht auf Schritt',
            targetFlow: 'Gehört auf',
            targetStep: 'Steht danach auf',
            chooseStep: 'Schritt wählen',
            noFlow: 'Ohne Ablauf',
            noStep: 'Ohne Schritt',
            alreadyRight: 'Diese Bewegung läuft bereits auf dem richtigen Ablauf. Hier ist nichts zu tun.',
            confirm: 'Umstellen',
        },
        purpose: {
            ISSUE: 'Ausgabe',
            RETURN: 'Rückgabe',
            EXCHANGE: 'Tausch',
            REQUEST: 'Anfrage',
        },
        state: {
            OPEN: 'Offen',
            DONE: 'Abgeschlossen',
            DECLINED: 'Abgelehnt',
            CANCELLED: 'Abgebrochen',
        },
        actor: {
            MEMBER: 'Mitglied',
            STATION: 'Wache',
            OWNER: 'Träger',
        },
        ack: {
            CONFIRMED: 'bestätigt',
            ASSERTED: 'von der Wache vermerkt',
            FORCED: 'erzwungen',
            CORRECTED: 'von Hand richtiggestellt',
        },
        subject: {
            OUTGOING: 'Abgehendes Teil',
            INCOMING: 'Ankommendes Teil',
        },
        queue: {
            title: 'Vorgänge',
            subtitle: 'Was gerade unterwegs ist, und wer als Nächstes dran ist.',
            empty: 'Hier ist gerade nichts unterwegs.',
            noMatch: 'Kein Vorgang passt zu dieser Auswahl.',
            create: 'Vorgang starten',
            export: 'Als PDF',
            download: 'Herunterladen',
            downloadCount: '{count} herunterladen',
            pickForExport: 'Mitnehmen',
            exportFieldsHint: 'Zusätzliche Felder für den Export auswählen:',
            selectAll: 'Alle auswählen',
            selectNone: 'Auswahl aufheben',
            columns: {
                turn: 'Dran',
                what: 'Worum es geht',
                member: 'Mitglied',
                purpose: 'Art',
                step: 'Schritt',
                since: 'Seit',
                created: 'Erstellt am',
                modified: 'Zuletzt bewegt',
                actions: '',
            },
            filter: {
                search: 'Nach Name, Teil oder Kennzeichnung suchen',
            },
            forTheStore: 'Für das Lager',
            acknowledge: 'Schritt bestätigen',
            correct: 'Richtigstellen',
            openDetail: 'Ablauf ansehen',
            correctPanel: {
                title: 'Vorgang richtigstellen',
                hint: 'Ein Vorgang hat keinen Status, der sich setzen lässt: wo er steht, wird an den Teilen '
                    + 'abgelesen. Sag also, wo die Teile sind, und der Ablauf folgt.',
                outgoing: 'Abgehendes Teil ist',
                incoming: 'Ankommendes Teil ist',
                leave: 'Unverändert lassen',
                detach: 'Ersatzteil wieder lösen',
                closeAs: 'Vorgang abschließen als',
                stayOpen: 'Offen lassen',
                reason: 'Grund',
                reasonPlaceholder: 'z. B. Rückgabe wurde nie eingetragen',
                confirm: 'Richtigstellen',
            },
        },
        wizard: {
            title: 'Vorgang starten',
            step: 'Schritt {current} von {total}',
            purpose: {
                title: 'Was soll passieren?',
                ISSUE: 'Etwas ausgeben',
                ISSUEHint: 'Ein Teil geht an ein Mitglied oder ins Lager.',
                RETURN: 'Etwas zurückgeben',
                RETURNHint: 'Ein Teil geht zurück an die Wache oder an den Träger.',
                EXCHANGE: 'Etwas tauschen',
                EXCHANGEHint: 'Ein Teil geht zurück und ein Ersatz kommt dafür.',
                REQUEST: 'Etwas anfragen',
                REQUESTHint: 'Die Wache bittet den Träger um ein Teil, das sie nicht hat.',
            },
            party: {
                title: 'Mit wem?',
                member: 'Mit einem Mitglied',
                store: 'Für das Lager der Wache',
                pickMember: 'Mitglied wählen',
            },
            subject: {
                titleItem: 'Um welches Teil geht es?',
                titleInventory: 'Um was geht es?',
                inventory: 'Inventar',
                size: 'Größe',
                anySize: 'Ohne bestimmte Größe',
                item: 'Teil',
                alreadyPicked: 'Teil ist schon ausgesucht',
            },
            reason: {
                title: 'Warum?',
                label: 'Grund',
                placeholder: 'z. B. zu klein geworden',
                newSize: 'Gewünschte Größe',
            },
            preview: {
                title: 'Das passiert danach',
                hint: 'Mit dem Start ist der erste Schritt bestätigt. Die weiteren Schritte bestätigt, wer dran ist.',
                noFlow: 'Für diese Kombination ist kein Ablauf hinterlegt. Lege einen unter Einstellungen → '
                    + 'Abläufe an, dann lässt sich der Vorgang starten.',
                toFlows: 'Zu den Abläufen',
                start: 'Vorgang starten',
            },
            next: 'Weiter',
            back: 'Zurück',
        },
    },
    itemDetail: {
        ownedElsewhereTitle: 'Gehört dem Verband',
        ownedElsewhereHint: 'Beschreibung und Größe legt der Verband fest. Ihr könnt es zurückgeben oder um ein anderes bitten.',
        handBack: 'Zurückgeben',
        askExchange: 'Tausch anfragen',
        movementStarted: 'Die Bewegung wurde angestoßen.',
        reportLossTitle: 'Verlust melden',
        reportLossHint: 'Der Verband erfährt vom Verlust erst, wenn ihr Ersatz anfragt. Der Gegenstand bleibt vermisst, ob Ersatz kommt oder nicht.',
        reportLoss: 'Verlust melden und Ersatz anfragen',
        reportLossNote: 'Notiz an den Verband',
        reportLossNotePlaceholder: 'Was ist passiert, und was braucht ihr?',
        reportLossDocument: 'Beleg',
        title: 'Gegenstand',
        checkHistory: {
            title: 'Prüfungsverlauf',
            empty: 'Dieser Gegenstand wurde noch nie geprüft.',
            byChecker: 'Geprüft von {name}',
            reportedBy: 'Angegeben von {name}',
            inContainer: 'in {container}',
            result: {
                CONFIRMED: 'Bestätigt',
                NOT_IN_POSSESSION: 'Fehlte',
                LOST: 'Verloren',
            },
        },
        name: 'Bezeichnung',
        internalId: 'Interne Nr.',
        size: 'Größe',
        owner: 'Eigentümer',
        ownerStation: 'Wache',
        ownerCluster: 'Träger',
        ownerClusterOffline: 'Träger (nicht in Ember)',
        custody: 'Im Besitz von',
        custodyValues: {
            WITH_OWNER: 'Beim Eigentümer',
            AT_STATION: 'Bei der Wache',
            WITH_MEMBER: 'Bei einem Mitglied',
            WITH_PARTNER: 'Verliehen',
            IN_TRANSIT: 'Unterwegs',
            LOST: 'Verloren',
        },
        status: 'Status',
        active: 'Aktiv',
        statusAssigned: 'Zugewiesen',
        available: 'Verfügbar',
        assignedTo: 'Zugewiesen an',
        location: 'Lagerort',
        edit: 'Bearbeiten',
        actions: 'Aktionen',
        assign: 'Zuweisen',
        unassign: 'Zuweisung aufheben',
        assigned: 'Gegenstand zugewiesen.',
        unassigned: 'Zuweisung aufgehoben.',
        assignTitle: 'Gegenstand zuweisen',
        selectMember: 'Mitglied auswählen...',
        markLost: 'Als verloren melden',
        markFound: 'Als gefunden melden',
        markedLost: 'Gegenstand als verloren markiert.',
        markedFound: 'Gegenstand als gefunden markiert.',
        history: 'Zuweisungsverlauf',
        noHistory: 'Keine Zuweisungen.',
        member: 'Mitglied',
        givenOut: 'Ausgegeben',
        returned: 'Zurückgegeben',
        corrected: 'Korrektur',
        current: 'aktuell',
    },
    checklist: {
        title: 'Checklisten',
        empty: 'Noch keine Checklisten angelegt.',
        create: 'Neue Checkliste',
        createTitle: 'Checkliste erstellen',
        name: 'Name',
        namePlaceholder: 'z. B. Sicherheitsbriefing 2026',
        description: 'Beschreibung',
        descriptionPlaceholder: 'Optionale Notiz für Verwalter.',
        columns: 'Spalten',
        addColumn: 'Spalte hinzufügen',
        columnLabel: 'Spaltentitel',
        columnLabelPlaceholder: 'z. B. Unterschrift erhalten',
        columnDescription: 'Spaltenbeschreibung',
        columnDescriptionPlaceholder: 'Optionaler Hinweis für Verwalter.',
        removeColumn: 'Spalte entfernen',
        memberSet: 'Mitglieder',
        followsFilter: 'Nach Merkmalen',
        followsFilterHelp: 'Rollen, Gruppen, Merkmale und einzelne Personen. Beim Anlegen wird daraus einmal eine Zeile je passender Person.',
        followsEvent: 'Nach einem Termin',
        followsEventHelp: 'Die Liste hält sich an einen einzelnen Termin. Auf sie kommt, wer für diesen Termin einen Platz hat.',
        followsEventGuests: 'Zusagen von Partnerwachen bleiben außen vor. Sie sind hier keine Mitglieder und können deshalb nicht auf die Liste.',
        followsEventRefresh: 'Auffrischen holt alle nach, die inzwischen zugesagt haben. Das passiert nicht von selbst, sondern erst wenn jemand den Knopf drückt.',
        followsEventHeader: 'Folgt dem Termin „{event}" am {date}.',
        occurrenceOn: 'am {date}',
        occurrencePlaceholder: 'Termin mit Anmeldung suchen…',
        occurrenceEmpty: 'Keine anstehenden Termine mit Anmeldung gefunden.',
        membership: 'Mitglieder bestimmen',
        membershipTitle: 'Mitglieder dieser Liste',
        membershipHelp: 'Hier legst du fest, woraus sich die Liste speist. Vorhandene Zeilen bleiben stehen; wer neu dazugehört, kommt erst beim Auffrischen dazu.',
        membershipSaved: 'Gespeichert. Auffrischen holt jetzt die passenden Personen.',
        cancel: 'Abbrechen',
        save: 'Speichern',
        savingError: 'Speichern fehlgeschlagen.',
        checkedCountUnknown: 'Wie viele Haken in dieser Spalte stehen, konnte nicht ermittelt werden. Beim Löschen gehen sie trotzdem verloren.',
        cellSaveFailed: 'Der Haken konnte nicht gespeichert werden.',
        noteHistoryFailed: 'Der Verlauf dieser Notiz konnte nicht geladen werden.',
        memberCount: '{count} Mitglieder',
        columnCount: '{count} Spalten',
        lastRefreshed: 'Zuletzt aktualisiert: {when}',
        neverRefreshed: 'Noch nicht aktualisiert',
        refresh: 'Auffrischen',
        refreshDone: '{added} neue Mitglieder hinzugefügt, {already} bereits vorhanden.',
        addMembers: 'Mitglieder hinzufügen',
        addMembersTitle: 'Mitglieder hinzufügen',
        addMembersIntro: 'Wähle Mitglieder, die manuell auf diese Liste sollen. Zuvor entfernte Mitglieder werden wiederhergestellt.',
        previouslyRemoved: 'Zuvor entfernt',
        addedToast: '{added} hinzugefügt, {restored} wiederhergestellt, {skipped} bereits dabei.',
        showRemoved: 'Entfernte anzeigen',
        notInFilter: 'Nicht im Filter',
        filterAny: 'Alle',
        filterChecked: 'Abgehakt',
        filterUnchecked: 'Offen',
        searchPlaceholder: 'Mitglied suchen…',
        bulkTickShown: 'Sichtbare abhaken',
        bulkClearShown: 'Sichtbare leeren',
        bulkConfirmTickTitle: 'Sichtbare Mitglieder abhaken',
        bulkConfirmClearTitle: 'Sichtbare Mitglieder leeren',
        bulkConfirmMessage: 'Diese Aktion betrifft {count} Mitglied(er) in dieser Spalte. Hinweise bleiben erhalten.',
        bulkDone: '{count} Zellen aktualisiert.',
        deleteRow: 'Mitglied von Liste entfernen',
        deleteRowMessage: 'Soll {name} von dieser Liste entfernt werden? Der Auffrischer fügt sie nicht erneut hinzu.',
        deleteChecklist: 'Checkliste löschen',
        deleteChecklistMessage: 'Soll die Checkliste „{name}" mit allen Spalten und Einträgen gelöscht werden? Diese Aktion kann nicht rückgängig gemacht werden.',
        deleteColumnTitle: 'Spalte löschen',
        deleteColumnMessage: 'Die Spalte „{name}" und {count} bereits abgehakte Zelle(n) werden entfernt. Trotzdem fortfahren?',
        editColumn: 'Spalte bearbeiten',
        editChecklist: 'Checkliste bearbeiten',
        columnOrder: 'Spaltenreihenfolge',
        columnOrderHint: 'Mit den Pfeilen sortieren, am Rechner auch durch Ziehen am Griff.',
        columnPosition: 'Position',
        notePlaceholder: 'Notiz zu dieser Zelle…',
        noteSave: 'Notiz speichern',
        noteHistory: 'Verlauf',
        noteHistoryEmpty: 'Noch keine Notizänderungen.',
        noteHistoryEntry: 'Geändert von {who} am {when}',
        noteHistoryDeletedActor: 'Unbekannt',
        noteCleared: '(Notiz entfernt)',
        noteAdded: '(Notiz hinzugefügt)',
        export: 'Export',
        exportCsv: 'Als CSV laden',
        exportPdf: 'Als PDF laden',
        matrixEmpty: 'Diese Liste hat noch keine Mitglieder. Frische sie auf oder füge Mitglieder von Hand hinzu.',
        rowFilteredOut: 'Keine Mitglieder entsprechen den aktiven Filtern.',
        columnsEmpty: 'Diese Liste hat noch keine Spalten.',
        addFirstColumn: 'Erste Spalte hinzufügen',
        backToList: 'Zur Übersicht',
        frozenSetHint: 'Diese Liste wurde aus einer festen Personenauswahl angelegt. Auffrischen prüft dieselbe Auswahl noch einmal und holt niemanden nach, der später dazugekommen ist.',
    },
    lostAndFound: {
        title: 'Fundbüro',
        empty: 'Keine Fundsachen vorhanden.',
        create: 'Fundgegenstand melden',
        createTitle: 'Neuen Fundgegenstand melden',
        description: 'Beschreibung',
        descriptionPlaceholder: 'Was wurde gefunden?',
        foundAt: 'Funddatum',
        noDescription: 'Keine Beschreibung',
        claim: 'Gehört mir',
        claimedBy: 'Beansprucht von {name}',
        claimedByYou: 'Von dir beansprucht',
        provided: 'Ausgegeben',
        release: 'Anspruch zurücknehmen',
        claimConfirmTitle: 'Gegenstand beanspruchen',
        claimConfirmMessage: 'Möchtest du diesen Gegenstand als deinen markieren? Die Verwaltung wird benachrichtigt.',
        claimFor: 'Für wen?',
        claimForMyself: 'Für mich selbst',
        providedConfirmTitle: 'Als ausgegeben markieren',
        providedConfirmMessage: 'Wurde der Gegenstand an die Person übergeben? Der Eintrag wird danach entfernt.',
        releaseConfirmTitle: 'Anspruch zurücknehmen',
        releaseConfirmMessage: 'Der Gegenstand steht danach wieder für alle zur Abholung bereit.',
        image: 'Bild',
        uploadImage: 'Bild hochladen',
        takePhoto: 'Foto aufnehmen',
        addImage: 'Bild nachreichen',
        retryImage: 'Bild erneut senden',
        keepWithoutImage: 'Ohne Bild behalten',
        savedWithoutImage: 'Der Eintrag ist gespeichert. Es fehlt nur noch das Bild.',
        imageUnavailable: 'Bild nicht verfügbar',
        imageUnreadable: 'Dieses Bild kann auf diesem Gerät nicht gelesen werden. Bitte ein JPEG oder PNG wählen.',
        imageUnreadableGuidance: 'Das liegt an der Datei und nicht an Ember. Öffne sie einmal in der Foto-App deines Geräts und speichere sie neu, dann klappt es meistens.',
    },
    demo: {
        title: 'Ember Demo',
        banner: 'Dies ist eine Demo-Instanz. Alle Daten werden regelmäßig zurückgesetzt. Passwort für alle Konten: demo',
        loginHint: 'Wähle ein Konto aus, um dich direkt als Demo-Nutzer anzumelden.',
        devLoginHint: 'Schnellanmeldung mit Testkonten',
        clusterLoginHint: 'Konten, die für einen Verband handeln',
        searchPlaceholder: 'Name, Adresse, Rolle, Gruppe oder Merkmal suchen…',
        searchEmpty: 'Keine Konten gefunden.',
    },
    inventoryMembers: {
        title: 'Mitglieder & Inventar',
        role: 'Rolle',
        allRoles: 'Alle Rollen',
        showEmpty: 'Ohne Inventar anzeigen',
        displayOptions: 'Angezeigte Informationen',
        optName: 'Name',
        optInternalId: 'Interne ID',
        optSize: 'Größe',
        empty: 'Keine Mitglieder gefunden.',
        export: 'Exportieren',
        exportFieldsHint: 'Zusätzliche Felder für den Export auswählen:',
        lost: 'Verloren',
        memberCount: 'Mitglieder',
    },
    procurement: {
        title: 'Beschaffung',
        create: 'Neue Beschaffung',
        createTitle: 'Beschaffung erstellen',
        empty: 'Keine Beschaffungsanfragen vorhanden.',
        inventory: 'Ausstattung',
        selectInventory: 'Ausstattung auswählen',
        member: 'Mitglied',
        selectMember: 'Mitglied auswählen',
        size: 'Größe',
        noSize: 'Keine Größe',
        notes: 'Notizen',
        notesPlaceholder: 'Optionale Notizen',
        submit: 'Beschaffung erstellen',
        fulfilled: 'Erledigt',
        open: 'Offen',
        fulfilledAt: 'Erledigt am',
        markFulfilled: 'Erledigt',
    },
    forms: {
        create: 'Umfrage erstellen',
        edit: 'Bearbeiten',
        delete: 'Löschen',
        publish: 'Veröffentlichen',
        close: 'Schließen',
        reopen: 'Wieder öffnen',
        clearResponses: 'Antworten zurücksetzen',
        settingsSaved: 'Einstellungen werden automatisch gespeichert. Nur die Fragen brauchen den Speichern-Knopf.',
        settingsSaving: 'Einstellungen werden gespeichert ...',
        settingsSaveFailed: 'Die Einstellungen konnten nicht gespeichert werden. Prüfe die Verbindung und ändere sie noch einmal.',
        responseCount: '{count} Antworten',
        actions: 'Aktionen',
        createdOn: 'Erstellt am {when}',
        lastActivityOn: 'Letzte Aktivität am {when}',
        closedOn: 'Abgelaufen am {when}',
        sortBy: 'Sortieren nach',
        sortOrder: 'Reihenfolge',
        sortActivity: 'Letzte Aktivität',
        sortCreated: 'Erstellungsdatum',
        sortTitle: 'Titel',
        sortDesc: 'Neueste zuerst',
        sortAsc: 'Älteste zuerst',
        title: 'Titel',
        description: 'Beschreibung',
        statusDraft: 'Entwurf',
        statusOpen: 'Offen',
        statusScheduled: 'Startet später',
        statusClosed: 'Geschlossen',
        responses: 'Antworten',
        noForms: 'Keine Umfragen vorhanden.',
        noAvailableForms: 'Keine Umfragen verfügbar.',
        startAt: 'Startdatum',
        endAt: 'Enddatum',
        shuffleQuestions: 'Fragen mischen',
        allowEdit: 'Antworten bearbeitbar',
        forced: 'Pflichtumfrage',
        stillHeldBy: 'Diese Umfrage steht noch auf folgenden Seiten: {pages}. Dort kann sie jetzt niemand mehr beantworten, bis du sie von diesen Seiten nimmst.',
        visibilityFailed: 'Die Sichtbarkeit dieser Umfrage konnte nicht geändert werden.',
        priorAnswerUnknown: 'Ob du diese Umfrage schon beantwortet hast, konnte nicht geprüft werden. Deine frühere Antwort steht möglicherweise noch, wird hier aber nicht angezeigt.',
        fillForm: 'Ausfüllen',
        fillForWhom: 'Für wen möchtest du die Umfrage ausfüllen?',
        fillForSelf: 'Für mich ({name})',
        fillForSelfDefault: 'mich selbst',
        otherPlaceholder: 'Eigene Antwort eingeben...',
        fillForMemberHint: 'Du füllst diese Umfrage für ein verwaltetes Mitglied aus.',
        fillForSingleManaged: 'Diese Umfrage wird für {name} ausgefüllt.',
        viewAnalytics: 'Auswertung',
        share: 'Zum Versenden verlinken',
        unlistedBadge: 'Nur per Link',
        openlyAddressed: 'Öffentlich erreichbar',
        openlyAddressedHint: 'Das Formular hat eine eigene Adresse, unter der es jeder öffnen kann. Das braucht es, wenn es auf einer öffentlichen Seite eingebunden ist.',
        unlistedHint: 'Das Formular ist nur über den Link erreichbar, mit dem du es verschickst. Ein neuer Link beendet jeden Weg hinein, der bisher vergeben wurde. Eingebunden auf einer öffentlichen Seite funktioniert es so nicht.',
        editResponse: 'Antwort bearbeiten',
        submit: 'Absenden',
        update: 'Aktualisieren',
        confirmPublish: 'Möchtest du die Umfrage veröffentlichen? Alle berechtigten Mitglieder werden benachrichtigt.',
        confirmClose: 'Möchtest du die Umfrage schließen? Es können keine weiteren Antworten abgegeben werden.',
        confirmReopen: 'Möchtest du die Umfrage wieder öffnen? Es können wieder Antworten abgegeben werden.',
        confirmClearResponses: 'Möchtest du alle {count} Antworten löschen? Die Umfrage selbst bleibt bestehen und kann erneut beantwortet werden. Diese Aktion kann nicht rückgängig gemacht werden.',
        confirmDelete: 'Möchtest du die Umfrage löschen? Diese Aktion kann nicht rückgängig gemacht werden.',
        confirm: 'Bestätigen',
        questionTitle: 'Fragetext',
        questionDescription: 'Beschreibung (optional)',
        questionRequired: 'Pflichtfeld',
        questionShuffle: 'Optionen mischen',
        questionTypes: {
            CHOICE: 'Auswahl',
            TEXT: 'Text',
            RATING: 'Bewertung',
            DATE: 'Datum',
            RANKING: 'Rangfolge',
            LIKERT: 'Likert-Skala',
        },
        choice: {
            multiSelect: 'Mehrfachauswahl',
            dropdown: 'Dropdown',
            allowOther: 'Sonstige Option',
            options: 'Optionen',
            addOption: 'Option hinzufügen',
            limitNone: 'Keine',
            limitEqual: 'Genau',
            limitAtMost: 'Höchstens',
            limitAtLeast: 'Mindestens',
            limitValue: 'Anzahl',
        },
        text: {
            longAnswer: 'Langantwort',
        },
        rating: {
            scale: 'Skala',
            iconStar: 'Stern',
            iconNumber: 'Zahl',
            iconHeart: 'Herz',
            iconThumbUp: 'Daumen hoch',
        },
        ranking: {
            options: 'Elemente',
            addOption: 'Element hinzufügen',
        },
        likert: {
            statements: 'Aussagen',
            addStatement: 'Aussage hinzufügen',
            scaleMin: 'Skala Min',
            scaleMax: 'Skala Max',
        },
        restrictions: {
            title: 'Einschränkungen',
            roles: 'Rollen',
            groups: 'Gruppen',
            tags: 'Tags',
            noRestrictions: 'Für alle sichtbar',
        },
        contactSubmissions: {
            title: 'Kontaktanfragen',
            empty: 'Noch keine Anfragen eingegangen.',
            acknowledge: 'Bestätigen',
            acknowledged: 'Bestätigt',
            acknowledgedBy: 'Bestätigt von',
            notAcknowledged: 'Noch nicht bestätigt',
        },
        analytics: {
            responseFailed: 'Diese Antwort konnte nicht geöffnet werden.',
            totalResponses: 'Antworten gesamt',
            noResponses: 'Noch keine Antworten.',
            noAnswersYet: 'Auf diese Frage hat noch niemand geantwortet.',
            otherAnswer: 'Sonstiges',
            tabCharts: 'Diagramme',
            tabIndividual: 'Einzelantworten',
            responseOf: '{current} von {total}',
            export: 'Exportieren',
            exportQuestions: 'Fragen auswählen',
            exportFields: 'Profilfelder hinzufügen',
            exportMember: 'Mitglied',
            exportCsv: 'Als CSV exportieren',
            missingTitle: 'Ausstehende Antworten',
            missingHelp: 'Diese Mitglieder können die Umfrage ausfüllen, haben es aber noch nicht getan.',
            missingCount: '{count} ausstehend',
            grouping: {
                title: 'Auswerten nach Mitgliedern',
                filter: 'Filtern',
                groupBy: 'Gruppieren nach',
                noGrouping: 'Nicht gruppieren',
                byUserType: 'Mitgliedsart',
                byGroup: 'Gruppe',
                byTag: 'Tag',
                byAge: 'Alter',
                only: 'Nur diese vergleichen',
                onlyPlaceholder: 'Alle zeigen',
                bounds: 'Grenzen',
                boundsHint: 'Wo eine neue Gruppe beginnt, zum Beispiel 14, 18, 27',
                userTypes: 'Mitgliedsarten',
                groups: 'Gruppen',
                tags: 'Tags',
                matchAny: 'eine davon',
                matchAll: 'alle davon',
                age: 'Alter',
                from: 'von',
                to: 'bis',
                addField: 'Profilfeld hinzufügen',
                removeField: 'Bedingung entfernen',
                choose: 'Auswählen',
                yes: 'Ja',
                no: 'Nein',
                reset: 'Zurücksetzen',
                matching: '{count} Antworten entsprechen der Auswahl',
                overlap: 'Ein Mitglied kann in mehreren Gruppen sein und zählt dann in jeder. Die Gruppen ergeben zusammen deshalb mehr Antworten als insgesamt.',
                tooMany: 'Zu viele Gruppen, um sie farblich zu unterscheiden. Sie werden als Tabelle gezeigt.',
                asTable: 'Als Tabelle',
                asChart: 'Als Diagramm',
                groupResponses: '{count} Antworten',
                average: 'Durchschnitt {value}',
                answers: 'Antworten',
                noAnswers: 'Keine Antworten',
                other: 'Sonstiges',
                none: {
                    USER_TYPE: 'Ohne Anmeldung',
                    GROUP: 'In keiner Gruppe',
                    TAG: 'Ohne Tag',
                    FIELD: 'Keine Angabe',
                    AGE: 'Alter unbekannt',
                },
            },
        },
    },
    boards: {
        title: 'Planer',
        labelsPlaceholder: 'Labels...',
        noLabelsFound: 'Keine Labels gefunden',
        manageTitle: 'Boards verwalten',
        createBoard: 'Board erstellen',
        boardName: 'Name',
        boardDescription: 'Beschreibung',
        shortKey: 'Kürzel',
        shortKeyHint: 'Wird als Ticket-Präfix verwendet (z.B. DEV)',
        preset: 'Vorlage',
        kbSearchFailed: 'Das Wiki konnte gerade nicht durchsucht werden.',
        searchFailed: 'Die Suche hat nicht funktioniert. Es heißt nicht, dass es den Vorgang nicht gibt.',
        labelCreateFailed: 'Das Label konnte nicht angelegt werden.',
        federationConfigUnknown: 'Mit welchen Partnerwachen dieses Board geteilt ist, konnte nicht geladen werden. Die Angaben unten sind unvollständig.',
        ticketCreatedPartly:'Der Vorgang {ticket} wurde angelegt, aber nicht alles daran konnte gespeichert werden. Drücke noch einmal auf Erstellen: der Vorgang wird dann nicht doppelt angelegt, sondern fertig ausgefüllt.',
        presetSimple:'Einfach (Offen, In Arbeit, Erledigt)',
        presetFeedback: 'Mit Feedback (Offen, In Arbeit, Feedback, Erledigt)',
        presetNone: 'Keine Vorlage',
        deleteBoard: 'Board löschen',
        deleteBoardConfirm: 'Soll dieses Board wirklich gelöscht werden? Alle Tickets gehen verloren.',
        noBoards: 'Noch keine Boards vorhanden.',
        settings: 'Einstellungen',
        lanes: 'Spalten',
        addLane: 'Spalte hinzufügen',
        hideDoneAfterDays: 'Erledigte Tickets ausblenden nach (Tagen)',
        createTicket: 'Ticket erstellen',
        ticketKey: 'ID',
        ticketTitle: 'Titel',
        ticketDescription: 'Beschreibung',
        priority: 'Priorität',
        priorityLowest: 'Sehr niedrig',
        priorityLow: 'Niedrig',
        priorityMedium: 'Mittel',
        priorityHigh: 'Hoch',
        priorityHighest: 'Sehr hoch',
        assignee: 'Zugewiesen',
        unassigned: 'Nicht zugewiesen',
        dueDate: 'Fällig am',
        checklist: 'Checkliste',
        addChecklistItem: 'Punkt hinzufügen',
        links: 'Verknüpfungen',
        linkedTickets: 'Verknüpfte Tickets',
        addLink: 'Ticket verknüpfen',
        linkRelatesTo: 'Bezieht sich auf',
        linkBlocks: 'Blockiert',
        linkBlockedBy: 'Blockiert von',
        linkCauses: 'Verursacht',
        linkCausedBy: 'Verursacht von',
        comments: 'Kommentare',
        transitions: 'Verlauf',
        movedFrom: 'Verschoben von',
        movedTo: 'nach',
        archived: 'archiviert',
        showArchived: 'Archivierte anzeigen',
        noTickets: 'Keine Tickets in dieser Spalte.',
        filterByAssignee: 'Nach Bearbeiter filtern',
        allMembers: 'Alle',
        viewAccess: 'Lesezugriff',
        editAccess: 'Bearbeitungszugriff',
        activityAll: 'Alle',
        clickToAddDescription: 'Beschreibung hinzufügen...',
        searchTickets: 'Tickets suchen...',
        weblinks: 'Weblinks',
        addWeblink: 'Weblink hinzufügen',
        weblinkTitle: 'Titel (optional)',
        attachments: 'Anhänge',
        addAttachment: 'Datei anhängen',
        watch: 'Beobachten',
        unwatch: 'Nicht mehr beobachten',
        fields: 'Felder',
        addField: 'Feld hinzufügen',
        fieldName: 'Feldname',
        fieldType: 'Typ',
        fieldTypeString: 'Text',
        fieldTypeNumber: 'Zahl',
        fieldTypeBoolean: 'Ja/Nein',
        fieldTypeEnum: 'Auswahl',
        fieldTypeDate: 'Datum',
        fieldTypeLaneAssignee: 'Spalten-Zuweisung',
        fieldLane: 'Spalte',
        fieldRequired: 'Pflichtfeld',
        fieldOptions: 'Optionen (kommagetrennt)',
        kbLinks: 'Lernsammung',
        addKbLink: 'Wiki verknüpfen',
        searchKb: 'Seite suchen...',
        backlog: 'Backlog',
        backlogTitle: 'Backlog',
        showBacklog: 'Backlog anzeigen',
        labels: 'Labels',
        clearLabelFilter: 'Label-Filter zurücksetzen',
        moreOptions: 'Mehr Optionen',
        federation: 'Föderation',
        federationShare: 'Board teilen',
        federationShareDesc: 'Wähle die Partner, mit denen dieses Board geteilt werden soll.',
        federationNoPermission: 'Du benötigst die Berechtigung „Board föderieren", um die Föderationseinstellungen zu ändern.',
        shareMode: 'Zugriffsmodus',
        shareModeReadOnly: 'Nur Lesen',
        shareModeFull: 'Lesen & Bearbeiten',
        accessMode: 'Zugriff',
        minViewRole: 'Sichtbar ab',
        requiredRoleUser: 'Alle Mitglieder',
        requiredRoleTeam: 'Team & Verwaltung',
        requiredRoleManager: 'Nur Verwaltung',
        requiredRole: 'Mindest-Rolle',
        federatedEditRoles: 'Wer darf bearbeiten?',
        federatedEditRolesDesc: 'Welche Mitgliedstypen der Partnerwache dürfen Tickets bearbeiten? Leer = alle mit Zugriff.',
        federatedBoards: 'Föderierte Boards',
        federatedBoardsDesc: 'Boards, die von Partnerwachen mit dir geteilt werden.',
        noFederatedBoards: 'Keine föderierte Boards verfügbar.',
        sourceStation: 'Quelle',
        bookmark: 'Lesezeichen',
        bookmarked: 'Gemerkt',
        bookmarkHint: 'Boards mit Lesezeichen erscheinen direkt in der Seitenleiste.',
        readOnlyBadge: 'Nur Lesen',
        fullAccessBadge: 'Vollzugriff',
        federatedFrom: 'Föderiert von',
        accessOverride: 'Zugriffseinschränkungen',
        accessOverrideDesc: 'Einschränkungen können den Zugriff nur weiter einschränken, nicht erweitern.',
        viewOverride: 'Leseeinschränkung',
        editOverride: 'Bearbeitungseinschränkung',
        historyActions: {
            PRIORITY_CHANGED: 'Priorität',
            LABEL_ADDED: 'Label hinzugefügt',
            LABEL_REMOVED: 'Label entfernt',
            TITLE_CHANGED: 'Titel geändert',
            DESCRIPTION_CHANGED: 'Beschreibung geändert',
            DUE_DATE_CHANGED: 'Fälligkeitsdatum',
            FIELD_CHANGED: 'Feld geändert',
            LINK_ADDED: 'Link hinzugefügt',
            LINK_REMOVED: 'Link entfernt',
            ASSIGNEE_CHANGED: 'Zuweisung',
        },
    },
    requirements: {
        title: 'Noch etwas zu erledigen',
        subtitle: 'Bevor es losgeht, gibt es ein paar Dinge, die noch erledigt werden sollten. Du kannst diese auch später nachholen.',
        profileTitle: 'Profil vervollständigen',
        profileText: 'Einige Pflichtfelder in deinem Profil sind noch nicht ausgefüllt.',
        formText: 'Diese Umfrage wartet auf deine Antworten.',
        quizText: 'Dieser Test steht für dich bereit.',
        fillOut: 'Ausfüllen',
        startQuiz: 'Test starten',
        skip: 'Später erledigen',
        allDone: 'Alles erledigt! Es gibt keine offenen Aufgaben.',
        selfCheckTitle: 'Ausrüstung prüfen',
        selfCheckText: 'Die Wache möchte wissen, was du von deiner Ausrüstung noch hast.',
        answerSelfCheck: 'Beantworten',
        registrationText: 'Der Termin am {date} stellt eine neue Frage. Deine Anmeldung braucht dafür noch eine Antwort.',
        registrationTextFor: 'Der Termin am {date} stellt eine neue Frage. Die Anmeldung von {name} braucht dafür noch eine Antwort.',
        updateRegistration: 'Anmeldung aktualisieren',
    },
    selfCheck: {
        intro: 'Geh deine Ausrüstung durch und sag zu jedem Stück, wie es aussieht. Du kannst zwischendurch speichern und später weitermachen.',
        dueOn: 'Gewünscht bis {date}',
        emptyPlace: 'Noch nichts erfasst',
        borrowed: 'Von einer Partnerwache',
        recordedLost: 'Als vermisst erfasst',
        notePlaceholder: 'Anmerkung (freiwillig)',
        identifierPlaceholder: 'Nummer auf dem Stück, falls vorhanden',
        sizeUnknown: 'Größe unbekannt',
        actualSize: 'Tatsächliche Größe',
        reportLost: 'Finde ich nicht mehr',
        lossRaised: 'Als vermisst gemeldet',
        requestExchange: 'Passt nicht mehr',
        reportBroken: 'Ist kaputt',
        exchangeCause: {
            DOES_NOT_FIT: 'Passt nicht mehr',
            BROKEN: 'Ist kaputt, Ersatz nötig',
        },
        exchangeRaised: 'Tausch läuft',
        lossWaiting: 'Verlustmeldung vorgemerkt',
        exchangeWaiting: 'Tausch vorgemerkt',
        raisedAtOnce: 'Beides gilt sofort und wartet auf niemanden. Es gehört nicht zu dem, was du abgibst.',
        raisedAfterCorrection:
            'Du stellst hier die Größe richtig. Was du zu diesem Stück meldest, wird vorgemerkt und geht hinaus, sobald die Wache die Richtigstellung übernommen hat, dann mit der Größe, die du angegeben hast.',
        reportFellAway:
            'Deine Vormerkung ist mit dieser Angabe weggefallen, denn sie hing daran. Du kannst sie neu stellen.',
        save: 'Zwischenspeichern',
        saved: 'Gespeichert. Du kannst später weitermachen.',
        submit: 'Abgeben',
        stillOpen: 'Noch {count} offen',
        waitingForReview: 'Abgegeben. {count} Angabe(n) werden noch angesehen.',
        closed: 'Diese Aufgabe ist abgeschlossen.',
        cameBack: 'Kam zurück: {reason}',
        answer: {
            HAVE_IT: 'Habe ich',
            DO_NOT_HAVE_IT: 'Habe ich nicht',
            TURNED_UP: 'Ist wieder da',
            WRONG_RECORD: 'Das ist nicht das, was ich habe',
            NEVER_HAD: 'Hatte ich nie',
            HAVE_ONE: 'Ich habe eins',
        },
        state: {
            OPEN: 'Offen',
            SUBMITTED: 'Abgegeben, wartet auf Durchsicht',
            DONE: 'Erledigt',
            OVERTAKEN: 'Durch eine Prüfung vor Ort ersetzt',
        },
        panel: {
            title: 'Selbstauskünfte',
            hint: 'Bitte Mitglieder, ihre Ausrüstung selbst durchzugehen. Wer eine Aufgabe hat, sieht sie auf seiner Startseite.',
            member: 'Mitglied',
            pickMember: 'Mitglied suchen',
            noneAskable: 'Alle Mitglieder haben bereits eine Selbstauskunft.',
            dueOn: 'Gewünscht bis',
            ask: 'Anfragen',
            none: 'Zurzeit läuft keine Selbstauskunft.',
            review: 'Ansehen',
        },
        review: {
            people: 'Angegeben von {reporter}, angefragt von {handedOut}',
            statedSize: 'Angegebene Größe: {size}',
            noSizeGiven: 'Keine Größe angegeben.',
            outstanding: 'Noch {count} Angabe(n) zu entscheiden',
            answeredBy: 'Angegeben von {name}',
            take: 'Übernehmen',
            correct: 'Erfassung richtigstellen',
            refuse: 'Zurückgeben',
            taken: 'Übernommen',
            refused: 'Zurückgegeben',
            nothingSaid: 'Zu dieser Aufgabe wurde nichts angegeben.',
            refuseTitle: 'Angabe zurückgeben',
            refuseHint: 'Nur für Angaben, die sich gar nicht klären lassen. Ist nur die Erfassung veraltet, stell sie richtig und übernimm die Angabe.',
            refuseReasonPlaceholder: 'Warum lässt sich das nicht klären?',
            raisedTitle: 'Ohne Rückfrage ausgelöst',
            raisedHint: 'Diese Dinge haben sofort gewirkt und warten auf niemanden.',
            raised: {
                LOSS: '{name} hat {item} als vermisst gemeldet',
                EXCHANGE: '{name} hat für {item} einen Tausch angefragt',
            },
            waitingTitle: 'Wartet auf deine Richtigstellung',
            waitingHint:
                'Das hat noch nichts ausgelöst. Es geht hinaus, sobald du die zugehörige Angabe richtigstellst, und zwar mit der richtiggestellten Größe. Gibst du die Angabe stattdessen zurück, fällt es weg.',
            waiting: {
                LOSS: '{name} meldet {item} als vermisst, sobald die Erfassung stimmt',
                EXCHANGE: '{name} fragt für {item} einen Tausch an, sobald die Erfassung stimmt',
            },
            typed: 'Eingetippt: {typed} -',
            heldBy: 'liegt bei {name}',
            free: 'liegt im Lager',
            container: 'Behälter „{name}"',
            finding: {
                NO_MATCH: 'nichts mit dieser Nummer gefunden',
                FREE: 'ein freies Stück gefunden',
                HELD: 'ein Stück gefunden, das jemand anderes hat',
                SEVERAL: 'mehrere Treffer, die Nummer ist nicht eindeutig',
                A_CONTAINER: 'das ist die Nummer eines Behälters',
                NOTHING_TYPED: 'nichts eingetippt',
            },
            settlement: {
                CONFIRMS_PIECE: 'Wird als vorhanden vermerkt.',
                RECORDS_NOT_HELD: 'Wird als nicht im Besitz vermerkt. Für Material einer Partnerwache entsteht dabei keine Verlustmeldung.',
                MARKS_FOUND: 'Das Stück gilt wieder als vorhanden.',
                CONFIRMS_GAP: 'Die Lücke wird als bestätigt vermerkt.',
                NEEDS_RECORD_PUT_RIGHT: 'Erst richtigstellen: das Mitglied hat etwas anderes als erfasst.',
                NEEDS_A_PIECE_NAMED: 'Erst ein Stück benennen: das Mitglied hat etwas, das nirgends erfasst ist.',
                ANCHOR_GONE: 'Das Stück dazu gibt es nicht mehr.',
            },
            removal: {
                BACK_TO_STORE: 'Das bisher erfasste Stück geht zurück ins Lager der Wache.',
                RETURNED_TO_OWNER: 'Das bisher erfasste Stück geht zurück an den Verband.',
                DELETED: 'Das bisher erfasste Stück wird gelöscht: es gibt kein Lager, in das es zurück könnte.',
                NOTHING: '',
            },
        },
    },
    federationShare: {
        enable: 'Mit Partnerwachen teilen',
        scope: 'Reichweite',
        scopeAll: 'Alle Partnerwachen',
        scopeSpecific: 'Bestimmte Partnerwachen',
        partners: 'Partnerwachen',
    },
    fileUpload: {
        choose: 'Datei wählen',
        maxSize: 'Max. {size}',
        tooLarge: 'Datei zu groß. Maximal {size} erlaubt.',
        uploadFailed: 'Upload fehlgeschlagen.',
    },
    adminClusters: {
        createTitle: 'Verband anlegen',
        createHint: 'Ein neuer Verband bekommt automatisch eine eigene, verborgene Wache als Identität. Danach braucht er eine Leitung: erst wer dort eingetragen ist, kann den Verband überhaupt öffnen.',
        nameLabel: 'Name',
        namePlaceholder: 'z.B. Kreisverband Musterstadt',
        descriptionLabel: 'Beschreibung',
        descriptionPlaceholder: 'Wofür steht dieser Verband?',
        empty: 'Es gibt noch keine Verbände.',
        deleteMessage: 'Soll der Verband „{name}" wirklich entfernt werden? Das geht nur, solange ihm keine Wache mehr angehört.',
        appoint: 'Leitung eintragen',
        appointHint: 'Wer hier eingetragen wird, verwaltet den Verband von da an selbst. Alles Weitere entscheidet der Verband.',
        appointPlaceholder: 'Name oder E-Mail-Adresse',
    },
    clusterSidebar: {
        cluster: 'Verband',
        overview: 'Übersicht',
        settings: 'Einstellungen',
        stations: 'Wachen',
        stationList: 'Wachen',
        stationGroups: 'Wachgruppen',
        applications: 'Beitrittsanfragen',
        governance: 'Vorgaben',
        modules: 'Module',
        lookAndFeel: 'Erscheinungsbild',
        storage: 'Speicherplatz',
        members: 'Mitglieder',
        team: 'Verbandsteam',
        teamList: 'Personen',
        memberList: 'Alle Mitglieder',
        memberGroups: 'Gruppen',
        memberManagement: 'Mitglieder der Wachen',
        fields: 'Profilfelder',
        fieldList: 'Profilfelder',
        inventory: 'Material',
        stock: 'Bestand',
        movements: 'Offene Schritte',
        knowledge: 'Wiki',
        news: 'Neuigkeiten',
        events: 'Termine',
    },
    clusterInventory: {
        tabStock: 'Lager',
        tabOut: 'Bei den Wachen',
        tabRequirements: 'Vorgaben',
        tabMovements: 'Bewegungen',
        tabStatistics: 'Zahlen',
        tabSettings: 'Einstellungen',
        outEmpty: 'Es ist nichts bei einer Wache.',
        inTransit: 'Unterwegs',
        itemCount: '{count} Stück',
        flowsTitle: 'Abläufe',
        flowsHint: 'Ein Ablauf ist die Reihe von Schritten, die jemand bestätigt, und wo das Material danach liegt. Pro Zweck gilt genau ein Ablauf: einen zweiten legst du erst an, wenn der alte stillgelegt ist.',
        flowsEmpty: 'Noch keine Abläufe angelegt.',
        flowNameLabel: 'Name',
        flowNamePlaceholder: 'z. B. Ausgabe an die Wache',
        flowPurposeLabel: 'Zweck',
        dispatch: {
            empty: 'Im Lager des Verbands liegt gerade nichts, was ihr weggeben könntet.',
            stationTitle: 'Wohin',
            stationHint: 'Alles, was ihr auswählt, geht in einer Sendung an diese Wache. Sie bestätigt einmal den Eingang, nicht jedes Teil einzeln.',
            stationLabel: 'Wache',
            stationPlaceholder: 'Wache auswählen',
            itemsTitle: 'Was',
            takeAll: 'Alle',
            reasonLabel: 'Notiz an die Wache',
            reasonPlaceholder: 'z. B. Ausstattung für die neue Gruppe',
            send: 'Sendung auf den Weg bringen ({count})',
        },
        lossReportTitle: 'Verlustmeldungen',
        lossReportHint: 'Was eine Wache mitschicken muss, wenn sie euch ein verlorenes Teil meldet und Ersatz anfragt. Über den Verlust selbst entscheidet ihr nicht, nur über den Ersatz.',
        lossReportRequires: {
            NOTHING: 'Nichts weiter',
            NOTE: 'Eine Notiz',
            DOCUMENT: 'Notiz und Beleg',
        },
        usesTitle: 'Material im Verband führen',
        usesHint: 'Ist das aus, verhalten sich alle Wachen so, als gäbe es keinen Verband über ihnen, wenn es um Material geht: sie tauschen und geben zurück in ihren eigenen Abläufen, ohne Schritte, die auf euch warten.',
        tags: {
            title: 'Empfohlene Tags',
            intro: 'Wörter, die ihr euren Abteilungen für ihr Material vorschlagt. Sie erscheinen dort '
                + 'neben den eigenen Wörtern der Abteilung.',
            standsBeside: 'Eine Empfehlung ersetzt nie das Wort einer Abteilung. Beide bleiben stehen und '
                + 'meinen dasselbe, denn verglichen wird ohne Rücksicht auf Groß- und Kleinschreibung. '
                + 'Zurückgezogen wird nur der Vorschlag, nichts bei den Abteilungen.',
            add: 'Wort empfehlen',
            edit: 'Empfehlung bearbeiten',
            name: 'Wort',
            namePlaceholder: 'z. B. Funk',
            color: 'Farbe',
            forStations: 'Für welche Abteilungen',
            allStations: 'Alle Abteilungen',
            empty: 'Ihr empfehlt noch keine Wörter.',
            deleteConfirm: 'Empfehlung „{name}“ zurückziehen? Bei den Abteilungen ändert sich nichts.',
        },
        empty: 'Dem Verband gehört noch kein Material.',
        custody: {
            WITH_OWNER: 'Im Lager',
            AT_STATION: 'Wache',
            WITH_MEMBER: 'Mitglied',
            WITH_PARTNER: 'Verliehen',
            IN_TRANSIT: 'Unterwegs',
            LOST: 'Verloren',
        },
        stationFilter: 'Wache',
        stationFilterAll: 'Alle Wachen',
        outNoMatch: 'Nichts passt zu den gewählten Filtern.',
        outColumn: {
            name: 'Bezeichnung',
            internalId: 'Kennung',
            size: 'Größe',
            holder: 'Getragen von',
            custody: 'Verbleib',
        },
        statsScope: 'Gezählt wird nur, was dem Verband gehört. Was eine Wache selbst gekauft hat, '
            + 'gehört ihr und steht hier nicht.',
    },
    clusterMovements: {
        hint: 'Hier stehen die Vorgänge, die auf einen Schritt warten, den nur der Verband bestätigen kann.',
        empty: 'Es wartet nichts auf euch.',
        unnamedStep: 'Schritt',
        station: 'Wache',
        purpose: {
            EXCHANGE: 'Tausch',
            RETURN: 'Rückgabe',
            ISSUE: 'Ausgabe',
            REQUEST: 'Anfrage',
        },
    },
    clusterFields: {
        hint: 'Diese Felder erscheinen im Profil jedes Mitglieds an allen Wachen des Verbands, neben den Feldern der Wache selbst.',
        add: 'Feld hinzufügen',
        addTitle: 'Neues Feld',
        editTitle: 'Feld bearbeiten',
        empty: 'Der Verband fragt noch nichts ab.',
        nameLabel: 'Bezeichnung',
        namePlaceholder: 'z.B. Führerscheinklasse',
        typeLabel: 'Typ',
        scopeLabel: 'Gilt für',
        positionLabel: 'Position',
        stationReadonlyLabel: 'Die Wache darf den Wert nur lesen',
        requiredLabel: 'Pflichtfeld',
        notifyLabel: 'Bei Änderung benachrichtigen',
        keepOnArchiveLabel: 'Wert bei ehemaligen Mitgliedern behalten',
        readonlyBadge: 'Nur lesbar',
        types: {
            TEXT: 'Text',
            NUMBER: 'Zahl',
            DATE: 'Datum',
            BOOLEAN: 'Ja/Nein',
            ENUM: 'Auswahl',
            AGE: 'Alter',
        },
        scopes: {
            MEMBER: 'Mitglieder',
            GUARDIAN: 'Erziehungsberechtigte',
            TEAM: 'Teammitglieder',
            MANAGER: 'Manager',
        },
    },
    clusterMembers: {
        rosterTitle: 'Verbandsteam',
        add: 'Aufnehmen',
        selectHint: 'Wähle links eine Person aus, um ihre Rechte zu sehen.',
        addTitle: 'Mitglied aufnehmen',
        addHint: 'Die Person handelt danach für den Verband, ist aber weiterhin in keiner Wache Mitglied.',
        emailLabel: 'E-Mail-Adresse',
        emailPlaceholder: "person{'@'}beispiel.de",
        newAccountHint: 'Zu dieser Adresse gibt es noch kein Konto. Mit Vor- und Nachnamen wird eines angelegt und eine Einladung zum Passwort setzen verschickt.',
        firstNameLabel: 'Vorname',
        lastNameLabel: 'Nachname',
        userTypeLabel: 'Rolle',
        empty: 'Für diesen Verband handelt noch niemand.',
        permissionsLabel: 'Eigene Rechte',
        permissionsHint: 'Rechte, die diese Person persönlich bekommt, zusätzlich zu denen aus ihren Gruppen.',
        groupsLabel: 'Gruppen',
        noGroups: 'Es gibt noch keine Gruppen.',
        effectiveLabel: 'Ergibt zusammen',
        effectiveNone: 'Diese Person darf im Verband bisher nichts weiter.',
    },
    clusterMemberGroups: {
        createTitle: 'Gruppe anlegen',
        createHint: 'Eine Gruppe trägt Rechte, die alle ihre Mitglieder bekommen.',
        namePlaceholder: 'z.B. Gerätewarte',
        nameLabel: 'Name',
        permissionsLabel: 'Rechte der Gruppe',
        membersLabel: 'Mitglieder der Gruppe',
        empty: 'Es gibt noch keine Gruppen.',
    },
    clusterMemberDetail: {
        fieldsTitle: 'Angaben',
        fieldCount: '{count} Felder',
        hint: 'Fragen der Wache und des Verbands stehen hier zusammen. Der Verband darf auch die beantworten, die für die Wache gesperrt sind.',
        saved: 'Gespeichert.',
        documents: {
            title: 'Dokumente',
            hint: 'Der Verband sieht die Dokumente der Person und legt neue ab. Ordnen, taggen und entfernen bleibt bei der Wache, denn das Dokument gehört der Wache, an der die Person ist.',
            none: 'Zu dieser Person ist nichts abgelegt.',
            upload: 'Dokument ablegen',
            uploadTitle: 'Dokument ablegen',
            uploadHint: 'Das Dokument gehört der Wache, an der die Person ist, und bleibt dort, wenn die Wache den Verband verlässt.',
            file: 'Datei',
            name: 'Bezeichnung',
        },
    },
    clusterMemberManagement: {
        create: {
            button: 'Mitglied aufnehmen',
            title: 'Mitglied aufnehmen',
            hint: 'Zuerst die Wache: ein Mitglied gehört zu einer Wache, und der Verband tritt hier für sie auf. Was der Verband selbst fragt, wird danach auf der Seite der Person beantwortet.',
            station: 'Wache',
            stationPlaceholder: 'Wache auswählen',
            firstName: 'Vorname',
            lastName: 'Nachname',
            userType: 'Rolle an der Wache',
            canLogin: 'Kann sich anmelden',
            email: 'E-Mail',
            noLoginHint: 'Ohne Anmeldung bekommt die Person eine Adresse, an die keine Post geht. Das ist derselbe Weg, den die Wache auf ihrem eigenen Bildschirm geht.',
            save: 'Aufnehmen',
        },
        blockedOwner: 'Wachleitung, kann vom Verband nicht bearbeitet werden',
        blockedSelf: 'Eigene Mitgliedschaft, kann vom Verband nicht bearbeitet werden',
        tooMany: 'Es werden die ersten {count} Mitglieder angezeigt. Grenze die Liste auf eine Wache ein.',
        searchPlaceholder: 'Name oder E-Mail suchen...',
        allStations: 'Alle Wachen',
        includeFormer: 'Ehemalige anzeigen',
        empty: 'Keine Mitglieder gefunden.',
        count: '{count} Treffer',
        former: 'Ehemalig',
        stationOwner: 'Wachleitung',
        archive: 'Als ehemalig führen',
        loadFailed: 'Die Mitglieder konnten nicht geladen werden.',
        actionFailed: 'Die Änderung war nicht möglich.',
        userTypes: {
            MEMBER: 'Mitglied',
            GUARDIAN: 'Erziehungsberechtigter',
            TEAM: 'Teammitglied',
            MANAGER: 'Manager',
        },
    },
    clusterModules: {
        hint: 'Ein abgeschaltetes Modul steht der Wache nicht zur Verfügung und kann dort auch nicht wieder eingeschaltet werden. Vorhandene Daten bleiben erhalten und sind wieder da, sobald ihr das Modul freigebt.',
        everyStation: 'Alle Wachen',
        groupHint: 'Abschaltungen addieren sich: Was ihr für alle Wachen abschaltet, bleibt auch hier abgeschaltet. Über diese Gruppe könnt ihr nur zusätzlich abschalten, nichts wieder freigeben.',
        saved: 'Die Modulvorgaben wurden gespeichert.',
    },
    clusterLookAndFeel: {
        hint: 'Was ihr hier setzt, bekommen alle Wachen des Verbands als Ausgangspunkt. Gesperrte Punkte können sie danach nicht mehr ändern.',
        saved: 'Das Erscheinungsbild wurde gespeichert.',
        settingsTitle: 'Vorgaben',
        feelLabel: 'Standard-Stil',
        noOpinion: 'Keine Vorgabe',
        locksTitle: 'Gesperrt',
        locksHint: 'Ein gesperrter Punkt ist in der Wache sichtbar, aber nicht änderbar.',
        lockTheme: 'Farbschema',
        lockColors: 'Eigene Farben',
        lockFeel: 'Stil',
        lockLogo: 'Logo',
    },
    clusterStorageBackend: {
        linkFromRoom: 'Eigenen Speicher verwalten →',
        backToRoom: '← Zurück zum Speicherplatz',
        policy: {
            title: 'Wofür der eigene Speicher gilt',
            hint: 'Der Verband entscheidet, wofür sein Speicher gilt. Verschoben wird nichts davon von allein: eine Entscheidung ist sofort geschrieben, eine Kopie braucht Zeit.',
            reach: 'Geltungsbereich',
            reaches: {
                NONE: 'Für nichts - alles liegt beim Standard der Instanz',
                OWN_FILES: 'Nur für die eigenen Dateien des Verbands',
                EVERY_STATION: 'Für den Verband und alle seine Wachen',
            },
            locked: 'Wachen dürfen nichts ändern',
            lockedHint: 'Gesperrt entscheidet allein der Verband, wo die Dateien einer Wache liegen. Offen darf eine Wache eigenen Speicher mitbringen - und zahlt ihn dann selbst, womit sie aus dem Kontingent des Verbands fällt.',
            save: 'Entscheidung speichern',
            drop: 'Eigenen Speicher aufgeben',
        },
        form: {
            title: 'Speicher des Verbands',
            hint: 'Zugangsdaten werden vor der Speicherung verschlüsselt. Zeigt die Konfiguration auf dasselbe Ziel, werden nur die Zugangsdaten erneuert und niemand muss umziehen.',
            type: 'Backend-Typ',
            types: {
                s3: 'S3 / S3-kompatibel',
                smb: 'SMB',
                sftp: 'SFTP',
            },
        },
        actions: {
            probeConfig: 'Verbindung testen',
            probeLive: 'Gespeicherten Speicher testen',
            probing: 'Teste …',
            apply: 'Speicher speichern',
            applying: 'Speichere …',
        },
        probe: {
            ok: 'Verbindung erfolgreich getestet.',
            failed: 'Verbindung fehlgeschlagen: {reason}',
        },
        placements: {
            title: 'Wo die Dateien liegen',
            hint: 'Eine Wache, deren Dateien noch nicht dort liegen, wo die Entscheidung es vorsieht, wird hier einzeln verschoben.',
            station: 'Wache',
            actual: 'Liegt auf',
            expected: 'Soll liegen auf',
            action: 'Aktion',
            ownStore: 'Eigener Ablageort',
            inPlace: 'Passt',
            move: 'Verschieben',
            moving: 'Verschiebe …',
            nothingToDo: 'Nichts zu tun',
            where: {
                ITS_OWN: 'Eigenem Speicher der Wache',
                THE_CLUSTERS: 'Speicher des Verbands',
                INSTANCE_DEFAULT: 'Standard der Instanz',
                WHEREVER_IT_IS: 'Bleibt, wo es ist',
            },
        },
        feedback: {
            policySaved: 'Entscheidung gespeichert.',
            backendSaved: 'Speicher gespeichert.',
            dropped: 'Der Verband hat keinen eigenen Speicher mehr. Wer noch darauf liegt, wird einzeln zurückgeholt.',
            moved: 'Verschoben. {copied} Dateien kopiert, {deleted} an der Quelle gelöscht.',
        },
        errors: {
            loadFailed: 'Konnte die Speicher-Konfiguration des Verbands nicht laden.',
            probeFailed: 'Verbindungstest fehlgeschlagen.',
            saveFailed: 'Speichern fehlgeschlagen.',
        },
    },
    clusterStorage: {
        poolTitle: 'Speicherplatz des Verbands',
        poolUsage: '{used} von {total} verteilt',
        noPool: 'Die Instanz hat dem Verband keine Obergrenze gesetzt.',
        noStations: 'Dem Verband gehört noch keine Wache an.',
        instanceDefault: 'Standard',
        defaultsTitle: 'Standard für alle Wachen',
        defaultsHint: 'Gilt für jede Wache, der der Verband nichts Eigenes zugeteilt hat. Das eigene Lager des Verbands zählt mit.',
        editDefaults: 'Standard ändern',
        leftToInstance: 'Der Instanz überlassen',
        emptyMeansInstance: 'Ein leeres Feld überlässt dieses Maß der Instanz.',
        emptyMeansDefaults: 'Ein leeres Feld fällt auf den Standard des Verbands zurück.',
        grantRoom: 'Platz zuteilen',
        grantRoomTo: 'Platz für {name}',
        handBack: 'Zuteilung zurücknehmen',
    },
    clusterStations: {
        createTitle: 'Wache anlegen',
        createHint: 'Eine hier angelegte Wache gehört dem Verband von Anfang an. Eine bestehende Wache kann nur beitreten, wenn ihre Leitung das selbst beantragt.',
        nameLabel: 'Name',
        namePlaceholder: 'z.B. Löschzug Nord',
        empty: 'Diesem Verband gehört noch keine Wache an.',
        release: 'Entlassen',
        releaseMessage: 'Soll „{name}" wirklich aus dem Verband entlassen werden? Ausrüstung des Verbands, die dort liegt, geht zurück an den Verband.',
    },
    clusterApplications: {
        empty: 'Es liegen keine Beitrittsanfragen vor.',
        askedOn: 'Angefragt am {date}',
        approve: 'Aufnehmen',
        deny: 'Ablehnen',
        denyTitle: 'Anfrage ablehnen',
        denyReasonPlaceholder: 'Warum wird die Anfrage abgelehnt?',
        status: {
            PENDING: 'Offen',
            APPROVED: 'Aufgenommen',
            DENIED: 'Abgelehnt',
            WITHDRAWN: 'Zurückgezogen',
        },
    },
    stationCluster: {
        memberHint: 'Diese Wache gehört zu diesem Verband. Der Verband entscheidet, wann sie ihn wieder verlässt.',
        pendingTitle: 'Anfrage läuft',
        pendingText: 'Die Anfrage an {name} wartet auf eine Antwort.',
        withdraw: 'Anfrage zurückziehen',
        applyTitle: 'Einem Verband beitreten',
        applyHint: 'Nur die Leitung der Wache kann den Beitritt beantragen. Der Verband entscheidet danach, ob er die Wache aufnimmt.',
        clusterLabel: 'Verband',
        clusterPlaceholder: 'Verband auswählen',
        apply: 'Beitritt beantragen',
        historyTitle: 'Frühere Anfragen',
    },
    clusterSwitcher: {
        title: 'Verband wechseln',
        noCluster: 'Kein Verband gewählt',
    },
    clusterOverview: {
        title: 'Verband',
        subtitle: 'Übersicht über den Verband, für den du gerade arbeitest',
        noCluster: 'Du arbeitest gerade für keinen Verband. Wähle oben in der Seitenleiste einen aus.',
        noDescription: 'Keine Beschreibung hinterlegt.',
        loadFailed: 'Der Verband konnte nicht geladen werden.',
        yourRoleTitle: 'Deine Rolle',
        permissionsTitle: 'Was du in diesem Verband darfst',
        permissionsNone: 'Du bist hier aufgenommen, hast aber noch keine Aufgabe zugewiesen bekommen.',
        administratorsTitle: 'Verbandsleitung',
        administratorsHint: 'An diese Personen wendest du dich, wenn du hier etwas brauchst.',
        administratorsNone: 'Für diesen Verband ist noch niemand als Leitung eingetragen.',
        role: {
            CLUSTER_USER: 'Mitglied',
            CLUSTER_ADMIN: 'Verwaltung',
            unknown: 'Unbekannt',
        },
    },
    clusterSettings: {
        title: 'Verbands-Einstellungen',
        subtitle: 'Name und Beschreibung des Verbands',
        generalTitle: 'Allgemein',
        nameLabel: 'Name',
        namePlaceholder: 'Name des Verbands',
        descriptionLabel: 'Beschreibung',
        descriptionPlaceholder: 'Wofür steht dieser Verband?',
        saved: 'Die Einstellungen wurden gespeichert.',
        loadFailed: 'Die Einstellungen konnten nicht geladen werden.',
        saveFailed: 'Die Einstellungen konnten nicht gespeichert werden.',
        federationTitle: 'Vernetzung der Wachen',
        autoFederateLabel: 'Wachen untereinander verbinden',
        autoFederateHint: 'Neue Wachen des Verbands werden automatisch mit allen anderen verbunden, sodass sie Termine, Wissen und Material teilen können.',
        autoFederateOffHint: 'Beim Ausschalten bleiben bestehende Verbindungen erhalten. Es werden nur keine neuen mehr angelegt.',
    },
    questionValue: {
        chooseOne: 'Bitte wählen',
        chooseMember: 'Mitglied wählen',
    },
    questionOptions: {
        add: 'Option hinzufügen',
        placeholder: 'Antwortmöglichkeit',
    },
    notificationSchedule: {
        title: 'Versandzeiten der Benachrichtigungen',
        hint: 'Gesammelte Benachrichtigungen gehen zu den hier genannten Zeiten als eine Mail raus, nach der Uhr der Station. Was dazwischen ankommt, wartet auf den nächsten Termin.',
        mode: 'Versand',
        modeOperator: 'Wie von der Instanz vorgegeben',
        modeTimes: 'Zu festen Uhrzeiten',
        modeHourly: 'Stündlich',
        times: 'Uhrzeiten',
        addTime: 'Uhrzeit hinzufügen',
        floorHint: 'Diese Instanz verschickt höchstens alle {minutes} Minuten eine Mail. Häufigere Wünsche werden auf diesen Abstand gebracht.',
        saved: 'Die Versandzeiten wurden gespeichert.',
    },
    files: {
        noPreview: 'Für diesen Dateityp gibt es keine Vorschau. Lade sie herunter, um sie anzusehen.',
        previewFailed: 'Die Datei ist da, lässt sich hier aber nicht anzeigen.',
        previewFailedGuidance: 'Ein zweiter Versuch ändert daran nichts. Lade sie herunter und öffne sie mit einem Programm auf deinem Gerät.',
        truncated: 'Die Vorschau zeigt nur den Anfang der Datei. Lade sie herunter, um alles zu lesen.',
        saveUnavailable: 'Diese Datei ließ sich auf dem Gerät nicht sichern. Öffne die Seite in Safari oder Chrome und versuche es dort noch einmal.',
    },
    contentDraft: {
        found: 'Es gibt noch ungespeicherte Änderungen von {when}. Möchtest du dort weitermachen?',
        restore: 'Weitermachen',
        discard: 'Verwerfen',
    },
    exportFormat: {
        title: 'Export',
        format: 'Format',
        csv: 'Tabelle (CSV)',
        pdf: 'PDF',
        separator: 'Trennzeichen',
        semicolon: 'Semikolon (;)',
        comma: 'Komma (,)',
        separatorHint: 'Tabellenprogramme auf Deutsch erwarten das Semikolon.',
    },
    common: {
        send: 'Abschicken',
        continue: 'Weiter',
        export: 'Exportieren',
        loading: 'Laden...',
        error: 'Das hat nicht funktioniert. Versuche es noch einmal, und melde es bitte, wenn es dabei bleibt.',
        empty: 'Leer - zum Bearbeiten klicken',
        cancel: 'Abbrechen',
        confirm: 'Bestätigen',
        previous: 'Zurück',
        notFound: 'Nicht gefunden',
        unknown: 'Unbekannt',
        edit: 'Bearbeiten',
        delete: 'Löschen',
        save: 'Speichern',
        saving: 'Speichern…',
        saved: 'Gespeichert',
        close: 'Schließen',
        enlargeImage: 'Bild vergrößern',
        add: 'Hinzufügen',
        back: 'Zurück',
        open: 'Öffnen',
        moveUp: 'Nach oben',
        moveDown: 'Nach unten',
        yes: 'Ja',
        no: 'Nein',
        refresh: 'Aktualisieren',
        remove: 'Entfernen',
        rename: 'Umbenennen',
        create: 'Erstellen',
        general: 'Allgemein',
        requiredField: 'Dieses Feld ist erforderlich.',
        next: 'Weiter',
        unisize: 'Einheitsgröße',
        createdBy: 'Erstellt von {name}',
        submittedBy: 'Eingereicht von {name}',
        actions: 'Aktionen',
        selectFile: 'Datei auswählen',
        noFileSelected: 'Keine Datei ausgewählt',
        selectAll: 'Alle',
        selectNone: 'Keine',
        noOptions: 'Keine Auswahl verfügbar.',
    },
    onboarding: {
        resume: 'Weiter mit der Einrichtung',
        /** Stands in for the child's name where a task is about nobody in particular. */
        child: 'dein Kind',
        card: {
            begin: 'Los geht’s',
            install: 'Ember installieren',
            confirm: 'Erledigt',
            skip: 'Überspringen',
            resume: 'Wieder aufnehmen',
            discard: 'Verwerfen',
            allDone: 'Alles erledigt. Schön, dass du dabei bist!',
            skippedTitle: 'Übersprungen',
            skippedHint: 'Verworfene Aufgaben verschwinden dauerhaft und kommen nicht zurück.',
            settledBy: 'von {name}',
            MEMBER: {
                title: 'Deine ersten Schritte',
                hint: 'Ich zeige dir, wo alles liegt. Du klickst selbst, dann kennst du den Weg beim nächsten Mal.',
            },
            STATION: {
                title: 'Eure Wache einrichten',
                hint: 'Das hier gilt für die ganze Wache. Was einer von euch erledigt, ist für alle erledigt.',
            },
            INSTANCE: {
                title: 'Die Instanz einrichten',
                hint: 'Das hier gilt für die ganze Instanz und ist für alle Administratoren dasselbe.',
            },
        },
        guide: {
            reveal: 'Hier steckt es drin. Klapp das auf, dann zeige ich dir weiter.',
            behindMenu: 'Das liegt im Menü, und das ist gerade zu. Tipp hier drauf, dann zeige ich dir weiter.',
            elsewhere: 'Das findest du unter „{page}". Soll ich dich hinbringen?',
            takeMeThere: 'Hinbringen',
            understood: 'Verstanden',
            skip: 'Aufgabe überspringen',
            dismiss: 'Später',
            blocked: 'Das geht noch nicht. Erledige zuerst, was auf der Seite darüber steht, dann wird der Knopf aktiv.',
            finished: 'Geschafft. Willst du zurück zu deiner Liste?',
            notYet: 'Die Schritte sind durch, die Aufgabe aber noch nicht: Ember hakt sie erst ab, wenn die Sache '
                + 'wirklich getan ist. Prüfe, ob deine Eingabe auch gespeichert wurde.',
            toList: 'Zurück zur Liste',
            stayHere: 'Hier bleiben',
        },
        routes: {
            'profile': 'Profil',
            'profile-notifications': 'Benachrichtigungen',
            'profile-absences': 'Abwesenheiten',
            'profile-managed': 'Verwaltete Profile',
            'events-upcoming': 'Termine',
            'events': 'Terminverwaltung',
            'kb-browse': 'Wiki',
            'quiz-training': 'Training',
            'members-create': 'Mitglied anlegen',
            'members-groups': 'Gruppen',
            'members-type-permissions': 'Berechtigungen',
            'station-mailing': 'Mailversand',
            'station-mail-import': 'Post-Import',
            'station-manage': 'Wachverwaltung',
            'station-federation-settings': 'Föderation',
            'account-avatar': 'Konto → Profil',
            'account-security': 'Konto → Sicherheit',
            'admin-legal': 'Rechtstexte',
            'admin-mailing': 'Mailversand der Instanz',
            'admin-stations': 'Wachen',
            'admin-settings': 'Einstellungen',
            'admin-security': 'Sicherheit',
            'admin-storage': 'Speicher',
            'admin-maps': 'Karten',
        },
        tasks: {
            member: {
                profile: {
                    title: 'Dein Profil durchsehen',
                    body: 'Schau nach, was über dich hinterlegt ist, und ergänze, was fehlt. Dauert zwei Minuten.',
                },
                notifications: {
                    title: 'Benachrichtigungen einstellen',
                    body: 'Du entscheidest für jede Art von Nachricht, ob du sie hier, per E-Mail oder im Feed '
                        + 'bekommst.',
                },
                eventAnswer: {
                    title: 'Die zwei Arten von Terminen kennenlernen',
                    body: 'Zu manchen meldest du dich an, bei anderen wirst du erwartet. Antworten musst du '
                        + 'dafür nichts.',
                },
                calendar: {
                    title: 'Termine in deinen Kalender holen',
                    body: 'Einmal abonnieren, danach stehen alle deine Termine automatisch in deiner Kalender-App.',
                },
                absence: {
                    title: 'Schauen, wo Abwesenheiten eingetragen werden',
                    body: 'Damit du weißt, wo es steht, wenn du mal nicht kannst. Eintragen musst du jetzt nichts.',
                },
                bookmark: {
                    title: 'Ember griffbereit halten',
                    body: 'Am Rechner als Lesezeichen, am Handy über das Menü auf den Startbildschirm legen.',
                },
                wiki: {
                    title: 'Ins Wiki schauen',
                    body: 'Merkblätter und Nachschlagewerke deiner Wache liegen hier.',
                },
                quiz: {
                    title: 'Ein Training ausprobieren',
                    body: 'Übe den Ausbildungsstoff, so oft du magst. Niemand sieht deine Ergebnisse.',
                },
            },
            guardian: {
                profile: {
                    title: 'Daten von {name} prüfen',
                    body: 'Schau einmal durch, was die Wache über {name} hinterlegt hat.',
                },
                username: {
                    title: 'Benutzernamen festlegen',
                    body: 'Damit meldet sich {name} an, auch ohne eigene E-Mail-Adresse.',
                },
                login: {
                    title: 'Anmeldung freischalten',
                    body: 'Du entscheidest, ab wann {name} selbst hereinkommt, und kannst es jederzeit '
                        + 'zurücknehmen.',
                },
                password: {
                    title: 'Passwort setzen',
                    body: 'Vergib das Passwort und gib es {name} zusammen mit dem Benutzernamen weiter.',
                },
                eventAnswer: {
                    title: 'Für {name} auf einen Termin antworten',
                    body: 'Anmelden oder abmelden, direkt in der Terminliste.',
                },
            },
            station: {
                groups: {
                    title: 'Mitgliedergruppen anlegen',
                    body: 'Gruppen sind die Grundlage für Berechtigungen, Termine und Auswertungen.',
                },
                mail: {
                    title: 'Mailversand einrichten',
                    body: 'Ohne Versand geht keine Einladung und keine Benachrichtigung raus.',
                },
                branding: {
                    title: 'Wache einkleiden',
                    body: 'Logo und Farben, damit eure Seite nach euch aussieht.',
                },
                firstEvent: {
                    title: 'Ersten Termin anlegen',
                    body: 'Damit eure Mitglieder etwas zum Zusagen haben.',
                },
                kbSeed: {
                    title: 'Erste Wiki-Seite anlegen',
                    body: 'Ein Merkblatt reicht für den Anfang.',
                },
                federation: {
                    title: 'Sichtbarkeit entscheiden',
                    body: 'Legt fest, was andere Wachen von euch sehen.',
                },
                invites: {
                    title: 'Erste Mitglieder einladen',
                    body: 'Anlegen allein reicht nicht: Erst mit erlaubter Anmeldung kommt jemand herein.',
                },
                memberTypes: {
                    title: 'Berechtigungen ansehen',
                    body: 'Schaut einmal durch, was Mitglieder, Team und Verwaltung dürfen.',
                },
            },
            instance: {
                ownAccount: {
                    title: 'Dein eigenes Konto absichern',
                    body: 'Eine erreichbare Adresse und ein zweiter Faktor. Daran hängt jede Wiederherstellung.',
                },
                legal: {
                    title: 'Rechtstexte hinterlegen',
                    body: 'Impressum und Datenschutz. Ohne sie läuft die Instanz nicht rechtssicher.',
                },
                mail: {
                    title: 'Mailversand der Instanz einrichten',
                    body: 'Die erste Mail überhaupt ist die Einladung an eure erste Wachenverwaltung.',
                },
                firstStation: {
                    title: 'Erste Wache anlegen',
                    body: 'Anlegen und jemandem übergeben, der sie führt.',
                },
                stationRegistration: {
                    title: 'Selbstregistrierung entscheiden',
                    body: 'Dürfen sich Wachen selbst anmelden, oder legst du sie an?',
                },
                security: {
                    title: 'Sicherheit festlegen',
                    body: 'Zwei-Faktor-Pflicht und Prüfung gegen bekannte Datenlecks.',
                },
                storage: {
                    title: 'Speicherziel wählen',
                    body: 'Wo die hochgeladenen Dateien liegen sollen.',
                },
                operations: {
                    title: 'Karten und Protokolle kennen',
                    body: 'Damit du im Ernstfall weißt, wo du nachschaust.',
                },
            },
        },
        steps: {
            member: {
                profile: {
                    0: 'Öffne dein Profil, dort steht alles über dich.',
                    1: 'Schau einmal durch, ob das alles noch stimmt: deine Wache plant damit. '
                        + 'Ändern musst du nichts, und wenn doch, denk ans Speichern.',
                },
                notifications: {
                    0: 'Deine Benachrichtigungen liegen hier.',
                    1: 'Stell für jede Zeile ein, wo du sie haben willst: hier, per E-Mail oder im Feed.',
                },
                eventAnswer: {
                    0: 'Hier stehen alle kommenden Termine.',
                    1: 'Zu manchen Terminen meldest du dich an. Dort ist der Platz erst deiner, wenn du '
                        + 'zugesagt hast, und deine Wache plant mit denen, die zugesagt haben. Antworten '
                        + 'musst du jetzt nichts.',
                    2: 'Bei den anderen wirst du erwartet, ohne dass du etwas tust. Da meldest du dich nur '
                        + 'ab, wenn du nicht kannst, damit niemand vergeblich auf dich wartet.',
                },
                calendar: {
                    0: 'Deinen Kalender-Zugang findest du bei den Benachrichtigungen.',
                    1: 'Erzeuge dir einen Feed-Token.',
                    2: 'Kopiere die Kalender-Adresse und trag sie in deiner Kalender-App als Abo ein.',
                },
                absence: {
                    0: 'Abwesenheiten stehen in deinem Profil.',
                    1: 'Hier trägst du Anfang und Ende ein und sagst kurz, warum. Deine Wache sieht dann, dass du '
                        + 'in der Zeit nicht kannst. Eintragen musst du jetzt nichts.',
                },
                wiki: {
                    0: 'Das Wiki deiner Wache.',
                    1: 'Öffne einen Eintrag und schau dich um.',
                },
                quiz: {
                    0: 'Das Training liegt hier.',
                    1: 'Wähle mindestens einen Katalog aus. Ohne Auswahl bleibt der Startknopf grau.',
                    2: 'Jetzt kannst du starten.',
                },
            },
            guardian: {
                profile: {
                    0: 'Die Profile, die du verwaltest, liegen hier.',
                    1: 'Wähle {name} aus.',
                    2: 'Prüfe die Angaben und speichere.',
                },
                username: {
                    0: 'Öffne die verwalteten Profile.',
                    1: 'Wähle {name} aus.',
                    2: 'Trag einen Benutzernamen ein. Nichts leicht Erratbares, bitte.',
                    3: 'Speichern.',
                },
                login: {
                    0: 'Öffne die verwalteten Profile.',
                    1: 'Schalte die Anmeldung frei.',
                },
                password: {
                    0: 'Öffne die verwalteten Profile.',
                    1: 'Vergib ein Passwort für {name}.',
                    2: 'Speichern, und gib es {name} weiter.',
                },
                eventAnswer: {
                    0: 'Hier stehen die Termine.',
                    1: 'Wähle {name} aus.',
                    2: 'Melde {name} an oder ab.',
                },
            },
            station: {
                groups: {
                    0: 'Gruppen liegen bei den Mitgliedern.',
                    1: 'Leg eure erste Gruppe an.',
                },
                mail: {
                    0: 'Der Mailversand eurer Wache.',
                    1: 'Trag einen Versandweg ein.',
                    2: 'Schick eine Testmail. Erst wenn die ankommt, funktioniert es wirklich.',
                },
                branding: {
                    0: 'Die Verwaltung eurer Wache.',
                    1: 'Lade euer Logo hoch.',
                },
                firstEvent: {
                    0: 'Die Terminverwaltung.',
                    1: 'Leg euren ersten Termin an.',
                },
                kbSeed: {
                    0: 'Euer Wiki.',
                    1: 'Leg den ersten Eintrag an.',
                },
                federation: {
                    0: 'Die Föderationseinstellungen.',
                    1: 'Entscheidet, was andere Wachen von euch sehen.',
                },
                invites: {
                    0: 'Hier legt ihr ein Mitglied an. Denkt daran, ihm auch die Anmeldung zu erlauben.',
                },
                memberTypes: {
                    0: 'Hier steht, was jeder Mitgliedstyp darf.',
                },
            },
            instance: {
                ownAccount: {
                    0: 'Hinterlege eine Adresse, unter der du wirklich erreichbar bist.',
                    1: 'Richte einen zweiten Faktor ein.',
                },
                legal: {
                    0: 'Die Rechtstexte der Instanz.',
                    1: 'Nimm eine Vorlage als Anfang. Sie ersetzt keine Rechtsberatung.',
                },
                mail: {
                    0: 'Der Mailversand der Instanz.',
                    1: 'Trag einen Versandweg ein.',
                    2: 'Schick eine Testmail.',
                },
                firstStation: {
                    0: 'Die Wachen dieser Instanz.',
                    1: 'Leg die erste an und übergib sie jemandem.',
                },
                stationRegistration: {
                    0: 'Die Einstellungen der Instanz.',
                    1: 'Entscheide, ob sich Wachen selbst registrieren dürfen.',
                },
                security: {
                    0: 'Die Sicherheitseinstellungen.',
                },
                storage: {
                    0: 'Hier legst du fest, wo Dateien liegen.',
                },
                operations: {
                    0: 'Karten und Protokolle. Merk dir den Weg hierher.',
                },
            },
        },
    },
    tour: {
        back: 'Zurück',
        next: 'Weiter',
        finish: 'Fertig',
        skip: 'Tour überspringen',
        restartButton: 'Einführungstour wiederholen',
        restartHint: 'Du kannst die Einführungstour jederzeit hier nochmal starten.',
        steps: {
            welcome: {
                title: 'Willkommen bei Ember!',
                body: 'Schön, dass du da bist! Wir zeigen dir jetzt die wichtigsten Seiten. Schau dir alles in Ruhe an - du kannst die Seite oben ganz normal ansehen, während wir dir hier unten erklären, was du dort findest.',
            },
            dashboard: {
                title: 'Dein Dashboard',
                body: 'Das hier ist deine Startseite. Du siehst hier Benachrichtigungen, deine nächsten Termine und ob es Neuigkeiten gibt. Schau hier einfach regelmäßig vorbei.',
            },
            news: {
                title: 'Neuigkeiten',
                body: 'Hier findest du alle Nachrichten deiner Wache. Du kannst sie lesen und auch Kommentare schreiben. Neue Beiträge erscheinen ganz oben.',
            },
            profile: {
                title: 'Dein Profil',
                body: 'Das ist dein Profil. Hier siehst du deine gespeicherten Daten. Manche Felder kannst du selbst bearbeiten.',
            },
            absences: {
                title: 'Abwesenheiten',
                body: 'Hier trägst du ein, wenn du mal nicht kommen kannst - zum Beispiel bei Krankheit oder Urlaub. So wissen alle Bescheid.',
            },
            events: {
                title: 'Termine',
                body: 'Hier siehst du alle kommenden Übungen und Veranstaltungen. Bei manchen Terminen kannst du dich an- oder abmelden - einfach auf den entsprechenden Knopf klicken.',
            },
            inventory: {
                title: 'Dein Inventar',
                body: 'Hier siehst du, welche Ausrüstung dir zugewiesen ist. Wenn etwas nicht mehr passt, kannst du hier einen Tausch beantragen.',
            },
            forms: {
                title: 'Umfragen',
                body: 'Hier findest du die Umfragen deiner Wache. Klicke auf „Ausfüllen", um eine zu öffnen. Schau ab und zu rein, ob es etwas Neues gibt.',
            },
            knowledgeBase: {
                title: 'Wiki',
                body: 'Hier findest du Wissen deiner Wache - Anleitungen, Dokumente und Videos. Alles ist in Ordnern sortiert und du kannst nach Begriffen suchen.',
            },
            lending: {
                title: 'Inventar-Ausleihe',
                body: 'Hier kannst du Ausrüstung von anderen Wachen anfragen oder sehen, was aktuell verliehen ist. Anfragen durchlaufen einen Genehmigungsprozess.',
            },
            managedProfiles: {
                title: 'Verwaltete Profile',
                body: 'Dir sind Mitglieder zugewiesen, deren Profile du hier bearbeiten kannst. Du kannst auch Abwesenheiten eintragen oder sie für Termine anmelden.',
            },
            members: {
                title: 'Mitgliederliste',
                body: 'Hier siehst du alle Mitglieder. Du kannst auf einen Namen klicken, um das Profil zu öffnen.',
            },
            attendance: {
                title: 'Anwesenheit erfassen',
                body: 'Hier startest du eine neue Anwesenheitserfassung. Wähle eine Vorlage und trage ein, wer da war. Vergangene Erfassungen findest du unter „Vergangene Anwesenheiten".',
            },
            eventManagement: {
                title: 'Termine verwalten',
                body: 'Hier verwaltest du alle Termine. Du kannst neue erstellen, bestehende bearbeiten oder Kategorien und Pausen anlegen. Klicke auf einen Termin für Details.',
            },
            federation: {
                title: 'Föderation',
                body: 'Hier verbindest du deine Wache mit anderen Jugendfeuerwehren. Ihr könnt dann Lernmaterial, Quiz-Kataloge und Prüfungsbögen teilen oder Ausrüstung verleihen.',
            },
            discovery: {
                title: 'Wachen-Verzeichnis',
                body: 'Im Verzeichnis findest du andere Wachen auf dieser Instanz. Du kannst ihnen eine Föderations-Anfrage schicken oder deren öffentliches Wiki durchstöbern.',
            },
            station: {
                title: 'Wache verwalten',
                body: 'Hier findest du die Einstellungen für deine Wache - Name, Anwesenheitsvorlagen und Mitglieder-Felder.',
            },
            settings: {
                title: 'Einstellungen',
                body: 'Hier kannst du einstellen, welche Benachrichtigungen du bekommst. Du siehst auch deine aktiven Sitzungen und kannst diese Tour jederzeit neu starten.',
            },
            helpCenter: {
                title: 'Das Hilfe-Center',
                body: 'Oben rechts findest du immer ein Fragezeichen (?). Klicke darauf und es öffnet sich das Hilfe-Center. Dort wird dir zu jeder Seite einfach erklärt, was du dort machen kannst.',
            },
            done: {
                title: 'Das war\'s!',
                body: 'Jetzt kennst du die wichtigsten Bereiche. Viel Spaß mit Ember! Du findest diese Tour jederzeit unter „Profil" → „Einstellungen".',
            },
        },
    },
    quiz: {
        // Top-level keys used by views
        questionTypes: {
            MULTIPLE_CHOICE: 'Multiple Choice',
            FILL_IN_THE_BLANK: 'Lückentext',
            FREE_ANSWER: 'Freitext',
            CONNECT: 'Zuordnung',
            IMAGE_TEXT: 'Bild & Text',
            TRUE_FALSE: 'Wahr/Falsch',
            ORDERING: 'Reihenfolge',
            ENUMERATION: 'Aufzählung',
        },
        statusDraft: 'Entwurf',
        statusActive: 'Aktiv',
        statusClosed: 'Geschlossen',
        statusSubmitted: 'Abgegeben',
        attemptCount: 'Teilnehmer',
        createCatalog: 'Neuer Katalog',
        points: 'Punkte',
        noAnswer: 'Keine Antwort',
        trueLabel: 'Wahr',
        falseLabel: 'Falsch',
        catalogs: {
            title: 'Fragenkataloge',
            create: 'Neuer Katalog',
            name: 'Name',
            description: 'Beschreibung',
            trainingEnabled: 'Für Training freigeben',
            questionCount: '{count} Fragen',
            noCatalogs: 'Noch keine Kataloge vorhanden.',
            deleteCatalog: 'Katalog löschen',
            deleteConfirm: 'Möchtest du diesen Katalog wirklich löschen? Alle Fragen gehen verloren.',
            export: 'Exportieren',
            import: 'Importieren',
            search: 'Fragenkataloge durchsuchen...',
            allStations: 'Alle Partnerwachen',
            importRejected: 'Die Datei wurde nicht übernommen. Es wurde nichts angelegt.',
            importNotReadable: 'Die Datei konnte nicht gelesen werden.',
            nameRequired: 'Bitte einen Namen für den Katalog angeben.',
        },
        categories: {
            title: 'Kategorien',
            create: 'Neue Kategorie',
            edit: 'Kategorie bearbeiten',
            name: 'Kategoriename',
            description: 'Beschreibung (optional)',
            noCategories: 'Keine Kategorien vorhanden.',
            deleteConfirm: 'Möchtest du diese Kategorie wirklich löschen?',
        },
        questions: {
            title: 'Fragen',
            create: 'Neue Frage',
            edit: 'Frage bearbeiten',
            questionTitle: 'Fragetext',
            description: 'Beschreibung / Hinweis',
            category: 'Kategorie',
            noCategory: 'Keine Kategorie',
            points: 'Punkte',
            autoPoints: 'Automatisch berechnen',
            image: 'Bild',
            uploadImage: 'Bild hochladen',
            noQuestions: 'Noch keine Fragen vorhanden.',
            allTypes: 'Alle Fragetypen',
            allCategories: 'Alle Kategorien',
            deleteConfirm: 'Möchtest du diese Frage wirklich löschen?',
            type: 'Fragetyp',
            config: {
                options: 'Antwortmöglichkeiten',
                addOption: 'Option hinzufügen',
                correctAnswer: 'Richtige Antwort',
                correctAnswers: 'Richtige Antworten',
                pointsPerCorrect: 'Punkte pro richtige Antwort',
                mcScoringHint: 'Jede richtige Antwort gibt Punkte, jede falsche zieht Punkte ab. Das Ergebnis ist mindestens 0.',
                fillText: 'Lückentext',
                fillTextPlaceholder: 'Text mit ___ als Lücken eingeben',
                fillAnswersHint: 'Die richtigen Wörter in der Reihenfolge der Lücken.',
                distractors: 'Falsche Wörter (Ablenkungen)',
                distractorsHint: 'Zusätzliche Wörter, die plausibel klingen aber falsch sind.',
                addDistractor: 'Falsches Wort hinzufügen',
                useDropdown: 'Dropdown-Auswahl verwenden',
                useDropdownHint: 'Wenn aktiviert, können Nutzer die Wörter aus einem Dropdown auswählen statt sie einzutippen. Auf Papier werden stattdessen Nummern eingetragen.',
                lines: 'Anzahl Zeilen',
                enumerationAnswers: 'Mögliche Antworten',
                addAnswer: 'Antwort hinzufügen',
                left: 'Links',
                right: 'Rechts',
                pairs: 'Paare',
                addPair: 'Paar hinzufügen',
                items: 'Elemente in korrekter Reihenfolge',
                addItem: 'Element hinzufügen',
                orderingHint: 'Die Reihenfolge hier ist die richtige Lösung.',
            },
        },
        tests: {
            title: 'Tests',
            tabTest: 'Test',
            tabTests: 'Tests',
            tabResults: 'Ergebnisse',
            noResults: 'Noch keine Ergebnisse vorhanden.',
            savedWithoutSections: 'Der Test wurde angelegt, aber seine Abschnitte und die Freigabe konnten nicht gespeichert werden. Lege ihn nicht noch einmal an, sondern öffne ihn und speichere sie dort.',
            create: 'Neuer Test',
            titlePlaceholder: 'Name des Tests',
            descriptionPlaceholder: 'Kurze Beschreibung',
            timeLimit: 'Zeitlimit (Minuten)',
            timeLimitEnabled: 'Zeitlimit aktivieren',
            timeLimitMinutes: 'Minuten',
            noTimeLimit: 'Kein Zeitlimit',
            timeLimitValue: '{minutes} Minuten',
            shuffle: 'Fragen mischen',
            forced: 'Pflichttest',
            startAt: 'Startzeit',
            endAt: 'Endzeit',
            startedAt: 'Gestartet',
            submittedAt: 'Abgegeben',
            noTests: 'Noch keine Tests vorhanden.',
            confirmDelete: 'Möchtest du diesen Test wirklich löschen?',
            statusDraft: 'Entwurf',
            statusActive: 'Aktiv',
            statusClosed: 'Geschlossen',
            activate: 'Aktivieren',
            confirmActivate: 'Möchtest du diesen Test wirklich aktivieren? Mitglieder können ihn dann schreiben.',
            close: 'Schließen',
            confirmClose: 'Möchtest du diesen Test wirklich schließen?',
            grantAccess: 'Zugang gewähren',
            grantAccessMember: 'Mitglied-ID',
            accessClosesAt: 'Zugang läuft ab am',
            accessManagement: 'Zugangsverwaltung',
            takeTest: 'Test schreiben',
            taken: 'Abgegeben',
            backToList: 'Zurück zur Liste',
            restrictions: 'Zugangseinschränkungen',
            searchMember: 'Mitglied suchen...',
            exportQuestions: 'Fragen-PDF',
            exportSolutions: 'Lösungs-PDF',
        },
        frozenQuestions: {
            title: 'Testfragen',
            generate: 'Fragen generieren',
            regenerate: 'Neu generieren',
            empty: 'Noch keine Fragen generiert. Generiere Fragen, um sie vor der Aktivierung zu überprüfen.',
            questionMissing: 'Frage nicht mehr vorhanden',
            randomReplace: 'Zufällig austauschen',
            pickReplace: 'Frage auswählen',
            pickTitle: 'Frage ersetzen',
            searchPlaceholder: 'Frage suchen...',
            noAvailable: 'Keine weiteren Fragen verfügbar.',
        },
        sections: {
            title: 'Abschnitte',
            add: 'Abschnitt hinzufügen',
            titlePlaceholder: 'Titel des Abschnitts',
            descriptionPlaceholder: 'Kurze Beschreibung',
            noSections: 'Noch keine Abschnitte vorhanden.',
            untitled: 'Unbenannter Abschnitt',
            sectionNumber: 'Abschnitt {n}',
            sources: 'Fragen-Quellen',
            addSource: 'Quelle hinzufügen',
            selectCatalog: 'Katalog wählen',
            allCategories: 'Alle Kategorien',
            questionCount: 'Anzahl Fragen',
        },
        attempt: {
            title: 'Test',
            submit: 'Test abgeben',
            submitted: 'Abgegeben',
            confirmSubmitTitle: 'Test abgeben?',
            confirmSubmitMessage: 'Möchtest du den Test wirklich abgeben? Du kannst danach keine Antworten mehr ändern.',
            answerNotSaved: 'Deine Antworten sind nicht gespeichert. Der Server hat sie nicht angenommen.',
            answerNotSavedGuidance: 'Schließe diese Seite nicht und geh nicht vom Platz. Was du geschrieben hast, steht im Moment nur in diesem Browserfenster und wäre sonst weg. Der Test lässt sich erst abgeben, wenn deine Antworten angekommen sind: versuche es gleich noch einmal, und hole die Prüfungsaufsicht, wenn es dabei bleibt.',
            next: 'Weiter',
            prev: 'Zurück',
            backToTest: 'Zurück zum Test',
            statusInProgress: 'In Bearbeitung',
            statusSubmitted: 'Abgegeben',
            statusGraded: 'Bewertet',
            noAttempts: 'Noch keine Teilnahmen.',
            groupUngraded: 'Noch nicht bewertet',
            groupGraded: 'Bewertet',
            points: '{points} / {max} Punkte',
            true: 'Wahr',
            false: 'Falsch',
            fillGapPlaceholder: 'Lücke {n}...',
            freeAnswerPlaceholder: 'Deine Antwort hier eingeben...',
            questionProgress: 'Frage {current} von {total}',
            submittedInfo: 'Dein Test wurde abgegeben. Das Ergebnis siehst du, sobald er ausgewertet wurde.',
        },
        evaluate: {
            title: 'Bewertung',
            studentAnswer: 'Antwort des Teilnehmers',
            correctAnswer: 'Richtige Antwort',
            sampleAnswers: 'Beispielantworten',
            correct: 'richtig',
            points: 'Punkte',
            total: 'Gesamt',
            finishGrading: 'Bewertung abschließen',
            gradedSuccess: 'Bewertung gespeichert.',
            gradedBy: 'Bewertet von',
            markCorrect: 'Richtig',
            markWrong: 'Falsch',
        },
        training: {
            selectCatalogs: 'Wähle Kataloge zum Üben',
            noCatalogs: 'Es sind keine Kataloge für das Training freigegeben.',
            noQuestions: 'Keine Fragen in den ausgewählten Katalogen.',
            start: 'Training starten',
            showAnswer: 'Antwort zeigen',
            correctAnswer: 'Richtige Antwort',
            next: 'Nächste Frage',
            finish: 'Fertig!',
            finished: 'Alle Fragen durchgearbeitet!',
            restart: 'Erneut starten',
            progress: 'Frage {current} von {total}',
            sampleAnswer: 'Beispielantwort',
            checkYourAnswer: 'Vergleiche deine Antwort mit der Musterlösung.',
            correctOrder: 'Richtige Reihenfolge',
            correctPairs: 'Richtige Zuordnung',
        },
        ai: {
            apiKey: 'API-Schlüssel',
            provider: 'Anbieter',
            model: 'Modell',
            generate: 'Generieren',
            generating: 'Generiere...',
            count: 'Anzahl',
            keyNotEncrypted: 'Hinweis: Der Schlüssel wird unverschlüsselt auf dem Server gespeichert.',
            keyLocalHint: 'Der Schlüssel wird nur in deinem Browser gespeichert und nicht an den Server gesendet.',
            saveOnServer: 'Schlüssel auch auf dem Server speichern',
            noKeyConfigured: 'Kein API-Schlüssel konfiguriert. Gehe zu den KI-Einstellungen in der Fragengenierung, um einen Schlüssel einzutragen.',
            fillMcAnswers: 'Antworten generieren',
            fillMcHint: 'Generiere falsche Antworten für alle Multiple-Choice-Fragen im Katalog. Fragen ohne markierte korrekte Antwort werden übersprungen.',
            questionsWithNewAnswers: 'Fragen mit neuen Antworten',
            newAnswersTotal: 'neue Antworten insgesamt',
            noNewAnswers: 'Keine neuen Antworten generiert.',
            additionalPrompt: 'Zusätzlicher Kontext',
            additionalPromptPlaceholder: 'z.B. Thema, Schwierigkeitsgrad, Zielgruppe...',
            modeAdd: 'Hinzufügen',
            modeFillTo: 'Auffüllen auf',
            defaultModel: 'Standard-Modell',
            settingsTitle: 'KI-Einstellungen',
            generateQuestions: 'Fragen generieren',
            userPrompt: 'Thema / Kontext',
            userPromptPlaceholder: 'Beschreibe das Thema der Fragen, z.B. "Brandschutz und Löschtechnik für Anfänger"',
            reviewHint: 'Klicke auf eine Frage, um sie ein- oder auszuschließen. Nur ausgewählte Fragen werden zum Katalog hinzugefügt.',
            acceptSelected: 'Ausgewählte übernehmen',
            regenerate: 'Neu generieren',
            addEntry: 'Fragetyp hinzufügen',
            providers: {
                openai: 'OpenAI',
                gemini: 'Google Gemini',
                claude: 'Anthropic Claude',
            },
        },
        csv: {
            import: 'CSV importieren',
            columnMapping: 'Spaltenzuordnung',
            questionColumn: 'Frage',
            answerColumn: 'Antwort',
            categoryColumn: 'Kategorie',
            typeColumn: 'Typ-Spalte',
            pointsColumn: 'Punkte-Spalte',
            descriptionColumn: 'Hinweis-Spalte',
            imageColumn: 'Bild-Spalte',
            distractorColumn: 'Falsche Antworten',
            distractorHint: 'Falsche Antworten machen aus einer Lücke eine Auswahl. Ohne diese Spalte gilt bei Multiple Choice die erste Antwort als richtig und der Rest als falsch.',
            pointsPerCorrectColumn: 'Punkte je Antwort',
            requiredCountColumn: 'Geforderte Anzahl',
            orderedRequiredColumn: 'Reihenfolge wichtig',
            showFurtherColumns: 'Weitere Spalten zuordnen',
            hideFurtherColumns: 'Weitere Spalten ausblenden',
            defaultType: 'Standard',
            answerSeparator: 'Trennzeichen für Antworten',
            preview: 'Vorschau',
            questionColumnRequired: 'Bitte wähle die Spalte mit den Fragen aus.',
            noQuestionsSelected: 'Es ist keine Frage zum Import ausgewählt.',
            aiCatalogContext: 'Katalog: {name}',
            importSuccess: '{count} Fragen importiert',
            generateWrongAnswers: 'Falsche Antworten per KI generieren (Multiple Choice)',
            wrongAnswerCount: 'Anzahl falscher Antworten',
            generateWrongAnswersHint: 'Die falschen Antworten werden vor der Vorschau erzeugt, damit du sie dort noch prüfen und ändern kannst.',
            generatingAnswers: 'Generiere falsche Antworten',
            noTitle: '(Ohne Titel)',
            include: 'Importieren',
            exclude: 'Überspringen',
            resplit: 'Neu aufteilen',
            noSplitItems: 'Keine Aufteilung notwendig.',
        },
        report: {
            action: 'Frage melden',
            title: 'Frage melden',
            hint: 'Sag kurz, was an der Frage nicht stimmt: eine falsche oder veraltete Antwort, eine unklare Formulierung oder zwei Antworten, die beide passen.',
            placeholder: 'Was stimmt an dieser Frage nicht?',
            submit: 'Melden',
            thanks: 'Danke, deine Anmerkung ist beim Katalog hinterlegt.',
            openCount: 'Offene Anmerkungen: {count}',
            acknowledge: 'Erledigt',
            unknownReporter: 'Unbekannt',
        },
        format: {
            show: 'Welches Format braucht die Datei?',
            hide: 'Format ausblenden',
            sheetTab: 'Tabelle (.csv)',
            fileTab: 'Katalogdatei (.json)',
            sheetIntro: 'Die erste Zeile enthält die Spaltenüberschriften, danach folgt eine Frage pro Zeile. Wie deine Spalten heißen, ist egal: du ordnest sie im nächsten Schritt selbst zu. Mehrere Antworten in einer Zelle trennst du mit einem Semikolon.',
            fileIntro: 'Eine Katalogdatei beschreibt einen ganzen Katalog. Kategorien werden über einen frei gewählten Schlüssel angesprochen, den die Fragen derselben Datei wieder verwenden.',
            required: 'Pflicht',
            optional: 'optional',
            columnsTitle: 'Spalten',
            catalogTitle: 'Block "catalog"',
            categoriesTitle: 'Liste "categories"',
            questionsTitle: 'Liste "questions"',
            typesTitle: 'Fragetypen',
            typesAnswerHint: 'Links steht der Fragetyp, in der Mitte die Schreibweisen, die die Typ-Spalte versteht, rechts ein Beispiel für die Antwort-Zelle.',
            typesConfigHint: 'Rechts steht, wie "config" für diesen Fragetyp aussieht. Die Punkte je Antwort sind überall optional.',
            downloadSheet: 'Beispieltabelle herunterladen',
            downloadFile: 'Beispieldatei herunterladen',
            downloadHint: 'Das Beispiel enthält je eine Frage pro Fragetyp und lässt sich unverändert importieren.',
            fields: {
                question: 'Der Fragetext. Zeilen ohne Fragetext werden übersprungen.',
                answer: 'Die richtige Antwort. Mehrere Antworten mit Semikolon trennen.',
                distractors: 'Falsche Antworten. Bei Lückentext wird daraus eine Auswahlliste, bei Multiple Choice gelten dann alle Antworten der Antwort-Spalte als richtig.',
                category: 'Name der Kategorie. Gleiche Namen landen in derselben Kategorie, vorhandene Kategorien der Wache werden weiterverwendet.',
                type: 'Der Fragetyp. Ohne diese Spalte gilt der Typ, den du im nächsten Schritt auswählst.',
                points: 'Punkte für die Frage. Ohne Angabe ein Punkt.',
                description: 'Zusatztext oder Hinweis unter der Frage.',
                image: 'Adresse eines Bildes, das schon im Netz erreichbar ist. Bilder werden nicht mitgeladen.',
                pointsPerCorrect: 'Punkte je richtiger Teilantwort.',
                requiredCount: 'Wie viele Antworten eine Aufzählung verlangt.',
                orderedRequired: 'Ob die Reihenfolge bei einer Aufzählung zählt (ja / nein).',
                catalogName: 'Name des Katalogs. Beim Anlegen kannst du ihn noch ändern.',
                catalogDescription: 'Beschreibung des Katalogs.',
                trainingEnabled: 'Ob Mitglieder mit dem Katalog üben dürfen.',
                language: 'Sprache der Fragen als Kürzel, etwa "de".',
                source: 'Woher die Fragen stammen.',
                author: 'Wer die Fragen geschrieben hat.',
                license: 'Unter welchen Bedingungen die Fragen genutzt werden dürfen.',
                categoryKey: 'Der Schlüssel, über den die Fragen dieser Datei die Kategorie ansprechen.',
                categoryName: 'Name der Kategorie.',
                categoryDescription: 'Beschreibung der Kategorie.',
                categoryPosition: 'Sortierung der Kategorie.',
                title: 'Der Fragetext.',
                questionType: 'Der Fragetyp, in Großbuchstaben.',
                config: 'Die Antworten der Frage. Der Aufbau hängt vom Fragetyp ab, siehe unten.',
                questionCategoryKey: 'Der Schlüssel einer Kategorie aus derselben Datei.',
                autoPoints: 'Ob die Punkte aus den Antworten berechnet werden.',
                position: 'Sortierung im Katalog.',
            },
        },
        import: {
            createTitle: 'Katalog aus Datei anlegen',
            appendTitle: 'Fragen aus Datei ergänzen',
            sourceTitle: 'Datei auswählen',
            sourceHint: 'Eine Katalogdatei (.json) oder eine Tabelle (.csv). Bei einer Tabelle wählst du im nächsten Schritt, welche Spalte was enthält.',
            newCatalogTitle: 'Neuer Katalog',
        },
        batch: {
            selected: 'ausgewählt',
            selectAll: 'Alle auswählen',
            deselectAll: 'Auswahl aufheben',
            toggleAutoPoints: 'Auto-Punkte',
            setPoints: 'Punkte setzen',
            setPointsPerCorrect: 'Punkte/korrekt',
            setCategory: 'Kategorie ändern',
            generate: 'Generieren',
            autoPointsLabel: 'Punkte automatisch berechnen',
            correctCount: 'Korrekte',
            wrongCount: 'Falsche',
            pairCount: 'Paare',
            gapCount: 'Lücken',
            sentenceCount: 'Sätze',
            action_autoPoints: 'Automatische Punkte',
            action_setPoints: 'Punkte setzen',
            action_pointsPerCorrect: 'Punkte pro korrekte Antwort',
            action_setCategory: 'Kategorie ändern',
            action_generate: 'Antworten generieren',
        },
    },
    federation: {
        addPartner: 'Partner hinzufügen',
        noPartners: 'Noch keine Föderationspartner.',
        active: 'Aktiv',
        suspended: 'Pausiert',
        pending: 'Ausstehend',
        suspend: 'Pausieren',
        resume: 'Fortsetzen',
        end: 'Beenden',
        createInvite: 'Einladung erstellen',
        createInviteHint: 'Generiere einen Einladungscode und teile ihn mit der anderen Wache.',
        generate: 'Code generieren',
        acceptInvite: 'Einladung annehmen',
        acceptInviteHint: 'Gib den Einladungscode der anderen Wache ein.',
        codePlaceholder: 'Einladungscode einfügen...',
        connect: 'Verbinden',
        connected: 'Erfolgreich verbunden!',
        refused: {
            malformed: 'Das ist kein Einladungscode. Bitte kopiere ihn noch einmal vollständig.',
            otherInstance: 'Dieser Code stammt von einer anderen Instanz und enthält keine Einladung. Über Instanzen hinweg funktioniert nur ein Einladungscode, den die andere Wache erzeugt hat.',
            hostRefused: 'Die Adresse in diesem Code darf von hier aus nicht aufgerufen werden. Sie muss eine öffentlich erreichbare HTTPS-Adresse sein.',
            remoteUnreachable: 'Die andere Instanz hat nicht geantwortet. Prüft gemeinsam, ob sie erreichbar ist, und versucht es dann noch einmal.',
            remoteTimeout: 'Die andere Instanz hat zu lange gebraucht. Versuche es später noch einmal.',
            remoteRefused: 'Die andere Instanz hat diese Wache nicht angenommen.',
            remoteStationGone: 'Die Wache, für die dieser Code erstellt wurde, gibt es auf der anderen Instanz nicht mehr.',
            contractMismatch: 'Die beiden Instanzen nutzen Föderations-Versionen, die nicht zueinander passen. Sobald beide Seiten auf demselben Stand sind, klappt die Verbindung.',
            unknownStation: 'Zu diesem Code gehört keine Wache auf dieser Instanz.',
            ownStation: 'Das ist der Code dieser Wache. Gib ihn der anderen Wache.',
            alreadyPartnered: 'Diese beiden Wachen sind bereits verbunden.',
            requestPending: 'Eine Anfrage an diese Wache wartet bereits auf Antwort.',
            spentToken: 'Dieser Code wurde bereits eingelöst. Lass dir einen neuen geben.',
            unknown: 'Der Code konnte nicht eingelöst werden.',
        },
        cap: {
            kb: 'Wiki',
            quiz: 'Quiz-Kataloge',
            protocol: 'Prüfungsprotokolle',
            event: 'Veranstaltungen',
            board: 'Boards',
            news: 'Neuigkeiten',
            inventory: 'Inventar-Ausleihe',
        },
        shared: 'Geteilt',
        partnerStation: 'Partnerwache',
        copyToStation: 'Auf meine Wache kopieren',
        manage: 'Verwalten',
        feature: 'Funktion',
        receive: 'Empfangen',
        send: 'Senden',
        version: 'Version',
        since: 'Verbunden seit',
        versionUnknown: 'Version unbekannt',
        versionUnknownHint: 'Die Föderationsversion des Partners ist noch nicht bekannt. Sie wird beim nächsten Kontakt automatisch abgeglichen; bis dahin ist die Föderation pausiert.',
        incompatible: 'Inkompatibel',
        incompatibleHint: 'Der Partner nutzt eine inkompatible Föderationsversion. Die Föderation ist pausiert, bis beide Wachen dieselbe Version verwenden.',
        partiallyCompatible: 'Teilweise kompatibel',
        partiallyCompatibleHint: 'Einzelne Funktionen nutzen eine inkompatible Version und sind pausiert. Details in der Partnerverwaltung.',
        featureIncompatible: 'Version inkompatibel - pausiert',
        featureIncompatibleHint: 'Diese Funktion nutzt beim Partner eine andere Vertragsversion und ist pausiert, bis beide Wachen dieselbe Version verwenden. Die Schalter kannst du weiterhin ändern; sie greifen wieder, sobald die Versionen übereinstimmen.',
        pairRequests: 'Föderationsanfragen',
        acceptRequest: 'Annehmen',
        requestSent: 'Föderationsanfrage gesendet! Die andere Wache muss die Anfrage bestätigen.',
    },
    discovery: {
        title: 'Wachen-Verzeichnis',
        subtitle: 'Hier findest du andere Wachen auf dieser Instanz. Du kannst eine Föderations-Verbindung anfragen, um Inhalte zu teilen.',
        empty: 'Keine Wachen im Verzeichnis gefunden.',
        connect: 'Verbinden',
        alreadyConnected: 'Verbunden',
        requestSent: 'Föderationsanfrage gesendet!',
        requestError: 'Anfrage konnte nicht gesendet werden.',
        viewStation: 'Öffentliche Seite',
        ownStation: 'Meine Wache',
        ungrouped: 'Ohne Verband',
        getCode: 'Code anfordern',
        inviteHint: 'Kopiere diesen Einladungscode und gib ihn in den Föderations-Einstellungen deiner Wache ein, um eine Verbindung herzustellen.',
        settings: {
            visibility: 'Sichtbarkeit',
            visibilityNone: 'Nicht sichtbar',
            visibilityInstance: 'Nur auf dieser Instanz',
            visibilityPublic: 'Öffentlich',
            description: 'Beschreibung',
            descriptionPlaceholder: 'Kurze Beschreibung eurer Wache...',
        },
    },
    protocol: {
        create: 'Neuer Prüfungsbogen',
        empty: 'Noch keine Prüfungsbögen erstellt.',
        noSections: 'Noch keine Abschnitte vorhanden.',
        search: 'Prüfungsbögen durchsuchen...',
        allStations: 'Alle Partnerwachen',
        name: 'Name',
        description: 'Beschreibung',
        passThreshold: 'Bestehensgrenze (Punkte)',
        passThresholdHint: 'Leer = keine Grenze',
        threshold: 'Grenze',
        deleteConfirm: 'Prüfungsbogen wirklich löschen?',
        total: 'Gesamt',
        addSection: 'Abschnitt hinzufügen',
        editSection: 'Abschnitt bearbeiten',
        addSubsection: 'Unterabschnitt',
        sectionName: 'Abschnittsname',
        maxPoints: 'Max. Punkte',
        addItem: 'Prüfpunkt hinzufügen',
        editItem: 'Prüfpunkt bearbeiten',
        itemLabel: 'Prüfpunkt',
        points: 'Punkte',
        runs: 'Prüfungsläufe',
        createRun: 'Neuer Prüfungslauf',
        noRuns: 'Noch keine Prüfungsläufe.',
        selectProtocol: 'Prüfungsbogen auswählen',
        runName: 'Name des Laufs',
        selectMembers: 'Einzelne Prüflinge',
        selectByRestriction: 'Prüflinge nach Rolle, Gruppe oder Tag',
        open: 'Offen',
        closed: 'Abgeschlossen',
        closeRun: 'Prüfung abschließen',
        completed: 'Abgeschlossen',
        locked: 'Gesperrt',
        pending: 'Ausstehend',
        grade: 'Prüfen',
        lockError: 'Prüfling ist bereits von einem anderen Prüfer gesperrt.',
        savedButNotFinished: 'Die Bewertung wurde gespeichert, aber der Schritt danach hat nicht geklappt. Speichere sie nicht noch einmal, sondern versuche den Schritt erneut.',
        saveAndExit: 'Speichern & Beenden',
        totalScore: 'Gesamtpunktzahl',
        prevSection: 'Zurück',
        nextSection: 'Weiter',
        finish: 'Prüfung abschließen',
        sections: 'Abschnitte',
        filterIncomplete: 'Nur unvollständige',
        markDone: 'Geprüft',
        unmarkDone: 'Zurücksetzen',
        markDoneAndNext: 'Geprüft & Weiter',
        markDoneAndExit: 'Geprüft & Beenden',
        evaluation: 'Auswertung',
        topic: 'Thema',
        maxPts: 'Max',
        avg: 'Ø',
        exportTable: 'Tabelle als PDF',
        exportAll: 'Alles als ZIP',
    },
    procedures: {
        createProcedure: 'Neuer Ablauf',
        name: 'Name',
        description: 'Beschreibung',
        dueDate: 'Fällig am',
        fromEvent: 'Zum Termin am {date}',
        assignees: 'Zugewiesene',
        noAssignees: 'Keine Zugewiesenen',
        open: 'Offen',
        resolved: 'Abgeschlossen',
        overdue: 'Überfällig',
        items: 'Schritte',
        progress: 'Fortschritt',
        resolve: 'Abschließen',
        reopen: 'Wieder öffnen',
        deleteConfirm: 'Ablauf wirklich löschen?',
        empty: 'Noch keine Abläufe vorhanden.',
        search: 'Abläufe durchsuchen...',
        filterAll: 'Alle',
        filterMine: 'Meine',
        createFromTemplate: 'Aus Vorlage',
        createManual: 'Manuell',
        selectTemplate: 'Vorlage auswählen',
        noTemplates: 'Keine Vorlagen vorhanden.',
        createTemplate: 'Neue Vorlage',
        editTemplate: 'Vorlage bearbeiten',
        templateName: 'Name',
        templateDescription: 'Beschreibung',
        archiveTemplate: 'Vorlage archivieren',
        archiveConfirm: 'Vorlage wirklich archivieren?',
        showArchived: 'Archivierte anzeigen',
        emptyTemplates: 'Noch keine Vorlagen vorhanden.',
        templateNotLoaded: 'Die Vorlage konnte nicht geladen werden. Das Formular ist deshalb leer geblieben.',
        addItem: 'Schritt hinzufügen',
        editItem: 'Schritt bearbeiten',
        deleteItem: 'Schritt löschen',
        itemTitle: 'Titel',
        itemDescription: 'Beschreibung',
        itemNote: 'Notiz',
        itemPublic: 'Öffentlich',
        itemPrivate: 'Intern',
        itemUserAssigned: 'Nutzer zugewiesen',
        dependencies: 'Abhängigkeiten',
        dependsOn: 'Hängt ab von',
        check: 'Erledigt',
        uncheck: 'Zurücksetzen',
        checkedBy: 'Erledigt von',
        locked: 'Gesperrt - Abhängigkeiten nicht erfüllt',
        loadFromTemplate: 'Aus Vorlage laden',
        noTemplate: 'Keine Vorlage (leer starten)',
        templateHint: 'Eine Vorlage füllt die Schritte vor. Du kannst sie vor dem Erstellen noch anpassen.',
        details: 'Details',
        publicProcedure: 'Sichtbar für Zugewiesene',
        selectAssignee: 'Person auswählen...',
        noItems: 'Noch keine Schritte hinzugefügt.',
        userCanCheck: 'Nutzer kann abhaken',
        editOnly: 'Nur durch Betreuer abhakbar',
        addDependency: 'Abhängigkeit hinzufügen...',
        untitledItem: '(Ohne Titel)',
    },
    kb: {
        allStations: 'Alle Partnerwachen',
        allTags: 'Alle Tags',
        search: 'Suchen...',
        newEntry: 'Neu',
        newFolder: 'Neuer Ordner',
        newFile: 'Neue Datei',
        newArticle: 'Artikel',
        uploadFile: 'Datei hochladen',
        addYoutube: 'YouTube-Video hinzufügen',
        addLink: 'Link hinzufugen',
        importDocument: 'Dokument importieren',
        importDocumentHint: 'Word (.docx), OpenDocument (.odt), RTF oder HTML-Dateien werden automatisch in Markdown umgewandelt.',
        importBtn: 'Importieren',
        linkUrl: 'URL',
        linkAutoFetch: 'Name und Beschreibung werden automatisch geladen',
        folderName: 'Ordnername',
        fileName: 'Dateiname',
        description: 'Beschreibung',
        noContent: 'Noch kein Inhalt vorhanden.',
        edit: 'Bearbeiten',
        preview: 'Vorschau',
        versions: 'Versionen',
        enableBlocks: 'Mit dem Seiten-Editor schreiben',
        enableBlocksHint: 'Der Text wandert in einen Textblock, den du danach aufteilst. Zurück geht es nicht: der gespeicherte Text wird von da an aus den Blöcken erzeugt.',
        revertUnavailableForBlocks: 'Bei einem Artikel aus Blöcken kannst du alte Stände ansehen, aber nicht wiederherstellen: gespeichert wird der aus den Blöcken erzeugte Text, und der würde nach dem Zurücksetzen nicht mehr zu den Blöcken passen.',
        revert: 'Zurücksetzen',
        revertConfirm: 'Möchtest du wirklich zu dieser Version zurückkehren?',
        youtubeUrl: 'YouTube-URL',
        searchResults: 'Suchergebnisse',
        noResults: 'Keine Ergebnisse gefunden.',
        root: 'Wiki',
        emptyFolder: 'Dieser Ordner ist leer.',
        deleteFolder: 'Ordner löschen',
        deleteFolderConfirm: 'Möchtest du diesen Ordner und alle Inhalte wirklich löschen?',
        deleteFile: 'Datei löschen',
        deleteFileConfirm: 'Möchtest du diese Datei wirklich löschen?',
        download: 'Herunterladen',
        version: 'Version',
        editFolder: 'Ordner bearbeiten',
        editFile: 'Datei bearbeiten',
        editMetadata: 'Eigenschaften',
        backToBrowse: 'Zurück zum Wiki',
        content: 'Inhalt',
        openLink: 'Link öffnen',
        shareLink: 'Teilen',
        restrictions: 'Zugriffsbeschränkung',
        unsavedChanges: 'Ungespeicherte Änderungen',
        goUp: 'Eine Ebene nach oben',
        tags: 'Tags',
        tagPlaceholder: 'Neues Tag...',
        share: 'Sichtbarkeit',
        reachPublic: 'Steht im öffentlichen Wiki',
        reachFederated: 'Alle verbundenen Wachen sehen das, die Öffentlichkeit nicht',
        reachNarrow: 'Geteilt, aber nicht mit allen',
        everyStation: 'Für alle Wachen des Verbands',
        everyPartner: 'Für alle verbundenen Wachen',
        reachStations: 'Welche Wachen des Verbands',
        reachPartners: 'Welche verbundenen Wachen',
        audienceNone: 'Nicht weitergeben',
        audienceSome: 'Nur ausgewählte Wachen',
        forStations: 'Nur für diese Wachen',
        shareHint: 'Wer diesen Eintrag sehen darf, und ob er im öffentlichen Wiki steht.',
        publicVisibility: 'Öffentliche Sichtbarkeit',
        publicVisibilityDefault: 'Standard (globale Einstellung)',
        publicVisibilityDefaultOpen: 'Standard: öffentlich sichtbar',
        publicVisibilityDefaultClosed: 'Standard: nicht öffentlich',
        publicVisibilityPublic: 'Öffentlich sichtbar',
        publicVisibilityHidden: 'Versteckt',
        publicVisibilityOff: 'Das öffentliche Wiki ist ausgeschaltet. Einzelne Einträge werden erst '
            + 'veröffentlicht, wenn es unter Wache verwalten eingeschaltet wird.',
        folderIcon: 'Ordner-Bild',
        lastEditedAt: 'Zuletzt bearbeitet',
        readOnlyHere: 'Diese Datei ist für dich schreibgeschützt.',
        readOnlyFrom: 'Schreibgeschützt - festgelegt im Ordner „{folder}".',
        accessLevel: 'Berechtigung',
        accessLevelHint: 'Legt für jede ausgewählte Zielgruppe einzeln fest, was sie hier tun darf. „Wie bisher" lässt die stationsweite Berechtigung entscheiden.',
        accessLevels: {
            inherit: 'Wie bisher (stationsweite Berechtigung)',
            read: 'Nur lesen',
            write: 'Lesen und bearbeiten',
            manage: 'Vollzugriff (auch löschen und freigeben)',
        },
        typeFolder: 'Ordner',
        typeArticle: 'Artikel',
        typeText: 'Text',
        typeImage: 'Bild',
        typeLink: 'Link',
        typePresentation: 'Präsentation',
        typeFile: 'Datei',
        federatedContentUnavailable: 'Diese Datei einer Partnerwache lässt sich hier nicht anzeigen. Kopiere sie in deine Wache, um sie zu öffnen.',
        downloadPdf: 'Als PDF',
        downloadFile: 'Herunterladen',
        itemActions: 'Aktionen',
        listView: 'Listenansicht',
        gridView: 'Kachelansicht',
        initialVersion: 'Erstversion (vollständiger Inhalt)',
        relatedFiles: 'Weiterführende Dateien',
        backlinks: 'Wird verwiesen von',
        searchRelated: 'Datei suchen...',
        noFilesFound: 'Keine Dateien gefunden',
        move: 'Verschieben',
        moveHint: 'Wohin soll „{name}"?',
        movePickerSearch: 'Ordner suchen...',
        moveNoFolders: 'Keine Ordner gefunden',
        moveTargetReadOnly: 'In diesem Ordner darfst du nichts anlegen.',
        moveReachInternal: 'nur innerhalb der Wache',
        moveReachNarrow: 'nur für einen Teil der Leser',
        moveReachFederated: 'für alle verbundenen Wachen',
        moveReachPublic: 'im öffentlichen Wiki',
        moveReachUnchanged: 'Sichtbar bleibt der Eintrag {reach}.',
        moveReachChanges: 'Nach dem Verschieben ist der Eintrag {reach} sichtbar.',
        moveReachWidens: 'Achtung: nach dem Verschieben ist der Eintrag {reach} sichtbar.',
        startSelecting: 'Mehrere auswählen',
        stopSelecting: 'Auswahl beenden',
        selectedCount: '{count} ausgewählt',
        clearSelection: 'Auswahl aufheben',
        tagSelected: 'Taggen',
        bulkMoveHint: '{count} Einträge verschieben. Alles in den ausgewählten Ordnern kommt mit.',
        bulkTagHint: 'Tags für {count} Einträge. Vorhandene Tags bleiben erhalten.',
        bulkTagsAdd: 'Hinzufügen',
        bulkTagsRemove: 'Entfernen',
        bulkMoved: '{count} verschoben.',
        bulkTagged: '{count} getaggt.',
        bulkDeleted: '{count} in den Papierkorb gelegt.',
        bulkDelete: 'Auswahl löschen',
        bulkDeleteCounting: 'Wird gezählt...',
        bulkDeleteHint: 'Das betrifft {folders} Ordner und {files} Einträge, alles in den Ordnern eingerechnet.',
        bulkDeleteRecoverable: 'Alles landet im Papierkorb und lässt sich von dort zurückholen.',
        deleteRecoverable: 'Der Eintrag landet im Papierkorb und lässt sich von dort zurückholen.',
        deleteEmbeddedOn: 'Der Artikel ist auf diesen Seiten eingebunden: {pages}. Dort steht danach nur noch ein Ersatztitel.',
        deleteEmbeddedOnPublic: 'Achtung: Der Artikel steht auf diesen veröffentlichten Seiten: {pages}. Dort bleibt danach nur ein Ersatztitel stehen, den niemand von außen als Fehler erkennt.',
        trash: 'Papierkorb',
        trashHint: 'Hier liegt, was du gelöscht hast. Du siehst nur Einträge, die du auch löschen durftest.',
        trashSize: 'Belegt {size}',
        trashNothing: 'Der Papierkorb ist leer.',
        trashEmpty: 'Papierkorb leeren',
        trashEmptyConfirm: 'Alles endgültig löschen? Das gibt {size} wieder frei und lässt sich nicht zurücknehmen.',
        trashEmptied: '{count} endgültig gelöscht.',
        trashRestore: 'Wiederherstellen',
        trashRestored: '„{name}" ist wieder da.',
        trashRestoredToRoot: '„{name}" ist wieder da, oben im Wiki: der ursprüngliche Ordner liegt selbst im Papierkorb.',
        trashPurge: 'Endgültig löschen',
        trashPurgeConfirm: '„{name}" endgültig löschen? Das lässt sich nicht zurücknehmen.',
        trashPurged: '„{name}" ist endgültig gelöscht.',
        trashDeletedAt: 'Gelöscht am {when}.',
        trashDeletedBy: 'Von {name}.',
        trashContains: 'Mit {count} Einträgen darin.',
        trashHolds: 'Belegt {size}.',
        bulkTargetRefused: 'In diesem Zielordner darfst du nichts anlegen.',
        refusalNoPermission: '„{name}": dazu fehlt dir die Berechtigung.',
        refusalNameTaken: '„{name}": im Zielordner gibt es schon einen Ordner mit diesem Namen.',
        refusalTargetInside: '„{name}": der Zielordner liegt in diesem Ordner.',
        refusalShareTooWide: '„{name}": der Eintrag ist weiter freigegeben als der Zielordner reicht.',
        refusalNotFound: '„{name}": nicht mehr vorhanden.',
        refusalUnnamed: 'Ein Eintrag',
        refusalMore: 'Und {count} weitere.',
        favourites: 'Favoriten',
        removeFavourite: 'Favorit entfernen',
        addFavourite: 'Zu Favoriten',
        entryCount: '{count} Einträge',
        favouriteUnavailable: 'Das lässt sich gerade nicht als Favorit merken. Vielleicht teilt die Partnerwache es nicht mehr.',
        favouritesLoadFailed: 'Deine Favoriten konnten nicht geladen werden.',
        presentationLoadFailed: 'Die Präsentation konnte nicht geladen werden.',
        copyToStationFailed: 'Der Artikel konnte nicht in das eigene Wiki übernommen werden.',
        saveContentFailed: 'Der Inhalt konnte nicht gespeichert werden.',
        reuploadFailed: 'Die Datei konnte nicht ersetzt werden.',
        tagsLoadFailed:'Die vorhandenen Schlagwörter konnten nicht geladen werden. Sie bleiben beim Speichern unverändert.',
        audienceChoicesFailed: 'Gruppen und Schlagwörter konnten nicht geladen werden, deshalb ist die Auswahl unvollständig.',
        audienceLoadFailed: 'Die aktuelle Sichtbarkeit konnte nicht geladen werden.',
        audienceNotSaved: 'Die Sichtbarkeit wurde nicht gespeichert, weil unklar ist, wie sie bisher eingestellt war.',
        audienceNotSavedGuidance: 'Sonst würde hier eine leere Auswahl über die bestehende geschrieben. Schließe das Fenster, lade die Seite neu und öffne es erneut.',
        presentationRenderFailed: 'Die Präsentation ist da, lässt sich hier aber nicht anzeigen.',
        presentationRenderFailedGuidance: 'Ein zweiter Versuch ändert daran nichts. Lade die Datei herunter und öffne sie mit einem Programm auf deinem Gerät.',
        pdfExportFailed: 'Der PDF-Export hat nicht geklappt.',
        downloadFailed: 'Die Datei konnte nicht heruntergeladen werden.',
        versionsLoadFailed: 'Der Versionsverlauf konnte nicht geladen werden.',
        partnerNoLongerShares: 'Diese Datei der Partnerwache lässt sich gerade nicht öffnen. Teilt die Partnerwache sie nicht mehr mit euch, kannst du sie aus deinen Favoriten entfernen.',
        present: 'Präsentieren',
        downloadOriginal: 'Original herunterladen',
        reupload: 'Erneut hochladen',
        conversionPending: 'Die Datei wird in ein PDF umgewandelt. Bitte warte einen Moment...',
        conversionFailed: 'Die Umwandlung ist fehlgeschlagen. Bitte lade die Datei erneut hoch.',
    },
    lending: {
        availableUnreadable: 'Es konnte nicht gelesen werden, welche Stücke gerade frei sind. Die Auswahl unten ist deshalb leer, obwohl vielleicht welche verfügbar sind.',
        tabs: {
            offers: 'Angebote',
            requests: 'Anfragen',
        },
        searchInventory: 'Inventar durchsuchen...',
        availableItems: 'Verfügbare Ausrüstung',
        noAvailable: 'Keine Ausrüstung von Partnerwachen verfügbar.',
        nothingShared: 'Derzeit teilt keine Partnerwache etwas mit eurer Wache.',
        nothingFree: 'Nichts frei im gewählten Zeitraum.',
        requestItem: 'Anfragen',
        available: 'verfügbar',
        blocks: 'Sperrzeiten',
        incoming: 'Eingehende Anfragen',
        outgoing: 'Ausgehende Anfragen',
        noIncoming: 'Keine eingehenden Anfragen.',
        noOutgoing: 'Keine ausgehenden Anfragen.',
        loadError: 'Fehler beim Laden der Daten.',
        createError: 'Fehler beim Erstellen der Anfrage.',
        backToList: 'Zurück zur Übersicht',
        requestingStation: 'Anfragende Wache',
        owningStation: 'Besitzende Wache',
        dateFrom: 'Von',
        dateTo: 'Bis',
        items: 'Gegenstände',
        itemType: 'Typ',
        quantity: 'Anzahl',
        noItems: 'Keine Gegenstände angegeben.',
        approve: 'Genehmigen',
        decline: 'Ablehnen',
        markReturned: 'Als zurückgegeben markieren',
        close: 'Abschließen',
        chat: 'Nachrichten',
        noMessages: 'Noch keine Nachrichten.',
        messagePlaceholder: 'Nachricht schreiben...',
        declineTitle: 'Anfrage ablehnen',
        declineReasonPlaceholder: 'Grund für die Ablehnung (optional)',
        addBlock: 'Sperrzeit hinzufügen',
        noBlocks: 'Keine Sperrzeiten eingetragen.',
        blockFrom: 'Gesperrt von',
        blockTo: 'Gesperrt bis',
        blockReason: 'Grund',
        blockReasonPlaceholder: 'z.B. Eigene Veranstaltung',
        blockScope: 'Inventare',
        blockScopeAll: 'Gesamtes Inventar',
        blockScopeHint: 'Ohne Inventarauswahl wird das gesamte Inventar gesperrt. Füge Inventare hinzu, um gezielt zu sperren.',
        blockItemAll: 'Alle Gegenstände',
        selectInventory: 'Inventar auswählen...',
        addInventory: 'Hinzufügen',
        noAvailableItems: 'Keine verfügbaren Gegenstände.',
        createRequest: 'Neue Anfrage',
        note: 'Bemerkung',
        notePlaceholder: 'Optionale Bemerkung zur Anfrage',
        sendRequest: 'Anfrage senden',
        assignItems: 'Ausrüstung zuweisen',
        selectItemsToLend: 'Wähle die Gegenstände aus, die du ausgeben möchtest:',
        lendOut: 'Ausgeben',
        overdue: 'Überfällig',
        itemsSelected: 'Gegenstände ausgewählt',
        status: {
            REQUESTED: 'Angefragt',
            APPROVED: 'Genehmigt',
            DECLINED: 'Abgelehnt',
            LENT: 'Ausgeliehen',
            RETURNED: 'Zurückgegeben',
            CLOSED: 'Abgeschlossen',
        },
    },
    publicKb: {
        search: 'Suche...',
        backToBrowse: 'Zurück',
        noContent: 'Kein Inhalt vorhanden.',
        openLink: 'Link öffnen',
        download: 'Herunterladen',
        empty: 'Keine Einträge vorhanden.',
    },
    beacon: {
        settingsTitle: 'Beacon-Einstellungen',
        settingsHint: 'Standardmäßig verlässt nichts die Instanz. Was hier eingeschaltet wird, gilt sofort und übersteht einen Neustart.',
        settingsSaved: 'Gespeichert.',
        enabled: 'An einen Beacon melden',
        url: 'Beacon-Adresse',
        forwardProblems: 'Neue Fehler automatisch weitergeben',
        forwardReports: 'Problemmeldungen automatisch weitergeben',
        metricsEnabled: 'Täglich anonyme Nutzungsmetriken senden.',
        receiving: 'Meldungen anderer Instanzen entgegennehmen',
        contactName: 'Kontaktname',
        contactMail: 'Kontaktadresse',
        contactHint: 'Deine eigenen Angaben, nicht die eines Mitglieds. Sie reisen mit jeder Meldung mit, damit ein Beacon zurückschreiben kann.',
        previewTitle: 'Was gesendet wird',
        pictureGoesWithIt: 'Dieses Bild geht mit der Meldung mit. Decke hier ab, was die Instanz nicht verlassen soll. Was einmal ohne Bild gesendet wurde, bekommt später keines mehr.',
        sendWithoutPicture: 'Ohne Bild senden',
        reviewReportPictures: 'Bilder vor dem Weitergeben ansehen',
        reviewReportPicturesHint: 'Meldungen mit Bild warten, bis jemand hier sie weitergibt. Ausgeschaltet gehen Meldung und Bild sofort zusammen raus.',
        previewHint: 'Genau diese Angaben verlassen die Instanz. Fehlermeldungen zitieren, was schiefging, und das ist manchmal eine Adresse oder ein Name.',
        send: 'Senden',
        sendOne: 'An Beacon senden',
        sendSelected: 'Ausgewählte senden ({count})',
        queued: '{count} an den Beacon übergeben.',
        notQueued: 'Konnte nicht übergeben werden: die Warteschlange der Instanz ist voll.',
        notQueuedGuidance: 'Versuche es in ein paar Minuten erneut. Es ist nichts kaputt, die Warteschlange arbeitet sich gerade ab.',
        day: 'Tag',
        subject: 'Art',
        accounts: 'Konten',
        stations: 'Wachen',
        instancesAffected: '{count} Instanzen',
        versionsSeen: 'Versionen',
        version: 'Version',
        resolvedIn: 'Behoben in',
        markResolved: 'Als behoben markieren',
        noFaults: 'Keine offenen Fehler anderer Instanzen.',
        noReports: 'Keine offenen Problemmeldungen anderer Instanzen.',
        noMetrics: 'Bisher hat keine Instanz Zahlen gemeldet.',
        notABeacon: 'Diese Instanz nimmt keine Meldungen entgegen.',
        contact: 'Kontakt',
        unknownInstance: 'Unbekannte Instanz',
    },
    adminProblems: {
        errors: 'Fehler',
        warnings: 'Warnungen',
        noProblems: 'Keine Probleme erkannt.',
        acknowledge: 'Bestätigen',
        acknowledgeAll: 'Alle bestätigen',
        showAcknowledged: 'Bestätigte anzeigen',
        messages: 'Nachrichten',
        copyStacktrace: 'Stacktrace kopieren',
    },
    adminDataTracking: {
        devOnlyNotice: 'Nur im Dev-Modus verfügbar',
        devOnlyDisabled: 'Diese Ansicht steht nur zur Verfügung, wenn der Server im Dev-Modus läuft.',
        totalTables: 'Tabellen gesamt',
        columnsVerified: 'Spalten verifiziert',
        needsReview: 'Zu prüfen',
        schemaHash: 'Schema-Hash',
        statusBreakdown: 'Status-Übersicht',
        stationTransfer: 'Stations-Transfer',
        gdprExport: 'DSGVO-Export',
        gdprDeletion: 'DSGVO-Löschung',
        tables: 'Tabellen',
        colName: 'Tabelle',
        searchPlaceholder: 'Tabelle suchen…',
        noMatches: 'Keine Treffer.',
        unverifiedColumns: 'Unverifizierte Spalten',
        strategyTooltip: 'DSGVO-Löschstrategie: {strategy}',
        cascadeFromTooltip: 'CASCADE über FK an {table}; Strategie dort: {effective}',
        cascadeMisleading: '⚠ Achtung: Der CASCADE feuert nur, wenn die Eltern-Zeile gelöscht wird. Da die übergeordnete Strategie keine Löschung ist, bleibt diese Zeile bei einer DSGVO-Löschung bestehen.',
        foreignKeyHint: 'Fremdschlüsselspalte',
        danglingHeader: 'Achtung: {n} Identitäts-Spalte(n) ohne Fremdschlüssel zu station_member - bei lokalen Mitgliedern besonders kritisch, da gelöschte Mitglieder keine FK-CASCADE auslösen.',
        context: {
            all: 'Alle Kontexte',
            transfer: 'Transfer',
            gdprExport: 'DSGVO-Export',
            gdprDeletion: 'DSGVO-Löschung',
        },
        filter: {
            all: 'Alle Status',
            TRACKED: 'Erfasst',
            IGNORED: 'Ignoriert',
            UNVERIFIED: 'Unverifiziert',
            NEEDS_REVIEW: 'Prüfen',
        },
        batch: {
            selected: '{n} ausgewählt',
            setStatusOn: 'Status setzen auf',
            apply: 'Anwenden',
            selectAllFiltered: 'Alle gefilterten auswählen',
            clear: 'Auswahl leeren',
            toggleRow: 'Zur Auswahl hinzufügen',
        },
        detail: {
            columns: 'Spalten',
            verified: 'Verifiziert',
            verifiedShort: 'verifiziert',
            verifyAll: 'Alle als verifiziert markieren',
            foreignKeys: 'Fremdschlüssel',
            lookups: 'Lookups',
            customScope: 'Custom-Scope',
            reason: 'Begründung',
            rationale: 'Anmerkung',
            reviewNote: 'Hinweis für die Prüfung (z. B. „nach Schema-Refresh erneut prüfen")',
            ignoredColumns: 'Ignorierte Spalten (kommagetrennt)',
            ignoredColumnsPlaceholder: 'spalte1, spalte2, …',
            identityColumns: 'Identitäts-Spalten',
            strategies: 'Löschstrategien',
            addStrategy: 'Strategie hinzufügen',
            noStrategies: 'Noch keine Strategien definiert.',
            strategyColumn: 'Spalte',
            strategyKind: 'Strategie',
            legalBasis: 'Rechtsgrundlage',
            legalBasisPlaceholder: 'z. B. handelsrechtliche Aufbewahrungspflicht (§257 HGB)',
        },
    },
    apiStatus: {
        totalRequests: 'Anfragen (3 Tage)',
        avgResponseTime: 'Ø Antwortzeit',
        serverErrors: 'Serverfehler',
        requestsOverTime: 'Anfragen über Zeit',
        statusCodes: 'Statuscodes',
        slowest: 'Langsamste',
        fastest: 'Schnellste',
        failing: 'Fehlerhaft',
        method: 'Methode',
        endpoint: 'Endpunkt',
        count: 'Anzahl',
        avg: 'Ø',
        min: 'Min',
        max: 'Max',
        errorRate: 'Fehlerrate',
        requests: 'Anfragen',
        errors: 'Fehler',
        recentRequests: 'Letzte 100 Anfragen',
        responseTimeLine: 'Antwortzeit',
        timestamp: 'Zeitpunkt',
        statusCode: 'Statuscode',
        duration: 'Dauer',
    },
    traffic: {
        admin: {
            subtitle: 'Eingehender und ausgehender Datenverkehr pro Wache und pro Auth-Klasse.',
        },
        station: {
        },
        controls: {
            window: 'Zeitraum',
            metric: 'Kennzahl',
            bucket: 'Zugriffsart',
        },
        window: {
            last24h: 'Letzte 24 Stunden',
            last3d: 'Letzte 3 Tage',
            last7d: 'Letzte 7 Tage',
            last30d: 'Letzte 30 Tage',
        },
        metric: {
            ingress: 'Eingang (Bytes)',
            egress: 'Ausgang (Bytes)',
            requests: 'Anfragen',
            inout: 'Ein- und Ausgang',
        },
        bucket: {
            ALL: 'Alle',
            AUTHENTICATED: 'Angemeldet',
            UNAUTHENTICATED: 'Öffentlich',
            FEDERATION: 'Föderation',
        },
        totals: {
            ingress: 'Eingang insgesamt',
            egress: 'Ausgang insgesamt',
            requests: 'Anfragen insgesamt',
        },
        unitBytes: 'Bytes',
        unitRequests: 'Anfragen',
        chart: {
            title: 'Verlauf',
            hint: 'Pro Stunde aufgeschlüsselt nach Zugriffsart. Bei mehr als 24 Stunden lässt sich der Bereich zoomen.',
            stationHint: 'Pro Stunde aufgeschlüsselt nach Zugriffsart deiner Wache.',
            directionBucket: {
                in: 'Eingang · {bucket}',
                out: 'Ausgang · {bucket}',
            },
        },
        stationLeaderboard: {
            title: 'Top-Wachen im Zeitraum',
            hint: 'Sortiert nach der gewählten Kennzahl. „Global" sind anonyme Anfragen ohne eindeutige Wache-Zuordnung (Admin-Routen, statische Inhalte).',
            station: 'Wache',
            global: 'Global',
        },
        noData: 'Für den gewählten Zeitraum liegen noch keine Daten vor.',
        help: 'Hilfe',
    },
    stationFeeds: {
        help: 'Hilfe',
        total: 'Eingerichtete Zugänge',
        activeLastWeek: 'In den letzten 7 Tagen abgerufen',
        searchPlaceholder: 'Nach Namen suchen',
        noneFound: 'Niemand hat bisher einen Zugang eingerichtet.',
        never: 'Nie',
        inUse: 'In Benutzung',
        dormant: 'Ruht',
        column: {
            member: 'Mitglied',
            since: 'Eingerichtet',
            calendar: 'Kalender zuletzt',
            notifications: 'Benachrichtigungen zuletzt',
            state: 'Status',
        },
    },
    insights: {
        title: 'Öffentliche Seiten - Statistik',
        subtitle: 'Aggregierte Zugriffe pro öffentlicher Seite. Keine IPs, keine Cookies, keine eindeutigen Besucher.',
        help: 'Hilfe',
        controls: {
            window: 'Zeitraum',
            includeBots: 'Bots mitzählen',
        },
        window: {
            last24h: 'Letzte 24 Stunden',
            last7d: 'Letzte 7 Tage',
            last30d: 'Letzte 30 Tage',
            last90d: 'Letzte 90 Tage',
            last365d: 'Letztes Jahr',
        },
        totals: {
            hits: 'Aufrufe insgesamt',
            pagesWithHits: 'Seiten mit Aufrufen',
            publicPages: 'Öffentliche Seiten',
        },
        leaderboard: {
            title: 'Top-Seiten',
            subtitle: 'Aufrufe pro Seite im gewählten Zeitraum. Klicke auf „Details" für die Aufschlüsselung.',
        },
        table: {
            page: 'Seite',
            hits: 'Aufrufe',
            botHits: 'Bot-Aufrufe',
            details: 'Details',
        },
        detail: {
            title: 'Detail: {page}',
            countries: 'Länder',
            referrers: 'Verweise',
        },
        chart: {
            hits: 'Aufrufe',
        },
        unitHits: 'Aufrufe',
        noData: 'Für den gewählten Zeitraum liegen noch keine Daten vor.',
        noPages: 'Diese Wache hat noch keine öffentlichen Seiten.',
    },
    feedMetrics: {
        subtitle: 'Renderzeiten, Volumen und Reader-Statistik der persönlichen Feeds (iCal, RSS, Atom).',
        help: 'Hilfe',
        totalRequests: 'Anfragen gesamt',
        totalRendered: 'Vollständig gerendert',
        cacheHits: 'Cache-Treffer (304)',
        avgDuration: 'Ø Renderzeit',
        requestsByType: 'Anfragen pro Feed-Typ',
        latencyHistogram: 'Latenz-Verteilung',
        latencyHistogramHint: 'So viele Renders fielen in den jeweiligen Zeitbereich. Grün ist schnell, rot ist langsam.',
        dailyVolume: 'Tägliches Volumen',
        statusBreakdown: 'Statuscodes',
        topUserAgents: 'Top Reader (User-Agents)',
        topUserAgentsHint: 'Globale Aggregation. Keine Zuordnung zu einzelnen Tokens oder Mitgliedern.',
        userAgent: 'User-Agent',
        requests: 'Anfragen',
        status: 'Status',
        count: 'Anzahl',
        lastSeen: 'Zuletzt',
        noData: 'Noch keine Daten gesammelt.',
    },
    linkAccess: {
        deadGuidance: 'Prüfe, ob du den Link vollständig kopiert hast. Wenn er nicht mehr gilt, frag dort nach einem neuen, wo du ihn bekommen hast.',
    },
    legal: {
        unavailable:'Dieses Dokument lässt sich gerade nicht anzeigen.',
        unavailableGuidance: 'Das liegt nicht an dir und nicht an der Adresse: der Text konnte nicht geladen werden. Versuche es in ein paar Minuten noch einmal.',
    },
    publicStation: {
        calendar: 'Kalender',
        knowledgeBase: 'Wiki',
        subscribeCal: 'Kalender abonnieren',
        icalHint: 'Füge diese URL in deiner Kalender-App als Abo hinzu, um automatisch neue Termine zu erhalten.',
        noEvents: 'Keine öffentlichen Termine.',
        backToDiscovery: 'Zurück zur Übersicht',
        waitlist: 'Warteliste',
        blog: 'Blog',
        blogTitle: 'Neuigkeiten',
        blogNoEntries: 'Noch keine Blog-Beiträge.',
        blogBy: 'von',
        stationGone: 'Diese Wache hat keine öffentliche Seite (mehr).',
        stationGoneGuidance: 'Entweder wurde die Seite abgeschaltet, oder die Adresse stimmt nicht ganz. Prüfe den Link, mit dem du hergekommen bist.',
        pageGone: 'Diese Seite gibt es nicht (mehr).',
        pageGoneGuidance: 'Vielleicht wurde sie zurückgezogen oder umbenannt. Über die Startseite der Wache findest du, was es aktuell gibt.',
        articleGone: 'Diesen Artikel gibt es nicht (mehr).',
        articleGoneGuidance: 'Er wurde zurückgezogen oder ist nicht mehr öffentlich. Im Wiki der Wache findest du, was aktuell frei zugänglich ist.',
        knowledgeBaseGone: 'Diese Wache hat kein öffentliches Wiki (mehr).',
        knowledgeBaseGoneGuidance: 'Entweder wurde es abgeschaltet, oder die Adresse stimmt nicht ganz. Auf der Startseite der Wache siehst du, was öffentlich ist.',
        blogEntryGone: 'Diesen Beitrag gibt es nicht (mehr).',
        blogEntryGoneGuidance: 'Er wurde zurückgezogen oder gelöscht. In der Übersicht der Neuigkeiten findest du die aktuellen Beiträge.',
        meta: {
            station: 'Öffentliche Seite der Wache {station}.',
            calendar: 'Alle öffentlichen Termine und Veranstaltungen der Wache {station} auf einen Blick.',
            blog: 'Neuigkeiten und Beiträge der Wache {station}.',
            blogEntry: 'Ein Beitrag aus dem Blog der Wache {station}.',
            knowledgeBase: 'Öffentliches Wiki der Wache {station}.',
            knowledgeBaseFolder: '{folder} im öffentlichen Wiki der Wache {station}.',
            knowledgeBaseFile: '{name} im öffentlichen Wiki der Wache {station}.',
            waitlist: 'Auf die Warteliste der Wache {station} setzen lassen.',
            form: 'Ein Formular der Wache {station} ausfüllen.',
        },
    },
    registrationStats: {
        title: 'Anmelde-Statistik',
        allCategories: 'Alle Kategorien',
        noData: 'Keine Statistiken vorhanden.',
        member: 'Mitglied',
        registered: 'Angemeldet',
        accepted: 'Bestätigt',
        denied: 'Abgelehnt',
        rate: 'Quote',
        score: 'Fairness',
        priority: 'Priorität',
        priorityHigh: 'Hoch',
        priorityMedium: 'Mittel',
        priorityLow: 'Niedrig',
        lastYear: 'Letzte 12 Monate',
        thisYear: 'Dieses Kalenderjahr',
    },
    restriction: {
        userTypes: 'Mitgliedstyp',
        groups: 'Gruppen',
        tags: 'Tags',
        members: 'Mitglieder',
        and: 'UND',
        or: 'ODER',
        reset: 'Zurücksetzen',
        noRestrictions: 'Keine Einschränkungen - für alle sichtbar.',
    },
    categoryManage: {
        title: 'Kategorien verwalten',
        create: 'Kategorie erstellen',
        edit: 'Kategorie bearbeiten',
        name: 'Name',
        namePlaceholder: 'z.B. Übungen',
        maxShownLabel: 'Max. angezeigte Termine',
        maxShownHint: '0 oder leer = alle Termine anzeigen',
        maxShown: 'max. {count}',
        empty: 'Keine Kategorien vorhanden.',
        deleteConfirmTitle: 'Kategorie löschen',
        deleteConfirmText: 'Soll die Kategorie „{name}" wirklich gelöscht werden? Termine werden nicht gelöscht, nur die Zuordnung entfernt.',
        public: 'Öffentlich sichtbar',
        color: 'Farbe',
        colorHint: 'Wird im Kalender als Hintergrund der Termine dieser Kategorie verwendet.',
    },
    eventTemplates: {
        duplicate: 'Duplizieren',
        copyOf: '{name} (Kopie)',
        title: 'Termin-Vorlagen',
        create: 'Vorlage erstellen',
        edit: 'Vorlage bearbeiten',
        name: 'Vorlagenname',
        namePlaceholder: 'z.B. Standard-Übung',
        eventTitle: 'Termintitel',
        eventTitlePlaceholder: 'Wird beim Anwenden als Titel übernommen',
        eventDescription: 'Beschreibung',
        category: 'Kategorie',
        noCategory: 'Keine Kategorie',
        eventType: 'Termintyp',
        noDefault: 'Kein Standard',
        empty: 'Keine Vorlagen vorhanden.',
        eventDefaults: 'Termin-Voreinstellungen',
        attendanceTemplate: 'Anwesenheitsvorlage',
        restriction: 'Zielgruppe',
        restrictionHint: 'Wer für Termine aus dieser Vorlage eingeplant wird. Beim Anwenden der Vorlage wird die '
            + 'Auswahl in den Termin übernommen und kann dort noch geändert werden.',
        viewRestriction: 'Sichtbarkeit',
        viewRestrictionHint: 'Wer von Terminen aus dieser Vorlage überhaupt erfahren darf. Ohne Auswahl alle. '
            + 'Wird beim Anwenden ebenfalls in den Termin übernommen.',
        applied: 'Vorlage angewendet.',
        loadTemplate: 'Vorlage laden...',
    },
    eventFields: {
        name: 'Feldname',
        namePlaceholder: 'z.B. Treffpunkt',
        type: 'Typ',
        typeString: 'Text',
        typeNumber: 'Zahl',
        typeTime: 'Uhrzeit',
        typeDate: 'Datum',
        typeBoolean: 'Ja/Nein',
        typeEnum: 'Auswahl',
        typeUrl: 'URL',
        typeTextarea: 'Langtext',
        typeLocation: 'Ort',
        typeMember: 'Mitglied',
        typeMemberList: 'Mitgliederliste',
        typeMemberOfGroup: 'Mitglied aus Gruppe',
        typeMemberListOfGroup: 'Mitgliederliste aus Gruppe',
        typeMemberOfType: 'Mitglied eines Benutzertyps',
        typeMemberListOfType: 'Mitgliederliste eines Benutzertyps',
        typeMemberOfTag: 'Mitglied mit Tag',
        typeMemberListOfTag: 'Mitgliederliste mit Tag',
        enumOptions: 'Optionen',
        overview: 'Übersicht',
        group: 'Gruppe',
        selectGroup: 'Gruppe wählen',
        userType: 'Benutzertyp',
        selectUserType: 'Benutzertyp wählen',
        tag: 'Tag',
        selectTag: 'Tag wählen',
        selfRegistration: 'Selbsteintragung erlauben',
        perDate: 'Pro Termin beantworten',
        perDateHint: 'Bei einer Terminserie hat jeder Termin eine eigene Antwort. Ohne Haken gilt eine Antwort für die ganze Serie.',
        perDateKeptAnswers: 'Bereits erfasste Antworten je Termin bleiben erhalten und gelten wieder, sobald der Haken zurückgesetzt wird.',
        selfRegister: 'Eintragen',
        removeSelf: 'Austragen',
        saveValue: 'Eintragung speichern',
        valueSaved: 'Gespeichert.',
        valueSaveFailed: 'Speichern fehlgeschlagen.',
        slotTaken: 'Belegt',
        selfRegisterConflict: 'Der Platz ist bereits vergeben.',
        selfRegisterForbidden: 'Du erfüllst die Voraussetzung für dieses Feld nicht.',
        selfRegisterFailed: 'Eintragung fehlgeschlagen.',
        attendanceLink: 'Anwesenheitsfeld',
        fromAttendance: 'Aus der Anwesenheitsvorlage übernehmen:',
        takeAllFromAttendance: 'Alle übernehmen',
        value: 'Wert',
        defaultValue: 'Standardwert',
        public: 'Öffentlich',
        addField: 'Feld hinzufügen',
    },
    batchCreate: {
        title: 'Termine erstellen',
        stepFields: 'Felder definieren',
        applyTemplate: 'Termin-Vorlage laden',
        stepSchedule: 'Zeitplan oder Import',
        stepTable: 'Termine bearbeiten',
        schedule: 'Zeitplan',
        csvImport: 'CSV-Import',
        intervalType: 'Intervall',
        dayOfWeek: 'Wochentag',
        startDate: 'Von',
        endDate: 'Bis',
        startTime: 'Startzeit',
        endTime: 'Endzeit',
        ignoreBreaks: 'Pausen ignorieren',
        generateDates: 'Termine generieren',
        uploadCsv: 'CSV-Datei hochladen',
        mapColumns: 'Spalten zuordnen',
        dateFormat: 'Datumsformat',
        dateFormatAuto: 'Automatisch erkennen',
        unmapped: 'Nicht zugeordnet',
        setAll: 'Alle setzen',
        removeRow: 'Zeile entfernen',
        create: 'Termine erstellen',
        creating: 'Erstelle Termine...',
        eventsCount: '{count} Termine werden erstellt',
        success: '{count} Termine erstellt',
        noRows: 'Keine Termine vorhanden.',
        eventName: 'Terminname',
        date: 'Datum',
        monday: 'Montag',
        tuesday: 'Dienstag',
        wednesday: 'Mittwoch',
        thursday: 'Donnerstag',
        friday: 'Freitag',
        saturday: 'Samstag',
        sunday: 'Sonntag',
    },
    failure: {
        report: 'Diesen Fehler melden',
        staleAfterAction: 'Das hat geklappt, aber die Ansicht konnte danach nicht aktualisiert werden.',
        OFFLINE: {
            message: 'Keine Verbindung zum Server.',
            guidance: 'Prüfe deine Internetverbindung und versuche es dann noch einmal. Wenn andere Seiten funktionieren, ist Ember gerade nicht erreichbar und du kannst es in ein paar Minuten erneut versuchen.',
        },
        TIMEOUT: {
            message: 'Der Server hat zu lange nicht geantwortet.',
            guidance: 'Versuche es gleich noch einmal. Deine Eingaben sind dabei möglicherweise schon gespeichert worden, prüfe das also, bevor du sie ein zweites Mal machst. Passiert das öfter, melde es bitte.',
        },
        SIGNED_OUT: {
            message: 'Deine Sitzung ist abgelaufen.',
            guidance: 'Melde dich erneut an. Danach kannst du dort weitermachen, wo du warst.',
        },
        DENIED: {
            message: 'Dafür fehlt dir die Berechtigung.',
            guidance: 'Das ist kein Fehler in Ember: Rechte vergibt deine Wache. Wenn du meinst, du solltest das dürfen, wende dich an die Leitung deiner Wache.',
        },
        GONE: {
            message: 'Das gibt es nicht mehr.',
            guidance: 'Vermutlich wurde es gelöscht oder verschoben, während diese Seite offen war. Lade die Seite neu, dann siehst du den aktuellen Stand.',
        },
        CONFLICT: {
            message: 'In der Zwischenzeit hat es jemand anderes geändert.',
            guidance: 'Lade die Seite neu und schau dir den aktuellen Stand an, bevor du deine Änderung noch einmal machst. Sonst überschreibst du die Arbeit der anderen Person.',
        },
        REJECTED: {
            message: 'Die Eingabe wurde nicht angenommen.',
            guidance: 'Prüfe die markierten Felder und versuche es dann noch einmal.',
        },
        TOO_LARGE: {
            message: 'Die Datei ist größer als erlaubt.',
            guidance: 'Wähle eine kleinere Datei, oder verkleinere sie vorher. Wie viel deine Wache aufbewahren darf, steht unter Verwalten.',
        },
        TOO_OFTEN: {
            message: 'Zu viele Versuche in kurzer Zeit.',
            guidance: 'Warte einen Moment und versuche es dann noch einmal.',
        },
        SERVER_FAULT: {
            message: 'Auf dem Server ist etwas schiefgelaufen.',
            guidance: 'Das ist ein Fehler in Ember und nicht deine Schuld. Bitte melde ihn und schreibe kurz dazu, was du gerade getan hast, dann lässt er sich finden.',
        },
        UNKNOWN: {
            message: 'Etwas hat nicht funktioniert, und Ember kann nicht sagen was.',
            guidance: 'Versuche es noch einmal. Bleibt es dabei, melde es bitte mit einer kurzen Beschreibung, was du gerade getan hast.',
        },
    },
    problemReport: {
        button: 'Fehler in Ember melden',
        forward: 'An das Beacon weitergeben',
        heldForReview: 'Wartet auf dich',
        forwarded: 'Weitergegeben',
        title: 'Fehler in Ember melden',
        notTheStation: 'Das geht an die Betreiber von Ember, nicht an deine Wache.',
        notTheStationDetail: 'Diese Meldung ist für Fehlverhalten der Anwendung selbst: etwas lässt sich nicht speichern, eine Seite zeigt Unsinn an, ein Knopf tut nichts. Niemand in deiner Wache sieht sie. Wenn es um fehlende Rechte, Ausrüstung, Termine oder eine Frage an die Wachleitung geht, sprich die zuständigen Personen in deiner Wache direkt an: hier kann dir dabei niemand helfen.',
        aboutThis: 'Das ist gerade schiefgegangen:',
        describe: 'Was hast du getan, und was hättest du erwartet?',
        describeHelp: 'Am hilfreichsten sind die Schritte davor, was du erwartet hast und was stattdessen passiert ist. Ohne das lässt sich ein Fehler meist nicht wiederfinden.',
        placeholder: 'Ich wollte … dann habe ich … erwartet hätte ich … stattdessen …',
        tooShort: 'Bitte beschreibe es in ein paar Worten mehr, sonst ist die Meldung nicht nachvollziehbar.',
        autoCapture: 'Aktuelle Seite, Berechtigungen, die letzten Anfragen und Browser-Informationen werden automatisch mitgesendet. Persönliche Daten aus deiner Wache nicht.',
        pictureTitle: 'Bild der Seite',
        pictureHelp: 'Freiwillig. Ein Bild zeigt sofort, was du meinst, und zeigt auch alles andere, was gerade auf dem Bildschirm steht. Decke ab, was niemanden etwas angeht.',
        pictureTake: 'Bild aufnehmen',
        pictureAttach: 'Bild auswählen',
        pictureDiscard: 'Bild verwerfen',
        pictureRefused: 'Es wurde kein Bild aufgenommen. Du kannst stattdessen eines auswählen, das du selbst gemacht hast.',
        pictureFailed: 'Das Bild konnte nicht aufgenommen werden. Wähle stattdessen eines aus, das du selbst gemacht '
            + 'hast, zum Beispiel mit der Bildschirmfoto-Taste.',
        coverHint: 'Ziehe über alles, was abgedeckt werden soll. Ein Klick auf eine Fläche nimmt sie wieder weg.',
        coverRemove: 'Abdeckung entfernen',
        coverPersonalData: 'Namen und Daten abdecken',
        coverClear: 'Abdeckungen zurücknehmen',
        send: 'Meldung absenden',
        sent: 'Deine Meldung ist angekommen. Danke!',
        sentDetail: 'Die Betreiber dieser Instanz sehen sie. Eine Antwort bekommst du hier nicht, es ist eine Meldung und keine Nachricht.',
        couldNotSend: 'Die Meldung selbst konnte nicht gesendet werden. Prüfe deine Verbindung und versuche es noch einmal.',
        showAcknowledged: 'Bestätigte anzeigen',
        acknowledgeAll: 'Alle bestätigen',
        acknowledged: 'Bestätigt',
        acknowledge: 'Bestätigen',
        empty: 'Keine offenen Problemmeldungen.',
        page: 'Seite',
        roles: 'Rollen',
        browser: 'Browser',
        screen: 'Bildschirm',
        recentRequests: 'Letzte Anfragen',
        reqTime: 'Zeit',
        reqMethod: 'Methode',
        reqUrl: 'URL',
        reqStatus: 'Status',
        reqDuration: 'Dauer',
        reqError: 'Fehler',
    },
    devTools: {
        button: 'Dev-Tools',
        title: 'Session-Informationen',
        noSession: 'Keine aktive Session.',
        account: 'Konto',
        name: 'Name',
        email: 'E-Mail',
        accountId: 'Konto-ID',
        instanceUserType: 'Instanz-Typ',
        profileComplete: 'Profil vollständig',
        station: 'Station',
        stationId: 'Station-ID',
        memberId: 'Mitglieds-ID',
        memberUid: 'Mitglieds-UID',
        userType: 'Mitglieds-Typ',
        publicKbMode: 'Öffentl. Wissens-Modus',
        permissions: 'Berechtigungen',
        groups: 'Gruppen',
        tags: 'Tags',
        managedMembers: 'Verwaltete Mitglieder',
        disabledModules: 'Deaktivierte Module',
        copyJson: 'Als JSON kopieren',
    },
    stationPages: {
        saveFailed: 'Die Seite konnte nicht gespeichert werden. Deine Änderungen stehen noch hier.',
        title: 'Seiten',
        createPage: 'Seite erstellen',
        empty: 'Noch keine Seiten vorhanden.',
        published: 'Veröffentlicht',
        draft: 'Entwurf',
        unlisted: 'Nur per Link',
        cells: {
            pageLinkUnresolved: 'Seite nicht mehr erreichbar',
        },
        landingPage: 'Startseite',
        duplicate: 'Duplizieren',
        publish: 'Veröffentlichen',
        unpublish: 'Zurückziehen',
        changeVisibility: 'Sichtbarkeit ändern',
        shareLink: 'Link zur Seite',
        visibilityDraft: 'Niemand',
        visibilityDraftHint: 'Die Seite ist nur innerhalb der Wache sichtbar.',
        visibilityUnlisted: 'Alle mit dem Link',
        visibilityUnlistedHint: 'Die Seite steht in keinem Menü und in keiner Sitemap. Sie steht außerhalb der Seitenstruktur: ohne übergeordnete Seite und ohne Unterseiten.',
        visibilityPublic: 'Alle',
        visibilityPublicHint: 'Die Seite steht im Menü der Wache und kann von Suchmaschinen gefunden werden.',
        setLandingPage: 'Als Startseite festlegen',
        deleteConfirm: 'Möchtest du die Seite „{title}" wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.',
        contentType: {
            markdown: 'Text',
            image: 'Bild',
            video: 'Video',
            callout: 'Hinweis',
            quote: 'Zitat',
            divider: 'Trennlinie',
            spacer: 'Abstand',
            accordion: 'Aufklappbereich',
            pdf: 'PDF',
            file_download: 'Datei',
            countdown: 'Countdown',
            featured_event: 'Hervorgehobenes Event',
            upcoming_events: 'Kommende Events',
            kb_article: 'Wiki-Artikel',
            news_teaser: 'News-Teaser',
            page_link: 'Seitenlink',
            map: 'Karte',
            address_card: 'Adresse',
            partner_stations: 'Partnerstationen',
            federated_event: 'Föderiertes Event',
            member_spotlight: 'Mitglied im Fokus',
            member_list_spotlight: 'Mitglieder-Spotlight',
            stats_counter: 'Zahlen',
            image_gallery: 'Bildergalerie',
            hero_banner: 'Hero-Banner',
            past_event_recap: 'Event-Rückblick',
            tabs: 'Tabs',
            achievements: 'Erfolge',
            external_link_card: 'Externer Link',
            newsletter_signup: 'Newsletter',
            audio_embed: 'Audio',
            poll_embed: 'Umfrage',
            quiz_teaser: 'Quiz',
            application_cta: 'Mitmachen',
            code_block: 'Code',
            nested_rows: 'Verschachtelte Zeilen',
        },
        imageFit: {
            cover: 'Füllen',
            contain: 'Einpassen',
            fill: 'Strecken',
        },
        callout: {
            info: 'Info',
            warning: 'Warnung',
            success: 'Erfolg',
            tip: 'Tipp',
        },
        editor: {
            newPage: 'Neue Seite',
            unsavedChanges: 'Ungespeicherte Änderungen',
            edit: 'Bearbeiten',
            preview: 'Vorschau',
            metadata: 'Seiteneinstellungen',
            titleLabel: 'Titel',
            titlePlaceholder: 'Seitentitel eingeben',
            slugLabel: 'URL-Pfad',
            slugPlaceholder: 'z. B. ueber-uns',
            parent: 'Übergeordnete Seite',
            noParent: 'Keine (Hauptseite)',
            metaDescription: 'Meta-Beschreibung',
            metaDescriptionPlaceholder: 'Kurze Beschreibung für Suchmaschinen',
            addRow: 'Zeile hinzufügen',
            addRowHere: 'Zeile hier einfügen',
            pasteRow: 'Zeile einfügen',
            pasteCell: 'Zelle einfügen',
            copyRow: 'Zeile kopieren',
            cutRow: 'Zeile ausschneiden',
            copyCell: 'Zelle kopieren',
            cutCell: 'Zelle ausschneiden',
            addColumn: 'Spalte hinzufügen',
            addCell: 'Element hinzufügen',
            addComponent: 'Komponente hinzufügen',
            swapComponents: 'Komponenten tauschen',
            rowMenu: 'Zeilen-Menü',
            uploadImage: 'Bild hochladen',
            imageFit: 'Darstellung',
            altText: 'Alternativtext',
            altTextPlaceholder: 'Bildbeschreibung',
            imageDescription: 'Bildunterschrift',
            imageDescriptionPlaceholder: 'Wird unter dem Bild angezeigt',
            maxHeight: 'Max. Höhe (px)',
            maxHeightPlaceholder: 'z. B. 400',
            imageSettings: 'Bildeinstellungen',
            imageSectionDisplay: 'Darstellung',
            imageSectionCrop: 'Zuschnitt',
            imageSectionStyle: 'Stil',
            imageSectionText: 'Text',
            cropTop: 'Oben (%)',
            cropRight: 'Rechts (%)',
            cropBottom: 'Unten (%)',
            cropLeft: 'Links (%)',
            borderRadiusPercent: 'Eckenradius (%)',
            borderWidthPx: 'Rahmenbreite (px)',
            borderColor: 'Rahmenfarbe',
            cropPreview: 'Vorschau',
            linkSearchPlaceholder: 'Seite suchen oder URL eingeben',
            linkPickCalendar: 'Veranstaltungskalender',
            linkPickKb: 'Wiki',
            browse: 'Durchsuchen',
            browseFiles: 'Dateien durchsuchen',
            browseFilesSearch: 'Datei suchen…',
            browseFilesEmpty: 'Noch keine hochgeladenen Dateien.',
            uploadNewFile: 'Neue Datei',
            uploadFailed: 'Hochladen fehlgeschlagen.',
            uploadMimeRejected: '{count} Datei(en) übersprungen - erwartet wird {prefix}*.',
            videoUrlPlaceholder: 'YouTube-URL oder Video-URL eingeben',
            addRowTitle: 'Neue Zeile',
            addRowHint: 'Wähle, wie viele Spalten die Zeile haben soll.',
            emptyCellTitle: 'Leerer Container',
            emptyCellHint: 'Wähle den Inhaltstyp aus.',
            chooseMarkdown: 'Text',
            chooseImage: 'Bild',
            chooseVideo: 'Video',
            resizeColumn: 'Spaltenbreite anpassen',
            swapCells: 'Spalten tauschen',
            editMarkdown: 'Text bearbeiten',
            markdownEmpty: 'Klicken, um Text hinzuzufügen…',
            chooseCallout: 'Hinweis',
            chooseQuote: 'Zitat',
            chooseDivider: 'Trennlinie',
            chooseSpacer: 'Abstand',
            chooseAccordion: 'Aufklappbereich',
            choosePdf: 'PDF',
            chooseFile: 'Datei',
            chooseCountdown: 'Countdown',
            chooseFeaturedEvent: 'Termin (Highlight)',
            chooseUpcomingEvents: 'Termine',
            chooseKbArticle: 'Wiki-Artikel',
            chooseNewsTeaser: 'News-Teaser',
            choosePageLink: 'Seitenlink',
            chooseMap: 'Karte',
            chooseAddress: 'Adresse',
            choosePartners: 'Partner',
            chooseMemberSpotlight: 'Mitglied',
            chooseMemberList: 'Mitglieder-Spotlight',
            chooseStats: 'Statistik',
            chooseGallery: 'Bildergalerie',
            chooseHero: 'Hero-Banner',
            choosePastEvent: 'Rückblick',
            chooseTabs: 'Tabs',
            chooseAchievements: 'Erfolge',
            chooseExternalLink: 'Externer Link',
            chooseBlogSignup: 'Blog-Abo',
            chooseAudio: 'Audio',
            choosePoll: 'Umfrage',
            chooseQuiz: 'Quiz',
            chooseFormsCta: 'Kontaktformular',
            chooseCode: 'Code',
            countdownTarget: 'Zieldatum (ISO 8601)',
            countdownLabel: 'Überschrift',
            countdownSublabel: 'Untertitel',
            eventTitle: 'Titel',
            eventDate: 'Datum',
            eventLocation: 'Ort',
            eventDescription: 'Beschreibung',
            ctaText: 'Button-Text',
            ctaUrl: 'Button-URL',
            listTitle: 'Überschrift',
            jsonItems: 'Einträge (JSON)',
            jsonItemsHint: 'Liste als JSON. Felder je nach Typ; Beispiel: [{"title":"…","date":"…"}]',
            galleryColumns: 'Spalten',
            galleryAspectMode: 'Bildverhältnis',
            galleryAspectSquare: 'Quadratisch zuschneiden',
            galleryAspectPreserve: 'Original beibehalten',
            galleryMaxItemHeight: 'Max. Höhe (px)',
            galleryImageIds: 'Bild-IDs (JSON-Array)',
            articleId: 'Artikel-ID',
            linkTitle: 'Bezeichnung',
            linkUrl: 'URL',
            linkDescription: 'Beschreibung',
            newsTitle: 'Titel',
            newsDate: 'Datum',
            newsSummary: 'Zusammenfassung',
            imageUrl: 'Bild-URL',
            linkImageDisplay: 'Bild-Darstellung',
            linkImageDisplayBanner: 'Banner über dem Text',
            linkImageDisplayIcon: 'Symbol links neben dem Text',
            pageId: 'Seiten-ID',
            mapLat: 'Breitengrad',
            mapLon: 'Längengrad',
            mapZoom: 'Zoom',
            mapHeight: 'Höhe (px)',
            mapLabel: 'Beschriftung',
            addrLabel: 'Überschrift',
            addrLine: 'Straße/Hausnummer',
            addrPostal: 'PLZ',
            addrCity: 'Ort',
            addrCountry: 'Land',
            addrMapUrl: 'Karten-Link',
            partnerName: 'Partner-Name',
            memberName: 'Name',
            memberRole: 'Funktion',
            memberBlurb: 'Kurzbeschreibung',
            showUserType: 'Nutzertyp anzeigen',
            showTag: 'Mitgliedstag anzeigen',
            imageId: 'Bild-ID (siehe Upload)',
            headline: 'Überschrift',
            subtitle: 'Untertitel',
            body: 'Text',
            blogSignupTitle: 'Überschrift',
            blogSignupDescription: 'Beschreibung',
            blogSignupFeedHint: 'Die Komponente verlinkt automatisch auf den öffentlichen Blog-Feed dieser Wache (Atom + RSS).',
            audioUrl: 'Audio-URL',
            audioTitle: 'Titel',
            pollPicker: 'Umfrage auswählen',
            pollShowResultsAfterVote: 'Ergebnisse nach Abgabe anzeigen',
            formsCtaPicker: 'Kontaktformular auswählen',
            formsCtaHeadlineOverride: 'Überschrift (optional)',
            formsCtaBodyOverride: 'Begleittext (optional)',
            formPickerPlaceholder: 'Formular suchen …',
            formPickerStatusOpen: 'Veröffentlicht',
            formPickerStatusDraft: 'Entwurf',
            formPickerStatusClosed: 'Geschlossen',
            formPickerClosedHint: 'Geschlossene Formulare können nicht ausgewählt werden.',
            quizTitle: 'Titel',
            quizDescription: 'Beschreibung',
            quizCatalogs: 'Fragenkataloge',
            quizCatalogsEmpty: 'Noch keine Kataloge sind als öffentlich markiert.',
            quizCatalogsHint: 'Mindestens einen Katalog auswählen, damit die Komponente eine Frage anzeigen kann.',
            quizReveal: 'Antwort zeigen',
            quizNext: 'Neue Frage',
            quizNoQuestions: 'Aktuell keine Frage verfügbar.',
            quizDefaultTitle: 'Teste dein Wissen',
            codeLanguage: 'Sprache',
            codeContent: 'Code',
            addEvent: 'Termin hinzufügen',
            addPartner: 'Partner hinzufügen',
            addOfficer: 'Funktion hinzufügen',
            addStat: 'Statistik hinzufügen',
            addTab: 'Tab hinzufügen',
            addAchievement: 'Erfolg hinzufügen',
            addImage: 'Bild hinzufügen',
            replaceImage: 'Bild ersetzen',
            pruneUnused: '{count} unbenutzte aufräumen',
            unusedBadge: 'Unbenutzt',
            editFileMeta: 'Standardtexte',
            filesTitle: 'Dateien',
            contactFormsTitle: 'Kontaktformulare',
            pollsTitle: 'Umfragen',
            newsTeaserSearchPlaceholder: 'Nachricht suchen…',
            memberSearchPlaceholder: 'Mitglied suchen…',
            pageSearchPlaceholder: 'Seite suchen…',
            eventSearchPlaceholder: 'Termin suchen…',
            partnerStationSearchPlaceholder: 'Wache suchen…',
            wikiSearchPlaceholder: 'Wiki-Artikel suchen…',
            stationNotPublic: 'Wache ist nicht öffentlich',
            eventLimit: 'Max. Einträge',
            includeFederated: 'Föderierte Termine einbeziehen',
            eventCategories: 'Kategorien',
            eventCategoriesAll: 'Keine Auswahl = alle Kategorien anzeigen.',
            eventCategoriesEmpty: 'Es sind noch keine öffentlichen Kategorien definiert.',
            autoFillFromPartners: 'Automatisch alle Partnerwachen anzeigen',
            partnerStationsList: 'Ausgewählte Wachen',
            removePartnerStation: 'Wache entfernen',
            memberListSource: 'Quelle der Mitgliederliste',
            memberListSourceGroup: 'Gruppe',
            memberListSourceTag: 'Tag',
            memberListSourceManual: 'Manuell',
            memberListGroupPick: 'Gruppe auswählen',
            memberListGroupPickEmpty: '- keine Gruppe gewählt -',
            memberListTagPick: 'Tag auswählen',
            memberListTagPickEmpty: '- kein Tag gewählt -',
            memberListManualList: 'Ausgewählte Mitglieder',
            memberListManualRemove: 'Mitglied entfernen',
            memberListDescriptionPlaceholder: 'Beschreibung (optional, z.B. Rolle)',
            memberListMissingMember: 'Mitglied nicht mehr verfügbar',
            memberListGroupTagDescriptionsTitle: 'Beschreibungen für Mitglieder',
            memberListGroupTagDescriptionsHint: 'Beschreibungen erscheinen unter dem Namen auf der Seite.',
            sortBy: 'Sortierung',
            sortByOrder: 'Reihenfolge (Drag-and-Drop)',
            sortByName: 'Alphabetisch',
            sortByRole: 'Nach Rolle',
            sortByJoinDate: 'Nach Eintrittsdatum',
            prunePrompt: '{count} unbenutzte Dateien dauerhaft löschen?',
            folders: 'Ordner',
            tags: 'Tags',
            newFolder: 'Neuer Ordner',
            editFolder: 'Ordner bearbeiten',
            folderName: 'Ordnername',
            folderDeletePrompt: 'Ordner "{name}" und alle Unterordner löschen? Dateien werden in den Stamm verschoben.',
            newTag: 'Neuer Tag',
            editTag: 'Tag bearbeiten',
            tagName: 'Tag-Name',
            tagDeletePrompt: 'Tag "{name}" entfernen?',
            allFiles: 'Alle Dateien',
            allTags: 'Alle',
            rootFolder: '- Stamm -',
            root: 'Stamm',
            selectedCount: '{count} ausgewählt',
            moveSelected: 'Verschieben',
            clearSelection: 'Auswahl aufheben',
            deleteSelected: 'Auswahl löschen',
            deleteSelectedPrompt: '{count} Datei(en) dauerhaft löschen?',
            multiSelectEnable: 'Mehrfachauswahl',
            multiSelectDisable: 'Mehrfachauswahl beenden',
            deleteFile: 'Löschen',
            deleteFilePrompt: 'Datei „{name}" dauerhaft löschen?',
            fileActions: 'Aktionen',
            perPage: 'Pro Seite',
            pageOf: 'Seite {current} von {total}',
            prevPage: 'Vorige',
            nextPage: 'Nächste',
            moveToFolder: 'In Ordner verschieben',
            partnerDistance: 'Distanz (km)',
            statLabel: 'Bezeichnung',
            statValue: 'Wert',
            statSuffix: 'Zusatz',
            tabTitle: 'Titel',
            tabBody: 'Inhalt',
            achievementTitle: 'Titel',
            achievementYear: 'Jahr',
            achievementDescription: 'Beschreibung',
            galleryImages: 'Bilder',
            heroImage: 'Hintergrundbild',
            recapImage: 'Bild',
            cellMenu: 'Aktionen',
            splitCell: 'Spalten teilen',
            width: 'Breite',
            depthWarning: 'Tief verschachtelt - überlege, ob es einfacher geht.',
            chooserSearch: 'Komponenten suchen…',
            catBasic: 'Grundlagen',
            catLayout: 'Layout',
            catFiles: 'Dateien & Medien',
            catEvents: 'Termine',
            catLinks: 'Inhalte verlinken',
            catGeo: 'Standort',
            catPeople: 'Menschen & Statistik',
            catEngage: 'Engagement',
            calloutVariant: 'Art',
            calloutTitle: 'Titel',
            calloutBody: 'Text',
            quoteText: 'Zitat',
            quoteAuthor: 'Autor',
            quoteAttribution: 'Quellen-Link',
            dividerLabel: 'Beschriftung',
            dividerLabelPlaceholder: 'optional',
            spacerHeight: 'Höhe (px)',
            accordionTitle: 'Überschrift',
            accordionOpenByDefault: 'Standardmäßig geöffnet',
            accordionBody: 'Inhalt',
            pdfUrl: 'PDF-URL',
            pdfHeight: 'Höhe (px)',
            fileUrl: 'Datei-URL',
            fileLabel: 'Anzeigename',
            fileDescription: 'Beschreibung',
        },
    },
    setup: {
        headerTitle: 'Wache einrichten',
        stepCounter: 'Schritt {current} von {total}',
        actions: {
            back: 'Zurück',
            skip: 'Überspringen',
            saveAndContinue: 'Speichern und weiter',
            getStarted: 'Loslegen',
            addRow: 'Zeile hinzufügen',
            removeRow: 'Zeile entfernen',
        },
        checklist: {
            title: 'Einrichtung abschließen',
            hint: 'Erledige die fehlenden Schritte, damit deine Wache einsatzbereit ist.',
            resume: 'Weiter im Assistenten',
            required: 'Pflicht',
            optional: 'optional',
        },
        steps: {
            welcome: {
                label: 'Willkommen',
                title: 'Willkommen!',
                hint: 'Wir richten deine Wache in wenigen Schritten ein.',
                body1: 'Dieser Assistent hilft dir, deine Wache startbereit zu machen.',
                body2: 'Du kannst zwischendurch jederzeit aussteigen - die offenen Punkte erscheinen auf dem Dashboard.',
                bullet1: 'Adresse und Pin auf der Karte hinterlegen.',
                bullet2: 'Module wählen, die deine Wache nutzt.',
                bullet3: 'Erste Mitgliedergruppen und Berechtigungen anlegen.',
            },
            address: {
                label: 'Adresse',
                title: 'Adresse und Standort',
                hint: 'Pflichtangabe - wird für Karte, Entfernungssuche und Föderation benötigt.',
                incompleteHint: 'Bitte fülle alle Adressfelder aus und setze einen Pin auf der Karte.',
            },
            modules: {
                label: 'Module',
                title: 'Module aktivieren',
                hint: 'Wähle, welche Funktionen deine Wache nutzen möchte. Du kannst das jederzeit ändern.',
            },
            'member-types': {
                label: 'Rollen',
                title: 'Berechtigungen je Mitgliedstyp',
                hint: 'Lege fest, welche Standard-Berechtigungen Mitglieder, Teammitglieder, Erziehungsberechtigter und Manager bekommen.',
                granted: 'erteilt',
                types: {
                    MEMBER: 'Mitglied',
                    GUARDIAN: 'Erziehungsberechtigter',
                    TEAM: 'Teammitglied',
                    MANAGER: 'Manager',
                },
            },
            groups: {
                permissionsLoadFailed: 'Die Rechte dieser Gruppe konnten nicht geladen werden. Was hier steht, ist nicht der aktuelle Stand.',
                label: 'Gruppen',
                title: 'Mitgliedergruppen',
                hint: 'Optional - du kannst jederzeit später Gruppen anlegen oder anpassen.',
                placeholder: 'Name der Gruppe',
                aboutTitle: 'Was sind Gruppen?',
                aboutBody: 'Gruppen bündeln Mitglieder, die zusammen agieren oder ähnliche Aufgaben übernehmen - z. B. ein Wissensstand oder ein Fachbereich. Mit Gruppen kannst du gezielt Einladungen, Benachrichtigungen und Berechtigungen vergeben, ohne jedes Mitglied einzeln anzufassen.',
                useCase1: 'Termine und News auf bestimmte Gruppen einschränken.',
                useCase2: 'Mehrere Mitglieder gleichzeitig zu einem Board zuordnen.',
                useCase3: 'Anwesenheitsfilter und Statistiken pro Gruppe auswerten.',
                editPermissions: 'Berechtigungen bearbeiten',
                hidePermissions: 'Berechtigungen ausblenden',
                colorExplainer: 'Die Farbe einer Gruppe taucht als kleine Markierung neben dem Namen auf - auf Mitglieder-Karten, in Filtern und überall, wo Gruppen erscheinen. So erkennst du auf einen Blick, wer wozu gehört.',
                sortExplainer: 'Die Reihenfolge bestimmt, in welcher Folge Gruppen in Listen und Filtern auftauchen. Die höchste Gruppe bestimmt außerdem die Farbe. Wichtige oder häufig genutzte Gruppen gehören nach oben.',
                listTitle: 'Gruppen',
                permissionsTitle: 'Berechtigungen für diese Gruppe',
                moveUp: 'Nach oben',
                moveDown: 'Nach unten',
                emptyHint: 'Noch keine Gruppen. Lege unten eine an.',
                selectHint: 'Wähle links eine Gruppe aus, um Farbe und Berechtigungen zu bearbeiten.',
            },
            mail: {
                label: 'Mailing',
                title: 'Mailing-Anbindung der Wache',
                hint: 'Optional - alle Funktionen der Wache funktionieren auch ohne eigene Mailing-Anbindung.',
                worksWithoutTitle: 'Du musst hier nichts einrichten',
                worksWithoutBody: 'Alle Funktionen der Wache laufen auch ohne eigene Mailing-Anbindung. Einladungen, Passwort-Mails und Bestätigungen verschickt das System sowieso über den Instanz-Relay. Eine eigene Wachen-Anbindung ist nur sinnvoll, wenn ihr viele Benachrichtigungen rausschickt und das Versand-Budget eures Hosts schonen wollt.',
                scopeNote: 'Wenn du die Anbindung einrichtest, läuft hierüber das wachen-eigene Benachrichtigungs-Aufkommen (Erinnerungen, Berichte, Digests). Pflicht-Mails wie Einladungen und Passwort-Resets bleiben weiter beim System-Relay.',
                provider: 'Anbieter',
                senderAddress: 'Absenderadresse',
                senderName: 'Absendername',
                smtpHost: 'SMTP-Server',
                smtpPort: 'Port',
                smtpSsl: 'SSL verwenden',
                smtpUser: 'Benutzer',
                smtpPassword: 'Passwort',
                apiKey: 'API-Schlüssel',
            },
            branding: {
                label: 'Erscheinungsbild',
                title: 'Erscheinungsbild',
                hint: 'Optional - wähle ein Theme, eigene Farben oder lade ein Logo hoch.',
                logoTitle: 'Logo',
                logoHint: 'Ein eigenes Logo erscheint im Header und auf öffentlichen Seiten.',
                uploadLogo: 'Logo hochladen',
                removeLogo: 'Logo entfernen',
                logoPresent: 'Logo hinterlegt.',
            },
            'first-event': {
                label: 'Erstes Event',
                title: 'Erstes Event anlegen',
                hint: 'Optional - lege gleich ein Beispiel-Event an, oder überspringe diesen Schritt.',
                aboutTitle: 'Was passiert hier?',
                aboutBody: 'Wir bringen dich zum vollständigen Event-Editor mit allen Optionen (Kategorien, Wiederholung, Föderation …). Nach dem Speichern landest du automatisch wieder hier im Assistenten.',
                openEditor: 'Event-Editor öffnen',
            },
            'kb-seed': {
                label: 'Wiki',
                title: 'Wiki starten',
                hint: 'Optional - lege eine erste Seite an, damit das Wiki nicht leer ist.',
                aboutTitle: 'Was ist eine Wiki-Seite?',
                aboutBody: 'Wiki-Seiten sind Dokumente, Videos, Präsentationen und mehr, die du jederzeit weiter füllen und verlinken kannst. Eine erste Seite (z. B. „Über uns" oder „Kontakt") gibt deinen Mitgliedern sofort einen Anlaufpunkt. Du kannst die Inhalte später im Wiki-Editor weiter ausbauen.',
                pageName: 'Seitenname',
                pageDescription: 'Beschreibung (optional)',
                pagePlaceholder: 'z. B. „Über uns"',
                pageContent: 'Inhalt',
                createPage: 'Seite anlegen',
                missing: 'Bitte gib einen Seitennamen an.',
            },
            federation: {
                label: 'Sichtbarkeit',
                title: 'Sichtbarkeit in der Föderation',
                hint: 'Optional - bestimme, ob deine Wache im öffentlichen Verzeichnis erscheint.',
                consequence: 'Wenn öffentlich gewählt ist, erscheint deine Wache im öffentlichen Verzeichnis. Du kannst das später jederzeit ändern.',
                visibility: 'Sichtbarkeit',
                visibilityPublic: 'Öffentlich gelistet',
                visibilityNone: 'Versteckt',
                description: 'Kurzbeschreibung im Verzeichnis',
                showKb: 'Öffentliche Wiki im Verzeichnis verlinken',
            },
            invites: {
                label: 'Einladungen',
                title: 'Mitglieder einladen',
                hint: 'Optional - lege deine Mitglieder direkt an. Jedes Mitglied erhält eine E-Mail, um sein Passwort zu setzen.',
                tabBulk: 'Schnell-Liste',
                tabRich: 'Detail-Formular',
                tabCsv: 'CSV-Import',
                csvIntro: 'Für den vollständigen CSV-Import mit Spalten-Mapping, Vorschau, Gruppen-Zuordnung und Eltern-Verknüpfung nutze die Mitglieder-Import-Seite.',
                csvHandoffNote: 'Wir leiten dich dort hin und bringen dich nach dem Import automatisch zurück in den Assistenten.',
                openMemberImport: 'Mitglieder-Import öffnen',
                bulkEmails: 'E-Mail-Adressen (Kommas oder Zeilenumbrüche)',
                bulkPlaceholder: "alice{'@'}example.com\nbob{'@'}example.com",
                userType: 'Mitgliedstyp',
                group: 'Gruppe',
                groupNone: '- keine Zuordnung -',
                expandedCount: '{n} Mitglieder werden angelegt.',
                firstName: 'Vorname',
                lastName: 'Nachname',
                email: 'E-Mail',
                guardiansToggle: 'Erziehungsberechtigte ({n})',
                addGuardian: 'Erziehungsberechtigter hinzufügen',
            },
            finish: {
                label: 'Fertig',
                title: 'Geschafft!',
                body: 'Deine Wache ist bereit. Du kannst die Tour starten oder direkt zum Dashboard.',
                missingHint: 'Es sind noch Pflicht-Schritte offen. Wir bringen dich zum nächsten.',
                takeTour: 'Geführte Tour starten',
                goDashboard: 'Zum Dashboard',
                mailReminder: 'Hinweis: Die Mailing-Anbindung der Wache ist noch nicht eingerichtet - Benachrichtigungen werden bis dahin nicht versendet.',
            },
        },
    },
}
