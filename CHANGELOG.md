...
- **An inventory is now either a stock or a collection.** A stock holds one thing in many copies, the shelf full of blousons; a collection holds different things that belong together, the box with twelve radios, a charging station and an antenna. One of the two is now chosen by name when an inventory is created, rather than by ticking a box; requirements, procurements, exchanges and sizes are offered on a stock only, and every inventory that exists today counts as a stock until a station says otherwise.
- **An inventory wears its own labels.** Wherever inventories are listed, each one now shows who owns it, whether it uses sizes and whether it is a stock or a collection, so a list of a dozen inventories can be read without opening any of them. A collection also says how many kinds are defined in it.
- **Quick search finds an inventory.** Typing part of an inventory's name now offers it beside the pages, members, appointments and wiki files the quick search already found, and says whether it is a stock or a collection.
- **A field is written down where the thing it describes lives.** An extra field belonging to one kind of thing is now added and edited on that kind, and one belonging to a single piece on that piece, so a colour on the radios is set where somebody would look for it rather than on a separate screen that asks what the field is for. The inventory screen keeps the fields that belong to the whole inventory, and every field written before this stands where it always belonged.
- **A stock list leads into its own settings and back.** The page listing what an inventory holds now carries a button into that inventory's settings, sizes and kinds, and that page carries one straight back, so changing a size no longer means going out to the list of all inventories and in again. The button appears only for somebody allowed to change the inventory.
