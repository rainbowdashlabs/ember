...
- **An inventory is now either a stock or a collection.** A stock holds one thing in many copies, the shelf full of blousons; a collection holds different things that belong together, the box with twelve radios, a charging station and an antenna. Which of the two it is is chosen by name rather than by ticking a box, requirements, procurements, exchanges and sizes are offered on a stock only, and every inventory that exists today counts as a stock until a station says otherwise.
