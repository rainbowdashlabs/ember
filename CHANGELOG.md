...
- **`IllegalArgumentException` handler now logs the full throwable.** The generic 400 handler for `IllegalArgumentException` now passes the exception as the trailing SLF4J argument so the problem-log appender captures the class, message, and stacktrace for the admin problem feed. The response body still echoes the (already curated) message; fully masking it requires per-site conversion of every deliberate user-facing throw to `BadRequestResponse` and is tracked as a follow-up.
