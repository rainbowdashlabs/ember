...
- **Admin panels.** A new `/station/manage/security` (station admin) and `/admin/settings/security` (instance admin) let operators require 2FA per `StationUserType`, see who has set it up, reset 2FA on behalf of a user who lost their device, and browse the full audit log instance-wide.
...
- **On by default — Breaking on upgrade.** `auth.twoFactor.enabled` now defaults to `true`. **Production deployments must set `TWO_FACTOR_SECRET_KEY`** (a 32-byte base64 random value) — the application refuses to boot otherwise. Existing sessions are not invalidated by the flip, but mandated user types (instance admins, station managers) will be prompted to enrol on next login.
...
- **Path-traversal hardening for admin legal-document routes.** `GET /admin/legal/{type}/{locale}`, `PUT /admin/legal/{type}/{locale}`, `GET /admin/legal/{type}/{locale}/files`, and `PUT /admin/legal/{type}/{locale}/files` now validate `{locale}` against `^[a-z]{2}(-[A-Z]{2})?$` and verify the resolved on-disk path stays inside the configured legal directory. Without the check, an instance administrator could craft a `locale=../../etc` value and write or list files outside `data/legal/...`. The risk was low (admin-only) but the construction was unsafe and is now rejected with a static `400 Invalid locale`.
- **Static-file serving removed from the Java backend.** The Java process previously read `EMBER_PUBLIC_DIR` (default `public/`) at boot and served whatever sat inside that directory as static files plus an SPA fallback. An operator who pointed the env var at a sensitive directory (project root, `/etc`, etc.) would have published its contents to the public internet on the API origin. The Java backend is API-only now — the Nuxt server owns every browser-facing route — so the `staticFiles.add(...)` + `spaRoot.addFile(...)` block and the `EMBER_PUBLIC_DIR` read are gone, along with the unused `src/main/resources/static/index.html` fallback bundle. Hitting the Java port at `/` now returns 404; only `/api/v1/...` and `/docs` (OpenAPI / Swagger) are served.
- **Dead salt field removed from password hash encoding.** The `PasswordHash` record carried an extra `salt` segment encoded as `{algorithm:hash:salt}` that was never read at verification time — BCrypt embeds its own salt inside the hash output, so the extra base64-encoded random bytes were dead data. The record now has only the `algorithm` and `hash` fields; new hashes are written as `{algorithm:hash}`. `parse(...)` still accepts the legacy three-segment form so existing stored credentials keep verifying — the `needsRehash` path migrates them to the new shorter form on the user's next successful login.
- **`confirmStationDeletion` tolerates corrupt metadata.** A `STATION_DELETE` token whose metadata cannot be parsed as an integer now returns `Optional.empty()` and clears the token, instead of letting an uncaught `NumberFormatException` propagate into the generic 500 handler and the problem feed. User-facing response shape matches the other token endpoints (`400 Invalid or expired token`).
- **`setPassword` logs which token type rotated the credential.** The endpoint accepts `SET_PASSWORD` (invite), `RESET_PASSWORD` (forgot), and `FORCE_PASSWORD_CHANGE` (post-login forced rotation) interchangeably; a new INFO-level log line on every successful rotation records the token type and account id so operators can correlate the flow without a dedicated audit table.
- **`ImageService.delete` surfaces I/O failures instead of swallowing them.** Per-file `IOException`s during a delete previously logged a warning and continued, then `Files.deleteIfExists(dir)` on the still-non-empty directory threw `DirectoryNotEmptyException` and was also swallowed — a "successful" delete could leave stale fragments behind. The lambda now rethrows as `UncheckedIOException` and the outer try/catch does the same, so the failure reaches the admin problem feed via the generic exception mapper.
- **API Jackson mapper pins `FAIL_ON_UNKNOWN_PROPERTIES = true`.** Strict deserialization was already the Jackson default, but the API mapper now sets it explicitly so a future contributor copying a tolerant mapper (e.g. `FederationHttpClient` / `DiscoveryHttpClient`, which intentionally disable the flag for cross-version peer compatibility) cannot accidentally regress inbound client payloads to silent-drop behaviour. Both HTTP-client mappers gained a Javadoc note explaining why the tolerance is intentional there.

### Changes

- **Schema** — `patch_14` is extended with the tables and columns required for public forms, page files, the page editor refactor, and the new `consent_proof` JSONB column on the three anonymous-submission tables; `data_tracking.json` is refreshed to mark every new column as verified. `patch_15` adds `station_traffic_hourly`. `patch_17` adds all 2FA tables, enums, and the `account_session.two_factor_verified_at` / `device_trust_id` columns.
- **Admin sidebar URLs reorganised** — every entry under the "Monitoring" sidebar group now lives under `/admin/monitoring/` (problems, problem-reports, storage, api-status, feed-metrics, traffic, discovery, maps) and the dev-only data-tracking inspector lives under `/admin/dev/`. Existing bookmarks to the old top-level paths (e.g. `/admin/storage`) need to be updated.
