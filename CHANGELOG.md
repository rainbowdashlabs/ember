...
- **Exchange requests can be narrowed down and put in order.** Above the list stand a search for the member, a choice of inventories and a choice of statuses; the last two are ticked off a list, so three ticked statuses show the rows of all three at once, while the three filters still narrow together and ticking nothing at all restricts nothing. The list opens with the statuses of a request still running ticked rather than on every request ever raised, where the filters leave nothing standing the page says so instead of reading like a station without exchanges, and it can be ordered by member, inventory, status or date, the status along the steps it passes through rather than the alphabet. The export takes exactly the rows the filters leave, so selecting all of them cannot quietly pull in what is hidden.
